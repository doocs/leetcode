...
> **思考**
>
> $n\le 10^6$。从位置 $i$ 可以向右跳到 $i+jump[i]$，或向左跳到任意 $j<i$。按层搜索最少按动次数是正确模型，但若每次把左侧全部位置入队，总边数可达平方级。
>
> 向左可到达的未访问下标构成一段不断右移的前缀。用指针 $mx$ 记录已经作为「左跳目标」考虑过的右端，新节点 $i$ 只需把 $[mx,i)$ 以及右跳点加入队列。同一位置至多入队一次，从而把 BFS 压到近似线性。
