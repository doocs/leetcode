...
> **思考**
>
> 目标形状是「红 + 黄 + 红」，$n\le 10^5$。若枚举两处分界点再统计修改次数，代价为平方级。同一前缀、同一所处段落会被重复计算。
>
> 用 $f[i][j]$ 表示处理到第 $i$ 片叶子且处于第 $j$ 段（左红、中黄、右红）的最少修改。段落只能按 $0\to 1\to 2$ 前进或停留，当前颜色决定转移是否加一。答案为 $f[n-1][2]$。
