...
> **思考**
>
> 每次操作把相邻元素的差调整 $1$，前缀变成公差为 $1$ 的等差数列，等价于把 $nums[i]-i$ 全部改成同一个数，最优目标是中位数。$n\le 10^5$，每个前缀都要求一次答案，不能对每个前缀重新排序。
>
> 用小根堆存较大一半、大根堆存较小一半，动态维护中位数及两侧元素和。新值先入小根堆再平衡到大根堆，使两侧个数至多多 $1$。操作次数由两侧和与中位数的偏差给出。
...
> **思考**
>
> 方法一已在 $O(n\log n)$ 内维护前缀中位数。堆只能弹栈顶，按值精确删除不够直观。两个有序集合分别保存较小半与较大半，可弹出最小或最大元素完成同样的对半划分，复杂度同阶。
