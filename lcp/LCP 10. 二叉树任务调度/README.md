...
> **思考**
>
> 依赖关系是二叉树，须先完成后继（子节点）再执行当前任务，且仅有两核可并行。节点数不超过 $10^3$，自底向上计算每棵子树的最短完工时间即可，无需枚举调度序列。
>
> 一棵子树的总工作量是确定的；两核至多把子节点工作对半分担，但又不能早于较重那棵子树自身的完工时刻。因此子树最短时间为 $\max(t_L,t_R,(s_L+s_R)/2)$ 再加上根的执行时间。后序递归同时回传子树工作量与该最短时间。
