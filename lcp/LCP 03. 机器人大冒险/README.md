...
> **思考**
>
> 终点与障碍的坐标可达 $10^9$，按指令逐步模拟全部路程不可行。指令串长度至多为 $10^3$，且至少含一个 $\texttt{U}$ 与一个 $\texttt{R}$，机器人的轨迹是同一段位移的周期重复。
>
> 记一轮指令后的净位移为 $(i,j)$，该轮内经过的相对格点构成有限集合 $vis$。任意可达点必可写成 $k\cdot(i,j)+p$，其中 $p\in vis$。因此将终点减去尽可能多的整轮位移后查询 $vis$，即可判断能否到达；对不超过终点的障碍做同样归约，若落入 $vis$ 则中途受阻。
