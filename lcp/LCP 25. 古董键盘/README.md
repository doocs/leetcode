...
> **思考**
>
> $k\le 5$、$n\le 26k$，按键序列共 $26^n$ 量级，不能直接枚举。字母之间地位对称，只需记录已使用的字母种数与已按下的次数，并限制每种字母至多 $k$ 次。
>
> 令 $f[i][j]$ 表示按了 $i$ 次、用了前 $j$ 个字母的方案数。枚举第 $j$ 个字母出现 $h$ 次（$0\le h\le\min(i,k)$），从 $f[i-h][j-1]$ 乘 $\binom{i}{h}$ 转移。答案为 $f[n][26]$。
