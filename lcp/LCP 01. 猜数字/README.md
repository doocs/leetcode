...
> **思考**
>
> 数组长度固定为 $3$，逐位比较 $guess$ 与 $answer$ 即可在常数时间内得到猜对的个数。题目只要求统计相等位置的数量，无需记录下标或处理冲突。
>
> 为此我们同步遍历两个序列，将对应位置相等的次数累加。$n$ 已为常量，这一线性扫描同时是最优的。
...
> **思考**
>
> 方法一已经用一次遍历得到答案。此处把「相等则累加」写成一次折叠，语义与方法一相同，仅实现更紧凑。
