...
> **思考**
>
> $n\le 10$、$k\le 5$，枚举所有长度为 $k$ 的传递序列可以接受，但同一「已走步数、当前所在玩家」会被反复访问。题目求的是方案数而非某条具体路径。
>
> 为此令 $f[i][j]$ 表示走 $i$ 轮到达 $j$ 的方案数。初始仅 $f[0][0]=1$，每条有向边 $a\to b$ 将上一轮在 $a$ 的方案累加到本轮的 $b$。答案为 $f[k][n-1]$。
...
> **思考**
>
> 方法一的 $f[i]$ 只依赖 $f[i-1]$，第一维无需整表保留。用长度为 $n$ 的两个数组交替填写，空间由 $O(kn)$ 降至 $O(n)$，转移关系不变。
