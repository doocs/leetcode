...
> **思考**
>
> 水缸数不超过 $100$，容量上限 $10^4$。升级应全部发生在蓄水之前，否则同等次数下每次倒入的水更少。蓄水次数 $x$ 确定后，第 $i$ 个桶至少要有 $\lceil vat_i/x\rceil$ 的容量，升级次数随之确定。
>
> $x$ 的上界是 $vat$ 的最大值（再多倒只会浪费）。枚举 $x$，累加各桶的升级次数 $y$，取 $x+y$ 的最小值。全部 $vat$ 为 $0$ 时无需操作。
