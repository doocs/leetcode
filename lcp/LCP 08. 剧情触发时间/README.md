...
> **思考**
>
> 天数 $m\le 10^4$，剧情数可达 $10^5$。对每个剧情按天扫描属性，总代价为二者乘积，无法接受。三项属性只增不减，第 $t$ 天的属性等于前 $t$ 天增量之和，且对天数单调。
>
> 先把 $increase$ 做成前缀和（并在开头插入全零以表示第 $0$ 天），再对每个需求在天数上二分，寻找三项同时达标的最早下标；找不到则记为 $-1$。
