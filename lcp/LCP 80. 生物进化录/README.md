...
> **思考**
>
> $n\le 10^4$。记录规则对应一次先序遍历：进入子节点写 $0$，回溯写 $1$。同一父节点下子树的记录顺序影响整串字典序，且指针最终不必回到根，末尾的回溯可以省略。
>
> 对每个节点收集各子树的记录串并按字典序排序后拼接，再包上 $0\cdots 1$。根的进入 $0$ 本来就不存在，去掉串首字符；末尾连续的 $1$ 全部删去，即得到最短且最小的合法串。
