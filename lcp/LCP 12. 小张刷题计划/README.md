...
> **思考**
>
> $n\le 10^5$，需把题目分成至多 $m$ 天，每天可跳过最耗时的一题，最小化其余耗时的最大值 $T$。直接枚举划分点不可行。$T$ 越大越容易在 $m$ 天内完成，具有单调性。
>
> 在 $[0,\sum time]$ 上二分 $T$。判定时从左到右尽量把题目装进当前天：累计和去掉当日最大值后一旦超过 $T$，就新开一天。分组数不超过 $m$ 则可行。
