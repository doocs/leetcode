...
> **思考**
>
> 从左上只能向右或向下走到右下。路径条数等于来自上方与左方的条数之和。
>
> $f[i][j]$ 由 $f[i-1][j]$ 与 $f[i][j-1]$ 相加得到，起点为 $1$。按行扫描即可。
...
> **思考**
>
> 方法一在边界上用条件累加。第一行与第一列只有一种走法，先填成 $1$，内部格子不再判断边界。
...
> **思考**
>
> 方法二仍占用整张表。同一行只依赖左侧与上一行同列，用长度为 $n$ 的数组滚动，$f[j]+=f[j-1]$。
