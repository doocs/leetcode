...
> **思考**
>
> 需要找出所有和为 $0$ 的不重复三元组。三重枚举在 $n\le 3000$ 时约为 $10^{10}$ 量级，无法通过；若不排序，去重也难以处理。
>
> 先排序后，枚举最小元 $nums[i]$，其余两数在右侧用相向双指针寻找相反数。有序使得指针移动具有单调性，也使相同值可以跳过，从而在 $O(n^2)$ 内得到不重复的全部三元组。若 $nums[i]>0$，其后皆为正，可以提前结束。
