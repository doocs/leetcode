...
> **思考**
>
> 需要多次查询子矩阵和。若每次现场累加，单次代价与矩形面积成正比，查询次数多时无法承受。
>
> 二维前缀和把矩形和化为四个角上前缀的容斥：$s[r_2+1][c_2+1]-s[r_2+1][c_1]-s[r_1][c_2+1]+s[r_1][c_1]$。初始化时按行、列递推填表，之后每次查询为常数时间。下标整体加一，是为了让边界上的减项自然变成 $0$。
