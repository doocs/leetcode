...
> **思考**
>
> 统计回文子串个数。枚举每个子串再判定是 $O(n^3)$。
>
> 回文由中心唯一确定。对每个位置分别按奇数、偶数长度向两侧扩展，每次成功匹配计一。扩展在失配处停止，总代价 $O(n^2)$。
...
> **思考**
>
> 中心扩展对每个中心都可能走到 $O(n)$。Manacher 利用已计算过的回文半径，在线性时间内得到每个中心的最大回文长度 $p[i]$，该中心贡献的回文个数为 $\lfloor p[i]/2\rfloor$。
