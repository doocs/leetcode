...
> **思考**
>
> $arr2$ 给出相对次序，其余元素升序接在后面。比较器若每次线性查找 $arr2$，排序过程中比较次数过多。
>
> 哈希表记下 $arr2$ 中的位次，不在其中的元素映射到 $1000+x$（值域不超过 $1000$）。按该键排序即可同时满足相对序与剩余元素的升序。
...
> **思考**
>
> 方法一仍是比较排序。值域很小，先计数再按 $arr2$ 的顺序输出，最后扫描值域补上未出现在 $arr2$ 中的数，线性完成。
