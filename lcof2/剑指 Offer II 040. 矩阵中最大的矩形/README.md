...
> **思考**
>
> 矩阵中全 $1$ 矩形的底边可以落在任意一行。若枚举上下边界再检查每列是否全 $1$，是立方级的。
>
> 把每一行当作柱状图底边：连续的 $1$ 累加高度，遇到 $0$ 则高度清零。对每行用直方图最大矩形求解，答案取全局最大。
...
> **思考**
>
> 方法一用两次单调栈分别求左右边界。一次从左到右扫描即可同时写下左边界，并在弹出时回填被弹出柱的右边界，常数更小。
