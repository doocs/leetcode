...
> **思考**
>
> 对每个 $i$ 单独统计二进制中 $1$ 的个数需要 $O(n\log n)$，在 $n\le 10^5$ 时可以通过，但相邻整数的二进制表示高度相关，不必每次从零开始。
>
> 注意到 $i\mathbin{\&}(i-1)$ 恰好比 $i$ 少掉最低位的 $1$，且该值严格小于 $i$。因此按升序填写 $f[i]=f[i\mathbin{\&}(i-1)]+1$，每个数只做一次常数时间转移。
