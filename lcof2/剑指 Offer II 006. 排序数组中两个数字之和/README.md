...
> **思考**
>
> 数组已按非递减排列，且恰好存在一对答案。对每个 $numbers[i]$ 在其右侧二分查找 $target-numbers[i]$，复杂度 $O(n\log n)$，在 $n\le 3\times 10^4$ 时可以通过。
>
> 有序保证了查找区间可以收窄到 $[i+1,n)$，从而既避免使用自身，也保证返回的下标递增。
...
> **思考**
>
> 方法一每次都做一次对数查找。两端指针相向移动时，和偏小则左端右移，偏大则右端左移，每个位置至多访问一次，线性即可定位。
