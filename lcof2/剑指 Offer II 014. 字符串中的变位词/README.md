...
> **思考**
>
> 判断 $s2$ 是否包含 $s1$ 的某个排列。枚举所有长度为 $|s1|$ 的窗口再排序比较，在长度 $10^4$ 时偏慢，且排序没有利用窗口只滑动一格的事实。
>
> 变位词只关心字符频次。先统计 $s1$ 与 $s2$ 首个等长窗口的计数，窗口右移时加入新字符、去掉滑出字符，计数相等即成功。
...
> **思考**
>
> 方法一每次滑动后都要比较两张表。用 $\textit{diff}$ 记录频次不同的字符种类，入窗、出窗时按计数穿越零的次数更新 $\textit{diff}$，$\textit{diff}=0$ 即覆盖成功，比较代价降为常数。
