...
> **思考**
>
> 迭代器要按中序依次给出结点。初始化时完整遍历并保存全部值，空间与结点数成正比。
>
> 先把中序序列装入数组，$\textit{next}$ 与 $\textit{hasNext}$ 退化为下标移动。实现简单，满足功能正确。
...
> **思考**
>
> 方法一在初始化时占用 $O(n)$。栈只保存「尚未访问的左链」，$\textit{next}$ 弹出栈顶后再把其右子树的左链压入，均摊 $O(1)$，空间与树高同阶。
