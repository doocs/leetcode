...
> **思考**
>
> 编码串由单词加 $\#$ 拼接，若一词是另一词的后缀则可省略。两两检查后缀是平方级的。
>
> 把单词反转后插入前缀树，公共后缀变成公共前缀。树上每个叶子对应一段必须写入的编码，其深度（含 $\#$）之和即最短长度。
...
> **思考**
>
> 方法一建完树再深度优先累加叶子。先按长度降序插入反转单词：若插入全程未创建新结点，说明它是已有更长单词的后缀，对长度贡献为 $0$。

<!-- thinking:end -->

将单词按长度从长到短排序，再依次把反转后的单词插入前缀树。若插入过程中所有节点都已存在，说明当前单词是某个更长单词的后缀，对编码长度贡献为 $0$；否则贡献 $|w|+1$。

时间复杂度 $O(L \log n + L)$，空间复杂度 $O(L)$。其中 $n$ 为单词个数，$L$ 为所有单词的长度之和。
