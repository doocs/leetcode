...
> **思考**
>
> 求无重复字符的最长子串。枚举端点再用集合检查，最坏 $O(n^2)$。
>
> 若当前右端字符已在窗口内，则左端必须右移直至该字符离开。集合记下窗口内字符后，每个位置进出至多一次，最长长度在扩张时同步更新。
...
> **思考**
>
> 方法一用哈希集合判定是否重复。字符落在 ASCII $[0,128)$ 内，布尔数组即可扮演同一角色，避免散列开销，逻辑不变。
