...
> **思考**
>
> 求数组中两数异或的最大值。枚举所有数对是平方级的，$n\le 2\times 10^5$。
>
> 从高位到低位确定答案：假设当前位能取 $1$，则在已有前缀集合中检查是否存在两个前缀异或等于该假设。能取则保留，否则该位为 $0$。
...
> **思考**
>
> 方法一按位枚举并维护前缀集合。二进制前缀树按高位到低位插入后，查询时尽量走相反位，使该位异或为 $1$，对每个数求与树中某数的最大异或再取全局最大。
