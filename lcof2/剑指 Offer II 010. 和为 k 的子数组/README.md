...
> **思考**
>
> 统计和为 $k$ 的连续子数组个数。数组含负数，滑动窗口不再单调，前缀和之差 $s_j-s_i=k$ 才是充要条件。
>
> 若枚举右端再回扫左端，仍是平方时间。把已经出现过的前缀和及其次数放入哈希表，到达 $s$ 时查询 $s-k$ 的次数即可在均摊常数时间内累加答案。初始放入 $0\mapsto 1$，以计入从开头出发的子数组。
