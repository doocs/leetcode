...
> **思考**
>
> 最底层最左侧的值，等于层序遍历最后一层的第一个节点。逐层 BFS 时，队列头部就是当前层最左节点。
>
> 每层开始时记下队首值，并展开左右孩子。循环结束时记下的便是最后一层最左值。层序保证深度最大的一层最后被处理。
...
> **思考**
>
> BFS 需要 $O(w)$ 队列。若改为 DFS，只需沿树高记录「首次达到的更大深度」。
>
> 先左后右的先序会先碰到每一层最左节点；仅当深度刷新最大值时更新答案。额外空间降为递归栈，结果与层序一致。
