...
> **思考**
>
> 斐波那契满足 $F_n = F_{n-1} + F_{n-2}$。直接递归会重复计算同一子问题；$n \le 30$ 虽小，仍宜改成递推。
>
> 用两个变量滚动保存相邻两项，迭代 $n$ 次即可得到 $F_n$，时间和空间均为线性与常数。
...
> **思考**
>
> 方法一已是 $O(n)$，对本题足够，但同一线性递推在 $n$ 更大时仍想压到对数级。
>
> $F_n$ 可由固定的 $2 \times 2$ 转移矩阵连乘得到。矩阵快速幂把 $n$ 次转移降为 $O(\log n)$ 次乘法，常数略大，适合需要加速同一递推的场景。
