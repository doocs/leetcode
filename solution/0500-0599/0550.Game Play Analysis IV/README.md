...
> **思考**
>
> 留存率是「首次登录次日仍登录」的玩家比例。先要每人的首日，再看次日是否有记录。
>
> 子查询按玩家取 `MIN(event_date)`，再左连接到原表，条件为同一玩家且日期差为 $1$。左连接使次日缺失的行保持 `NULL`，对 `event_date IS NOT NULL` 取平均即得比率。
...
> **思考**
>
> 方法一需要聚合后再连接。窗口可以在同一趟里取出下一次登录日，并给每次登录编号。
>
> `LEAD` 得到次日，`RANK` 标出首次登录。只保留排名为 $1$ 的行，再统计次日恰好相差一天的比例。少一次显式自连接，语义与方法一相同。
