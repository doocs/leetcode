...
> **思考**
>
> 重塑只是按行优先把元素重新切成 $r \times c$。元素总数对不上则无法重塑，返回原矩阵。
>
> 二维下标与一维序号一一对应：新位置 $i$ 来自原矩阵的 $i//n,\ i\% n$，写入 $i//c,\ i\% c$。一次线性搬运即可。
...
> **思考**
>
> 方法一已经用线性下标完成映射。方法二把同一公式写在目标矩阵的循环里，只是遍历顺序从「原下标」换成「新下标」，没有新的渐近改进。
>
> 实现语言不同，对应关系不变。
