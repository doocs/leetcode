...
> **思考**
>
> 缩写形如「首字母 + 中间长度 + 尾字母」，冲突时要加长前缀直到唯一。对每对单词比较前缀在 $n \le 400$ 时尚可，但要反复求最长公共前缀。
>
> 只有长度与尾字母都相同的单词才会缩写冲突，故按 $(|w|, w[-1])$ 分组。组内插入字典树并在节点上计数；查询时走到计数为 $1$ 的深度就是可区分前缀长度。短于「缩写所能节省」时退回原词。
