...
> **思考**
>
> 每次消除一段连续同色，得分为长度平方，消除顺序影响后续拼接。区间 DP 的状态还要带上右侧已经连着的同色个数，否则无法表达「先消中间再与右边合并」。
>
> $dfs(i,j,k)$ 表示区间 $[i,j]$ 且 $boxes[j]$ 右侧已有 $k$ 个同色。先把 $j$ 左侧连续同色收进 $k$，再选择直接消掉 $j$ 或找到更左的同色 $h$ 先清开中间再与 $j$ 合并。记忆化避免重复区间。
