...
> **思考**
>
> 把每个节点改成「不小于它的所有值之和」。BST 中这些值正好是反中序（右-根-左）已经走过的节点。
>
> 反中序累加到 $s$，再写回当前节点。右子树先于左子树处理，保证用到的都是更大的键。一次遍历完成。
...
> **思考**
>
> 递归反中序需要 $O(h)$ 栈。Morris 用右子树的最左空指针暂存后继，把递归栈省掉。
>
> 向右走时穿线，回到节点时累加并写回，再拆线向左。访问顺序仍是右-根-左，语义与方法一相同，额外空间为常数。
