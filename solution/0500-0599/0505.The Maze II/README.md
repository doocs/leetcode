...
> **思考**
>
> 球会沿一个方向滚到墙边才停下，因此边不是单步网格，而是「一次滚动」。朴素搜索若按格子逐步走，会把同一条滚动重复展开。
>
> 状态应是停止点。从起点 BFS，每次向四向滚到尽头，用新路径长度松弛该停止点；更短才入队。由于滚动长度不一，这是按停点松弛的最短路，而不是无权 BFS。到达终点的最短停点距离即为答案，不可达则为 $-1$。
