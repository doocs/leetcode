...
> **思考**
>
> 省份是连通块。邻接矩阵给出城市之间是否直接相连，从每个未访问城市 DFS/BFS 即可染完一块。
>
> 扫一遍城市，遇到未访问的起点就开一次 DFS 并把答案加一。访问数组保证每个城市只进入一次。
...
> **思考**
>
> DFS 依赖递归栈与访问数组。并查集按边合并：初始 $n$ 个分量，每条互连且尚未同根的边把答案减一。
>
> 只枚举上三角避免重复边。路径压缩保证查找接近常数。最终答案即剩余根的个数。
