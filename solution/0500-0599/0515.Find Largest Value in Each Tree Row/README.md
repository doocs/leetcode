...
> **思考**
>
> 每一层的最大值可在层序遍历时当场求出。将根入队，按层弹出，维护本层最大值并加入孩子。
>
> 一层处理完就得到一个答案。空树直接返回空列表。时间与节点数成正比。
...
> **思考**
>
> BFS 按层很自然，但需要队列。DFS 只要带着深度往下走：深度等于答案长度时追加，否则与已有值取最大。
>
> 先序保证每一层第一次到达时创建槽位。额外空间是递归深度，结果与层序相同。
