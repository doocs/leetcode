...
> **思考**
>
> 每次在资本允许的项目中选利润最大者，共做 $k$ 次。若每次扫描全部项目，$O(kn)$ 在 $n,k \le 10^5$ 下偏慢。
>
> 资本门槛只升不降，因此可先按启动资本建小根堆；当前资本够格的项目再进入按利润排序的大根堆。每次从大根堆取出最大利润并更新资本。两个堆保证「刚解锁」与「最优可选」都是堆顶操作，总复杂度受排序与堆操作支配。
