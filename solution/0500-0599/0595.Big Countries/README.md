...
> **思考**
>
> 大国定义为面积或人口达到阈值。一行过滤即可。
>
> `WHERE area >= 3000000 OR population >= 25000000` 选出符合任一条件的国家。两谓词用或连接，避免漏掉只满足一侧的行。
...
> **思考**
>
> 方法一用 `OR`。也可以把两个条件写成两次查询再用 `UNION` 合并，自动去重。
>
> 当优化器对 `OR` 不走索引时，两次范围扫描再合并可能更稳。结果集合与方法一相同。
