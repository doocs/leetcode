...
> **思考**
>
> 最短无序子数组排序后应使整个数组有序。把数组排序再与原数组比较，两端第一个不同的位置就是要排序的区间。
>
> $n \le 10^4$，排序可以接受。若两端从未错位则数组已有序，长度为 $0$。
...
> **思考**
>
> 排序多花了对数因子。从左扫，一旦当前值小于左侧最大值，它就必须落入待排序区间，右端点持续更新；从右扫则用右侧最小值更新左端点。
>
> 两端初值为 $-1$，若右端从未更新则返回 $0$。线性两遍，空间常数。
