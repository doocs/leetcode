...
> **思考**
>
> 转盘每次转到 `key` 的下一个字符，可顺时可逆时，还要按确认键。若对每个字符枚举所有环上位置，朴素搜索分支随 `|key|` 增长。
>
> 设 $f[i][j]$ 为拼出 `key` 前 $i+1$ 个字符且指针停在 $j$ 的最少步数。同一字符的候选位置可预处理。从上一字符的位置转移到当前位置，代价是环上较短弧长再加一次确认。最后在 `key` 末字符的所有位置上取最小即可。
