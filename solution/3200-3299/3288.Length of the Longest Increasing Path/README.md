...
> **思考**
>
> 二维点要严格 $x$、$y$ 都递增，且必须经过给定点 $k$。$n\le 10^5$，对点做 $O(n^2)$ 的 LIS 不可行。路径是「$k$ 左侧的 LIS」加「$k$ 右侧的 LIS」减一。
>
> 按 $x$ 排序、$y$ 用树状数组或二分维护 LIS；左侧只看严格小于 $k$ 的点，右侧只看严格大于 $k$ 的点。题面暂无实现代码，思考依此拆成两段二维 LIS。
