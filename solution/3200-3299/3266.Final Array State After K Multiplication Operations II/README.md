...
> **思考**
>
> 规则与 I 相同，但 $k\le 10^9$，不能逐次模拟。注意到一旦所有值都不小于原数组最大值 $m$，之后每次都是「当前最小变为新的最大」，每 $n$ 次恰好每人一轮。
>
> 先用堆把仍小于 $m$ 的元素乘到 $\ge m$ 或用尽 $k$。剩余次数按 $k//n$ 与 $k\% n$ 分配，快速幂写入并取模。$\textit{multiplier}=1$ 时直接返回。
