...
> **思考**
>
> $n\le 100$ 且组长度固定为 $3$，枚举每个起点再检查三格是否交替即可。环上越界需要取模，写法略繁。
>
> 将环展开为长度 $2n$ 的线性扫描后，用 $\textit{cnt}$ 记录当前交替段长度，相邻同色则重置。仅当 $i\ge n$ 且 $\textit{cnt}\ge 3$ 时计入，从而每个原位置作为组的右端点恰好统计一次，额外空间 $O(1)$。
