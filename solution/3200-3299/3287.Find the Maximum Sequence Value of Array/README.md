...
> **思考**
>
> 从左右各选 $k$ 个数，求两段 OR 之 XOR 的最大。$n\le 400$、$k\le n/2$，值 $<128$，枚举划分点后子集 OR 可用布尔 DP。
>
> $f[i][j][x]$ 表示前 $i$ 个里选 $j$ 个能否 OR 出 $x$，后缀 $g$ 对称。在切点 $i\in[k,n-k]$ 枚举能达到的 $x,y$，更新 $x\oplus y$。值域 $2^7$ 使三维布尔表可接受。
