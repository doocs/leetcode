...
> **思考**
>
> 子序列的合法条件只约束相邻两项之和对 $k$ 取模相等。本题 $k=2$，而 $n\le 2\times 10^5$，若枚举子序列或对每个位置再扫一遍前驱，则无法在时限内完成。
>
> 由 $(a+b)\equiv(b+c)\pmod 2$ 可得 $a\equiv c\pmod 2$，即奇数位残量相同、偶数位残量相同。合法序列的形态因此被最后两项的残量对 $(x,y)$ 完全刻画。为此维护 $f[x][y]$：以残量 $x$ 结尾、倒数第二项残量为 $y$ 的最长长度。扫到 $x$ 时枚举目标和 $j$，由 $y=(j-x)\bmod 2$ 从 $f[y][x]$ 转移，一次线性扫描即可。
