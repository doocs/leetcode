...
> **思考**
>
> 题目要求的子矩阵必须包含左上角 $(0,0)$，故候选只有 $O(mn)$ 个，而 $m,n\le 10^3$。若每次重新扫描子矩阵则总时间立方，无法接受。
>
> 二维前缀和可在 $O(1)$ 内得到 $[0,0]$ 到 $(i,j)$ 中 `X` 与 `Y` 的个数。扫到每个格点时用容斥更新前缀，若 `X` 个数为正且与 `Y` 相等则计入。一遍 $O(mn)$ 即可。
