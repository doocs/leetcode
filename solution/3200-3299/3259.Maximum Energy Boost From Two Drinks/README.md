...
> **思考**
>
> 每小时喝 A 或 B，换饮料须空一小时。$n\le 10^5$，枚举切换点不可行。第 $i$ 小时的最优只依赖上一小时喝的是哪一种。
>
> $f[i][0]$ / $f[i][1]$ 分别为第 $i$ 小时选 A / B 的最大能量：可继续同一种并加上当日值，或从上一种「空档」转移而不加当日。答案取最后一行较大者。
...
> **思考**
>
> 方法一的转移只读 $f[i-1]$，整表可以压成两个变量。滚动 $f,g$ 分别表示当前选 A、选 B 的最优，空间降为 $O(1)$，时间不变。
