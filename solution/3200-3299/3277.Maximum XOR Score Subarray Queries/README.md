...
> **思考**
>
> 子数组的「异或得分」按题面递归定义，查询次数 $10^5$、$n\le 2000$，不能每次现算。该得分等于区间 DP：$f[i][j]=f[i][j-1]\oplus f[i+1][j]$，且询问要的是区间内任意子区间得分的最大值。
>
> 顺带维护 $g[i][j]=\max(f[i][j],g[i][j-1],g[i+1][j])$。按长度（或 $i$ 降序 $j$ 升序）填表后，每个询问 $O(1)$ 取 $g[l][r]$。
