...
> **思考**
>
> 单次查询的 $k$-约束计数可用 I 的滑窗在线性内完成，但查询达 $10^5$、$n$ 亦 $10^5$，必须预处理。对每个左端，右端的合法范围是一段前缀，滑窗可求出每个 $i$ 右边第一个非法位置 $d[i]$。
>
> 再让 $\textit{pre}[j]$ 为「右端 $\le j$」的合法子串个数。询问 $[l,r]$ 拆成「左端为 $l$ 且右端不超过 $\min(r+1,d[l])-1$ 的三角形个数」加上 $\textit{pre}[r+1]-\textit{pre}[p]$。预处理 $O(n)$，询问 $O(1)$。
