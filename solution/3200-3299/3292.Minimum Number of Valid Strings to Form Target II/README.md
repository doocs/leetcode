...
> **思考**
>
> 题意与 I 相同，但 $|target|$ 与词长达到 $5\times 10^4$，字典树加记忆化的 $O(n^2)$ 不再可行。从 $i$ 能匹配的最长前缀对长度具有单调性，可用哈希加二分在对数时间内求出。
>
> 按前缀长度分组存单词哈希。$f(i)$ 二分最长 $L$ 使 $\textit{target}[i..i+L)$ 的哈希落在该长度集合中。得到每起点的最远覆盖后，问题化为「跳跃游戏」：维护当前能到的最远，到边界就再跳一次。
