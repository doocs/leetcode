...
> **思考**
>
> 要从 $(0,0)$ 走到对角且不碰圆。坐标达 $10^9$，离散化网格不可行；圆至多 $10^3$，障碍由圆及其在矩形内的相交并决定。
>
> 若起点或终点落在某圆内则不可达。若干圆在矩形内相交并同时碰到「左或上」与「右或下」，则把对角路径切断。从碰到左/上边的圆 DFS，沿矩形内相交边走到碰到右/下边的圆则失败。几何判定控制合并是否发生在矩形内部。
