...
> **思考**
>
> 子串需满足 $c_1\ge c_0^2$。$n\le 4\times 10^4$，枚举两端再计数为平方。由不等式知 $c_0\le\sqrt{n}$，固定左端后只需跳着枚举接下来的若干个 $0$。
>
> 预处理 $\textit{nxt}[i]$ 为 $i$ 起第一个 $0$ 的位置。对每个左端 $i$，沿 $0$ 的位置跳转并更新 $c_0$，在 $c_1$ 足够时把右端可滑动的长度计入。每起点至多 $O(\sqrt{n})$ 次跳跃。
