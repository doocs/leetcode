...
> **思考**
>
> 本题将模数从 $2$ 推广到任意 $k\le 10^3$，且 $n\le 10^3$。枚举子序列仍不可行，但相邻和同余同样推出奇数位残量相同、偶数位残量相同，状态仍只需最后两项的残量。
>
> $n\times k\le 10^6$，足以对每个数枚举目标和 $j\in[0,k)$，由 $y=(j-x+k)\bmod k$ 更新 $f[x][y]=f[y][x]+1$。与 $k=2$ 时同一套转移，只是表的边长改为 $k$。
