...
> **思考**
>
> 环上要支持改色，并查询长度为 $\textit{size}$ 的交替组个数。$n,q\le 5\times 10^4$，每次按 I/II 的线性扫描会超时。交替组由极大交替段决定，改一点只影响邻近两段的拆合。
>
> 用有序集合维护段，并用树状数组按段长统计「长度为 $x$ 的段有多少」。查询 $size$ 时，段长 $\ell$ 贡献 $\max(\ell-\textit{size}+1,0)$ 个组，前缀即可回答。题面暂无实现代码，思考依此段树/树状数组框架。
