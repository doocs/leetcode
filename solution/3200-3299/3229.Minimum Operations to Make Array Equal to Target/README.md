...
> **思考**
>
> 一次操作给一段子数组同时加一或减一，要把 $\textit{nums}$ 变成 $\textit{target}$。$n\le 10^5$，区间枚举不可行。差值数组 $d_i=\textit{target}_i-\textit{nums}_i$ 上，一段同号区间可以共享前缀操作。
>
> 同号时只需为多出的绝对值再付费；变号则必须为新值单独付 $|d_i|$。从左扫，维护与前一项的关系累加即可，额外空间 $O(1)$。
