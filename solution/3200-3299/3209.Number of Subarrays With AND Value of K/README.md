...
> **思考**
>
> $n\le 10^5$，枚举全部子数组的按位与为 $O(n^2)$，无法接受。固定右端点后，左端点左移只会使与值单调不增，且值域不超过 $10^9$，不同与值至多约 $30$ 种。
>
> 为此用计数表维护「以上一位置为右端的与值 $\to$ 出现次数」。扫到 $x$ 时，将表中每个旧值与 $x$ 相与得到新表，再计入单点 $x$，并把值为 $k$ 的次数累入答案。每种右端点只处理对数个键。
