...
> **思考**
>
> 每次把某个 $10$ 变成 $01$，即把一个 $1$ 右移一格。$n\le 10^5$，模拟每一步移位次数可能达平方。一次操作对应「某个 $1$ 越过其后的一个 $0$」，且同一段 $1$ 可以反复越过后续每个 $0$ 段。
>
> 从左统计已出现的 $1$ 的个数 $\textit{cnt}$，每当遇到由 $1$ 转入 $0$ 的边界，这 $\textit{cnt}$ 个 $1$ 都可以再向右移动一次，答案加上 $\textit{cnt}$。一遍线性。
