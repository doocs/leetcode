...
> **思考**
>
> 好整数是「存在某个 $k$ 回文数与它数字相同」的 $n$ 位数。$n\le 10$，枚举全部 $n$ 位数不可行，但回文由前半唯一决定，前半约 $10^{\lceil n/2\rceil}$ 个。
>
> 枚举前半拼出回文，能被 $k$ 整除则按其数位多重集合计一次排列（最高位非零），用排序串去重以免同一多重集合重复加。阶乘表计算 $\frac{(n-x_0)(n-1)!}{\prod x_i!}$。
