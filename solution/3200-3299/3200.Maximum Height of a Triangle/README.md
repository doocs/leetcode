...
> **思考**
>
> 高度为 $h$ 时需消耗 $h(h+1)/2$ 个球，故 $h$ 至多为 $O(\sqrt{\textit{red}+\textit{blue}})$。在 $1\le \textit{red},\textit{blue}\le 100$ 下即便枚举高度与每行着色，规模也可接受，但相邻行必须异色，整座三角形的着色由第一行唯一决定。
>
> 因此只需枚举第一行取红或取蓝，再按行长 $1,2,\ldots$ 交替扣减对应颜色的剩余数量。若当前行所需个数已超过该色存量则停止，并在两种起点中取最大可行行数。异或翻转下标即可在常数空间内完成这一过程。
