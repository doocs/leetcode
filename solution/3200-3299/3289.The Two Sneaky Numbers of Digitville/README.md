...
> **思考**
>
> 数组含 $0\ldots n-1$ 各一次，另有两个数各多一次。$n\le 100$，计数后收集频数为 $2$ 的值即可。
>
> 哈希或长为 $n$ 的数组统计出现次数，等于 $2$ 的两个数就是答案。
...
> **思考**
>
> 方法一用了线性额外空间。两个重复数的异或等于「数组全部异或再异或 $0\ldots n-1$」。取该异或值的最高不同位，按这一位把数分成两组分别异或，即可在 $O(1)$ 额外空间分离两个答案。
