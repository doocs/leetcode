...
> **思考**
>
> 每轮固定消耗 $2$ 枚 $75$ 与 $8$ 枚 $10$。$x,y\le 100$，模拟每一轮也可，但轮数由两者整除次数的较小值直接给出。
>
> 令 $k=\min(\lfloor x/2\rfloor,\lfloor y/8\rfloor)$，扣减后再看是否还能再走半轮（仍有一枚 $75$ 且至少 $4$ 枚 $10$）。能则多走的是 Alice，否则 Bob。判断为 $O(1)$。
