...
> **思考**
>
> 要构造长度为 $n$ 且能被 $k$ 整除的最大回文数，$n\le 10^5$、$k\le 9$。从大到小枚举回文不可行；只需定前半，后半由镜像确定，并对 $k$ 取模。
>
> 按 $k$ 的因子分类（如 $1,2,4,5,8$ 只约束尾数，$3,9$ 约束数位和，$6,7$ 需同时照顾模），从高位贪心填 $9$ 并修正最低几位使整模为 $0$。题面暂无实现代码，思考依此「定前半、按 $k$ 修尾」的构造。
