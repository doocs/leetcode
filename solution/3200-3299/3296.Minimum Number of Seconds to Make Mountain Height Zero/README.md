...
> **思考**
>
> 工人 $i$ 降低 $h$ 层耗时 $wt_i\cdot h(h+1)/2$，可并行，求使总降低量达到 $H$ 的最短时间。$H\le 10^5$、工人 $10^4$，枚举时间不可行。时间越大完成量越大，可二分。
>
> $\textit{check}(t)$ 对每个工人解不等式得到其在 $t$ 内最多降低的层数并求和。二分上界取足够大的常数（如 $10^{16}$），`bisect_left` 找最小可行 $t$。
