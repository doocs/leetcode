...
> **思考**
>
> 每行至多选一个、所选值互不相同，求最大和。行列至多 $10$，值 $\le 100$，按行枚举会与「值是否用过」纠缠。改按数值从大到小考虑是否选用，行集合用位压。
>
> $f[i][S]$ 为考虑到值 $i$、已选行集合 $S$ 的最大分。不选 $i$ 则继承 $f[i-1][S]$；选则枚举 $i$ 所在的行 $k\in S$。值到行的映射预处理成集合。
