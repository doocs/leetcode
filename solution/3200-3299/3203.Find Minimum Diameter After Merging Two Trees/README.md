...
> **思考**
>
> 需要在两棵树之间加一条边，使新树直径最小。$n,m\le 10^5$，若枚举连接端点再求直径，则次数为 $nm$ 量级，无法接受。
>
> 新直径只有两类：仍落在某一棵原树内，即 $\max(d_1,d_2)$；或穿过新边，此时最优是把两边的中心附近连起来，长度为两棵树半径之和再加 $1$。因此只要求出两棵树的直径。树上任取一点走最远到达 $a$，再从 $a$ 走最远到达 $b$，$a$ 到 $b$ 即为直径，两次 DFS 即可，总时间线性。
