...
> **思考**
>
> 行与列须同时回文，且 $1$ 的个数为 $4$ 的倍数。四个对称位置必须同值，各取变为全 $0$ 或全 $1$ 的较小代价。中心行/列的配对会牵动 $1$ 的个数模 $4$。
>
> 先处理 $2\times 2$ 轨道。若行列皆奇，正中心只能翻成 $0$。中轴上成对格子：不同则记 $\textit{diff}$，同为 $1$ 则记入 $\textit{cnt1}$。$\textit{cnt1}\bmod 4=0$ 或存在 $\textit{diff}$ 时用 $\textit{diff}$ 次即可凑模，否则还需额外 $2$ 次。
