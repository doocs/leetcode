...
> **思考**
>
> 要把 $nums[i]$ 拆成 $arr1[i]+arr2[i]$，且 $arr1$ 不降、$arr2$ 不升。$n\le 2000$、$nums[i]\le 50$，若每位枚举拆法再校验则指数。状态可取「考虑到 $i$ 且 $arr1[i]=j$」。
>
> $arr1$ 不降要求上一值 $j'\le j$；$arr2$ 不升给出 $j'\le j+nums[i-1]-nums[i]$。右端是一段前缀，对 $f[i-1]$ 做前缀和后 $O(1)$ 转移。答案为最后一行合法 $j$ 之和。
