...
> **思考**
>
> 与上一题类似，但查询保证新边互不交叉覆盖，规模更大，每次 BFS 不再可行。一条 $u\to v$ 会吃掉中间所有必须经过的城市，最短路长度减少被覆盖的点数。
>
> 用 $\textit{nxt}[i]$ 表示 $i$ 当前跳到的下一城。若新边真正缩短，则沿 $\textit{nxt}$ 把 $[u,v)$ 内旧跳重置并减少 $\textit{cnt}$。每个位置至多被删一次，总时间线性。
