...
> **思考**
>
> 每次询问后都重新枚举子序列并计算 GCD，在 $n,q\le 5\times 10^4$ 下无法接受。注意到只有 $p$ 的倍数可能进入 GCD 恰为 $p$ 的子序列，非倍数可视为不参与。
>
> 将每个位置存为「是 $p$ 的倍数则取其值，否则为 $0$」，全体候选的 GCD $g$ 即可在线段树上维护。若 $g\neq p$，答案必为否；若 $g=p$ 且并非全部元素都是倍数，直接取全部候选即可，长度已小于 $n$。
>
> 仅当 $n\le 6$ 且全部元素都是倍数时，才需要删去一个位置并用区间 GCD 判断剩余是否仍为 $p$。$n>6$ 时，整体 GCD 已为 $p$ 则必可删去一个元素而不破坏该条件。
