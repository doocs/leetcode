...
> **思考**
>
> $n,m\le 10^3$，对阈值暴力枚举全部边权再求最短路可以接受量级，但仍有更干净的判定。一条边相对阈值是轻或重，有效路径至多含 $k$ 条重边。
>
> 若固定阈值 $T$，将 $w\le T$ 的边视为代价 $0$、$w>T$ 的边视为代价 $1$，则存在有效路径当且仅当 $0$–$1$ 最短路不超过 $k$。阈值对判定单调，可在边权上二分 $T$。
>
> 仓库中该题尚无实现代码，思考止于「二分阈值 + 重边次数最短路」。
