...
> **思考**
>
> $n\le 10^5$，不能枚举翻转集合。禁止子序列 `011` 与 `110`，意味着串中不能同时「先出现 $0$ 再出现两个 $1$」或「先出现两个 $1$ 再出现 $0$」的跨位置组合。
>
> 连贯串的形态因此非常受限：大致只能是全 $0$、全 $1$，或「若干 $1$ 后接若干 $0$」、以及避免两段 $1$ 被 $0$ 隔开的少数模式。最少翻转等于把原串改成某一种合法形态的汉明距离最小值。
>
> 仓库中该题尚无实现代码，思考止于「合法串形态有限、答案为到这些形态的最小修改次数」。
