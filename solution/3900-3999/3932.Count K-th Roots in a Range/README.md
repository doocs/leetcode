...
> **思考**
>
> $r\le 10^9$，不能枚举区间内每个整数。完全 $k$ 次幂的个数等于满足 $l\le x^k\le r$ 的非负整数 $x$ 的个数。
>
> $k=1$ 时答案为区间长度。否则 $x$ 至多约 $r^{1/k}$，对 $x=0,1,2,\ldots$ 计算 $y=x^k$，越上界即停，落在 $[l,r]$ 则计数。
>
> 枚举量随 $k$ 增大迅速变小，足以覆盖 $k\le 30$。
