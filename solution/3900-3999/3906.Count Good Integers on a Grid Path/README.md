...
> **思考**
>
> 区间上界达 $9\times 10^{15}$，无法枚举整数。路径由六个 `D`/`R` 决定，在 $4\times 4$ 网格上访问恰好 $16$ 个格子中的一条轨迹，关键格上的数位须关于访问顺序非降。
>
> 将 $l,r$ 上的计数化为 $[0,r]$ 与 $[0,l-1]$ 之差后，对固定 $16$ 位数做数位 DP。状态 $(\textit{pos},\textit{last},\textit{lim})$ 记录当前位、上一个关键格数字以及是否贴上界。
>
> 非关键格可从 $0$ 起填；关键格的下界为 $\textit{last}$。路径上的关键格由 $\textit{directions}$ 预计算为布尔数组 $\textit{key}$。
