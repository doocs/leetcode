...
> **思考**
>
> Alice 选 $k>1$ 与子数组，能被 $k$ 整除的加到她，其余加到 Bob，分数差是「段内 $k$ 的倍数和减去非倍数和」。这等于 $2\cdot(\text{倍数和})-\text{段和}$。
>
> 对每个可能的 $k$，倍数出现位置构成若干段，最大差在这些段的前缀和上取。再在达到最大差的 $k$ 中取最小者，乘差取模。
>
> 仓库中该题尚无实现代码，思考止于「按 $k$ 收集倍数段」。
