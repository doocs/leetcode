...
> **思考**
>
> 一次变换分别循环移位偶下标子序列与奇下标子序列。两个串等价当且仅当它们的偶数位多重集相同且奇数位多重集相同——循环移位不改变多重集，且任意循环可达到同一多重集的任何轮换，对长度较短的一侧通常还要核对是否真能旋成。
>
> 若只比较排序后的偶串与奇串，则等价类由这两串的排序元组刻画，分组数等于不同元组个数。仓库中该题尚无实现代码，思考止于「偶/奇子序列的循环移位不变量」。
