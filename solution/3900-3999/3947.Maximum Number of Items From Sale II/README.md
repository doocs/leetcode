...
> **思考**
>
> 相对 I，物品数与预算都升到很大，背包不再可行。赠送图由因子关系决定，$\textit{factor}$ 不超过 $n$，可按倍数计数每个因子能带走多少种类。
>
> 最优策略仍是「花一笔钱触发一次赠送，其余预算全买最便宜」。需要在线性或线性对数时间内对每个候选第一种物品算出赠送量，再与单价比较。
>
> 仓库中该题尚无实现代码，思考止于「倍数计数替代背包」。
