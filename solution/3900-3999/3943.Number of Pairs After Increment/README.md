...
> **思考**
>
> $\textit{nums1}$ 长度至多 $5$，区间加与配对询问却达 $5\times 10^4$。暴力对每次类型 $2$ 枚举两数组可以过 $\textit{nums1}$ 一侧，但仍需快速统计 $\textit{nums2}$ 中等于 $tot-a$ 的个数，并支持区间加。
>
> 对 $\textit{nums2}$ 用树状数组或线段树维护值域计数，区间加可用懒标记或差分把计数整体平移。因 $\textit{nums1}$ 极短，每次询问对其中每个 $a$ 查询 $tot-a$ 的出现次数。
>
> 仓库中该题尚无实现代码，思考止于「短数组枚举 + 长数组可区间加的频次结构」。
