...
> **思考**
>
> 规则覆盖一段且位置不可再用，因此是把 $\textit{source}$ 划分成若干互不重叠的匹配段，每段用一条规则变成 $\textit{target}$ 的对应段。通配符 `*` 增加额外费用。
>
> 对每个起点预处理能覆盖到的规则，再在下标上做最短路：从 $i$ 连向 $i+|\textit{pattern}|$，边权为规则费用加星号个数，仅当模式匹配且替换结果等于目标子串。
>
> 仓库中该题尚无实现代码，思考止于「下标最短路拼接规则」。
