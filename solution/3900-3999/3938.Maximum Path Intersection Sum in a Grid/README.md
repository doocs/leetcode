...
> **思考**
>
> 两条路径交叉的共享格必须同时落在「左上到右下」与「左下到右上」的某对路径上。$m,n$ 达 $10^3$，枚举路径对不可行。
>
> 共享格大致形成从某一列穿越的一条「走廊」。对每个可能的相交带，把两条路径在带外的强制走法与带内格子值相加，可用前缀 DP 预计算从四角到达每格的最优部分和。
>
> 仓库中该题尚无实现代码，思考止于「四角到格的路径 DP + 枚举相交带」。
