...
> **思考**
>
> 树高可达 $n$，递归按层收集后再按之字形取和，空间与实现都偏重。题目要求的是每一层按方向截取到「缺少对应子结点」为止的结点值之和，而非整层之和。
>
> 层序遍历自然按层给出结点。用布尔量 $\textit{left}$ 标记当前方向，先把下一层入队，再按该方向扫描本层：取 $q[i]$ 或 $q[m-i-1]$，并检查其左/右孩子是否存在；一旦缺失则停止累加。
>
> 方向在层间翻转，队列替换为下一层，即可在一次 BFS 中得到全部答案。
