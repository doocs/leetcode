...
> **思考**
>
> 对每个位置再扫一遍左侧最大与右侧最大，时间为 $O(n^2)$。$n\le 100$ 可以接受，但判定条件只依赖左右两侧的最大值。
>
> 从右向左预处理后缀最大值 $\textit{right}$，从左向右维护前缀最大值 $\textit{left}$。元素合法当且仅当它严格大于 $\textit{left}$、位于末位、或严格大于右侧后缀最大。
>
> 一次预处理加一次扫描即可收集全部合法元素。
