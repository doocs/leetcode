...
> **思考**
>
> 格子规模 $n\cdot m\le 10^5$，逐源独立 BFS 会在重叠区域重复染色。同一时刻多个颜色到达同一格时须取较大颜色，因此各源必须按时间同步扩散。
>
> 多源 BFS 把所有源同时入队，每一时间步用哈希表 $\textit{vis}$ 汇聚本步新触及的格子，并对同一格保留最大颜色，再统一写入答案并进入下一轮。
>
> 已着色格子不再扩展，保证每个格子至多被确定一次，总时间为网格规模的线性。
