...
> **思考**
>
> 子序列排序只能把所选位置变成非降的 $0$、$1$，等价于在这些位置上把 $0$ 尽量左移。目标串中的 `?` 可任填，故对每个模式判断是否存在一种填法，使 $s$ 能通过若干次「把一些 $1$ 右移过 $0$」变成它。
>
> $0$ 的个数必须匹配（`?` 灵活），且 $s$ 中每个 $1$ 的位置不能比目标中对应 $1$ 更靠右到无法通过左移 $0$ 调整。仓库中该题尚无实现代码，思考止于「子序列排序 = 0 左移，再与模式匹配」。
