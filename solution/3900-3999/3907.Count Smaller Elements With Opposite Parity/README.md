...
> **思考**
>
> 对每个位置向右扫描更小且奇偶相反的元素，时间为 $O(n^2)$，在 $n\le 10^5$ 下无法接受。所需信息只依赖「尚未处理、奇偶相反、且数值更小」的个数。
>
> 从右向左扫描时，右侧元素恰好是尚未处理的集合。用两个有序列表分别存放偶数与奇数，对当前值在相反奇偶的列表中做 $\textit{bisect\_left}$，再把自身插入对应列表。
>
> 这样每次查询与插入均为对数时间，整体 $O(n\log n)$。
