...
> **思考**
>
> 询问中 $k$ 在相邻两次之间变化不超过 $10$，适合把「当前第 $k$ 大所在的一侧」做成滑动窗口。用两个有序集合 $l$、$r$ 分割数组：$r$ 始终保存最大的 $k$ 个元素。
>
> 插入新值后先放进 $r$，再按 $|r|$ 与 $k$ 的关系在两侧之间搬运最小/最大元素，使 $r[0]$ 恰为第 $k$ 大，再快速幂更新 $p$。
>
> 相邻 $k$ 变化很小，摊还搬运次数有限。
...
> **思考**
>
> 方法一要维护两个有序集合并按 $k$ 搬运。本题数据范围同样允许一份有序列表：插入后直接取 $sl[-k]$ 作为第 $k$ 大。
>
> 实现更短，不再依赖「$k$ 变化幅度小」这一条件，常数与代码量都更低。
