...
> **思考**
>
> 若存在长度为 $L$ 的唯一子数组，则更长的子数组必然也存在唯一者（把它扩一格仍只出现一次的那个核）。因此最短唯一长度对 $L$ 单调，可以二分。
>
> 判定「是否存在长度为 $\textit{mid}$ 且全局只出现一次的子数组」时，枚举所有窗口并用哈希计数即可。滚动哈希把窗口比较降为 $O(1)$ 滑动，整次判定 $O(n)$。
>
> 二分 $O(\log n)$ 次判定，总时间 $O(n\log n)$。
