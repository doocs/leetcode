...
> **思考**
>
> 每次插入后若重新排序再取第 $k$ 大，单次 $O(n\log n)$，总长 $2\times 10^4$ 时偏慢且实现啰嗦。需要一个能动态插入并按下标取第 $k$ 大的有序容器。
>
> 有序列表维护当前全部元素，插入 $val$ 后 $sl[-k]$ 即为第 $k$ 大，再把 $p$ 更新为 $p^x\bmod (10^9+7)$。
>
> 插入对数时间，快速幂对数时间，整体 $O((n+m)\log(n+m))$。
