...
> **思考**
>
> 本题与「最小稳定下标 I」定义相同，但 $n\le 10^5$，对每个 $i$ 重算左右最值已不可行。
>
> 不稳定值仍只由前缀最大值与后缀最小值决定，二者均可线性递推。先从右向左写出 $\textit{right}[i]$，再从左向右维护 $\textit{left}$，第一次满足 $\textit{left}-\textit{right}[i]\le k$ 的下标即为答案。
