...
> **思考**
>
> 点数至多 $20$，坐标不超过 $6$，但代数扩张若按点对枚举中点，集合会迅速重复。中点运算 $\lfloor(a+b)/2\rfloor$ 在每个坐标上独立，且坐标很小。
>
> 一维上看，$x$ 能被生成当且仅当它落在已有点该维坐标的凸包内，并满足对 $2^k$ 的同余障碍（向下取整会丢失低位）。三维同时成立时的最小 $k$ 即答案。
>
> 仓库中该题尚无实现代码，思考止于「各维独立、由整除向下取整的可生成性分析 $k$」。
