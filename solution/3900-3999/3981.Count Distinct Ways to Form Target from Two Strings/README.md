...
> **思考**
>
> 从两个串中按递增下标交错取字符拼出 $\textit{target}$，且两边至少各用一次。双串子序列 DP 的状态是「$\textit{target}$ 的前 $i$ 位、word1 用到 $j$、word2 用到 $k$」，还要标记两边是否已用过。
>
> $|word|$ 与 $|\textit{target}|$ 的乘积决定能否三维滚动。转移枚举当前位来自哪一串，并在对应串中跳到下一个匹配字符。
>
> 仓库中该题尚无实现代码，思考止于该双指针子序列计数 DP。
