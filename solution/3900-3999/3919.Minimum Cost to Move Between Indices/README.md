...
> **思考**
>
> 数组严格递增且询问多达 $10^5$，不能对每个 $[l,r]$ 现场模拟逐步移动。相邻一步的代价由局部三元的间距大小决定，且左往右与右往左的规则不对称。
>
> 对每条相邻边预先算出「向右走」代价 $c_1$ 与「向左走」代价 $c_2$，分别写入前缀和 $s_1$、$s_2$。询问 $l<r$ 取 $s_1[r]-s_1[l]$，否则取 $s_2[l]-s_2[r]$。
>
> 这样每次询问 $O(1)$，预处理 $O(n)$。
