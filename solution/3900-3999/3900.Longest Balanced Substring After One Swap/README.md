...
> **思考**
>
> 若枚举交换的两个位置再对每个子串统计 $0$、$1$ 的个数，时间复杂度为 $O(n^3)$，在 $n\le 10^5$ 下无法接受。即使只枚举子串两端，交换带来的全局扰动仍使朴素扫描过慢。
>
> 平衡子串中 $0$ 与 $1$ 个数相等，等价于将 `1` 记为 $+1$、`0` 记为 $-1$ 后前缀和差为 $0$。一次交换至多把某段的差调整 $2$，因此除 $\textit{pre}$ 相同的两端外，还需考察差为 $\pm 2$ 的两端，并确认段外仍有可换入的相反字符。
>
> 为此用哈希表记录每个前缀和出现过的位置：最早位置给出最长候选，若该段无法从外部换入所需字符，再退到次早位置。
