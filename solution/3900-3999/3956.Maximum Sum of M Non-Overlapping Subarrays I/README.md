...
> **思考**
>
> 至多 $m$ 段、每段长度在 $[l,r]$，求段和最大。若 DP 再枚举段长，复杂度随 $n,m,r-l$ 而定；I 的数据通常允许 $O(nm(r-l))$ 或前缀和优化后的 $O(nm)$。
>
> 令 $f[i][t]$ 为前 $i$ 个元素至多用 $t$ 段的最大和，转移为不选 $f[i-1][t]$，或枚举最后一段 $[j,i)$ 且长度合法时 $f[j][t-1]+(s_i-s_j)$。至少选一段，答案取 $t\ge 1$ 的最大。
>
> 仓库中该题尚无实现代码，思考止于该段划分 DP。
