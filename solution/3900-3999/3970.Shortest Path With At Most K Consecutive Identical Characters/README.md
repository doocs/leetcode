...
> **思考**
>
> 普通最短路忽略标签，可能走出超过 $k$ 个连续相同字符。状态必须带上「当前连续字符及其已经连续的长度」。
>
> 以 $(\textit{node},\textit{run})$ 为 Dijkstra 状态，$\textit{run}\le k$，转移时若下一标签相同则 $\textit{run}+1$，否则重置为 $1$。$n$ 与 $k$ 的乘积决定状态数。
>
> 仓库中该题尚无实现代码，思考止于「最短路状态扩展连续段」。
