...
> **思考**
>
> 将 $n$ 转为字符串后检查首位与是否出现 $x$，在 $n\le 10^5$ 下可行，但题目只需判断「含 $x$ 且最高位不是 $x$」。
>
> 不断取末位并整除 $10$，即可在算术上遍历各位：途中若某位等于 $x$ 则记下，循环停在最高位。此时同时具备「出现过 $x$」与「最高位不是 $x$」即为有效。
>
> 循环条件取 $n>9$，保证退出时 $n$ 恰为最高位。
