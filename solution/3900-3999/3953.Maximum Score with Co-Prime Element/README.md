...
> **思考**
>
> 选定值 $v$ 后，所有与 $v$ 不互质的位置必须改掉，代价为这些位置的个数，得分 $v$ 减去该代价。$v\le\textit{maxVal}$，需要快速知道数组中有多少个数与 $v$ 不互质。
>
> 按质因子把数组计数到最小质因子或整除计数上，枚举 $v$ 时用容斥得到不互质的个数。$v$ 的上界与数组值决定枚举是否对全体 $\textit{maxVal}$ 进行。
>
> 仓库中该题尚无实现代码，思考止于「枚举选定值 + 互质计数」。
