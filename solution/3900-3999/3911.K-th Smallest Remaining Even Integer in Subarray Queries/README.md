...
> **思考**
>
> 对每个询问枚举正偶数并检查是否落在子数组中，在 $n,q\le 10^5$ 且 $k_i$ 可达 $10^9$ 时不可行。$\textit{nums}$ 严格递增，子数组内被移除的偶数是一段有序集合。
>
> 第 $k$ 个剩余正偶数等于在序列 $2,4,6,\ldots$ 中跳过该段偶数后的第 $k$ 项。因此关键在于快速统计区间内偶数的个数，并据此把 $2k$ 向右平移相应位置。
>
> 仓库中该题尚无实现代码，思考止于上述归约，具体结构需在线性对数时间内支持区间偶数计数与定位。
