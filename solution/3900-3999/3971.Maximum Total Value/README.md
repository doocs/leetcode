...
> **思考**
>
> 第 $t$ 次选下标 $i$ 得到 $\textit{value}[i]-\textit{decay}[i]\cdot(t-1)$，这是一个递减的算术序列。$m$ 次选择应始终取当前还能给出的全局最大下一项。
>
> 每个下标贡献一条递减序列，可用优先队列每次弹出当前最大的下一项并推入下一项，共 $m$ 次。若衰减很快，序列会变为非正则应停止该下标。
>
> 仓库中该题尚无实现代码，思考止于「多路递减序列的堆选择」。
