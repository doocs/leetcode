...
> **思考**
>
> 允许的操作是旋转与翻转的组合，并不能任意重排。排列若能排序，从 $0$ 出发沿环看必须是单调递增或沿反方向单调递增。
>
> 找到 $0$ 的位置 $\textit{zero}$，检查步长 $+1$ 与 $-1$ 两种环序是否递增。对每种合法方向，旋转次数与「先翻再旋再翻」的次数可直接由 $\textit{zero}$ 与 $n$ 算出，取最小；两种方向都不成立则无法排序。
>
> 验证为 $O(n)$，无需模拟每一步操作。
