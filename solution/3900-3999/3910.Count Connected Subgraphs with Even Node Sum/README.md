...
> **思考**
>
> 连通诱导子图的数量在一般图上难以直接计数，但 $n\le 13$，全部非空子集只有 $2^n-1\le 8191$ 个，可以逐个检验。
>
> 先按位权和筛掉奇数和；对偶数和子集，用位掩码 $\textit{vis}$ 标记「不在子集中」的点，再从子集中任一点 DFS，只在子集内走边。若最终 $\textit{vis}$ 填满全部 $n$ 位，则诱导子图连通。
>
> 邻接表保证 DFS 为 $O(n+m)$，总时间 $O(2^n(n+m))$。
