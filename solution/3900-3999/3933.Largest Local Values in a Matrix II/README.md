...
> **思考**
>
> 每个非零格要以自身值 $x$ 为半径检查周围（去掉四角距离恰为 $x$ 的格），朴素实现为 $O(nm\cdot x^2)$，$x$ 可达 $200$，偏紧。局部最大意味着邻域内没有更大值。
>
> 可按值从大到小扫描：更大的值已经占据过的区域会屏蔽较小的候选。也可用二维稀疏表在 $O(1)$ 查询矩形最大，再扣掉被忽略的四角。
>
> 仓库中该题尚无实现代码，思考止于「邻域最大值查询 + 按值处理」。
