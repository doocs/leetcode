...
> **思考**
>
> 若对整串排序再写回，辅音位置会被打乱。题目只重排元音，且同频时须保持各元音首次出现的相对顺序。
>
> 先扫描一遍，用计数器统计元音频次，并把首次出现的元音依次记入列表，再按频次非增排序该列表。第二次扫描时，用指针按排序后的顺序消耗次数，把原元音位替换为当前应写入的字母。
>
> 辅音原位不动，频次耗尽则指针右移，保证高频元音优先填满。
