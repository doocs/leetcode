...
> **思考**
>
> $n\le 5\times 10^4$、$k\le 50$，枚举反转子集不可行。一次反转把整棵子树乘 $-1$，两次反转在重叠处抵消；距离至少为 $k$ 限制了链上相邻两次反转的间隔。
>
> 树上 DP：对每个结点记录「距离上次反转还有 $t$ 步」时，选或不选反转的最大子树和。转移沿父边把剩余冷却距离减一，冷却未结束则不能再反转。
>
> 仓库中该题尚无实现代码，思考止于「带冷却距离的树上反转 DP」。
