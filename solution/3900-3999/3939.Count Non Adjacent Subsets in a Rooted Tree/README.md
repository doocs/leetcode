...
> **思考**
>
> $n\le 1000$，子集指数枚举不可行。独立集（父子不同时选）上对 $k$ 取模的计数是树上背包：每个子树维护「选/不选根」且权和模 $k$ 的方案数。
>
> 合并子树时做卷积，注意不选根则子树可任选其合法状态，选根则每个孩子必须走「不选该孩子」的状态。对 $10^9+7$ 取模。
>
> 仓库中该题尚无实现代码，思考止于「树上独立集的模 $k$ 背包」。
