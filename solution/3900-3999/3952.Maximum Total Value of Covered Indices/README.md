...
> **思考**
>
> 每个标记至多左移一格且只能移动一次，标记之间不能重叠到同一下标。$n\le 10^5$，需要线性决策。
>
> 从左到右看，一个标记要么留在原位，要么移到 $i-1$（若该格空且未被占用）。贪心应把标记移向更大的 $\textit{nums}$ 值，并保证左侧空位先被更左的标记使用。
>
> 仓库中该题尚无实现代码，思考止于「每个标记至多左移一格的贪心分配」。
