...
> **思考**
>
> 数组已保证为双调，峰值唯一，不必再验证升降形态。若先找峰再分别求和，需要两次扫描；一次扫描即可同时切开左右两段。
>
> 令 $\textit{l}$ 为递增段之和、$\textit{r}$ 初值为全数组之和。沿相邻对向右走，只要仍在上升，就把新元素并入 $\textit{l}$、把旧元素从 $\textit{r}$ 中去掉（峰值留在右侧）。遇到下降即停。
>
> 最后比较 $\textit{l}$ 与 $\textit{r}$ 即可判定哪一侧更大。
