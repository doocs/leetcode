...
> **思考**
>
> 至多 $k$ 次任意交换，等价于从数组中选出若干「更大的外部元素」换入某段子数组。$n\le 1500$，可枚举子数组两端，再考虑把段内最小的若干元与段外最大的若干元交换，次数不超过 $k$。
>
> 对每个 $[l,r]$ 取出段内最小与段外最大各至多 $k$ 个，排序后做前缀改善。也可用堆在扩展右端时维护。
>
> 仓库中该题尚无实现代码，思考止于「枚举段 + 段内外 top-$k$ 交换」。
