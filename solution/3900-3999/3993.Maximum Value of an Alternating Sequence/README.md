...
> **思考**
>
> 相邻差至多为 $m$，且严格交替升降。为使某个元素尽量大，上升应尽量用满 $m$，下降应尽量只用 $1$，以便下一次再上升。
>
> $n=1$ 时答案为 $s$。否则先升后降可完成 $\lfloor n/2\rfloor$ 次上升，最大峰值为 $s+\lfloor n/2\rfloor(m-1)+1$。先降后升会先变小，不会更好。
>
> $n,s$ 达 $10^9$，必须 $O(1)$ 闭式。
