...
> **思考**
>
> 每次取前缀 MEX 并切掉该前缀，$n\le 10^5$，不能对每个可能的 $k$ 重算 MEX。字典序最大意味着应尽早切出尽可能大的 MEX，即第一次让某个小整数缺失时就切。
>
> 用出现次数维护当前窗口的 MEX，一旦 MEX 无法再靠向右延伸而增大（或继续延伸会把已有 MEX 变小），就在该处切开。重复直至数组空。
>
> 仓库中该题尚无实现代码，思考止于「贪心尽早输出当前能达到的最大 MEX」。
