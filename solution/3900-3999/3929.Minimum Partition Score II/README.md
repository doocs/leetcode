...
> **思考**
>
> 子数组值为 $\mathrm{sum}(\mathrm{sum}+1)/2$，分割分数对段和二次。枚举分割位置的朴素 DP 为 $O(n^2k)$，$n\le 5\times 10^4$ 不可接受。
>
> 二次费用暗示对前缀和做凸优化或四边形不等式加速的 $k$ 段划分。令 $s_i$ 为前缀和，$f[i][t]$ 表示前 $i$ 个元素分成 $t$ 段的最小费用，转移为 $f[j][t-1]+(s_i-s_j)(s_i-s_j+1)/2$。
>
> 仓库中该题尚无实现代码，思考止于「二次段和费用下的 $k$ 段划分 DP」。
