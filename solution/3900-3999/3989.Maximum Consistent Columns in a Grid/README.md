...
> **思考**
>
> 删列后相邻保留列在每一行的差的绝对值不得超过 $\textit{limit}$。列的相对顺序不变，问题是选一条最长的列子序列，使每行都满足相邻差约束。
>
> 列数通常不大时可对列做 LIS 式 DP：$\textit{dp}[j]$ 为以 $j$ 结尾的最长一致列数，转移要求 $j$ 与 $i$ 在所有行上 $|g[r][j]-g[r][i]|\le\textit{limit}$。
>
> 仓库中该题尚无实现代码，思考止于该列 DP。
