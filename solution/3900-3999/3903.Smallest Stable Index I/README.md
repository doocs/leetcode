...
> **思考**
>
> 对每个下标 $i$ 单独扫描 $[0,i]$ 的最大值与 $[i,n-1]$ 的最小值，时间为 $O(n^2)$。$n\le 100$ 可以接受，但同一信息可一次性预处理。
>
> 下标 $i$ 的不稳定值只依赖「左侧（含自身）最大值」与「右侧（含自身）最小值」。后者与 $i$ 无关地从右向左递推，前者则在从左向右扫描时同步维护。
>
> 因此先得到后缀最小值数组 $\textit{right}$，再边走边更新前缀最大值 $\textit{left}$，遇到 $\textit{left}-\textit{right}[i]\le k$ 即返回最小稳定下标。
