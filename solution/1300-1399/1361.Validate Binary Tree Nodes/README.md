...
> **思考**
>
> 左右孩子数组描述 $n$ 个节点，判断是否恰为一棵二叉树。非法情形是多父、成环或不止一个连通块。并查集在连边时检测：孩子已有父、或两端已连通（成环）则失败；成功则合并并减少分量。最后分量必须为 $1$。
...
> **思考**
>
> 并查集要维护父标记与分量。改为先数入度：没有入度 $0$ 则无根；有则从该根 BFS，遇已访问孩子说明多父或环，最后访问数须等于 $n$。无需并查数组。
