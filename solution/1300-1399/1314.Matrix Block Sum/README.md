...
> **思考**
>
> 每个位置要正方形邻域和。若对每个 $(i,j)$ 现场累加，$k$ 较大时为 $O(mnk^2)$。同一子矩形会被多次求和，二维前缀和把任意矩形降为四点容斥。
>
> 先建 $s[i][j]$ 表示左上 $i \times j$ 的和，再对每个格子把窗口裁到矩阵边界，用 $s[x_2+1][y_2+1]-s[x_1][y_2+1]-s[x_2+1][y_1]+s[x_1][y_1]$ 取值。
