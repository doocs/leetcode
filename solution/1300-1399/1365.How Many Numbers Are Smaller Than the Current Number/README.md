...
> **思考**
>
> 对每个 $x$ 统计严格更小的元素个数。双重循环在 $n \le 500$ 可过，但排序后 $x$ 的名次就是答案。复制一份升序数组，再对每个 $x$ 二分第一个不小于它的位置，该下标即更小元素个数。
...
> **思考**
>
> 值域只有 $[0,100]$，不必比较排序。计数后再做前缀和，$s[x]$ 即为小于 $x$ 的个数，线性扫一遍原数组即可作答，去掉 $O(n\log n)$ 的排序。
