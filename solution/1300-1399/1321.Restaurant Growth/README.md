...
> **思考**
>
> 每个日期需要含当日在内往前 $7$ 天的金额总和与日均，且窗口必须满 $7$ 天。按天先聚合金额，再对日期排序做 $\mathrm{ROWS}\,6\,\mathrm{PRECEDING}$ 的窗口和，并用窗口名次丢掉前 $6$ 天，剩下的行即为合法的七日均值。
...
> **思考**
>
> 窗口函数并非唯一手段。取出互异日期 $a$，与明细 $b$ 按日期差在 $[0,6]$ 连接并求和，再限制 $a$ 不早于最小日期加 $6$ 天，同样得到满七日的滑动和，不依赖窗口语法。
