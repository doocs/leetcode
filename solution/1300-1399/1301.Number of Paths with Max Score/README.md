...
> **思考**
>
> 从右下走到左上，每步只能向左、向上或左上。若枚举全部路径再比较得分，$n \le 100$，路径条数随格子增长而迅速增加，无法完成。同一格子会被多条路径到达，且题目同时要求最大得分及其方案数。
>
> 因此从终点倒推：记 $f[i][j]$ 为到达 $(i,j)$ 的最大得分，$g[i][j]$ 为取得该得分的方案数。格子 $(i,j)$ 只可能来自 $(i+1,j)$、$(i,j+1)$、$(i+1,j+1)$ 三者中可达且得分最优的前驱；得分严格更优则覆盖方案数，得分相同则累加。障碍与起点不计入数字，最后对方案数取模。
