...
> **思考**
>
> 每个水龙头覆盖一段 $[i-r_i,i+r_i]$，要覆盖 $[0,n]$ 的最少个数。枚举子集，$n \le 10^4$ 不可行。覆盖同一左端点时，右端更远者更优，问题与跳跃游戏相同：把每个区间改写成「从左端能跳到的最远右端」$\textit{last}[l]$。
>
> 再沿位置推进，维护当前能到的最远 $mx$ 与上一段终点 $\textit{pre}$；走到 $\textit{pre}$ 就新开一个水龙头。若某点 $mx \le i$，则无法覆盖。
