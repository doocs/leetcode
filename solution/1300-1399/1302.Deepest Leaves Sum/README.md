...
> **思考**
>
> 最深一层叶子的和，可以先求树高再扫一遍累加，但两次遍历并无必要。层序遍历按深度推进，每一层的节点值和都可以在出队时当场求出。
>
> 队列为空时所记录的那一层，恰为最深一层。为此用队列存放当前层，逐层覆盖 $\textit{ans}$，遍历结束时 $\textit{ans}$ 即为最深叶子之和。
...
> **思考**
>
> 广度优先需要与最宽一层成正比的额外空间。若改为递归，只需在下降时携带深度：等于当前最大深度则累加，超过则重置和与最大深度。一次遍历即可同时维护最深叶子之和。
