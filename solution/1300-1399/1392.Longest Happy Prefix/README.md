...
> **思考**
>
> 最长真前缀且同时为后缀。$n \le 10^5$ 时从短到长逐一比较是平方级。从最长的真前缀开始，检查 $s[:-i]$ 是否等于 $s[i:]$，第一次相等即为最长快乐前缀；若都不相等则空串。
...
> **思考**
>
> 逐长度比较最坏仍要扫完整段。KMP 的 $\textit{next}$ 数组恰为「真前缀等于真后缀」的最长长度：在 $s$ 末尾加分隔符后求 $\textit{next}$，末位即为所求长度，线性完成。
