...
> **思考**
>
> 把 $s$ 变成回文的最少插入次数。枚举所有插入方案，$n \le 500$，状态空间过大。子问题由左右端点决定：若 $s[i]=s[j]$，只需处理开区间；否则在左侧或右侧插入一个与对端相同的字符，取两者较小者再加一。
>
> 区间互相重叠，同一 $(i,j)$ 会被反复问到。记忆化 $dfs(i,j)$ 后每个区间只算一次，复杂度 $O(n^2)$。
...
> **思考**
>
> 记忆化依赖递归栈与缓存。同一转移写成区间 DP 表 $f[i][j]$ 后，按 $i$ 从大到小、$j$ 从小到大填表，即可保证 $f[i+1][j-1]$、$f[i+1][j]$、$f[i][j-1]$ 均已就绪，去掉递归。
...
> **思考**
>
> 按端点下标填表只是一种合法顺序。改为枚举区间长度 $k$，再枚举左端 $i$（右端 $j=i+k-1$），较短区间先于较长区间计算，转移完全相同，实现上只需一层长度循环。
