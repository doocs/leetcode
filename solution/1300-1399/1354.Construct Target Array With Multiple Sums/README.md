...
> **思考**
>
> 从全 $1$ 反复把某位置换成当前数组和，问能否得到 $\textit{target}$。正向不知选哪一维，$n \le 5 \times 10^4$ 且值可达 $10^9$。逆向时每一步必须还原当前最大值：$mx$ 由「其余和 $t$」替换而来，故上一值是 $mx \bmod t$（避免线性递减）。大根堆反复替换直到全为 $1$，或 $t=0$、$mx-t<1$ 时失败。
