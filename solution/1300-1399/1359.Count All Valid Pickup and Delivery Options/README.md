...
> **思考**
>
> $n$ 个订单，每个有取件、送达且取件必须在送达前，求排列数。$n \le 500$，不能枚举 $(2n)!$ 再过滤。第 $i$ 个订单的送达可当作当前序列的最后一次送达，其取件可插在前 $2i-1$ 个空隙之一，并与前 $i-1$ 个订单的方案相乘，再乘上选择哪一单放最后的 $i$ 种。滚动 $f \leftarrow f \cdot i \cdot (2i-1)$ 并对模取余。
