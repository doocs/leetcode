...
> **思考**
>
> 数组值为相邻差的绝对值之和，允许翻转一段。$n \le 3 \times 10^4$，枚举左右端为平方级。翻转只改动与翻转边界相邻的那几条边。
>
> 不翻转的和记为 $s$。翻转贴着左端或右端时，枚举另一端即可 $O(n)$ 更新。内部翻转把相邻对看成 $(x,y)$，增量写成四个线性组合的最大差，对各组合维护 $a-b$ 的最大与 $a+b$ 的最小，一次扫描得到最优增量。
