...
> **思考**
>
> 统计同时含 $a,b,c$ 的子串个数。$n \le 5 \times 10^4$，枚举两端为平方级。以 $i$ 为右端时，左端只需落到三个字母最近出现位置的最小值（含）之前。维护三个字母的最后下标，每次加上 $\min(d[a],d[b],d[c])+1$。
...
> **思考**
>
> 方法一依赖「最近出现」的闭式。也可用计数窗口：右端纳入后，当三种字母都出现时收缩左端，直到缺一种；此时以 $r$ 为右端的合法左端个数恰为当前的 $l$。两种线性写法，后者不显式存最后位置。
