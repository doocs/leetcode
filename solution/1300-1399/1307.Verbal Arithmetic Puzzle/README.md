...
> **思考**
>
> 将字母映射到互异数字，使竖式成立，且前导零不合法。不同字母至多 $10$ 个，穷举全部排列再验算整式，排列数为 $10!$ 量级，再乘上每次验算的位数，偏紧。
>
> 按列从低位向高位处理，可以把进位作为约束提前剪枝：当前列各加数位减去结果位，加上旧进位后，个位必须为 $0$ 才能进入下一列。已有映射则直接代入；尚未映射则枚举空闲数字，并禁止多位数字的前导零。全部列处理完时进位应为 $0$。
