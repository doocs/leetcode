...
> **思考**
>
> 需要恰好 $\textit{level}$ 跳的好友所看视频，并按出现次数、名称排序。沿好友关系盲目深搜难以卡在精确层数，也容易重复访问。从 $\textit{id}$ 做 $\textit{level}$ 轮广度优先，第 $\textit{level}$ 轮结束时队列里恰为该层好友。
>
> 再用计数表统计这些人所看视频，按 $(\textit{cnt}[v], v)$ 排序后输出名称。访问集合避免同一人进入多层。
