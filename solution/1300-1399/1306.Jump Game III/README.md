...
> **思考**
>
> 从 $\textit{start}$ 每次跳到 $i \pm \textit{arr}[i]$，询问能否到达值为 $0$ 的下标。$n \le 5 \times 10^4$，若对同一位置反复搜索，会在环上空转。每个下标最多进入一次队列即可判定可达性。
>
> 将问题看作有向图上的可达：出队时若值为 $0$ 则成功；否则把该格标为已访问（写入 $-1$），再把合法且未访问的两步入队。访问标记兼作剪枝，保证 $O(n)$。
