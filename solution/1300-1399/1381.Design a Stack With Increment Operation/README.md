...
> **思考**
>
> $\textit{increment}$ 要把栈底 $k$ 个元素都加 $\textit{val}$。若每次扫描栈内，三种操作各 $10^3$ 次尚可，但可以做到 $O(1)$：差分数组 $\textit{add}[i]$ 表示位置 $i$ 及以下应加的增量。增量记在第 $\min(k,\textit{size})-1$ 位；弹出时把该位增量传给下一层并清零，使懒标记随出栈下沉。
