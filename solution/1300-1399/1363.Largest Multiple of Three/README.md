...
> **思考**
>
> 用给定数位组成能被 $3$ 整除的最大整数。贪心删掉模 $3$ 余数最小的若干位即可，但同等长度下还要尽量靠左放较大数位，实现上容易漏边界。数位和模 $3$ 为 $0$ 当且仅当所选集合模 $3$ 为 $0$，因此先升序，再用 $f[i][j]$ 表示前 $i$ 个数中选出和模 $3$ 为 $j$ 的最大个数。
>
> 从 $f[n][0]$ 逆推所选数位：能转移则取当前位。升序保证同等长度时较大位更晚被考虑、因而更先被写入答案；去掉前导零后即为最大整数。无法选出任何位（含单独的 $0$ 之外的空集）则返回空串。
