...
> **思考**
>
> 树形汇报关系下，从负责人把消息传到全员的时间，边权为该员工的通知耗时。$n \le 10^5$，不能对每个叶子单独累加路径。建出下属邻接表后，$dfs(i)$ 为 $i$ 通知完其整棵下属树的时间，等于 $\textit{informTime}[i]$ 加上各直接下属 $dfs(j)$ 的最大者。答案为 $dfs(\textit{headID})$。
