...
> **思考**
>
> 每个节点要改成「同层节点值和减去自己与兄弟的和」。若对每个节点再扫一层，$n \le 10^5$ 会重复求和。同层总和只依赖深度，可先一次性求出。
>
> 第一次 DFS 累加各层和 $s[depth]$；第二次在父节点处用子层总和减去两兄弟的值写回。根没有堂兄弟，置 $0$。
...
> **思考**
>
> 方法一用两次递归并保存全部层和。按层 BFS 时，当前层已在队列中，可先累加下一层节点值，再按父节点扣掉兄弟和写回，省去显式层和数组与第二遍 DFS。
