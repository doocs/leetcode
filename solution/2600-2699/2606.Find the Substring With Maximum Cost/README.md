...
> **思考**
>
> 子串开销是区间价值和，空串开销为 $0$。若枚举左右端点再求和，$n \le 10^5$ 不可行。
>
> 固定右端后，最大开销等于当前前缀和减去历史上的最小前缀和（含空前缀 $0$）。因此扫一遍维护前缀和 $tot$ 及其最小值 $mi$，用 $tot-mi$ 更新答案。
>
> 字符价值先放入哈希表，未出现的字母取默认序号。
...
> **思考**
>
> 方法一显式维护全局最小前缀。同一问题等价于最大子数组和：若当前累计为负，则丢弃前缀再从下一个字符开始。
>
> Kadane 形式 $f=\max(f,0)+v$ 省去前缀数组，答案同样覆盖空串（初值 $0$）。
