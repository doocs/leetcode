...
> **思考**
>
> 答案至多两位，且每位分别来自两个数组。两数组长度不超过 $9$，枚举全部数对即可覆盖「取公共数字」与「拼接两个不同数字」两种形态。
>
> 若 $a=b$ 则一位数字 $a$ 优于任何两位；否则比较 $10a+b$ 与 $10b+a$。全程维护最小值。
...
> **思考**
>
> 方法一对每个数对都比较一次。公共数字一旦存在，最小公共值就是答案，不必再看拼接；若不存在，只需两侧最小值拼接。
>
> 用集合求交后取 $\min$，或直接取 $\min(nums1)$ 与 $\min(nums2)$ 拼接，将双重循环降为线性扫描。
...
> **思考**
>
> 数字只在 $1$ 到 $9$，集合可用位掩码代替。两掩码按位与非空则最低位置即最小公共数字；否则分别取两侧最低位拼接。
>
> 与方法二同一观察，只是把哈希集合换成常数空间的位运算。
