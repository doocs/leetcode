...
> **思考**
>
> 从 $(0,0)$ 每次可向右或向下跳不超过格子上标注的步数，求到达右下的最少访问格数。朴素 BFS 对每个格子枚举全部可达点，$m,n \le 10^5$ 且格子总数达 $10^5$，会重复松弛大量过期跳。
>
> 到达 $(i,j)$ 只需「同行左侧仍能跳到 $j$」或「同列上方仍能跳到 $i$」中距离最小者。对每行、每列用小根堆按距离排序，并弹出已经跳不到当前列（行）的堆顶。
>
> 按行列顺序填 $dist$，能更新则把该格压入对应行堆与列堆。
