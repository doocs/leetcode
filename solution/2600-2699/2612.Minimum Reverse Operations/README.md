...
> **思考**
>
> 每次翻转长为 $k$ 的窗口，求 $1$ 从 $p$ 到每个位置的最少次数。状态有 $n \le 10^5$ 个，若对每个位置枚举全部合法窗口，$k$ 亦达 $10^5$，转移过密。
>
> 窗口平移时，下标 $i$ 的像以步长 $2$ 变化，故奇偶分离。像的范围可由边界公式收成 $[mi,mx]$。
>
> 用两个有序集合保存尚未访问的奇、偶下标（去掉 $banned$ 与起点），BFS 时在有序集上截取 $[mi,mx]$ 内元素并立刻删除，避免重复入队。
