...
> **思考**
>
> `AB`、`CD` 可删除且可能连锁。反复 `replace` 最坏平方。相邻可消时与括号匹配相同，栈顶与当前字符恰好组成这两对则弹出，否则入栈。
>
> 预先压入空哨兵省去空栈判断，剩余长度减一即答案。
...
> **思考**
>
> 方法一显式栈。字符串长度 $\le 100$，也可不断用正则去掉 `AB|CD` 直至长度不再变化，写成递归一行。最坏多次扫描，常数仍可接受。
