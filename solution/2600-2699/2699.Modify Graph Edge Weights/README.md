...
> **思考**
>
> 要把 $-1$ 边改成正权，使 $source$ 到 $destination$ 的最短路恰为 $target$。乱改边权组合过多；$n \le 100$ 允许反复 Dijkstra。
>
> 先忽略 $-1$ 边：若已有正权最短路小于 $target$，无解；等于则把其余 $-1$ 边设成上限以免抄近路。大于则逐条把 $-1$ 边暂设为 $1$ 再求最短路，一旦 $\le target$ 就把该边补到恰好 $target$，其后的 $-1$ 边封顶。
>
> 全程仍短于目标则失败。
