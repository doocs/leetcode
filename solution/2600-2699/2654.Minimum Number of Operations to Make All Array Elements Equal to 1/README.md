...
> **思考**
>
> 一次操作用相邻 gcd 覆盖其中一个数。若已有 $1$，其余位置各需一次覆盖，共 $n-cnt$。否则须先在某处制造 $1$。
>
> gcd 具有区间递降性质，最短能得到 $1$ 的子数组长度 $mi$ 即制造 $1$ 的最少步数 $mi-1$，再把这个 $1$ 扩到其余 $n-1$ 个位置。
>
> $n \le 50$，枚举区间累计 gcd 即可；若全局 gcd 大于 $1$ 则无解。
