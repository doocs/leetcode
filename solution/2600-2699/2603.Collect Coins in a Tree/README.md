...
> **思考**
>
> 收集全部金币后须回到起点，每条边至多走两次。若对整棵树做欧拉遍历，会把远离金币的枝条和「站在距金币 $2$ 步处即可收取」的叶子一并计入。$n \le 10^5$，需要线性剥层。
>
> 无金币的叶子永远不必访问，可反复删去直至剩下的叶子都带金币。题目允许距离 $2$ 内收取，故再剥两层叶子后，剩余连通块才是必须往返的骨架。
>
> 用队列做拓扑式删叶：先清掉无金币叶子，再删两轮当前叶子，最后统计仍两端都在骨架上的边数并乘 $2$。
