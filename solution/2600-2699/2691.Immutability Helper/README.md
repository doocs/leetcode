...
> **思考**
>
> 要对不可变 JSON 做看似就地的修改，却生成新对象且不改原件。深拷贝整棵树在 $10^5$ 次 `produce`、载荷达 $4\times 10^5$ 时浪费未改动的分支。
>
> 用代理记录 mutator 写过的路径，按写时复制沿途分叉，未触及的子树共享原引用。题目保证不删键、不调方法、不写入对象，代理只需拦截赋值与读出。
