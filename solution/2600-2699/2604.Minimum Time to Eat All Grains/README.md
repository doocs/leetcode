...
> **思考**
>
> 若干只鸡同时出发吃完所有谷物，一只鸡一段连续行程的耗时取决于折返。若枚举每只鸡负责哪一段谷物，划分方案随只数指数增长；$n,m \le 2\times 10^4$，不可直接搜索。
>
> 时间 $t$ 具有单调性：能在 $t$ 内吃完则更大时间亦可。排序后，位置更左的谷物应优先交给更左的鸡，于是可用双指针检验给定 $t$。
>
> 对每只鸡，根据谷物在其左或右计算折返代价，尽量向右多吃，直到下一粒会超过 $t$。二分最小可行 $t$ 即可。
