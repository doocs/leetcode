...
> **思考**
>
> 两个排列的前缀公共元素个数。每次重建集合求交，$n \le 50$ 可行，但前缀只增长一个元素。
>
> 分别计数 $A$、$B$ 的出现，对值域 $[1,n]$ 取 $\min$ 再求和，即当前公共个数。
...
> **思考**
>
> 方法一每步 $O(n)$ 扫值域。排列中每个值恰出现两次（两个数组各一次），用 $vis$ 异或翻转：第二次见到时公共数加一，从而 $O(1)$ 更新。
...
> **思考**
>
> 方法二仍用 $O(n)$ 数组。$n \le 50$，可用两个整数作位集，前缀或运算后 `bit_count(x & y)` 即公共个数，额外空间降为常数。
