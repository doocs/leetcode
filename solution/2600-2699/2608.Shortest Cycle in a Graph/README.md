...
> **思考**
>
> 无向图最短环可对每条边「删边后求两端最短路」。$n \le 1000$、$m \le 1000$，每次 BFS 为 $O(n+m)$，总时间 $O(m(n+m))$ 可接受。
>
> 环必然包含某条边 $(u,v)$，删去它后 $u$ 到 $v$ 的最短路长度加 $1$ 即含该边的最短环。对全部边取最小；若均不连通则无环。
...
> **思考**
>
> 方法一对每条边各做一次 BFS。改为以每个顶点为源做一次 BFS：首次遇到非父节点的已访问邻居，即闭合一条环，长度为两边距离之和加 $1$。
>
> 边数较大时源点次数更少，复杂度变为 $O(n(n+m))$，实现上无需反复删边。
