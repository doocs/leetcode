...
> **思考**
>
> 完全连通分量要求块内任意两点都有边。枚举块后再两两检查边，$n \le 50$ 尚可，但一遍 DFS 已能同时得到点数与边数。
>
> 无向图中边被两端各计一次，故完全图满足 $x(x-1)=y$。对每个未访问点 DFS 累加 $(x,y)$ 并检验该等式。
...
> **思考**
>
> 方法一显式统计边数。改为对每个点保存含自身的邻域集合：同一完全分量里这些集合应相同，且集合大小等于该集合出现次数。
>
> 按集合分组计数，避免单独判断边数，适合点少、要用邻域判等的写法。
