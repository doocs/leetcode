...
> **思考**
>
> 每个位置至多减一次质数，且须使整个数组严格递增。若从左向右贪心，当前值压得过低可能让后续无法衔接；若对每个位置枚举全部质数，$n \le 1000$ 且值域不超过 $1000$，时间尚可，但需要确定扫描方向。
>
> 注意到右端 $nums[n-1]$ 越大越宽松，因此从右往左处理：当 $nums[i] \ge nums[i+1]$ 时，必须减去一个严格大于 $nums[i]-nums[i+1]$ 且小于 $nums[i]$ 的质数，并取其中最小者，以免过度压缩左侧空间。
>
> 为此先筛出 $1000$ 以内质数表 $p$，再在 $p$ 上二分定位满足下界的最小质数。若无此类质数则不可行。
...
> **思考**
>
> 方法一每次在质数表上二分，定位最小可行质数。值域仅 $1000$，可改为按下标直接取：令 $p[x]$ 表示不小于 $x$ 的最小质数，则 $nums[i]-nums[i+1]$ 的下界对应一次数组访问即可，省去二分。
>
> 其余仍从右向左、每次减去该最小质数，逻辑与方法一相同。
