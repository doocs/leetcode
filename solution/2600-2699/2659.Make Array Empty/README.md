...
> **思考**
>
> 每次可循环左移或删除当前最小值，求清空次数。直接模拟每次旋转，$n \le 10^5$ 不可行。删除顺序即升序，代价是相邻两个最小值在环上的距离，并扣除中间已删位置。
>
> 记录原下标后排序，用有序列表维护已删下标，间隔 $j-i$ 减去其间已删个数；若后一个下标更靠前，还需加上本轮剩余长度（绕环一周）。
...
> **思考**
>
> 方法一用有序列表查区间已删个数。树状数组同样支持前缀计数：下标 $+1$ 后，`query(j+1)-query(i+1)` 即其间已删量，更新为单点加一。其余绕环逻辑不变。
