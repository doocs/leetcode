...
> **思考**
>
> 相邻长度为 $k$ 的子数组和相等，化简后得到 $arr_i=arr_{i+k}$。数组又是环状的，故间隔 $k$ 与间隔 $n$ 的完全剩余系必须取同一值。
>
> 由裴蜀定理，这些下标按 $\gcd(n,k)$ 分成若干独立链。链内元素须改成同一个数，操作次数为到某目标的绝对差之和，中位数使其最小。
>
> 因此对每个剩余类排序后取中位数，累加偏差。
