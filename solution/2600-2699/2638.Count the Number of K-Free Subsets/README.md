...
> **思考**
>
> 子集中不能同时包含差为 $k$ 的两数。枚举 $2^n$ 个子集在 $n \le 50$ 时不可行。差为 $k$ 的数模 $k$ 同余，不同余类互不影响。
>
> 同类内排序后，相邻是否恰相差 $k$ 决定能否同时选取，于是化为「相邻互斥」的线性 DP：冲突则 $f[i]=f[i-1]+f[i-2]$，否则每个元素可自由选不选，$f[i]=2f[i-1]$。
>
> 各类方案数相乘，空集计入初值 $f[0]=1$。
