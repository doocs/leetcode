...
> **思考**
>
> 平衡子串形态固定为若干 $0$ 后接同样多的 $1$。$n \le 50$，枚举全部 $O(n^2)$ 子串再 $O(n)$ 检查，总时间 $O(n^3)$ 可通过。
>
> 对每个区间统计 $1$ 的个数，并保证所有 $0$ 都出现在第一个 $1$ 之前，且 $1$ 的个数恰为长度一半。
...
> **思考**
>
> 方法一立方扫区间。合法串只能由一段连续 $0$ 紧接一段连续 $1$ 构成，因此只需维护当前这两段的长度。
>
> 遇 $0$ 时若已有 $1$ 则开启新段；遇 $1$ 则用 $2\times\min(\textit{zero},\textit{one})$ 更新答案。一次遍历即可。
