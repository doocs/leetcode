...
> **思考**
>
> 三种标记的取值固定，若枚举取了几件 $1$、几件 $0$、几件 $-1$，组合数随 $k$ 线性增长。题目要求恰好取 $k$ 件且和最大，而 $k$ 与各计数均不超过 $50$，穷举可行，但并无必要。
>
> 和由 $1$、$0$、$-1$ 线性叠加，多取一件 $1$ 严格优于多取一件 $0$，多取一件 $0$ 又优于多取一件 $-1$。因此最优策略唯一：先取尽 $1$，不足再取 $0$，仍不足才取 $-1$。
>
> 比较 $k$ 与 $\textit{numOnes}$、$\textit{numZeros}$ 即可在常数时间内落到上述三段之一，无需真正构造选取方案。
