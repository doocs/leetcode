...
> **思考**
>
> 窗口第 $x$ 小若为负则输出该值，否则 $0$。值域仅 $[-50,50]$，不必对每个窗口排序。$n \le 10^5$ 时 $O(nk\log k)$ 偏紧。
>
> 用长度 $101$ 的计数数组维护窗口，查询时从负数一侧累加计数，首次达到 $x$ 即第 $x$ 小；若全非负则返回 $0$。
...
> **思考**
>
> 方法一依赖狭小值域。对偶堆把较小的 $x$ 个数放在大根堆一侧，堆顶即候选美丽值，再以延迟删除处理滑出窗口的元素。
>
> 值域更大时仍可 $O(n\log k)$ 维护第 $x$ 小，本题作为对计数法的结构替换。
