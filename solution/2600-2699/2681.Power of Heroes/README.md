...
> **思考**
>
> 子序列贡献为 $\max^2 \cdot \min$。顺序无关，排序后枚举最小值 $a_i$，右侧每个 $a_j$ 作为最大值的系数呈二的幂。直接枚举子序列 $2^n$ 不可行，$n \le 10^5$。
>
> 从右向左维护加权平方和 $p$，当前 $x$ 贡献 $x^3$ 以及 $x \cdot p$，再令 $p \leftarrow 2p+x^2$，即可在线性时间内累加。
