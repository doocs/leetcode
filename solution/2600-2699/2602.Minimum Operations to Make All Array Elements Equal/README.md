...
> **思考**
>
> 每次查询要把全体元素改成同一个目标 $x$，一次操作增减 $1$。若对每个查询线性扫一遍 $nums$，$n,q \le 10^5$，总时间 $O(nq)$ 不可接受。
>
> 元素之间互不影响，操作次数等于 $\sum |nums_i-x|$。排序后，$x$ 将数组分成小于 $x$ 与大于 $x$ 两段，两段的绝对偏差均可由前缀和在 $O(1)$ 内算出。
>
> 因此先排序并做前缀和，再对每个 $x$ 二分出分界下标，分别累计抬升与压低的代价。
