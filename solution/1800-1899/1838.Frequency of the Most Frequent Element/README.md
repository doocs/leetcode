...
> **思考**
>
> 每次把一个元素加一，最多 $k$ 次，求能达到的最大频数。目标值一定是原数组中的某个数，且操作对象应是不大于它的一段连续数。若枚举右端再暴力找左端，$n\le 10^5$ 会超时。
>
> 排序后，长度 $m$ 可行则更短也可行，可二分频数。前缀和判断长为 $m$ 的窗口把全部元素抬到右端值的花费是否不超过 $k$。
...
> **思考**
>
> 方法一额外需要前缀和与二分。窗口花费随右端增大而单调，可用双指针：$i$ 右移时把窗口内已有元素都抬到 $nums[i]$，若花费超过 $k$ 则右移 $j$。线性扫一遍即可得到最大窗口。
