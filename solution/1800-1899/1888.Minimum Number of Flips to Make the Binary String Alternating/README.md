...
> **思考**
>
> 可把串看成环（类型 1 把首字符移到末尾），再翻转若干位变成交替串。枚举所有旋转再逐位比较为 $O(n^2)$，$n\le 10^5$ 不可行。
>
> 目标只有 $0101\ldots$ 与 $1010\ldots$ 两种，且二者代价互补。把 $s+s$ 上长为 $n$ 的窗口看作一次旋转，滑动时增减两端与目标的差异，取 $\min(cnt,n-cnt)$ 的最小值。
