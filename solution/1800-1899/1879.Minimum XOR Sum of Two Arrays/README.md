...
> **思考**
>
> 将 $nums2$ 重排后与 $nums1$ 对应异或再求和，求最小值。$n\le 14$，排列有 $n!$ 种，穷举偏紧，适合状压。
>
> $f[i][j]$ 表示已配 $nums1$ 的前 $i$ 个、所用 $nums2$ 下标集合为 $j$ 的最小异或和。枚举 $j$ 中最后一个匹配的 $k$，由 $f[i-1][j\oplus 2^k]$ 转移。
...
> **思考**
>
> 方法一的三重循环里，状态 $j$ 的二进制 $1$ 的个数应等于 $i$，大量 $j$ 无效。可以去掉 $i$ 这一维，按 $nums1$ 的元素依次更新，并对 $j$ 逆序枚举，使转移用到的是上一轮的值，时间仍为 $O(n^2 2^n)$ 但常系数更好。
...
> **思考**
>
> 也可以按掩码 $i$ 本身枚举：令 $k=i$ 中 $1$ 的个数减一，表示正在放置 $nums1[k]$，再枚举 $i$ 中的某一位作为所用的 $nums2[j]$。这样每个状态只处理一次，空间为 $O(2^n)$，时间为 $O(n\,2^n)$。
