...
> **思考**
>
> 一个传感器会丢掉某个值并把后续数据前移，另一个保持正确。需要判断哪一个损坏，或无法区分。两数组只在错位后不同，暴力对齐所有丢弃位置为 $O(n^2)$，而 $n$ 虽不大，结构上只需一次错位比较。
>
> 先找到第一个不同下标 $i$，此后把 $sensor1[i+1:]$ 与 $sensor2[i:]$、以及反过来的一对比较。哪一侧无法对齐，即对应传感器丢失了数据；两边都能对齐则无法判定。
