...
> **思考**
>
> 每笔订单都可能与当前积压中价格更优的对手盘成交，剩余部分再进入积压。若每次成交都线性扫描全部积压订单，单次匹配可至 $O(n)$，整体为 $O(n^2)$。$n \le 10^5$，无法接受。
>
> 采购只要不高于报价的最低卖单，销售只要不低于报价的最高买单。为此分别用小根堆维护卖单、大根堆维护买单，堆顶即当前最优对手价。按输入顺序弹出并扣减数量，未成交部分再入堆，最后统计两堆剩余数量并对 $10^9+7$ 取模。
