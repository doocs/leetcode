...
> **思考**
>
> 普通前缀树只回答「是否存在」，本题还要返回某单词的出现次数、某前缀下的单词个数，并支持删除。若每次查询都扫描全部已插入字符串，操作次数与串长的乘积会过大。
>
> 在每个节点额外维护两个计数：$v$ 为以此节点结尾的单词次数，$pv$ 为经过该节点的单词次数。插入时沿路径累加 $pv$，终点累加 $v$；查询单词或前缀只需走到对应节点读取计数；删除则沿路径递减，从而各操作仍与单词长度成正比。
