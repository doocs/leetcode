...
> **思考**
>
> CPU 空闲时在已到达任务中选处理时间最短、下标最小者，求执行顺序。若每次在全部任务中扫描，单次 $O(n)$，总时间 $O(n^2)$。$n\le 10^5$，无法通过。
>
> 按入队时间排序，用小根堆存放当前已到达任务的 $(\textit{processingTime},\textit{index})$。时间跳到下一任务或堆顶任务结束，再把新到达者入堆。堆保证每次取出的即调度规则所选任务。
