...
> **思考**
>
> 一次操作是变成上一个字典序排列，问要多少次才能变成最小排列，即有多少个排列严格小于 $s$。$|s|\le 3000$，不能枚举排列。
>
> 从左到右看每一位：比 $s[i]$ 更小的字母放在该位时，后面是剩余字母的多重排列。用预处理的阶乘与逆元在模 $10^9+7$ 下计算 $m\times(n-i-1)!/\prod n_c!$，再从计数中去掉当前字母。各位贡献之和即为答案。
