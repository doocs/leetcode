...
> **思考**
>
> 每次操作把前半段映射到偶数下标、后半段映射到奇数下标，问最少几次回到初始排列。若完整模拟整个数组，单轮为 $O(n)$，轮数最坏也是 $O(n)$ 量级，虽然可通过，但不必维护全部位置。
>
> 一次操作对每个下标的映射是固定的，除 $0$ 与 $n-1$ 外任意位置回到原处即意味着整个排列复原。跟踪下标 $1$：落在前半则变为 $i\ll 1$，否则变为 $(i-(n\gg 1))\ll 1\mid 1$，直至再次等于 $1$，步数即为答案。
