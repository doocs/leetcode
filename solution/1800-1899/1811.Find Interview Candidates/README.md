...
> **思考**
>
> 候选人需满足「连续三场及以上获奖」或「至少三枚金牌」。奖牌分散在三列中，直接按原表筛选难以同时表达连续场次与金牌次数。
>
> 先把金银铜统一成 $(contest\_id,user\_id,type)$，金牌次数可用分组计数得到。连续场次则对同一用户按比赛编号排序，用 $contest\_id$ 减去行号得到常数差，相同差即一段连续比赛；再按该差分组，场数不少于 $3$ 者入选。最后与用户表连接输出姓名与邮箱。
