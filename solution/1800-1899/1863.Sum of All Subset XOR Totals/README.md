...
> **思考**
>
> 求所有子集异或和的总和。$n\le 12$，子集只有 $2^n$ 个，枚举可以通过。
>
> 用掩码 $i\in[0,2^n)$ 表示子集，按位把选中元素异或起来并累加。实现无需递归。
...
> **思考**
>
> 方法一用整数掩码，需要再开一层循环检查每一位。也可以 DFS：对第 $i$ 个元素选择「不选」或「异或进去」，到达末尾把当前异或和加入答案。时间同阶，写成选或不选更直观。
