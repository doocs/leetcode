...
> **思考**
>
> 至多替换 $nums1$ 中一个位置，使 $\sum|nums1[i]-nums2[i]|$ 最小。若对每个位置枚举所有可替换值，时间为 $O(n^2)$。$n\le 10^5$，无法通过。
>
> 先算出不替换时的总和 $s$。替换 $i$ 的收益是 $|nums1[i]-nums2[i]|$ 与「$nums1$ 中最接近 $nums2[i]$ 的值」之差的差。把 $nums1$ 排序后对每个 $nums2[i]$ 二分相邻候选，记录最大收益 $mx$，答案为 $s-mx$。
