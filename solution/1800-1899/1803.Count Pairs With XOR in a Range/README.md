...
> **思考**
>
> 需要统计满足 $low \le nums[i] \oplus nums[j] \le high$ 的下标对数目。若枚举全部数对再判断异或，时间复杂度为 $O(n^2)$。$n \le 2 \times 10^4$，无法通过。
>
> 区间计数可改写成「异或小于 $high+1$ 的对数」减去「异或小于 $low$ 的对数」。将已处理的数按二进制从高到低插入 $0$-$1$ 字典树，并在节点上记录子树大小。查询时根据 $limit$ 的当前位决定：该位为 $1$ 则可把与 $x$ 同位的整棵子树计入答案，再走向相反位；该位为 $0$ 则只能走向相同位。对每个 $x$ 先查询再插入，避免与自身配对。
