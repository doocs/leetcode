...
> **思考**
>
> 题目要求的是连续且严格递增的子数组之和的最大值。若枚举全部子数组再检验是否升序，时间复杂度为 $O(n^2)$。$n \le 100$，该做法可以通过，但会把大量不满足升序的区间一并检查。
>
> 升序一旦在某对相邻元素处断开，则跨越该处的任何区间都不再合法。因此只需从左到右维护当前这一段升序的和 $t$：能接上则累加并更新答案，否则从当前元素重新起段。一次遍历即可得到全局最大值。
