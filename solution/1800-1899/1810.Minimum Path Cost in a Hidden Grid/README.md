...
> **思考**
>
> 网格不可见，只能通过 $\textit{canMove}$、$\textit{move}$ 与 $\textit{isTarget}$ 探路，且移动具有花费。在未知图上直接边走边求最短路，无法保证回退与重复访问的代价被正确计入。
>
> 网格不超过 $100\times 100$，把起点放在 $(100,100)$，用 DFS 配合反向移动完整建图，记录每条边进入格子的花费，并记下终点。图已知后，边权非负，再用堆优化 Dijkstra 从起点计算到终点的最小花费；若 DFS 未发现终点则返回 $-1$。
