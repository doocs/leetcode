...
> **思考**
>
> 三条跑道上有障碍，只能在同一点侧跳，求到达终点的最少侧跳次数。若按位置与跑道搜索全部路径，状态虽为 $O(n)$，但若不记忆化会重复扩张。
>
> 定义 $f[j]$ 为当前点处于跑道 $j$ 的最少侧跳。有障碍的跑道置为无穷；其余跑道可沿原跑道前进，或先跳到当前点上花费最少的跑道再侧跳一次。滚动长度为 $3$ 的数组即可从起点推到终点。
