...
> **思考**
>
> 除最后一段外，每走完一条路若未整点则向上取整休息。可跳过若干次休息，求最少跳过次数使总时间不超过 $hoursBefore$。枚举跳过哪些路为指数级。
>
> $f[i][j]$ 为走完前 $i$ 条路且跳过 $j$ 次的最短用时：不跳则对 $f[i-1][j]+d_i/s$ 向上取整，跳则直接累加。浮点取整用 $eps$ 修正。从 $j=0$ 起找第一个不超过时限的 $j$。
