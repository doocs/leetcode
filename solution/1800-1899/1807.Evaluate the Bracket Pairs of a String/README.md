...
> **思考**
>
> 括号对中的键需要替换为知识库中的值，缺失则改为 `'?'`。若对每个括号都线性扫描 $\textit{knowledge}$，在串长与词条数均达 $10^5$ 时会超时。
>
> 先把 $\textit{knowledge}$ 建成哈希表，再从左到右扫描 $s$：遇 `'('` 找到匹配的 `')'`，用键查询表并写入答案；其余字符原样追加。每个字符访问常数次，替换在期望 $O(1)$ 内完成。
