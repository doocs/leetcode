...
> **思考**
>
> 每个询问要在面积不小于 $\textit{minSize}$ 的房间中找与 $\textit{preferred}$ 编号最接近者。若对每个询问扫描全部房间，$n,k\le 10^5$ 无法通过。
>
> 询问互不影响，可按 $\textit{minSize}$ 升序离线处理。房间同样按面积排序后，用有序集合维护「当前仍满足面积」的编号，面积不够的房间不断删去。在有序编号上二分 $\textit{preferred}$ 的左右邻居即可。
