...
> **思考**
>
> 子序列的 GCD 种类数最多不超过数组最大值 $mx$，但子序列有 $2^n$ 个，$n\le 10^5$，不能枚举。
>
> 一个数 $x$ 能成为某子序列的 GCD，当且仅当数组中 $x$ 的若干倍数的 GCD 恰好为 $x$。对每个 $x\in[1,mx]$ 枚举其倍数并累积 gcd，一旦得到 $x$ 即可计数并停止。倍数枚举的调和级数总复杂度为 $O(mx\log mx)$。
