...
> **思考**
>
> 要对长度为 $m$ 的窗口去掉最小与最大各 $k$ 个数后求平均，且插入频繁。若每次对窗口排序，单次为 $O(m\log m)$，操作次数大时无法接受。
>
> 用队列保存插入顺序，并用三个有序集合分别存放最小 $k$ 个、中间一段与最大 $k$ 个，同时维护中间和 $s$。插入或弹出后把溢出的元素在三个集合之间搬移，使两端长度回到 $k$。平均数即为 $s/(m-2k)$。
...
> **思考**
>
> 方法一要同时维护三棵有序树，实现与平衡条件较繁。也可以只保留一个有序集合表示整个窗口：窗口刚满时建表并累加中间和，之后根据新元素、旧元素落入的名次增量调整 $s$。结构更简单，单次仍是对数时间。
