...
> **思考**
>
> 一组开心当且仅当轮到它时余下甜甜圈恰为 $0$，即此前人数之和能被 $\textit{batchSize}$ 整除。组数不超过 $30$，穷举排列为 $30!$，不可行。
>
> 余数为 $0$ 的组不影响后续，可先全部计入答案。其余组只保留对 $\textit{batchSize}$ 的余数；每个余数至多出现 $30$ 次，可用 $5$ 位计数，全部余数装进一个整数 $state$。记忆化 $dfs(state,mod)$ 枚举再安排一个余数，并在 $mod=0$ 时给当前组加一分。
...
> **思考**
>
> 方法一只按余数个数压缩状态，实现依赖位宽约定。也可以把「尚未安排的组」直接做成二进制掩码：$m$ 为余数非 $0$ 的组数，$m\le 30$ 但相同余数可去重枚举。$dfs(state,x)$ 在掩码上选未用组，前缀余数为 $0$ 时加分。状态数变为 $O(2^m)$，组很少时更直观。
