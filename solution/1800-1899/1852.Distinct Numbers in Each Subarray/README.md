...
> **思考**
>
> 对每个长为 $k$ 的窗口统计不同数字个数。$n\le 10^5$，每个窗口单独用集合去重会超时。
>
> 维护窗口内的频次表：右端进入加一，左端离开减一至 $0$ 则删除。表的大小即当前窗口的不同个数，窗口滑动一遍即可。
...
> **思考**
>
> 方法一用哈希表，常数与哈希开销较大。数值范围不超过 $10^5$，可用数组代替映射。窗口逻辑不变，只是把计数下标换成数值本身。
