...
> **思考**
>
> 从左侧能看到 $k$ 根的排列数，看见当且仅当它是到目前为止的最高。$n\le 1000$，枚举排列不可行。
>
> 设 $f[i][j]$ 为 $i$ 根中恰看见 $j$ 根的方案。最长一根放在末尾则一定看见，前 $i-1$ 根看见 $j-1$ 根；否则末尾是其余 $i-1$ 种高度之一，看见数不变。按此递推到 $f[n][k]$。
...
> **思考**
>
> 方法一的 $f[i][j]$ 只依赖上一行的 $j$ 与 $j-1$，二维数组可以压成一维。为避免覆盖尚未使用的 $f[j-1]$，内层按 $j$ 从大到小更新，空间降为 $O(k)$。
