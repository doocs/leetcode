...
> **思考**
>
> 较短句须能通过在某一处插入一段连续单词变成较长句，即较短句是较长句的一个「前缀加后缀」拼接。若枚举插入位置并逐词比较，单词数虽不大，但逻辑易与「可在多处插入」混淆。
>
> 将两句按空格拆成词数组并保证第一句不短于第二句。从左对齐统计公共前缀，从右对齐统计公共后缀；若二者长度之和覆盖较短句的全部单词，则中间缺口至多一段，恰为一次插入。
