...
> **思考**
>
> 子数组按位或与 $k$ 的最小绝对差。枚举左右端再算或值是 $O(n^2)$。或值随区间变长单调不减，可用双指针把或值压到 $k$ 附近。
>
> 右端纳入时或值只增；过大则左端右移。移出时需按位计数，某位计数归零才能把该位从 $s$ 清掉。
>
> 维护 $cnt[h]$ 与当前或值 $s$，每次更新 $|s-k|$。指针各前进至多 $n$ 次，再乘上位数。
...
> **思考**
>
> 方法一要按位维护计数，实现较长。固定右端时，不同或值至多 $O(\log M)$ 个，因为左端左移只会把或值的某些零位变成一。
>
> 用集合保存以当前右端结尾的全部或值，纳入 $x$ 时对集合每个元素或上 $x$ 并加入 ${x}$。
>
> 对集合内每个值更新 $|y-k|$。集合大小始终为位数级，渐近时间相同而代码更短。
