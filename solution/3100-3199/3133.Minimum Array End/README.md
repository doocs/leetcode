...
> **思考**
>
> 构造长为 $n$ 的严格递增正整数列，使全体按位与为 $x$，并最小化末项。从 $x$ 起逐个试下一个候选会在 $n$ 很大时过慢。
>
> 第一项必须是 $x$，其后各项只能在 $x$ 的零位上填入递增二进制，才能保持与值仍为 $x$。忽略这些被占用的一位后，序列恰为 $0,1,2,\ldots,n-1$。
>
> 因此把 $n-1$ 的各位依次写入 $x$ 的空位（低位优先），更高位整体接到第 $31$ 位之上。得到的数即为最小末项。
