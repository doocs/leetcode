...
> **思考**
>
> 只能把元素升高，目标值至少是当前最大值。$n\le 10^5$，不能对每个目标模拟每一次 $+1$。
>
> 若 $2\cdot cost1\le cost2$，双升从不更优，答案退化为缺口总和乘 $cost1$。否则应对缺口配对使用操作二，并注意最大缺口过大时无法任意配对。
>
> 目标还可继续升高以改善配对，升高范围受最大缺口约束，枚举量与 $n$ 无关。对每个候选目标用缺口总和与最大缺口计算两种操作的次数，取最小费用并对 $10^9+7$ 取模。
