...
> **思考**
>
> 每列须改为同一数字，且相邻列数字不同。若对每列枚举数字再递归后续列，状态数为 $10^n$。
>
> 列间只有「上一列选了哪个数字」会影响转移，格子取值仅 $0$ 到 $9$。先统计本列各数字出现次数，改成 $j$ 的代价为 $m-cnt[j]$。
>
> 定义 $f[i][j]$ 为前 $i$ 列且第 $i$ 列取 $j$ 的最小代价，由 $k\neq j$ 的 $f[i-1][k]$ 转移。最后取 $f[n-1]$ 的最小值。
