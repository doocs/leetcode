...
> **思考**
>
> 选一个伤害 $x$ 就不能再选 $[x-2,x+2]$ 中的其他值，相同 $x$ 可全选。对每个互异伤害做子集枚举是指数的。
>
> 排序后决策变为：跳过当前值的全部副本，或收下它们并跳到第一个大于 $x+2$ 的位置。下一位置可用二分预计算。
>
> 记忆化 $dfs(i)=\max(dfs(i+cnt[x]),\,x\cdot cnt[x]+dfs(nxt[i]))$。每个互异伤害只展开一次。
