...
> **思考**
>
> 矩形高度不受限，覆盖只由横坐标与宽度 $w$ 决定。若枚举每个点所属矩形的划分，状态随点数指数增长。
>
> 最优策略是按 $x$ 排序后尽量把当前矩形向右铺满：一旦某点落在已覆盖范围外，必须新开一个以该点为左端、长度为 $w$ 的矩形。
>
> 为此排序后维护当前矩形右边界 $x_1$，遇 $x>x_1$ 则答案加一并令 $x_1=x+w$。贪心覆盖次数即为最少矩形数。
