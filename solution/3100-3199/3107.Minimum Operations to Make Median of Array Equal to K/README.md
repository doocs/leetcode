...
> **思考**
>
> 中位数由有序后的中间位置决定。若不排序而枚举哪些元素增减，组合数过大；$n$ 允许 $O(n\log n)$ 排序。
>
> 将中间元改为 $k$ 是必要的。若它大于 $k$，其右侧本已不小于它，只需把左侧仍大于 $k$ 的值压到 $k$；若它不大于 $k$，则只需把右侧仍小于 $k$ 的值抬到 $k$。
>
> 为此先排序并定位中位下标 $m$，把 $|nums[m]-k|$ 计入答案，再按上述方向单侧扫描补齐。每侧遇已满足 $k$ 的元素即可停止。
