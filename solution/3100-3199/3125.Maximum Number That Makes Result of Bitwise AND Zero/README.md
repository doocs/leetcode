...
> **思考**
>
> 要找不超过 $n$ 的最大 $x$ 使 $[x,x+1]$ 的按位与为 $0$。从 $n-1$ 向下试会在最坏情况下线性扫描。
>
> $x$ 与 $x+1$ 互为进位相邻，其与为 $0$ 意味着 $x$ 的二进制是一段低位全 $1$。不超过 $n$ 的最大此类数，恰是 $n$ 最高位以下的掩码。
>
> 因此取 $n$ 的最高位置位，返回 $2^{\textit{bit\_length}(n)-1}-1$。该数与其后继的与必为 $0$，且已是上界。
