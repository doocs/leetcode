...
> **思考**
>
> 排列的字典序排名等于有多少个排列严格小于它。若对每个前缀枚举剩余数字再递归计数，时间随阶乘增长，即使 $n$ 中等也无法承受。
>
> 第 $i$ 位若选比 $perm[i]$ 更小且尚未使用的数，后面 $n-i-1$ 位可任意排，贡献为「更小可用数个数」乘 $(n-i-1)!$。各位贡献相加即排名。
>
> 树状数组按值域维护已出现的数，则小于当前值的已用个数为一次前缀查询。从左到右累加 $(perm[i]-1-\textit{query}(perm[i]))\times(n-i-1)!$ 并标记当前值，即可在 $O(n\log n)$ 内得到答案。
