...
> **思考**
>
> 稳定数组要求任意连续相同段长度不超过 $limit$。在 $zero$ 个 $0$ 与 $one$ 个 $1$ 上直接回溯，方案数随组合爆炸。
>
> 填完后的合法性只依赖剩余 $0$、$1$ 个数以及下一位准备填的数字。连续超限等价于「连续填了 $limit+1$ 个相同位且再往前是另一种位」。
>
> 定义 $dfs(i,j,k)$ 为剩余 $i$ 个 $0$、$j$ 个 $1$ 且下一位填 $k$ 的方案。转移为续填同色或改色，并减去超限的那一支。记忆化后状态量为 $O(zero\cdot one)$。
...
> **思考**
>
> 方法一的状态转移没有后效，递归与缓存的常系数在 $zero,one$ 接近上界时偏大。
>
> 同一递推可按剩余个数从小到大填表：$f[i][j][k]$ 表示用掉 $i$ 个 $0$、$j$ 个 $1$ 且最后一位为 $k$。
>
> 边界为只填一种数字且长度不超过 $limit$ 的串。迭代加上模运算后直接读取 $f[zero][one][0]+f[zero][one][1]$。
