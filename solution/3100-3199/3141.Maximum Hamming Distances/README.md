...
> **思考**
>
> 对每个值求它与数组中某元素的最大汉明距离。若对每个数枚举其余数，在 $m$ 达 $17$、$n$ 较大时过慢。
>
> 最大距离等于 $m$ 减去到「某数组元素」的最小距离。从数组中的数出发，在超立方体上按翻一位做 BFS，即可得到每个掩码到数组的最短汉明距离。
>
> 因此 $dist[x]=0$ 当 $x$ 在数组中，多层扩展翻比特；对询问值 $x$ 取 $m-dist[x\oplus(2^m-1)]$，即到其补码的最短距离改写成最大距离。
