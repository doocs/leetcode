...
> **思考**
>
> 要求每名员工的任务总时长（重叠只计一次）以及最大并发数。直接对每对任务比重叠在 SQL 中不便。
>
> 把每名员工的所有起止时刻去重排序后，相邻时刻构成原子区间；区间被多少任务覆盖即并发，长度累加即去重时长。
>
> 因此合并 $start\_time$ 与 $end\_time$，用 `LEAD` 得到下沿，再回连 `Tasks` 统计覆盖数，最后按员工求和并取最大并发。
