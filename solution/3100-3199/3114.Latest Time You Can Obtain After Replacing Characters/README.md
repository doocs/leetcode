...
> **思考**
>
> 合法时刻只有 $12\times 60$ 种。从大到小枚举小时与分钟，检查与模式串在非 `?` 位是否一致，第一次命中即最大合法时间。
>
> 状态空间为常数，无需回溯或逐位推最优。按 $11{:}59$ 到 $00{:}00$ 的顺序保证字典序最大。
>
> 因此双重循环生成 `HH:MM`，用逐位比较过滤 `?`，返回第一个匹配的串。
...
> **思考**
>
> 方法一枚举全部时刻，虽为常数但仍做了数百次格式化与比较。每位的取值范围由 12 小时制独立确定，可以直接写下。
>
> 小时十位只能是 $0$ 或 $1$，个位在十位为 $1$ 时至多为 $1$；分钟十位至多为 $5$，个位可取 $9$。
>
> 为此按从左到右把每个 `?` 换成仍合法的最大数字：先定 $s[0]$ 再定 $s[1]$，分钟两位分别填 $5$ 与 $9$。一次扫描即可。
