...
> **思考**
>
> 兑换阈值每次成功后都会加一，若试图用闭式公式一次算出总瓶数，需处理空瓶与递增阈值之间的二次关系，推导并不轻松。$n$ 与 $\textit{numExchange}$ 至多为 $100$，按规则逐步兑换的循环次数与 $\sqrt{n}$ 同阶，足以在时限内完成。
>
> 注意到满瓶可先全部喝掉，此后能否继续只取决于当前空瓶数是否不少于阈值。每换一瓶便喝掉它，空瓶净减 $\textit{numExchange}-1$，阈值同时加一。
>
> 因此先把 $\textit{numBottles}$ 计入答案，再在空瓶不少于当前阈值时减去阈值、阈值加一、答案与空瓶各加一。循环结束时的累计瓶数即为最多能喝到的数量。
