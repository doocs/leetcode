...
> **思考**
>
> 排列分数是相邻项 $|perm[i]-nums[perm[i+1]]|$ 再加闭合项，且要字典序最小。$n\le 14$，全排列为 $14!$ 不可行。
>
> 循环移位不改变分数，字典序最小迫使首位为 $0$。其余位置用子集 $mask$ 与上一个值 $pre$ 即可 DP。
>
> 记忆化 $dfs(mask,pre)$ 枚举未使用的 $cur$，加上 $|pre-nums[cur]|$。填满后补上回到 $0$ 的边。再用同一最优值回溯构造字典序最小序列。
