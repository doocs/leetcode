...
> **思考**
>
> 好子序列允许至多 $k$ 次相邻值不同。$n$ 与 $k$ 都不大，可对结尾位置与已用不同次数 DP。
>
> 转移到 $i$ 时需看更早的 $j$：值相等则不消耗次数，否则消耗一次。
>
> 令 $f[i][h]$ 为以 $i$ 结尾、不同次数不超过 $h$ 的最长长度，枚举 $j<i$ 更新，答案为 $\max f[i][k]$。
...
> **思考**
>
> 方法一对每个 $(i,h)$ 再扫全部 $j$，在 I 的约束下可通过，但已暴露 $O(n^2k)$ 的瓶颈。
>
> 相等转移只需要「同一值在次数 $h$ 下的最大 $f$」，不等转移只需要「次数 $h-1$ 下的最大与次大（避开当前值）」。
>
> 用 $mp[h][x]$ 与三元组 $g[h]$ 维护这些最值，把内层 $j$ 去掉，为 II 的 $O(nk)$ 做准备。
