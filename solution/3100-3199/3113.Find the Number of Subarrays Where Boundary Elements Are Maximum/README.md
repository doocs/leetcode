...
> **思考**
>
> 合法子数组要求两端相等且为区间最大值。枚举两端再检查中间最大值，时间为 $O(n^2)$，在 $n$ 较大时偏慢。
>
> 以当前值 $x$ 为两端时，中间不能出现更大的数。从左扫描时，上一个更大元素会隔断所有以 $x$ 为右端的更长段。
>
> 维护单调递减栈，栈中记录值及其连续可延伸次数。弹出所有小于 $x$ 的段后，若栈顶等于 $x$ 则次数加一，否则新开一段。每次把栈顶次数加入答案，即统计了全部以 $x$ 为右边界的合法子数组。
