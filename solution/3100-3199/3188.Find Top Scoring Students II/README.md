...
> **思考**
>
> 除平均 GPA 外，还要求必修全 $A$、选修至少两门且均为 $A/B$。多层过滤若拆成多个查询，空成绩与未选课容易漏。
>
> 先用 CTE 筛出 GPA $\ge 2.5$ 的学生，再连专业课程并左连成绩，用条件求和表达三类约束。
>
> `HAVING` 比较必修 $A$ 数与必修门数、选修非空且成绩在 $A/B$、以及选修门数 $\ge 2$。左连让未选课的 $grade$ 为 `NULL`。
