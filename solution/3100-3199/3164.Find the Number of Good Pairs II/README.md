...
> **思考**
>
> 条件与 I 相同但长度达到 $10^5$，双重循环不可用。
>
> $x$ 能配 $y$ 当且仅当 $(x/k)$ 是 $y$ 的倍数（且 $k$ 整除 $x$）。按商计数后，枚举 $y$ 的倍数即可。
>
> 先把 $nums1$ 中 $k$ 的倍数收成 $cnt1$，再对 $cnt2$ 的每个 $x$ 以步长 $x$ 扫到 $mx$，累加 $cnt1[y]\cdot v$。空 $cnt1$ 直接返回 $0$。
