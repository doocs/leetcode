...
> **思考**
>
> 网格固定为 $3\times 3$，一次改色后须出现同色 $2\times 2$。枚举改哪个格子再检查，情况虽少但比直接看每个 $2\times 2$ 更绕。
>
> 一个 $2\times 2$ 已有三格同色时，一次修改即可补齐；四格已经同色则无需修改。等价于该窗内黑白个数不等。
>
> 因此枚举四个可能的 $2\times 2$，统计 `W` 与 `B`，一旦不等即可返回真。网格尺寸为常数。
