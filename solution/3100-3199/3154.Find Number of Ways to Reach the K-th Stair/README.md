...
> **思考**
>
> 两种操作：向下走一格（不能连续），或向上跳 $2^{jump}$ 并令 $jump$ 加一。从 $1$ 出发到达 $k$ 的方案数。盲目搜索分支随跳次指数增长。
>
> 位置一旦超过 $k+1$ 便无法再下来回到 $k$。跳的次数约为 $O(\log k)$，状态 $(i,j,jump)$ 很少，其中 $j$ 标记上次是否刚向下。
>
> 记忆化 $dfs(i,j,jump)$：到达 $k$ 先计 $1$，允许时向下，并始终尝试上跳。越界返回 $0$。
