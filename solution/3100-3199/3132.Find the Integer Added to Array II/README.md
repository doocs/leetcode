...
> **思考**
>
> 从 $nums1$ 删两个元素后再整体加 $x$ 得到 $nums2$。枚举删除哪一对是 $O(n^2)$，再验证匹配。
>
> 排序后，$x$ 只能是 $nums2[0]$ 减去 $nums1$ 的前三个候选之一：被删的两个数若都大于最小保留值，才可能跳过前两项。
>
> 对每个候选 $x$ 用双指针扫描，统计无法对齐的个数，不超过 $2$ 则可行。取可行 $x$ 的最小值。
