...
> **思考**
>
> 节点会在给定时刻消失，到达时间必须严格早于消失时刻。朴素对每个节点单独搜路会重复松弛同一条边。
>
> 这仍是单源最短路：边权非负，只需在松弛时额外要求 $dist[u]+w < \textit{disappear}[v]$。不满足则该边不可用。
>
> 因此建无向图后用堆优化 Dijkstra 从 $0$ 出发更新 $dist$，最后将 $dist[i]<\textit{disappear}[i]$ 的距离留下，否则记为 $-1$。
