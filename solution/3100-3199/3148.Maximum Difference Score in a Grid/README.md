...
> **思考**
>
> 路径只能向右或向下，得分是相邻差之和，展开后等于终点减起点。枚举所有起点终点是 $O(m^2n^2)$。
>
> 对每个终点，最优起点是其左上方（不含自身）的最小格子。该最小值可由上方与左方的前缀最小值递推。
>
> 令 $f[i][j]$ 为到达 $(i,j)$ 时路径上见过的最小值（含自身），答案取 $grid[i][j]-\min(f[i-1][j],f[i][j-1])$ 的最大者。
