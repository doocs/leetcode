...
> **思考**
>
> 每次改一个位置后求不相邻子序列的最大和，询问多达 $10^5$。每次重做线性 DP 是 $O(nq)$。
>
> 不相邻选取在线段合并时只需区分左右端点是否被选，四个状态 $s_{00},s_{01},s_{10},s_{11}$ 即可拼出父区间。
>
> 建树后单点修改并查询整段的 $s_{11}$（可选两端）。每次询问累加该值并对 $10^9+7$ 取模。
