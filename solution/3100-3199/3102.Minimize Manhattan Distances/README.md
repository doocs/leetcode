...
> **思考**
>
> 删除一个点后，剩余点对的最大曼哈顿距离需要重新计算。若对每个被删点再两两扫描，时间为 $O(n^3)$，在 $n$ 达到数千时不可接受。
>
> 曼哈顿距离 $|x_1-x_2|+|y_1-y_2|$ 可改写成 $x+y$ 与 $x-y$ 两组坐标差的最大值。全集的最大距离因此只取决于这两套坐标的两端极值。
>
> 将全部 $x+y$ 与 $x-y$ 放入有序集合后，枚举每个点并暂时移除它，用两端极值更新答案再放回。每次删除与插入为对数时间，从而在 $O(n\log n)$ 内得到最优删除。
