...
> **思考**
>
> 正方形以原点为中心、边平行坐标轴，内部标签须互异，求最多点数。枚举边长再检查标签，边长取值多达坐标范围。
>
> 点 $(x,y)$ 被边长 $2d$ 的正方形盖住当且仅当 $\max(|x|,|y|)\le d$。按 $d$ 从小到大加入点，一旦出现重复标签便不能再扩大。
>
> 用哈希表按 $d$ 分桶并升序遍历：一层内若有标签已出现则返回此前点数，否则把本层全部加入已见集合。
