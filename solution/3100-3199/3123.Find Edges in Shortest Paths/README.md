...
> **思考**
>
> 要判断每条边是否落在 $0$ 到 $n-1$ 的某条最短路上。对每条边单独重算最短路会把 Dijkstra 重复 $m$ 次。
>
> 边 $(a,b,w)$ 在最短路上，当且仅当 $dist[a]+w+dist'[b]=dist[n-1]$（或对称），其中 $dist'$ 是从终点出发的距离。无向图中从终点沿 $dist[a]=dist[b]+w$ 回溯即可标出全部这样的边。
>
> 因此先用堆优化 Dijkstra 求出 $dist$，再从 $n-1$ 广度优先，把满足松弛等式的边标为真。不可达则全部为假。
