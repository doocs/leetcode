...
> **思考**
>
> 交替子数组要求相邻元素互异。若枚举全部 $O(n^2)$ 个区间再逐段检查，大量相邻比较会被重复执行。
>
> 以 $i$ 结尾的交替子数组，要么只含 $nums[i]$，要么在 $nums[i] \neq nums[i-1]$ 时把以 $i-1$ 结尾的段整体延长一位。相等则此前的交替段无法越过 $i$。
>
> 为此维护以当前位置结尾的交替长度 $s$，相邻不同则加一，否则重置为 $1$，并累加到答案。一遍扫描即可统计全部合法子数组。
