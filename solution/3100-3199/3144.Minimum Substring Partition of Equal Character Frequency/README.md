...
>
> 因此记忆化 $dfs(i)$：维护 $cnt$ 与 $freq$，每当 $freq$ 仅含一种次数就用 $1+dfs(j+1)$ 更新。空后缀返回 $0$。
...
> **思考**
>
> 方法一额外用 $freq$ 哈希表维护「出现次数的次数」，常数偏大，且每种次数的插入删除容易写繁。
>
> 平衡条件等价于段长等于「最大频次 $\times$ 种类数」。只需 $cnt$ 与当前最大频次 $m$。
>
> 扩展 $j$ 时更新 $m$，当 $j-i+1=m\cdot|cnt|$ 再递归。状态仍是起点 $i$，渐近时间不变而实现更短。
