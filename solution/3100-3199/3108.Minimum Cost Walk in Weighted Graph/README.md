...
> **思考**
>
> 路径代价是沿途边权的按位与，多次经过边只会让结果不增。若对每个查询单独搜最短（最小与）路，$q$ 较大时重复遍历同一连通块。
>
> 正整数反复按位与只会单调变小，同一连通分量内走遍所有边得到的与值即该分量能达到的最小代价。不同分量之间不存在路径。
>
> 因此用并查集合并所有边，再对每个分量把其边权全部与进根上的 $g[\textit{root}]$。查询时若两端同根则返回该值，同点为 $0$，否则为 $-1$。
