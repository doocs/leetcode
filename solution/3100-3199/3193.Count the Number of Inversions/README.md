...
> **思考**
>
> 统计满足若干前缀逆序对个数约束的排列数。插入第 $i$ 个数可与前面 $0..i$ 个数构成逆序，全排列 DP 的状态是「已放个数、已有逆序数」。
>
> 约束把某些 $i$ 的逆序数钉死，其余层仍需枚举可能的 $j$。$m\le 400$，三维枚举可过。
>
> 令 $f[i][j]$ 为 $[0..i]$ 上逆序数为 $j$ 的方案。若该层有要求则只填那一列，否则对 $k\le\min(i,j)$ 累加 $f[i-1][j-k]$。首项逆序必须为 $0$。
