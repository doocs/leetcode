...
> **思考**
>
> 自包含子串要求其中每个字符的全部出现都落在该区间内，且不能是整个串。枚举全部子串再检查 $26$ 个字符的首末位置，时间为 $O(n^2|\Sigma|)$，偏大。
>
> 合法区间的左端必是某个字符的首次出现：再往左扩只会引入新字符或其更早出现，破坏自包含。字符种类至多 $26$，以首次出现为左端的候选很少。
>
> 为此记录每个字符的首末下标，枚举左端 $i$，向右推进 $j$ 并维护必须覆盖到的最右位置 $mx$。一旦某字符的首次出现落在 $i$ 左侧便停止；当 $mx=j$ 且区间不是全集时更新最长长度。
