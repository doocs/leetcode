...
> **思考**
>
> $n < \min(2001, k \cdot 8)$，重复 $k$ 次的子序列长度至多为 $\lfloor n/k \rfloor \le 7$。字符须至少出现 $k$ 次才可能进入答案。
>
> 在长度受限时，按字典序从短到长扩展候选，比 DFS 更易保证「最后一次成功」即为最长且字典序最大（同长时后扩展的字母更大）。
>
> 队列从空串出发，仅把出现次数 $\ge k$ 的字母按升序追加；`check` 用双指针判断 $t$ 在 $s$ 中能否匹配 $k$ 遍。
