...
> **思考**
>
> $n \le 12$，全部子序列仅 $2^n$ 个，枚举可行。两个子序列还须下标不交，直接再套一层会变成 $4^n$ 量级，常数偏大。
>
> 对每个掩码用双指针跳过未选位，即可判定对应子序列是否回文，并记入数组 $p$。
>
> 若掩码 $i$ 回文，则在其补集上枚举子掩码 $j$，乘积为两边的 $1$ 的个数。子掩码枚举总量为 $3^n$，与预处理 $2^n n$ 同属可接受范围。
