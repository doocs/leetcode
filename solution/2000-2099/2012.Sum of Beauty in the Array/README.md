...
> **思考**
>
> $n \le 10^5$，对每个 $i$ 再扫左右最值会达到平方。美丽度 $2$ 要求 $nums[i]$ 严格大于左侧全部、小于右侧全部；美丽度 $1$ 只看相邻。
>
> 右侧最小值可预处理：$right[i]$ 为后缀最小。左侧最大值在从左往右扫描时用变量 $l$ 维护。
>
> 对每个中间下标先用 $l$ 与 $right[i+1]$ 判定 $2$ 分，否则再看相邻三项是否严格递增。
