...
> **思考**
>
> $n \le 16$，子集至多 $2^{16}$。全体或即为最大或值 $mx$，只需统计或等于 $mx$ 的非空子集（空集或为 $0$，一般不是最大）。
>
> DFS 对每个下标选或不选，到达末尾时比较当前或值。递归深度 $n$，无需额外存储全部掩码。
...
> **思考**
>
> 方法一依赖递归。二进制枚举每个掩码，按位累或，同时维护当前最大与计数，可在一层循环内完成，且不必预先知道 $mx$。
>
> 代价是多一个 $n$ 的内层扫描，空间降为常数。
