...
> **思考**
>
> $n \le 200$，枚举全部 $i<j$ 并判断绝对值是否为 $k$，次数约 $2 \times 10^4$，约束允许。值域与 $k$ 都很小，这一方法不依赖额外结构。
>
> 双重循环直接对应题目定义，实现与正确性都直观。
...
> **思考**
>
> 方法一对每个数对都扫描一次，虽能通过，但仍是平方。注意到只需统计差值恰为 $k$ 的个数，对当前 $x$，此前出现的 $x-k$ 与 $x+k$ 均可配对。
>
> 为此边遍历边用计数器累加，再写入 $x$，避免重复计数且只需线性时间。
