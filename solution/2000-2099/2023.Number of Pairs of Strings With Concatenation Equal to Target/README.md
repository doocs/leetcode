...
> **思考**
>
> $n \le 100$ 且串长短，枚举有序对 $(i,j)$ 做拼接比较即可。$i \neq j$ 允许同一字符串用两次仅当它在数组中出现两次。
>
> 双重循环对应题目定义，无需预处理。
...
> **思考**
>
> 方法一每次拼接耗时与 $m$ 有关，在 $n$、$m$ 同阶时偏慢。合法对必是 $target$ 的某个前缀加对应后缀。
>
> 先统计每个串的出现次数，再枚举切分点。前后缀相同须用 $c(c-1)$，以免同一下标配对。
