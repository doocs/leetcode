...
> **思考**
>
> 胜负只与价值和模 $3$ 有关，$n \le 10^5$ 不能模拟到取完。余 $0$ 不改变当前模，可穿插使用；先手第一枚不能取 $0$。
>
> 先手选 $1$ 后，最优序列为 $1,1,2,1,2,\ldots$（对称地从 $2$ 开始）。把余 $0$ 的个数计入回合长度后，若总回合为奇数且两种非零余数尚未耗尽成平局，则先手胜。
>
> 对 $cnt$ 与对调后的 $(cnt[0],cnt[2],cnt[1])$ 各判定一次，存在一种开局即可。
...
> **思考**
>
> 方法一用闭式判断回合奇偶，推导较长。同一余数计数下，也可按序列逐步扣减，在所需余数耗尽时再看余 $0$ 能否把当前回合变成奇数。
>
> 实现仍是 $O(n)$ 统计加常数次模拟，便于核对边界。题面第二种写法与第一种共用同一计数框架。
