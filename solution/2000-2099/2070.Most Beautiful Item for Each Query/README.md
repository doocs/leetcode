...
> **思考**
>
> 每个查询要价格 $\le q$ 的最大美丽值。$n,m \le 10^5$，在线对每个 $q$ 扫描物品不可行。离线后查询与物品都按价格升序，指针只向前。
>
> 扫到价格不超过当前 $q$ 的物品时更新美丽最大值，写入对应下标。排序决定复杂度。
...
> **思考**
>
> 方法一打乱查询顺序。若需在线回答，可在物品按价格排序后预处理「不超过该价的最大美丽」，再对每个 $q$ 二分。
>
> `mx` 数组单调，`bisect_right` 找到最后一个可买下标。
