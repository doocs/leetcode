...
> **思考**
>
> 机器人遇障右转，房间 $\le 300 \times 300$。会循环，终止条件是同一位置同一朝向再现。清扫过的空地标记后不再重复计数。
>
> 状态 $(i,j,k)$ 用集合记录。能前进则沿朝向 DFS，否则就地转向。空地首次访问时答案加一。
...
> **思考**
>
> 方法一用递归模拟，深度可达 $O(mn)$。同一转移写成循环即可去掉调用栈： vis 记录三元组，前进或转向直到状态重复。
>
> 计数规则不变。
