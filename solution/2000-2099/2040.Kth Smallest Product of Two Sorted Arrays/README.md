...
> **思考**
>
> 两数组有序，乘积共可达 $2.5 \times 10^9$ 个，不能物化。第 $k$ 小对阈值单调：乘积 $\le p$ 的个数随 $p$ 不减。
>
> 在 $[-M,M]$ 上二分 $p$，计数时对 $nums1$ 的每个 $x$ 分类：正数则 $nums2[i] \le p/x$，负数则不等式反向，零则视 $p$ 是否非负。有序保证可用二分完成计数。
>
> `bisect_left` 找到最小使计数 $\ge k$ 的 $p$。
