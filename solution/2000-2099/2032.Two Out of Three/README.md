...
> **思考**
>
> 值域仅 $[1,100]$，三个数组长度也 $\le 100$。先各自去重，再枚举 $1..100$，看它落入几个集合。出现不少于两次即加入答案。
>
> 实现不依赖位运算，与约束完全吻合。
...
> **思考**
>
> 方法一固定枚举 $1..100$，换值域便失效。用掩码记录每个数出现在哪些数组：第 $i$ 个数组置第 $i$ 位。
>
> `v & (v-1)` 非零当且仅当至少两位为 $1$，即至少两个数组含该数。一次遍历即可，不依赖值域上界。
