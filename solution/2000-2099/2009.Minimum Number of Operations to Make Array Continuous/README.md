...
> **思考**
>
> 连续数组的值域长度为 $n$，重复元素必须改掉。$n \le 10^5$，不能枚举每个目标区间。去重排序后，合法窗口是「最大值与最小值之差 $\le n-1$」的一段原有数。
>
> 枚举窗口左端 $nums[i]$，则右端不得超过 $nums[i]+n-1$。二分找到第一个越界位置 $j$，$j-i$ 即保留个数，操作为 $n-(j-i)$。
>
> 排序决定复杂度，查询为 $O(\log n)$。
...
> **思考**
>
> 方法一每次用二分定位右端，窗口随 $i$ 右移时右端也单调右移，不必重复折半。
>
> 为此用双指针推进 $j$，在同样 $O(n)$ 扫描内得到所有窗口，省去二分的对数因子。
