...
> **思考**
>
> 边权相同，但红绿灯使等待与路径长度相关，且要的是严格次短到达时间。$n \le 10^4$，普通最短路只保留最小值会丢掉次短。
>
> 对每个点记录最短与严格次短的边数。BFS 扩展：新距离优于最短则更新最短；介于两者之间则更新次短。到达 $n$ 的次短边数确定后，再按 $time$ 与 $change$ 模拟红灯等待。
>
> 队列中的状态是「点数、边数」，保证次短路径被保留。
