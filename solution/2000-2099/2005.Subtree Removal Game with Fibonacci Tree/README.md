...
> **思考**
>
> 斐波那契树由 $order(n-2)$ 与 $order(n-1)$ 递归拼成，结点个数随 $n$ 指数增长，显式建树不可行。$n \le 100$，需要的是先手必胜判定，而非模拟对局。
>
> 每次删除一个结点及其子树，等价于在若干独立子游戏上做减法博弈。根的左右子树的 Grundy 值决定整棵树是否为 $N$ 局面。
>
> 递推 $order(n)$ 的胜负即可：空树与单结点为必败，$n \ge 2$ 时由左右子树的异或（或等价的奇偶规律）得到先手是否获胜。题面代码栏为空，思考仍按这一博弈归约书写。
