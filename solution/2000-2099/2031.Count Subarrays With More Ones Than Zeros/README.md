...
> **思考**
>
> 将 $0$ 视为 $-1$ 后，子数组 $1$ 多于 $0$ 当且仅当前缀和严格上升。枚举右端时，需统计历史前缀中小于当前 $s$ 的个数。$n$ 较大时，有序插入须对数结构。
>
> 前缀和取值在 $[-n,n]$，平移 $n+1$ 后可用树状数组做前缀计数。初始放入 $0$ 一次，每步先查询再更新，并对 $10^9+7$ 取模。
...
> **思考**
>
> 方法一自行实现树状数组，需要下标平移。有序集合可直接对前缀和二分：`bisect_left(s)` 即为小于 $s$ 的历史个数。
>
> 语义与方法一相同，只是用平衡树代替树状数组，省去离散化。
