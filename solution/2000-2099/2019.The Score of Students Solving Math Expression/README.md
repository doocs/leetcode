...
> **思考**
>
> 运算符至多 $15$ 个，错误答案来自不同加乘结合次序。枚举所有加括号方案是卡特兰数级，但区间 DP 可合并出全部可能值。正确答案按「乘优先于加」单独计算。
>
> $f[i][j]$ 为第 $i$ 到 $j$ 个数字能算出的值集合；枚举分界 $k$ 与运算符做加减乘，并丢掉超过 $1000$ 的中间值（评分只关心 $[0,1000]$）。
>
> 对 $answers$ 计数：等于正确值得 $5$，否则若落在 $f[0][m-1]$ 中得 $2$。
