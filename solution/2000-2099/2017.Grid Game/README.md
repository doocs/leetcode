...
> **思考**
>
> 两行网格，第一个机器人从左上到右下只能先右后下再右，拐弯列 $j$ 唯一决定其路径。第二个机器人在剩余格子中取最优，只会走「第一行 $j$ 右侧」或「第二行 $j$ 左侧」之一。
>
> $n \le 5 \times 10^4$，枚举 $j$ 须 $O(1)$ 得到两段和。用第一行剩余后缀 $s_1$ 与第二行已走前缀 $s_2$，第二个机器人得 $\max(s_1,s_2)$。
>
> 第一个机器人要最小化该值，故对每个 $j$ 取 $\min$。
