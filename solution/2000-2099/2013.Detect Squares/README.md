...
> **思考**
>
> 总调用 $\le 5000$，坐标在 $[0,1000]$。每次 `count` 若枚举全部三点组合，最坏偏慢且实现繁琐。轴对齐正方形由一条水平边唯一确定另外两点。
>
> 用 $cnt[x][y]$ 记录重点。对查询 $(x_1,y_1)$，枚举已有横坐标 $x_2 \ne x_1$，边长 $d=x_2-x_1$，则上下两个正方形的另外两点坐标随之确定。
>
> 方案数为对应三个位置计数的乘积。`add` 为 $O(1)$，`count` 对横坐标线性。
