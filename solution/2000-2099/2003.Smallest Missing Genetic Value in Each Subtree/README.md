...
> **思考**
>
> $n \le 10^5$，对每个结点单独扫描子树会重复访问，总复杂度过高。基因值互不相同，因此 mex 大于 $1$ 的结点只能落在「含基因 $1$ 的结点到根」这条链上，其余结点答案恒为 $1$。
>
> 沿该链向上时，子树只会扩大：父结点的基因集合是当前子树再并上其余兄弟。已标记过的基因不必重扫。
>
> 为此自 $idx$ 向根做 DFS，用 $has$ 累积出现过的基因，$i$ 单调递增找最小缺失值，并把结果写回 $ans[idx]$。
