...
> **思考**
>
> 定长 $k$ 窗口的分数是 $\sum x^{\textit{freq}(x)}\bmod (10^9+7)$，要求所有窗口的最大分数。每个窗口从头计算需对频次做快速幂，窗口数为 $n-k+1$ 时偏慢。
>
> 相邻窗口只进一个元素、出一个元素。用哈希表维护频次，分数按 $x^{c+1}-x^c=(x-1)x^c$ 增量更新（新出现则加 $x$，频次降到 $0$ 则减 $x$）。进出相同则分数不变。
