...
> **思考**
>
> 要在正整数中为两个数组分别选出指定个数的互异元素，且第一数组不能含 $\textit{divisor1}$ 的倍数、第二数组不能含 $\textit{divisor2}$ 的倍数，最小化所用到的最大整数。最大值可达很大，不能从 $1$ 起逐个分配。
>
> 若上界 $x$ 可行，则更大的上界也可行，故对 $x$ 二分。$[1,x]$ 中不能被 $d$ 整除的个数为 $x-\lfloor x/d\rfloor$。第一、第二数组分别需要足够多的非倍数，且两数组合计不能超过 $[1,x]$ 中不能同时被二者整除（即不能被 $\operatorname{lcm}$ 整除）的个数。$\textit{bisect\_left}$ 找出最小可行 $x$。
