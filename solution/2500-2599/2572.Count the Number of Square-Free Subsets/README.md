...
> **思考**
>
> 子集乘积无平方因子，空集不计，$nums[i]\in[1,30]$。$2^n$ 在 $n\le 10^3$ 时不可枚举，但取值只有 $30$ 种，且含平方因子的数不能出现。
>
> 不超过 $30$ 的质数共 $10$ 个，子集的质因数集合可用 $10$ 位掩码表示。$f[\textit{state}]$ 为凑出该掩码的方案数；$1$ 可任意选取，故 $f[0]=2^{\textit{cnt}[1]}$。按 $0$-$1$ 背包从大到小转移每个无平方的 $x$。全部状态之和减去空集即为答案。
