...
> **思考**
>
> 完全二叉树中加边 $(a,b)$ 形成的环长等于 $a$ 到 $b$ 的路径长度加一。若对每个查询建树再求距离，$n$ 可达 $30$，节点数为 $2^n-1$，建树本身不可接受。
>
> 父结点编号为 $\lfloor x/2\rfloor$，即右移一位。两结点沿父指针上移，始终先移动编号较大者，直至相遇，所走步数加一即为环长。这正是完全二叉树中求 LCA 的过程。
