...
> **思考**
>
> 两个区间只要有重叠就必须分到同一组，求分成两组的方案数。枚举 $2^n$ 种分配在 $n\le 10^5$ 下不可行。
>
> 重叠关系具有传递性：排序后合并，每个连通块必须整块放入某一组。块与块不相交，故每块独立选组，方案数为 $2^{\textit{cnt}}$，用快速幂取模。
...
> **思考**
>
> 方法一先计数再幂。发现新块时当场把答案乘 $2$，省去单独的幂运算，合并逻辑不变。
