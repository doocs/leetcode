...
> **思考**
>
> 每次查询要找二进制串中值为 $first\oplus second$ 的最短（同长则最左）子串。查询达 $10^5$，值是 $32$ 位整数，现场扫描每个查询过慢。
>
> 任意 $32$ 位子串已覆盖全部可能取值。从每个起点向右至多延伸 $32$ 位，首次出现的值记入哈希表。遇到前导零的全零串立即停止，以免更长的等价表示覆盖更短者。查询时直接查表。
