...
> **思考**
>
> 三类操作：$nums1$ 的区间 $0/1$ 翻转；$nums2$ 整体加上 $p\times\sum nums1$；查询 $\sum nums2$。$n,q\le 10^5$，不能对每次翻转扫区间。
>
> $\sum nums2$ 只需一个变量：$nums1$ 的区间和决定第二次操作的增量。线段树维护 $nums1$ 的区间 $1$ 的个数，懒标记表示整段翻转，翻转后区间和变为长度减去原和。第三次操作把当前 $s$ 记下即可。
