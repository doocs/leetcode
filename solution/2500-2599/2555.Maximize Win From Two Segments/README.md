...
> **思考**
>
> 两条长不超过 $k$ 的线段覆盖奖品坐标，求最多覆盖个数，线段可重叠。枚举两条线段的端点是四次方级。
>
> 奖品坐标已有序。固定第二条线段右端为第 $i$ 个奖品时，左端由 $\textit{bisect}$ 得到，覆盖 $i-j$ 个。第一条必须完全落在这 $j$ 个奖品之前，其最优值由前缀 $\textit{dp}$ $f[j]$ 给出。边扫边更新 $f[i]$ 为“右端不超过 $i$ 的单条线段最优”。
