...
> **思考**
>
> 下标 $i$ 是 $k$-big 当且仅当左侧有至少 $k$ 个严格更小的数、右侧亦然。对每个 $i$ 扫描左右在 $n\le 10^5$ 时为平方级。
>
> 值域可离散到 $n$，用树状数组维护“某个值出现了多少次”。先把全部元素插入右侧树，从左到右扫描时先从右侧删去当前值，查询两侧小于 $v$ 的个数，再把 $v$ 插入左侧树。每个下标 $O(\log n)$。
