...
> **思考**
>
> 好子序列要求其中出现过的每种字母次数相同且非空。子序列个数为 $2^n$，不能枚举；字母只有 $26$ 种，真正约束的是各字母频次。
>
> 枚举公共次数 $i=1,\ldots,mx$。对频次 $v\ge i$ 的字母，可选 $\binom{v}{i}$ 种取法或完全不取，方案数为 $\binom{v}{i}+1$；频次不足 $i$ 的字母必须不出现。各字母方案相乘后减去全不选，再对 $i$ 求和。阶乘与逆元预处理组合数。
