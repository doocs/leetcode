...
> **思考**
>
> 进入格子的时刻必须不小于其上数字，每步耗时 $1$，可在已访问格子间来回等待。起点的两邻若都大于 $1$，则第一步都走不出去。
>
> 其余情形总能靠来回把奇偶调整到合法到达时刻。这是带等待的最短路：小根堆按到达时间扩展。若 $t+1$ 已不小于格子值则下一步即 $t+1$；否则等到 $grid[x][y]$，并在与 $t+1$ 奇偶不同时再多等一秒。
