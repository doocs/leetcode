...
> **思考**
>
> 从左上走到右下只能向右或向下，路径长度固定为 $m+n-1$。若该长度为奇数，则 $0$ 与 $1$ 的个数不可能相等，直接否定。$m,n\le 100$，枚举全部路径数量过大。
>
> 记目标为各出现 $s=(m+n-1)/2$ 次。状态 $(i,j,k)$ 表示走到 $(i,j)$ 且路径上 $1$ 的个数为 $k$。若 $k$ 或 $0$ 的个数已超过 $s$ 则可剪枝。记忆化后状态量为 $O(mn(m+n))$。
