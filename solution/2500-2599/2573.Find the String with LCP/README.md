...
> **思考**
>
> 由 $lcp$ 矩阵还原字典序最小的串。$lcp[i][j]>0$ 当且仅当 $s[i]=s[j]$，故相等关系把下标分成若干类，应把更靠前的类先涂上较小字母。
>
> 从 `'a'` 起，遇到未填位置就用当前字母填满所有 $lcp[i][j]\ne 0$ 的 $j$。字母用尽仍有空位则无解。再从大到小按 $lcp$ 递推验证：相等时期望值为后缀 $lcp$ 加一，不等则必须为 $0$。
