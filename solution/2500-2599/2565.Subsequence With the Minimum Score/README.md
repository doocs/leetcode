...
> **思考**
>
> 从 $t$ 中删一段 $[L,R]$（可为空或整串），使剩下的左右两段能作为 $s$ 的不相交子序列拼起来，最小化删除长度。枚举删段再匹配是平方级以上。
>
> 删除长度越大越容易成功，故对长度 $x$ 二分。$f[j]$ 为贪心匹配 $t[0..j]$ 时用到的 $s$ 最左下标，$g[j]$ 为匹配 $t[j..]$ 时用到的 $s$ 最右下标。删掉 $[k,k+x)$ 后若 $f[k-1]<g[k+x]$，两段在 $s$ 上不重叠。
