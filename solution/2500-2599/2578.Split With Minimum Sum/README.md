...
> **思考**
>
> 把 $num$ 的数位分成两个整数，使二者之和最小。高位应尽量小，故较小的数位应轮流放到两个数的高位。
>
> 统计 $0$–$9$ 的次数后，从小到大交替接到两个数的末尾，等价于让两个数的位数尽量接近且高位都尽量小。
...
> **思考**
>
> 方法一用手写计数保证线性。位数至多十位，排序字符串后取偶数位与奇数位分别拼成两个整数，分配规则相同，实现更短。
