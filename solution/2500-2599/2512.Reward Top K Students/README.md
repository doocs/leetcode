...
> **思考**
>
> 每名学生的得分由报告中正面词 $+3$、负面词 $-1$ 累加得到，再按得分降序、编号升序取前 $k$ 名。若每次在词表中线性查找，词表与报告长度乘积在约束下偏大。
>
> 把正、负词放入集合后，$O(1)$ 判断每个单词。将 $(\textit{score},\textit{id})$ 收集起来按 $(-\textit{score},\textit{id})$ 排序，截取前 $k$ 个编号即可。
