...
> **思考**
>
> 树的根未知，猜测是有向边。若以某点为根时猜中不少于 $k$ 条，则该点可行。对每个根单独 DFS 需 $O(n^2)$。
>
> 先以 $0$ 为根统计猜中数 $\textit{cnt}$。换根到邻点 $j$ 时，边 $(i,j)$ 不再由父指向子，$(j,i)$ 则变为由父指向子，故 $\textit{cnt}$ 增减对应猜测是否存在。第二次 DFS 遍历每个根并计数。
