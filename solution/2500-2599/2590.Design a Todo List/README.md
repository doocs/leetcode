...
> **思考**
>
> 待办需按用户隔离，未完成任务按截止日期升序返回，并支持按标签过滤与按编号完成。操作次数有限，线性扫描完成亦可。
>
> 用哈希表映射到每个用户的有序集合，元素为 $(\textit{due},\textit{desc},\textit{tags},\textit{id},\textit{done})$，插入按截止日期有序。查询时跳过已完成项；完成则按 $\textit{id}$ 找到后打标。
