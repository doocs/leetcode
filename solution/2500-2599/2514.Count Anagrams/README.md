...
> **思考**
>
> 原串按空格分成若干单词，答案是各单词变位词个数的乘积，对 $10^9+7$ 取模。单词长度之和达 $10^5$，不能对每个单词枚举排列。
>
> 单词 $w$ 的变位词数为 $|w|!\,/\,\prod(c_i!)$，其中 $c_i$ 为各字母出现次数。从左到右扫描时，$\textit{ans}$ 累乘当前位置下标（即阶乘），$\textit{mul}$ 累乘当前字母已出现次数（即分母）。最后乘 $\textit{mul}$ 的模逆元即可一次算完所有单词。
