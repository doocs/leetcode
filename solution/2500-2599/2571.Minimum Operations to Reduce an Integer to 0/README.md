...
> **思考**
>
> 每次可加或减一个 $2$ 的幂，把 $n$ 变为 $0$，求最少次数。把每一位单独加减需要与 $1$ 的个数相同的步数，但连续的 $1$ 可以加一个更高位再减，用更少步消掉。
>
> 自低向高扫描连续 $1$ 段：长度为 $1$ 则一次减去该位；更长则一次加上更高位，相当于把进位留给后面（计数留 $1$）。扫完后若仍剩一段，长度为 $1$ 再加一次，更长则加两次（先加后清）。
