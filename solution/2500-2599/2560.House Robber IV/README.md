...
> **思考**
>
> 能力定义为所偷房屋金额的最大值，房屋不能相邻，要偷至少 $k$ 间并最小化该能力。枚举子集在 $n\le 10^5$ 下不可行。
>
> 能力 $x$ 越大越容易凑满 $k$ 间，故对 $x$ 二分。检验时从左到右贪心：金额不超过 $x$ 且与上一间不相邻就偷，能偷满 $k$ 则 $x$ 可行。$\textit{bisect\_left}$ 给出最小可行能力。
