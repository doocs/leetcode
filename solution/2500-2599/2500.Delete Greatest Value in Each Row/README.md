...
> **思考**
>
> 每次操作要从每一行删去当前最大值，再把这些被删值中的最大者累加到答案。若按题意逐轮扫描，则每轮在各行找最大值需 $O(mn)$，共 $n$ 轮，在 $m,n\le 50$ 时尚可接受，但同一行被删去的序列其实就是该行降序排列的结果。
>
> 因此先将每一行升序排序，第 $j$ 次操作对应各行的第 $j$ 个元素（从右往左），答案即为按列取最大值之和。排序后用 $\textit{zip}$ 按列聚合即可，无需再模拟删除。
