...
> **思考**
>
> 至多加两条边且不能与已有边重复，要使全部度数变为偶数。握手引理表明奇数度顶点个数必为偶数；加一条边至多改变两个顶点的奇偶，故奇数度顶点超过 $4$ 个时不可能。
>
> 用邻接集合建图后取出奇数度顶点 $vs$。$|vs|=0$ 已满足；$|vs|=2$ 时优先直接连这两点，若已相邻则找一个与二者都未相连的第三点同时连出；$|vs|=4$ 时枚举三种配对方式，存在一对互不相邻的匹配即可。
