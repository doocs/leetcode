...
> **思考**
>
> 把数组分成两个子序列（每个元素入且只入一侧），要求两侧元素和都不小于 $k$。全部 $2^n$ 种分法在 $n\le 10^3$、$k\le 10^3$ 时不能直接枚举；若某一侧和已经小于 $k$，则该分法不合法。
>
> 若总和不足 $2k$，两侧不可能同时达标，答案为 $0$。否则先算全部 $2^n$ 个划分，再减去“某一侧和小于 $k$”的坏划分。坏划分对应选出一个和小于 $k$ 的子集作为其中一侧，这是容量为 $k-1$ 的 $0$-$1$ 背包；每一侧都可扮演“小和”角色，故坏方案数为子集数的两倍。
