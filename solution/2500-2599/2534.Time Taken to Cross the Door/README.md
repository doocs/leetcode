...
> **思考**
>
> 每秒门只让一人通过；两侧都有人时沿用上一秒的方向，空闲后默认先出。按到达时间模拟即可，$n\le 10^5$ 时需避免每秒空转过久，但到达时间与 $n$ 同阶，逐秒处理仍是线性。
>
> 用两个队列分别收进入与离开的请求。时钟 $t$ 前进时把已到达者入队；两队皆非空则弹出门当前方向 $st$ 的队首，仅一队非空则改 $st$ 为该方向，皆空则把门重置为离开。每人的通过时刻写入答案。
