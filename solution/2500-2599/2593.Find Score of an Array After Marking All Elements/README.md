...
> **思考**
>
> 反复选出未标记的最小元素（并列取最左）加入分数，并标记它与左右邻居，直至全部标记。每次线性找最小是平方级。
>
> 小根堆按 $(值,下标)$ 弹出全局最小未标记者。标记其邻域后，堆顶若已标记则继续弹出。每个元素至多入堆一次。
...
> **思考**
>
> 方法一用堆动态取最小。操作顺序其实就是按 $(值,下标)$ 的静态序，事先排序后依次检查：未被邻域波及则计入并标记左右。避免堆的弹出循环。
