...
> **思考**
>
> 从 $[1,n]$ 中选尽可能多的互异整数，避开 $\textit{banned}$，且和不超过 $\textit{maxSum}$。选小数更利于多选，故从 $1$ 起贪心。
>
> $n\le 10^4$ 时枚举每个候选，用集合判禁并累加，和一旦超限即停。
...
> **思考**
>
> 方法一的时间随 $n$ 线性增长，在 $n$ 更大的变体中不可用。禁选点把 $[1,n]$ 切成若干连续段，段内前缀和是等差数列，可二分这段能取多少个。从左到右取满能取的，直到额度用尽。
