...
> **思考**
>
> 分配需在下标最小的连续空闲段中放入 $\textit{size}$ 个单元，释放则回收某一 $\textit{mID}$ 占据的全部单元。$n,q\le 10^3$，即便每次扫描整段内存也只需 $O(nq)$。
>
> 为此用长度为 $n$ 的数组标记占用：空闲记 $0$，分配时累计连续 $0$ 的个数，凑满 $\textit{size}$ 后写入 $\textit{mID}$；释放时扫一遍把等于 $\textit{mID}$ 的位置清零并计数。
...
> **思考**
>
> 方法一对每次调用都线性扫描整段内存。若只维护已占用区间，则空闲段是相邻占用块之间的空隙。
>
> 用有序集合保存占用区间并在两端各放哨兵 $(-1,-1)$ 与 $(n,n)$，分配时按空隙长度做首次适配；再用哈希表记录每个 $\textit{mID}$ 对应的若干区间，释放时按块删除。单次代价降为对占用块数量的对数。
