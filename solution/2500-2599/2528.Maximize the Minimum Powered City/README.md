...
> **思考**
>
> 一座电站覆盖半径 $r$ 内的城市，还可再新建至多 $k$ 座，目标是最大化“供电最少的那座城市”的电量。$n\le 10^5$，不能对每个 $k$ 的分配方案搜索。
>
> 最小电量对新建数量单调，故二分目标值 $x$。差分数组先算出每座城市的初始供电。检验时从左到右：当前城市不足 $x$ 则把缺口建在能覆盖它的最右侧，使后续城市尽量受益，并用差分数组记录这段增量。缺口超过剩余 $k$ 则 $x$ 不可行。
