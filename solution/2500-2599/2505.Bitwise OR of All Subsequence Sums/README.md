...
> **思考**
>
> 子序列和共有 $2^n$ 个，$n\le 10^5$ 时不能枚举。所求是这些和的按位或，因此只需判断每一位能否在某个子序列和中出现。
>
> 某一位上的 $1$ 既可直接来自原数，也可由更低位成对进位产生。统计各位 $1$ 的个数后自低向高扫描：该位计数非零则或入答案，并把 $\lfloor cnt[i]/2\rfloor$ 累加到下一位，从而刻画全部可能的进位。
