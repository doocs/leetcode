...
> **思考**
>
> 一次移动必须从己方城堡出发、穿过一段连续空地、停在敌方城堡，俘虏的是中间的 $0$。$n\le 1000$，枚举所有起终点即可，但不必对每个 $i$ 都重新向右扫描。
>
> 用指针 $i$ 停在非零格子，再让 $j$ 跳过随后的 $0$ 落到下一非零格。若两端异号，则中间长度 $j-i-1$ 可更新答案，然后把 $i$ 接到 $j$，整段只走一遍。
