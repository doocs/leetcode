...
> **思考**
>
> 每次只能从当前串的左端或右端取走一个字符，要使 $a,b,c$ 各至少取到 $k$ 个，并最小化取走次数。枚举左右各取多少在 $n\le 10^5$ 下为平方级。
>
> 取走的是一段前缀加一段后缀，留下的是中间一个窗口。等价于在剩余字符中每种仍不少于 $k$ 的前提下最大化窗口。先统计全串频次，不足则无解；再滑动窗口扩大右端、在某种字符不足 $k$ 时收缩左端，最长合法窗口对应最少取走长度。
