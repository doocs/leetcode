...
> **思考**
>
> 每个查询要求从左上角出发，只能进入值严格小于查询值的格子，统计可达格数。若对每个查询单独做搜索，$k$ 与网格规模同阶时会重复访问大量格子。
>
> 查询相互独立且对阈值单调：阈值更大时可达集合只会扩张。将查询离线排序后，用小根堆维护当前边界上最小的格子，阈值升高时弹出所有小于该阈值的格子并扩展四邻。每个格子入堆一次，答案按原下标写回。
