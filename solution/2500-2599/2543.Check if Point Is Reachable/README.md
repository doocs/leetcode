...
> **思考**
>
> 从 $(1,1)$ 出发，可把当前点变为 $(x+y,y)$、$(x,x+y)$ 或把某一坐标翻倍。坐标可达 $10^9$，不能搜索。
>
> 前两种移动不改变 $\gcd$，后两种只把 $\gcd$ 乘上 $2$ 的幂，故终点的 $\gcd$ 必须是 $2$ 的幂。反之，从终点反复除二并把较大奇数换成 $(\textit{x}+\textit{y})/2$ 可一直缩小到 $(1,1)$。因此判断 $\gcd(x,y)$ 是否为 $2$ 的幂即可，即 $x\&(x-1)=0$。
