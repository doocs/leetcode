...
> **思考**
>
> 判断 $num$ 是否完全平方。从 $1$ 扫到 $\sqrt{num}$ 为 $O(\sqrt n)$。在值域上对 $x^2$ 二分即可。
>
> 在 $[1,num]$ 中找最小的 $x$ 使 $x^2\ge num$，再检查是否相等。代码用 `bisect_left` 配 $x\mapsto x^2$。
...
> **思考**
>
> 二分依赖有序平方。奇数和 $1+3+\cdots+(2n-1)=n^2$，故反复减递增奇数，减到 $0$ 则为平方。实现更短，时间为 $O(\sqrt n)$。
