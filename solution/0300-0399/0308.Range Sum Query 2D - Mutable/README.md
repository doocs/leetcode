...
> **思考**
>
> 二维矩阵既要改单格，又要求子矩形和。对每次询问现场累加，或每次修改后重建二维前缀，都无法同时压低两类操作。
>
> 代码为每一行建一棵树状数组，行内用前缀和差得到 $[col1,col2]$，再对 $row1\ldots row2$ 的行求和。修改只触及对应行。行数通常不大，故实现比真正的二维树状数组更直接。
...
> **思考**
>
> 方法一对每行独立维护前缀和，矩形询问需扫过涉及的每一行。改为每行一棵线段树后，单点修改与行内区间和同样是 $O(\log n)$，语义不变。
>
> 子矩形仍按行累加各自行内区间和。结构换成显式区间结点，便于理解「一段列范围」的聚合。
