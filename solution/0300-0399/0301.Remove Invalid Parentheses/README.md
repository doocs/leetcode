...
> **思考**
>
> 合法串须删除最少括号且枚举全部最优结果。若对每个括号独立选择删或不删，长度为 $n$ 时分支过多，且多数路径删除次数并不最小。
>
> 先扫描得到必须删除的左、右括号下界 $l$、$r$。搜索时维护剩余删除额度与当前左右括号计数：剩余字符不足以删完、或右括号已多于左括号，则剪枝。每个位置优先尝试删除（额度允许时），再选择保留，并用集合去重。因此只展开删除次数恰好为最小的路径。
