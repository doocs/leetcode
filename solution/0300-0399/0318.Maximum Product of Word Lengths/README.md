...
> **思考**
>
> 两词无公共字母时长度乘积才计入。集合求交为 $O(\sigma)$，词数较多时需更快的正交判定。
>
> 把每个词压成 $26$ 位掩码，按位与为零即无公共字母。对每对 $i>j$ 检查掩码并更新最大乘积。$n\le 1000$ 时 $O(n^2)$ 可接受。
...
> **思考**
>
> 方法一对相同字母集合的词会重复比较。用哈希表保留每种掩码的最大长度，枚举时只与已出现掩码比较。相同集合只留最长串，比较次数下降。
