...
> **思考**
>
> 统计 $x$ 位数以内各位数字均不同的个数，含 $0$。枚举再判重在 $n=8$ 时仍可，但数位 DP 统一处理前导零与已用数字。
>
> $dfs(i,mask,lead)$ 表示还剩 $i+1$ 位、已用数字集合为 $mask$、是否仍前导零。前导零不占用数字；否则把 $j$ 记入 $mask$。$n$ 位从 $i=n-1$ 开始，记忆化后状态很少。
