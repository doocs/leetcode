...
> **思考**
>
> 要求 $nums[0]<nums[1]>nums[2]<\cdots$，且原数组无此序。就地重排即可，不必稳定。
>
> 排序后从中位数向前取小的放偶数位、从末尾向前取大的放奇数位，可避免相邻相等。中位数与最大值反向填充，使相等元素被隔开。
...
> **思考**
>
> 方法一需要 $O(n\log n)$ 排序与额外数组。值域为 $[0,5000]$，用桶计数后从大到小先填奇数位再填偶数位，同样把大数放峰、小数放谷，时间线性。
