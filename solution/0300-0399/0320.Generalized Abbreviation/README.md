...
> **思考**
>
> 每个字符可保留或并入一段连续省略的数字。$n\le 15$，全部方案可枚举。
>
> $dfs(i)$ 处理后缀：要么保留 $word[i]$ 再递归，要么把 $[i,j)$ 收成数字并接上 $word[j]$（若还有）与更短后缀。空后缀返回 $[""]$ 作为拼接基底。
...
> **思考**
>
> 递归按位置分支，结构与 $2^n$ 种取舍一一对应。用二进制位表示「该位是否省略」，连续 $1$ 压成计数、遇到 $0$ 则写出字母。实现改为迭代，避免递归栈。
