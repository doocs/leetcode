...
> **思考**
>
> 戳气球的顺序会改变两侧相邻关系，直接按戳破次序搜索会重复子区间。两端各补 $1$ 后，区间边界成为固定乘数。
>
> 若 $k$ 是开区间 $(i,j)$ 中最后戳破的气球，则左右子区间互不干扰，得分多出 $arr[i]\cdot arr[k]\cdot arr[j]$。为此定义 $f[i][j]$ 为戳完 $(i,j)$ 内全部气球的最大得分，按区间长度递增（代码中 $i$ 降序、$j$ 升序）填表，答案为 $f[0][n+1]$。
