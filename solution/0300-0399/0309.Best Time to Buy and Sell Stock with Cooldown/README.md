...
> **思考**
>
> 买卖可进行多次，卖出后须冷却一天。若按「第几天、是否持有、是否处于冷却」穷举决策树，同一状态会被反复展开。
>
> 状态可压成 $(i,j)$：从第 $i$ 天起、是否持有股票。每天可跳过；持有则可卖出并跳到 $i+2$，空仓则可买入并进入持有。记忆化后每个状态只算一次，卖出多走一天即表达冷却。
...
> **思考**
>
> 方法二中 $f[i]$ 只依赖 $i-1$ 与 $i-2$，无需整张表。用三个变量滚动「前日空仓、昨日空仓、昨日持有」，按原转移更新即可把空间降到 $O(1)$。
