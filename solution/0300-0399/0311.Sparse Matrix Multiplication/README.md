...
> **思考**
>
> 矩阵乘法按定义对每个 $(i,j)$ 累加 $k$ 维内积，规模受约束时 $O(mnk)$ 可接受。稀疏矩阵中大量乘积为零，但仍按三层循环完整计算。
>
> 为此先写出三重循环，保证语义正确，再考虑跳过零元。
...
> **思考**
>
> 方法一仍会把零与任意数相乘。将两矩阵压成每行非零 $(列,值)$ 列表后，只对非零对累加 $ans[i][j]\mathrel{+}=x\cdot y$。零元不再进入内层，常数显著下降，最坏仍为 $O(mnk)$。
