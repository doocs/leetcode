...
> **思考**
>
> 求最大二叉搜索子树的结点数。对每个结点单独验证 BST 会重复遍历。子树是否为 BST 取决于左右极值与自身大小关系。
>
> 后序返回 $(最小值,最大值,规模)$。若左最大 $<$ 根 $<$ 右最小，则合并规模并更新答案；否则返回哨兵 $(-\infty,\infty,0)$ 使祖先无法再并入。空树返回 $(\infty,-\infty,0)$ 以便叶子成立。
