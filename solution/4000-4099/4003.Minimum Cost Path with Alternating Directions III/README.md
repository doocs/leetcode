...
> **思考**
>
> 格子上既可移动也可原地等待，下一步的行列奇偶会翻转，且移动代价还取决于方向是否与当前奇偶相符。$mn\le 10^5$，若只按格子做分层递推，难以同时处理「等待改奇偶」与「方向不匹配时额外罚款」。
>
> 把位置与「下一步是奇步还是偶步」合成状态后，等待与四向移动都成为非负权边：等待只付 $\textit{penalty}$ 并翻转奇偶；移动则加上目标格代价，并在方向与当前奇偶不一致时再付一次罚款。
>
> 该状态图上的最短路即首次到达终点的最小代价，用 Dijkstra 求解。
