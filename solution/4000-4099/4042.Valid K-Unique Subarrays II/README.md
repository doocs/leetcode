...
> **思考**
>
> 判定条件与 I 相同，但询问由上一次答案在线生成，且 $n,q$ 达 $5\times 10^5$，连较高的对数复杂度都偏紧。
>
> 生成式询问不能离线重排，必须支持在线。频次全为偶数仍可用线性预处理的前缀异或哈希在常数时间判断；不同值个数则依赖每种值的出现位置或预计算的前缀结构。
>
> 单次判定应降到常数或极短对数，再按规则推出下一对端点。
