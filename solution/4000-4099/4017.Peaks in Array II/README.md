...
> **思考**
>
> 查询与单点修改均为 $10^5$，不能对每个区间再扫描寻找峰值。
>
> 子数组 $[i,j]$ 是峰值子数组，当且仅当开区间 $(i,j)$ 内存在峰，因此区间答案只依赖其中的峰位置。单点修改至多改变相邻三个下标是否成峰。
>
> 用线段树维护峰的分布以及「覆盖至少一个峰的子数组个数」，即可在修改后局部更新，并在 $[l,r]$ 上直接合并出询问答案。
