...
> **思考**
>
> 合并总是删除右侧字符，最终保留的是从左到右的一条子序列，且相同字符在答案中的相邻距离必须大于 $k$。若按题意反复扫描并执行合并，最坏每次只删一个字符。
>
> 从左扫描时，若当前字符与答案中上一次出现的位置之差不超过 $k$，则它一定会被某次合并删掉；否则应当保留。
>
> 哈希表记下每个字符在答案中的最近下标，按该规则一次扫描即得到最终串。
