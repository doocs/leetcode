...
> **思考**
>
> 把 $n$ 拆成 $k$ 个正整数的方案数为 $\binom{n-1}{k-1}$。乘积为偶数当且仅当至少一份为偶数，因而答案等于总方案数减去「全部为奇数」的方案数。
>
> 全部为奇数当且仅当 $n$ 与 $k$ 同奇偶：令每份 $2a_i+1$，条件化为 $\sum a_i=(n-k)/2$，对应组合数 $\binom{(n+k)/2-1}{k-1}$；否则该项为 $0$。
>
> $n$ 与 $k$ 可达 $5\times 10^5$，单次求组合数必须依赖阶乘与逆元。为此预处理到上限，每次询问 $O(1)$ 取模相减。
