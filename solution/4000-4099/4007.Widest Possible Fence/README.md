...
> **思考**
>
> 同一行栅栏必须等高，木板可以单根使用、两根同高叠放，或两根不同高叠放。若按木板逐根搜索组合，方案数随 $n$ 增长过快。
>
> 高度先计数后，每一种目标行高只来自「高度 $h$ 本身」「两根 $h/2$」或「一对 $x+y=h$」。同一根木板不会同时进入两种配对，故可按高度对直接累加列数。
>
> $n\le 1000$，枚举所有有序高度对是 $O(m^2)$，取这些列数的最大值即可。
