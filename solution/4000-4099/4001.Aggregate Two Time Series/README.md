...
> **思考**
>
> 两段序列均已按时间戳严格递增，长度可达 $10^5$。若先把时刻丢进哈希表再逐点回填缺失值，既要处理「对端尚未出现」的边界，也多一次随机访问。
>
> 对齐本质是有序归并。双指针同时前进，每次取出较早的时刻，并把另一序列当前指针上的值加到该点；时刻相同则两指针一起前进。
>
> 某一序列耗尽后，对端不再提供新的更新，剩余点按原值追加即可。
