...
> **思考**
>
> 两个不重叠的轴对齐正方形，其行区间或列区间必然不相交，因此一定可以被一条水平线或一条垂直线分开。若枚举分割线后再在半边里暴力寻找最大全 $1$ 正方形，总时间过高。
>
> 以格点为角的最大正方形边长可用标准 DP 递推，再对每一行做前缀、后缀最大值，即可在 $O(mn)$ 内处理所有水平分割；两侧边长取较小值后平方，即该分割下的面积。
>
> 垂直分割把矩阵转置后复用同一过程，答案取二者较大值。
