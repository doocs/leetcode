...
> **思考**
>
> 班次依次消耗循环任务队列。若每个班次都从当前任务线性向后模拟，$n$ 与 $m$ 相乘可能偏大。
>
> 任务耗时的前缀和单调，剩余工时还能再完成到哪一项可以二分定位。先用前缀和回答「从某任务之后再做若干完整任务」的耗时，再按班次更新当前下标与已做完的部分工时。
>
> 当剩余时间足够清空整段队列时，指针回到队首，该班次结束时未完成数为 $0$。
