...
> **思考**
>
> 环上每次只能把 $1$ 个单位交给相邻位置，$n\le 1000$。若按移动次序搜索，状态与步数都无法控制。
>
> 盈余格必须把多余单位送到亏空格，单位沿环走的步数即操作次数。这正是以盈余为源、亏空为汇、邻接边容量无穷且代价为 $1$ 的最小费用流；所需流量等于亏空之和。若余额总和为负，则无解。
>
> 环双向连通且边权为 $1$，连续用 SPFA 增广即可；本题 $n$ 不大，最坏 $O(n^3)$ 可以接受。
