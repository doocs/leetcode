...
> **思考**
>
> 箱子必须按序连续装车，每趟有件数、重量上限，行程包含往返仓库以及相邻不同码头的跳转。$f[i]$ 为送完前 $i$ 箱的最少行程，枚举上一趟的分界 $j$。
>
> 码头跳转次数用前缀和 $cs$ 在 $O(1)$ 内得到，再加上往返的 $2$。$n$ 达 $10^5$ 时该 $O(n^2)$ 转移仅作为正确但不实用的基线。
...
> **思考**
>
> 方法一在窗口 $[i-\textit{maxBoxes},i)$ 上求 $f[j]-cs[j]$ 的最小，且还受重量前缀约束。单调队列维护候选 $j$，使转移摊还 $O(1)$。
>
> 队首超出件数或重量则弹出，队尾按 $f-cs$ 保持递增，总时间 $O(n)$。
