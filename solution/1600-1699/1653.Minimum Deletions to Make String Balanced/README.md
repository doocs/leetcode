...
> **思考**
>
> 平衡串是若干 `a` 后接若干 `b`。$n$ 达 $10^5$，不能枚举所有删除方案。从前到后决策时，已出现的 `b` 个数决定「删当前 `a`」还是「删掉这些 `b`」。
>
> $f[i]$ 为前 $i$ 个字符变平衡的最少删除：遇 `b` 则 $f[i]=f[i-1]$ 并累加 $b$；遇 `a` 则 $f[i]=\min(f[i-1]+1,b)$。
...
> **思考**
>
> 方法一按前缀递推。也可枚举分界：左边只留 `a`（删光 `b`），右边只留 `b`（删光 `a`），两边计数之和即删除数。
>
> 维护左侧 `b` 个数与右侧 `a` 个数，扫一遍取最小和，空间同样 $O(1)$。
