...
> **思考**
>
> 秩由同行同列中更小的值决定，相同值还必须同秩。若无重复，按值从小到大扫描、取所在行与列已有最大秩加一即可；$m,n \le 500$ 且存在重复时，同值可能通过「同行/同列」连锁。
>
> 把同一数值的格子按行、列并查（列编号平移 $m$），每个连通块共享「块内行、列最大秩加一」。
>
> 按值排序处理，用行、列最大秩数组更新答案后重置并查集，以免不同数值互相粘连。
