...
> **思考**
>
> 山形数组在峰两侧严格升降。删最少元素等于留下最长山形子序列。$n \le 1000$，可对每个峰算左侧 LIS 与右侧 LDS。
>
> $\textit{left}[i]$、$\textit{right}[i]$ 分别为以 $i$ 结尾的最长上升、以 $i$ 开头的最长下降；峰须满足两者都大于 $1$。
>
> 最少删除为 $n-\max(\textit{left}[i]+\textit{right}[i]-1)$。
