...
> **思考**
>
> 长度为 $n$ 的非降元音串等价于从 $5$ 种字母中可重复地按序选取。$n$ 通常不超过 $50$，按「已放 $i$ 个、最后字母编号为 $j$」搜索即可。
>
> $dfs(i,j)$ 在 $i=n$ 时计 $1$，否则枚举 $k \ge j$ 继续；$\texttt{cache}$ 避免重复状态。
...
> **思考**
>
> 方法一的二维状态可压成「长度为 $i$ 时以各元音结尾的个数」。后一长度是前一长度的后缀和。
>
> 用长为 $5$ 的数组滚动，每次把前缀和写回，最后求和。空间降为常数。
