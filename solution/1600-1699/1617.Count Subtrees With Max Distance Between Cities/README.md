...
> **思考**
>
> 城市数 $n \le 15$，子集只有 $2^n$ 个。一个子集构成子树当且仅当诱导子图连通；其最大距离即该子树直径。
>
> 连通性可用一次 DFS/BFS 看是否走完整点集：把掩码中访问过的位置清掉，结束时掩码为零则连通。直径则用树上两次搜索：先任取一点找到最远点，再从该点出发得最长路径。
>
> 跳过单点掩码，对每个至少两点的 $\textit{mask}$ 做上述检验，将直径 $mx$ 计入 $\textit{ans}[mx-1]$。
...
> **思考**
>
> 方法一用递归 DFS 求直径。同一套「清掩码判连通 + 两次最远点」也可以改成按层 BFS，避免递归深度并明确层次即距离。
>
> 算法结构不变，仅把搜索实现换成队列。
