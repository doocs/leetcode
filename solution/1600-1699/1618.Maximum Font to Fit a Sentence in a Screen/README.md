...
> **思考**
>
> 字体数组升序，字号越大越难放下，可行性关于下标单调，适合二分最大可行字号。
>
> 判定时高度不能超过 $h$，各字符宽度之和不能超过 $w$，均通过 $\texttt{FontInfo}$ 查询。
>
> 在下标区间上二分，取中点字号做 $\texttt{check}$，可行则抬左端，否则收右端；最后再验证 $\textit{fonts}[\textit{left}]$。
