...
> **思考**
>
> 题目给出的 $\textit{Successor}$ 按「当前人最年长且尚未列入顺序的孩子，否则回溯到父亲」递推，若每次查询都按定义从国王一路模拟，单次查询需要反复扫描子女列表，在出生次数达到 $10^5$、家族规模随之增大时，查询会落到平方级扫描。
>
> 注意到该递推恰好对应多叉树的前序遍历：先输出本人，再按出生先后递归各子树。死亡并不改变树形，只是从最终顺序中剔除，因此只需维护子女邻接表与死亡集合。
>
> 为此用哈希表 $g$ 按插入顺序记录子女，用集合 $\textit{dead}$ 标记死亡；出生、死亡均为摊还常数，查询时从国王做一次 DFS，活着的结点按前序写入答案。
