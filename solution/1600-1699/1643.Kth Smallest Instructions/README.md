...
> **思考**
>
> 路径由 $v$ 个 $\texttt{V}$ 与 $h$ 个 $\texttt{H}$ 组成，字典序第 $k$ 条不能把全部 $\binom{h+v}{h}$ 条路径列出来。
>
> 最高位若取 $\texttt{H}$，以 $\texttt{H}$ 开头的条数是 $C_{h+v-1}^{h-1}$。$k$ 大于该数则最高位只能是 $\texttt{V}$ 并扣掉这些方案，否则取 $\texttt{H}$。
>
> 从左到右逐位用组合数决策，直到 $h$ 用尽则其余全为 $\texttt{V}$。
