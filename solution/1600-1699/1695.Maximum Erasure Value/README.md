...
> **思考**
>
> 求元素互异的连续子数组的最大和。$n$ 达 $10^5$，用上次出现位置把左端 $j$ 推到重复点之后，前缀和给出区间和。
>
> 数组 $d$ 记下每个值最近下标，扫到 $v$ 时 $j=\max(j,d[v])$，用 $s[i]-s[j]$ 更新答案。
...
> **思考**
>
> 方法一用前缀和与下标表。也可用滑动窗口维护当前互异段的和：右端纳入时若重复则左端弹出，省掉前缀数组。
