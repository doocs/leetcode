...
> **思考**
>
> 冲突由「更年长反而得分更低」引起。若先按得分升序、得分相同再按年龄升序，则只需保证所选序列年龄不降，即可无冲突。
>
> $n \le 1000$，以某人收队的最大得分可用 $O(n^2)$ 的 LIS 式转移：枚举更早且年龄不超过自己的队员，取其 $f[j]$ 再加自身得分。
>
> 答案为所有 $f[i]$ 的最大值。
...
> **思考**
>
> 方法一枚举前驱是 $O(n^2)$。同一排序下，需要的是「年龄不超过当前值的最大得分」，这是前缀最大值查询。
>
> 树状数组按下标维护最大值：查询 $[1,\textit{age}]$ 的最优，再把 $\textit{score}$ 加回去更新该年龄。复杂度降为 $O(n\log m)$。
