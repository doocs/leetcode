...
> **思考**
>
> 环形数组上每个位置替换为后 $k$ 项或前 $\lvert k \rvert$ 项之和，$k=0$ 则全零。$n$ 与 $\lvert k \rvert$ 都很小，直接按定义累加即可。
>
> 下标对 $n$ 取模处理环；正 $k$ 扫 $[i+1,i+k]$，负 $k$ 扫 $[i+k,i)$。
...
> **思考**
>
> 方法一对每个 $i$ 重复相加。把数组拼接一份求前缀和后，任意环形区间变成两次前缀相减，总时间 $O(n)$。
