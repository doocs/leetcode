...
> **思考**
>
> 两串长度不超过 $100$，合法子串对恰好有一个位置不同。以该不同位置为中心向两侧扩展，左右连续相等的长度决定有多少种截取方式。
>
> 枚举 $(i,j)$ 且 $s[i]\ne t[j]$，向左、向右统计相同前缀/后缀长度 $l,r$，贡献 $(l+1)(r+1)$。
...
> **思考**
>
> 方法一在每个差异点现场向两侧延伸，存在重复比较。可预处理以 $(i,j)$ 结尾的最长相等后缀 $f$ 与开头的最长相等前缀 $g$。
>
> 再枚举差异点时贡献变为 $(f[i][j]+1)(g[i+1][j+1]+1)$，总时间 $O(mn)$。
