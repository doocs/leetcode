...
> **思考**
>
> 每次查询要判断子数组能否重排成等差数列，$n,m \le 500$，对每个区间排序检查公差可行，但更直接的是用最值确定公差再验证每一项是否出现。
>
> 子数组最小、最大值分别为首末项，若 $a_n-a_1$ 不能被 $n-1$ 整除则不可能；否则公差 $d$ 固定，只需集合中包含 $a_1, a_1+d, \ldots, a_n$。
>
> 对每组 $[l,r]$ 建集合并按上述检验，得到布尔数组。
