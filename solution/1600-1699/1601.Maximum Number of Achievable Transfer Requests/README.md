...
> **思考**
>
> 请求条数 $m \le 16$，若对每条请求决定是否采纳，状态空间仅为 $2^m \le 65536$，再对每栋楼核对净流入是否为零，复杂度可以接受。
>
> 可行方案的充要条件是每栋楼的迁入次数等于迁出次数，不必显式模拟人员移动。
>
> 因此枚举掩码 $\textit{mask}$，用 $\textit{bit\_count}$ 作为采纳条数；仅当条数优于当前答案时才检查各楼计数是否全零，从而在保证正确性的同时减少无效校验。
