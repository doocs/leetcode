...
> **思考**
>
> 偶数层须为严格递增的奇数，奇数层须为严格递减的偶数，判断依赖「同一层从左到右的相邻值」。层序遍历恰好按层提供这一顺序。
>
> 结点可达 $10^5$，一次 BFS 即可，不必按层物化数组。
>
> 用队列分层，层内维护前驱 $\textit{prev}$ 与奇偶标志 $\textit{even}$，不满足奇偶或单调性则立即返回。
...
> **思考**
>
> 方法一用队列保存一层。先序 DFS 同样按从左到右访问同层结点，只需记住每层最近一次见到的值。
>
> 哈希表 $d$ 以深度为键记录前驱，递归时同步检查奇偶与单调，空间转为递归栈而非显式队列。
