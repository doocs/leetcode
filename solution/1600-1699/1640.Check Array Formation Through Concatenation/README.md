...
> **思考**
>
> 零件内部顺序固定且元素互异，只需沿 $arr$ 依次找到以当前值为首的零件并核对整段。$n \le 100$，每次线性查找零件即可。
>
> 指针 $i$ 扫描 $arr$，在 $\textit{pieces}$ 中找首元素等于 $arr[i]$ 的一段，匹配失败则无法拼接。
...
> **思考**
>
> 方法一每次扫描全部零件。元素互异，可用首元素映射到整段零件，查找降为 $O(1)$。
>
> 哈希表 $d[p[0]]=p$，再按段比较 $arr[i:i+|p|]$ 是否等于 $p$。
