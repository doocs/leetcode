...
> **思考**
>
> 树中只有加法与字母叶结点，加法满足交换律与结合律，表达式等价当且仅当两侧字母的出现次数相同。结点可达约 $5000$，一次遍历即可统计。
>
> 对第一棵树把字母计数加一，对第二棵减一，最后所有计数为零则等价。
>
> DFS 忽略 `+` 结点，只在叶子上更新计数器 $cnt$。
...
> **思考**
>
> 方法一只处理加法。进阶若出现减法，字母不再是无符号计数，而应视为整数系数。
>
> 对每棵树递归得到长度为 $26$ 的系数向量：叶子置 $1$，`+` 相加，`-` 将右子树取反后相加，两向量相等则等价。
