...
> **思考**
>
> 顾客至多 $10$ 人，每种数的存量要完整分给若干顾客。子集分配可用 $2^m$ 表示谁已被满足。
>
> 先把频次收成 $arr$，并预处理顾客子集的需求之和 $s[j]$。$f[i][j]$ 表示前 $i$ 种数字能否满足顾客集合 $j$。
>
> 枚举 $j$ 的子集 $k$ 分给第 $i$ 种数，当 $s[k]\le arr[i]$ 且剩余集合已被前 $i-1$ 种满足时转移。答案是 $f[n-1][2^m-1]$。
