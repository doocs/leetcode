...
> **思考**
>
> 需要支持在序列末尾追加，以及对当前整个前缀做区间加、区间乘，并单点查询，操作达 $10^5$。对每个元素暴力更新不可行。
>
> 追加相当于在动态右端写入新值，加与乘都作用在 $[1,\textit{idx}]$ 上，这是典型的区间仿射更新。线段树对每个结点维护值以及懒标记 $(\textit{mul},\textit{add})$，先乘后加即可组合。
>
> 动态开点覆盖下标 $[1,10^5]$：追加写单点，`addAll`/`multAll` 改前缀，`getIndex` 查单点，全程取模 $10^9+7$。
