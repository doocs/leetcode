...
> **思考**
>
> 允许的两种操作分别对应「改最低位」与「改最低的 $1$ 的高一位」，$n$ 可达 $10^9$，逐步模拟每种操作不可行。
>
> 这两种规则恰为二进制格雷码的生成关系：把 $n$ 看成格雷码，所求最少步数等于其对应的自然二进制值。
>
> 格雷码转二进制是各位从高到低的前缀异或，写成循环即为反复 $\textit{ans} \oplus= n$ 再右移，直至 $n$ 变为 $0$。
