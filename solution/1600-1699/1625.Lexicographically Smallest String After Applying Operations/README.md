...
> **思考**
>
> 累加与轮转可反复施加，状态图的每个结点至多两条出边。字符串长度不超过 $100$ 且仅含数字，可达状态远小于 $10^{100}$，可用 BFS 穷尽。
>
> 从 $s$ 出发，对奇数位统一加 $a$（模 $10$）以及右旋 $b$ 位，用集合去重，遍历中维护字典序最小串。
...
> **思考**
>
> 方法一按图搜索，队列与哈希有额外开销。累加周期为 $10$，轮转周期不超过 $n$，且 $b$ 为偶数时累加只影响奇数位。
>
> 因此先枚举至多 $n$ 次轮转，再对奇数位枚举 $10$ 次累加；若 $b$ 为奇数，偶数位也可累加，再套一层 $10$ 次枚举，直接取最小串。
