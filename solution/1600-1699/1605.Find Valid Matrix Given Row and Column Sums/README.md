...
> **思考**
>
> 需要构造非负矩阵使各行之和、各列之和分别等于给定数组，且 $\sum \textit{rowSum} = \sum \textit{colSum}$。逐格搜索填数在 $m,n \le 500$ 下不可行。
>
> 对位置 $(i,j)$ 放入 $x=\min(\textit{rowSum}[i],\textit{colSum}[j])$ 并同时扣减对应行、列剩余，剩余问题仍满足行和与列和相等，因而可以继续贪心。
>
> 按行优先扫描整张表即可得到一组可行解，不必回溯。
