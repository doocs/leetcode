...
> **思考**
>
> 前跳 $a$、后跳 $b$ 且不能连续后跳，还有禁止坐标，求到 $x$ 的最少步数。状态是（位置，上一步是否可后跳），边权均为 $1$，适合 BFS。
>
> 位置可能越过 $x$ 再回来，需要右界：综合 $a,b$ 与禁区范围，取 $6000$ 即可覆盖所有有用状态。
>
> 队列存 $(i,k)$，$k=1$ 表示可以后跳；访问过的 $(i,k)$ 不再入队，禁区与越界丢弃。
