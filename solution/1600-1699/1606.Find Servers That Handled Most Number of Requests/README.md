...
> **思考**
>
> 请求数与服务器数均可到 $10^5$，对每个请求线性扫描 $k$ 台机器会超时。需要在空闲集合中快速找到编号不小于 $i \bmod k$ 的第一台，若无则回到编号最小者。
>
> 忙机按结束时间出队后应立刻回到空闲集合，因此用优先队列维护 $(\textit{end},\textit{server})$，用有序集合维护空闲编号。
>
> 请求到达时先释放所有已完成的任务，再在 $\textit{free}$ 上 $\texttt{bisect\_left}(i \bmod k)$；同时用数组 $\textit{cnt}$ 统计处理次数，最后收集达到最大值的编号。
