...
> **思考**
>
> 每次只能删左右端点，使删除之和为 $x$ 且次数最少，等价于中间留下和为 $\sum nums-x$ 的最长子数组。$n$ 达 $10^5$，需要线性查找该子数组。
>
> 前缀和哈希：记下和第一次出现的下标，当 $t-s$ 已出现则更新最长长度，答案为 $n$ 减该长度。
...
> **思考**
>
> 方法一用哈希表。元素均为正，前缀和严格递增，可用双指针把窗口和压到 $s$，省掉额外空间。
