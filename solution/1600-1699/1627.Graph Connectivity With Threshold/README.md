...
> **思考**
>
> 两城连通当存在一条公约数大于 $\textit{threshold}$ 的边链。若对每对城市试 $\gcd$，询问与 $n$ 都可达 $10^4$ 量级时过慢。
>
> 大于阈值的每个 $z$ 会把它的倍数全部连在一起，预先按倍数连边即可覆盖全部直接边。
>
> 并查集在 $z \in (\textit{threshold}, n]$ 上把 $z,2z,3z,\ldots$ 合并，每个询问判断两点是否同根。
