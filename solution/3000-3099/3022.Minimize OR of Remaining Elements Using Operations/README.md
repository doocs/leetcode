...
> **思考**
>
> 每次把相邻两项换成它们的按位与，最多 $k$ 次，$n \le 10^5$。最终序列的 OR 希望尽量小。
>
> OR 的高位比低位更值钱，应从高到低尝试关掉每一位。关掉某位，意味着可以用至多 $k$ 次与运算，把所有「该位为 $1$」的段合并掉。
>
> 对每一位构造探测掩码，扫描数组统计必须额外消耗的合并次数；若不超过 $k$ 则该位可以关掉，否则记入答案。
