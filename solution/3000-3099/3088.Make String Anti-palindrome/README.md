...
> **思考**
>
> 反回文要求 $s[i] \ne s[n-1-i]$，并在所有可达串中取字典序最小。$n$ 为偶数且 $\le 10^5$。
>
> 字典序最小先把字符排序。若中缝两侧已经不同，则整串对位也不同；若中缝相同，说明后半有一段与前半对称冲突，需把后半中更靠后的不同字母换到中缝一带。
>
> 排序后若 $s[m]=s[m-1]$，从后半找到下一段不同字母，与中缝起的对称位置交换；换尽仍冲突则无解。
