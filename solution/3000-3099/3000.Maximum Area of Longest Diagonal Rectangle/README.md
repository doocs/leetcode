...
> **思考**
>
> 矩形数量不超过 $100$，枚举所有矩形并比较对角线与面积均可在线性时间内完成。直接对对角线开方再比较，会引入浮点误差。
>
> 对角线长短只取决于 $l^2+w^2$，面积则是 $l \times w$。若某矩形的平方和更大，则它独占最长对角线；若平方和相同，则应在这些矩形中取面积最大者。
>
> 为此只需一遍扫描，维护当前最大平方和 $\textit{mx}$ 以及对应面积 $\textit{ans}$。平方和更新时改写面积，相等时对面积取最大值。
