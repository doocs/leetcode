...
> **思考**
>
> 子集须排成 $x, x^2, x^4, \ldots, x^{2^{k}}, \ldots, x^4, x^2, x$，长度为奇数，$n \le 10^5$。枚举所有子集不可行。
>
> 链由反复平方确定，中间元素只出现一次，其余每个值需要偶数次。从每个 $x$ 沿平方走，直到次数不足 $2$。
>
> $x=1$ 的平方链退化，单独按「尽量取奇数长度」处理。哈希表提供 $O(1)$ 次数查询。
