...
> **Thinking**
>
> A subset must form $x, x^2, x^4, \ldots, x^{2^{k}}, \ldots, x^4, x^2, x$ and have odd length. $n \le 10^5$ rules out subset enumeration.
>
> The chain is generated by repeated squaring. The middle value is used once; every other value needs an even count. From each $x$ we square until fewer than two copies remain.
>
> $x=1$ collapses under squaring and is handled separately as the largest odd count. A hash map supplies the frequencies.
