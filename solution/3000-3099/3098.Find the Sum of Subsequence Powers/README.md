...
> **思考**
>
> 子序列能量是其中任意两数差的最小值，求所有长为 $k$ 的子序列能量之和。$n \le 50$。
>
> 排序后相邻被选元素的差才可能成为该子序列的最小差，状态可用「当前位置、上一个选中下标、剩余个数、当前最小差」描述。
>
> 记忆化 $\textit{dfs}(i,j,k,\textit{mi})$：不选 $i$ 则保持 $j$；选 $i$ 则用 $\textit{nums}[i]-\textit{nums}[j]$ 更新最小差。排序保证差为非负。
