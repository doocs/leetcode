...
> **思考**
>
> 在 I 的基础上增加「把某下标直接置零」的操作，且 $n,m \le 5000$。单调性仍在，但每个下标可以选择「用置零秒清空」或「用普通减一清空」。
>
> 置零更适合 $nums[i]$ 较大的下标，且每个下标至多使用一次最早的置零机会。判定某个 $t$ 时，应用优先队列在「省下的减一次数」与「置零占用的秒」之间权衡。
>
> 对 $t$ 二分，倒序扫描前 $t$ 秒：首次出现的下标若 $nums[i]>0$ 则压入堆，必要时弹出堆顶改用减一，使剩余秒数足够标记所有下标。
