...
> **思考**
>
> 任意两串之间可交换字符，目标是让尽量多的串成为回文。$n \le 1000$，总长有限。
>
> 回文只关心偶数位置的成对字母；所有串的字母可视为一个公共池。奇数次字母各浪费一个，剩余成对容量按串长从短到长分配最省。
>
> 异或掩码统计奇数次字母个数，总长减去该个数得到可用偶数位。按长度排序后依次扣 $2\lfloor |w|/2 \rfloor$。
