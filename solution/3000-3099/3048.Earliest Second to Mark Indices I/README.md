...
> **思考**
>
> 第 $s$ 秒只能把 $\textit{changeIndices}[s]$ 对应的值减一或标记（需已为 $0$）。$n,m \le 2000$。能在 $t$ 秒完成则更长时间也能完成。
>
> 每个下标应尽量在其于前 $t$ 秒中最后一次出现时再标记，以便把更早的秒用来减其它数。
>
> 对 $t$ 二分：用最后出现时刻表模拟，非最后出现的秒当作一次减一额度；最后出现时额度必须够 $nums[i]$。
