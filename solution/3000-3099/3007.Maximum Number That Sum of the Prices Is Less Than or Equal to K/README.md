...
> **思考**
>
> 价值随 $\textit{num}$ 单调不减，而 $k$ 可达 $10^{15}$，不能逐个累加。
>
> 判定「$1$ 到 $\textit{mid}$ 的价值是否 $\le k$」是一个典型的数位计数问题：每一位是否计分只取决于其下标是否为 $x$ 的倍数。
>
> 因此对 $\textit{num}$ 二分，并用记忆化按二进制位统计：在受限/非受限下累计已经置位的有价位个数。
