...
> **思考**
>
> 对每个询问串，在容器中找最长公共后缀，并列时取更短串，再并列取更小下标。总长约 $10^4 \times 5 \times 10^3$，两两比较后缀不可行。
>
> 公共后缀是反转后的公共前缀，适合字典树。每个结点维护该后缀下最优的（最短长度、最小下标）。
>
> 倒序插入容器串并沿途更新最优，询问时倒序尽量走深，无法再走则返回当前结点的下标。
