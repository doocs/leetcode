...
> **思考**
>
> 对每个子序列 $S$，再统计 $S$ 的和为 $k$ 的子序列个数，最后对所有 $S$ 求和。$n,k \le 100$。
>
> 元素有三种身份：不在 $S$、在 $S$ 但不在求和子序列 $T$、在 $T$ 中。前两种对和的贡献相同，系数均为上一状态。
>
> 因此 $f[i][j]=2f[i-1][j]+f[i-1][j-x]$，初值 $f[0][0]=1$。
...
> **思考**
>
> 方法一的转移只依赖上一行的 $j$ 与 $j-x$，第一维可以去掉。
>
> 倒序更新一维数组，避免把本轮已写入的 $j-x$ 再次使用，空间降为 $O(k)$。
