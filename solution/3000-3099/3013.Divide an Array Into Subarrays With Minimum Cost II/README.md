...
> **思考**
>
> 与 I 不同，$k$ 与窗口跨度 $\textit{dist}$ 可变且 $n \le 10^5$。第一段仍固定带走 $\textit{nums}[0]$，其余 $k-1$ 个段首必须落在长度为 $\textit{dist}+1$ 的窗口内。
>
> 每个窗口要求其中最小的 $k-1$ 个数之和。朴素对每个窗口排序过慢。
>
> 用两个有序集合分别保存窗口内最小的 $k-1$ 个数与其余数，并维护前者的和。窗口滑动时按大小决定插入左或右集合，再平衡两边的基数。
