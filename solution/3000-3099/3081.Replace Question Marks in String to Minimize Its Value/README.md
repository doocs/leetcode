...
> **思考**
>
> 把 `?` 换成小写字母，使 $\sum \textit{freq}(\textit{freq}-1)/2$ 最小，且结果字典序尽量小。$n \le 10^5$。
>
> 该平方和在频次越均衡时越小，因此每次应把一个 `?` 给当前出现次数最少的字母。得到的字母多重集再按字典序填回 `?` 的位置。
>
> 小根堆维护 $26$ 个 $(\textit{cnt},c)$，弹出写入列表后排序，再按原串顺序替换问号。
