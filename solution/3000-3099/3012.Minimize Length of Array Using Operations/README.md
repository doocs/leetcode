...
> **思考**
>
> 操作把两个正数换成其余数，长度减一，$n \le 10^5$ 不能模拟到只剩少数元素。
>
> 余数严格小于被减的较大数，故全局最小值 $\textit{mi}$ 一旦存在且并非所有数都是它的倍数，就能造出比 $\textit{mi}$ 更小的值，再与其余数配对后只剩一个。
>
> 若所有数都是 $\textit{mi}$ 的倍数，则无法造出更小正数，剩下的只能是 $\textit{mi}$ 本身，两两合并后长度为 $\lceil \textit{cnt}/2 \rceil$。
