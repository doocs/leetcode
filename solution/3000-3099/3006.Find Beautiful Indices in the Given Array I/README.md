...
> **思考**
>
> $|s| \le 10^5$，而 $|a|,|b| \le 10$，朴素在每个位置比较模式可行，但随后对出现位置两两判定 $|i-j| \le k$ 会达到平方级。
>
> 模式匹配可用 KMP 在线性时间内列出 $a$、$b$ 的全部出现下标。两组下标均单调，对每个 $i$ 只需在 $b$ 的列表上推进指针，寻找距离不超过 $k$ 的最近 $j$。
>
> 代码先求前缀函数再搜索，随后用双指针扫描两份出现列表。
