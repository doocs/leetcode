...
> **思考**
>
> 统计 $[a,b]$ 中各位互不相同的整数。区间可写成 $f(b)-f(a-1)$。按位枚举会遇到前导零与重复数字。
>
> 用二进制掩码记录已用数字，并带上是否贴上界，即可按数位 DP 计数。$b \le 1000$，状态很少。
>
> 记忆化 $\textit{dfs}(\textit{pos}, \textit{mask}, \textit{limit})$，前导零不占用掩码，空掩码表示尚未形成数字。
...
> **思考**
>
> $a,b \le 1000$，数位 DP 的状态开销并无必要。
>
> 直接枚举区间内每个整数，用集合判断各位是否互异即可，实现更短。
