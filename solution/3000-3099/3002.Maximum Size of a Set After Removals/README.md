...
> **思考**
>
> 两数组等长 $n$ 且为偶数，$n \le 2 \times 10^4$，需各删去 $n/2$ 个元素后使并集最大。枚举删除方案不可行。
>
> 并集大小受三部分约束：仅出现在 $\textit{nums}_1$ 的值、仅出现在 $\textit{nums}_2$ 的值，以及交集。每边最多保留 $n/2$ 个位置。
>
> 因此先把两边的独有元素尽量填满配额，剩余空位再由交集填补，最后与 $n$ 取最小值。集合差与交一次性给出这三类计数。
