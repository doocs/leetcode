...
> **思考**
>
> 在位置 $i$ 收集 $k$ 个 $1$，可用邻格一步拿走、用限额把 $0$ 变成邻格再拿走，或从更远处移来。$n \le 10^5$。
>
> 站立位置值得枚举。邻格与 $\textit{maxChanges}$ 的用量是贪心确定的；更远的 $1$ 应按距离由近到远，其个数与代价可用前缀个数、前缀下标和在倍增区间上二分。
>
> 对每个 $i$ 先用自身与左右，再消耗限额，剩余用二分半径 $d$ 使两侧 $[i-d,i-2]\cup[i+2,i+d]$ 内的 $1$ 够用，并用前缀和计算移动步数。
