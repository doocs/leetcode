...
> **思考**
>
> 棋盘仅 $8 \times 8$，枚举车、象的走法并模拟阻挡在常数时间内可行。车与象各自最多走一步即可到达任意与自己共线的格子，因此答案至多为 $2$。
>
> 真正需要判定的是「一步能否直接吃到皇后」。车同行列、象同对角线时，只要中间没有另一枚己方棋子挡住，一步即可。
>
> 代码用乘积 $(d-b)(d-f)>0$ 等判断挡子是否落在开区间外侧。四类共线均不成立时，车绕行两步总能吃到皇后。
