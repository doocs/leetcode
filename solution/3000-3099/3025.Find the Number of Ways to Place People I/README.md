...
> **思考**
>
> $n \le 50$，枚举左上角与右下角后检查矩形内是否有其它点，最坏 $O(n^3)$ 可过，但重复检查较多。
>
> 将点按 $x$ 升序、$x$ 相同时按 $y$ 降序排列后，从左到右扫描时候选右下角的 $y$ 必须严格递增，否则会落入已形成的矩形内部。
>
> 为此对每个左上角维护已选用的最大 $y_2$，仅当 $\textit{maxY} < y_2 \le y_1$ 时计一对并更新 $\textit{maxY}$。
