...
> **思考**
>
> 把整数看作十进制串，求两数组间最长公共前缀长度。$n \le 5 \times 10^4$，对每对数字比前缀过慢。
>
> 一个数的全部前缀即不断整除 $10$ 得到的值。把 $\textit{arr}_1$ 的前缀放进哈希集合后，对 $\textit{arr}_2$ 的每个数从自身向短前缀查找。
>
> 命中的最大数值即最长前缀对应的整数，其十进制位数为答案。
