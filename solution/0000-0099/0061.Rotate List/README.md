...
最容易想到的是把链表转成数组，右移 $k$ 位再重建。节点数 $n \le 500$，空间上没问题；但 $k$ 最大到 $2 \times 10^9$，不能真的转 $k$ 圈。

慢在「转多少次」和「新头在哪」。右旋 $k$ 次等价于右旋 $k \bmod n$ 次，新头就是倒数第 $k$ 个节点。链表没有下标，只能先量出 $n$，再在一次遍历里卡到断点。

快慢指针间距恰好是有效的 $k$，同步走到尾时慢指针自然停在新头前驱，不必再数 $n-k$。整段只改几个指针，额外空间 $O(1)$。
