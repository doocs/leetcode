...
最容易想到的是先尽量往一行里塞单词，再用空格补齐。单词数 $\le 300$、行宽 $\le 100$，线性扫描就能做，不必回溯。

难点不在复杂度，而在空格规则：最后一行和只含一个单词的行必须左对齐；其余行要把多余空格尽量均分，多出来的空格优先给左边的间隙。

所以先贪心取出本行能放下的单词，再按是否末行 / 单词数是否为 $1$ 分成两种排版。从左到右处理每一行即可。
