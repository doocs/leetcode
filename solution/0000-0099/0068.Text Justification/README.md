...
> **思考**
>
> 题目要求尽可能多地往每一行放置单词，再用空格补齐到 $\textit{maxWidth}$。单词数不超过 $300$、行宽不超过 $100$，从左到右贪心取词即可，不必回溯。
>
> 复杂度并不构成瓶颈，需要仔细处理的是空格分配：最后一行以及只含一个单词的行必须左对齐；其余行应将多余空格尽量均分，不能均分时左侧间隙多放一格。因此先取出本行能放下的单词，再按是否为末行、单词数是否为 $1$ 分成两种排版。
