...
最容易想到的是枚举每个子串再用栈检查是否有效。正确，但 $n \le 3 \times 10^4$ 时 $O(n^2)$ 乃至 $O(n^3)$ 过不了。

慢在大量重叠：以某个位置结尾的最长有效括号，可以由更短的有效段拼出来。

有效串只能以右括号结尾。若它和前一位配成一对，长度就是再往前那段加 $2$；若前一位也是右括号，则先跨过已经配好的一段，再看这段之前能不能再配上一个左括号。

于是定义 $f[i]$ 为以 $s[i-1]$ 结尾的最长有效长度，按这两种配对方式转移，答案取 $\max f[i]$。
...
方法一已经 $O(n)$，但转移要分「前一位是不是左括号」以及「跨过一段有效括号再往前配对」，边界容易漏。

我们缺的不是更快的复杂度，而是更贴合「配对」的记录方式：尚未匹配的左括号下标，以及上一段失效从哪开始。

栈底先放 $-1$ 当基准，遇左括号压下标，遇右括号弹出；栈空则当前位置成为新基准，否则用栈顶更新长度。时间和空间仍是 $O(n)$，换的是匹配过程更直观。
