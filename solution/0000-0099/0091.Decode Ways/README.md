...
最容易想到的是递归：走到第 $i$ 位，可以单独解 $s[i]$（非 $0$），或与前一位拼成 $10$–$26$。$n \le 100$，不加记忆会指数爆炸，同一前缀会被反复计算。

卡点是「前 $i$ 个字符有多少种解法」只取决于更短前缀，而且合法划分只有「一位 / 两位」两种。这就是爬楼梯：把字符串换成数字编码的约束。

于是令 $f[i]$ 为前 $i$ 个字符的方案数。空串一种，$0$ 不能单拆，$10$–$26$ 才能双拆。按位递推即可，不必真去枚举所有划分。
...
方法一已经正确。转移只用到 $f[i-1]$ 和 $f[i-2]$，整表没有必要留下。滚动两个变量即可把空间压到 $O(1)$，时间仍是一遍扫描。
