...
若枚举矩形的左上、右下两个角点，再检查内部是否全为 $1$，时间复杂度为 $O(m^2 n^2)$。$m, n \le 200$，无法通过。

同一片区域会被反复检查。注意到：固定底边所在行 $i$ 后，第 $j$ 列向上连续 $1$ 的个数，恰好是一根柱子的高度。因此每一行都可以化成一道「柱状图中最大的矩形」，而后者已由第 84 题用单调栈在线性时间内解决。

为此我们逐行维护 $\textit{heights}[j]$：当前格为 $1$ 则高度加一，为 $0$ 则清零，再对当前直方图求最大矩形。总时间 $O(mn)$。
