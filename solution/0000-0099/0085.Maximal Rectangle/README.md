...
最容易想到的是枚举任意两点当矩形对角，再检查内部是否全 $1$。正确，但时间 $O(m^2 n^2)$。$m, n \le 200$，必超时。

慢在二维范围被反复检查。观察：以第 $i$ 行为底、第 $j$ 列向上连续 $1$ 的个数，正好是一根柱子的高度。于是每一行变成一道「柱状图中最大的矩形」。

这就是第 84 题。逐行维护 $\textit{heights}[j]$：遇 $1$ 加一，遇 $0$ 清零，再对当前直方图跑单调栈。总时间 $O(mn)$。
