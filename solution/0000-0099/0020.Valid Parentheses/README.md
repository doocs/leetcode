...
最容易想到的是反复把 $()$、$[]$、$\{\}$ 从串里抠掉，直到抠不动。正确，但最坏 $O(n^2)$。$n\le 10^4$ 勉强，写法也笨。

括号匹配是「最后打开的最先关上」，缺的是后进先出。左括号只是在等自己的右括号；遇到右括号时，必须与当前尚未匹配的最近一个左括号成对。扫完栈应为空，否则有左括号没关上。

于是我们用栈存左括号，右括号到来时弹出比对。
