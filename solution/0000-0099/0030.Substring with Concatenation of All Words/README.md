...
最容易想到的是枚举每个起点，再判断长度为 $n\cdot k$ 的窗口是不是 $words$ 的某个排列。$m \le 10^4$、$n \le 5000$、$k \le 30$，每个窗口再切 $n$ 段，最坏接近 $O(mnk)$，过不了。

慢在相邻窗口大量重叠却从头统计。单词等长，窗口只能按 $k$ 对齐滑动，词频可以用哈希表差分维护。

对齐方式只有 $k$ 种（起点 $0,1,\ldots,k-1$）。每种里右端每次吃一个长为 $k$ 的词：不在 $cnt$ 里就整窗作废；某词超了就从左边吐，直到频次合法；窗口刚好 $n$ 个词则记下左端。

每个字符最多进出窗口常数次，总时间 $O(mk)$。
