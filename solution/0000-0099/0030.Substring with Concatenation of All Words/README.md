...
> **思考**
>
> 串联子串的长度固定为 $n\cdot k$。若枚举 $s$ 的每个起点再判断该窗口是否为 $words$ 的某个排列，每个窗口还需切成 $n$ 段，时间约为 $O(mnk)$。在 $m \le 10^4$、$n \le 5000$、$k \le 30$ 的范围下无法通过。
>
> 单词等长，合法窗口的起点必然落在模 $k$ 的同一剩余类中，且相邻窗口只相差一个单词。因此词频可以用哈希表沿窗口差分维护，而不必每个起点重新统计。对齐方式只有 $k$ 种。
>
> 为此对每个余数 $i \in [0, k)$ 维护一个滑动窗口：右端每次纳入一个长为 $k$ 的词；词不在 $cnt$ 中则整窗作废；某词次数过多则收缩左端；窗口恰好含 $n$ 个词时记下左端。每个字符进出窗口常数次，总时间 $O(mk)$。
