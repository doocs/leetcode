...
最容易想到的是 DFS/BFS 搜索所有可达点，或 DP 判断每个下标能否到达。正确，但最坏 $O(n^2)$。$n \le 10^4$ 偏紧，而且我们只关心终点，不必记下每一步怎么跳。

慢在「对每个位置展开全部跳跃」。能到达的下标是一段前缀：只要维护当前最远可达 $mx$，若某 $i > mx$ 就断了。

于是从左扫一遍，用 $i + \textit{nums}[i]$ 更新 $mx$，能扫完全部就说明终点可达。
