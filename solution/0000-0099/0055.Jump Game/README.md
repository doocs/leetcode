...
题目只关心最后一个下标是否可达。从每个位置展开全部跳跃，或用动态规划判断每个下标能否到达，最坏时间均为 $O(n^2)$。$n \le 10^4$ 偏紧，且不必记下中间每一步如何跳。

能到达的下标构成一段前缀：维护当前最远可达位置 $mx$，若出现 $i > mx$，则之后均不可达。

因此从左扫描，用 $i + \textit{nums}[i]$ 更新 $mx$；若能扫完全部下标，终点即可达。
