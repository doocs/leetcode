...
最容易想到的是枚举长度为 $2n$ 的所有括号串，再筛合法者。$n \le 8$，一共 $2^{16}$ 条，能过，但绝大多数中途已经非法。

慢在「先生成再判断」：一旦某前缀里右括号多于左括号，或某一边超过 $n$，后面怎么补都救不回来。

合法前缀只需维护左、右括号个数 $l$、$r$：始终 $l \ge r$，且二者都不超过 $n$。$l=r=n$ 时收进答案。

于是 DFS 每次尝试追加 `(` 或 `)`，用上述三个不等式剪枝。搜索树比全枚举小得多，空间只占当前串的 $O(n)$。
