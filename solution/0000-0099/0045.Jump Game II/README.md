...
最容易想到的是动态规划：令 $f[i]$ 为到达 $i$ 的最少跳跃次数，从每个 $i$ 更新 $i+1..i+\textit{nums}[i]$。正确，时间最坏 $O(n^2)$。$n \le 10^4$ 勉强能过，但我们只关心终点，不必算出每个下标的精确次数。

慢在「对每个位置维护最少步数」。观察：第 $k$ 步能覆盖一段连续区间，这段里能探到的最远位置，就是第 $k+1$ 步的覆盖右端。

于是扫一遍，维护当前这一跳的边界和下一跳能到的最远。走到边界就不得不跳，步数加一。贪心按层扩展，等价于 BFS 却不用队列。
