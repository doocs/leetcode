...
> **思考**
>
> 若先收集所有小于 $x$ 的节点，再收集其余节点并重建链表，结果正确，$n \le 200$ 时间上也没有压力。然而这样要么拷贝节点，要么二次遍历，还容易打乱分区内部的相对次序。
>
> 题目要求的是稳定划分：小于 $x$ 的相对顺序、以及大于等于 $x$ 的相对顺序都不变。因此只需一遍扫描，把节点按取值分流到两条链上，最后把较小一侧的尾接到较大一侧的头。
>
> 指针接到原节点即可，不必另开值拷贝；较大一侧的尾要置空，以免与原后继成环。
