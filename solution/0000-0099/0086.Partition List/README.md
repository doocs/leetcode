...
最容易想到的是扫两遍：先收集小于 $x$ 的节点，再收集其余节点，重建链表。正确，$n \le 200$ 完全够，但要额外节点或二次遍历，也容易打乱相对顺序。

慢在「拆开再拼」没有一次走完。题目只要稳定划分：小于的相对次序、大于等于的相对次序都不变。

于是开两条链，一遍分流，最后把小的尾巴接到大的头上。尾指针接到原节点即可，不必拷贝；大链尾要置空，避免成环。
