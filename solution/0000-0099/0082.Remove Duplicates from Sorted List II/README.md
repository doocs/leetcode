...
最容易想到的是哈希计数：扫一遍记频次，再建一条只含出现一次的节点。正确，且 $n \le 300$ 完全够，却浪费了「已排序、重复必相邻」，还要 $O(n)$ 额外空间。

慢在没利用相邻性。同一值挤成一段，扫到一段就能决定整段去留。

关键是头节点也可能被删，要用哑节点；以及「整段都删」时前驱不能跟着走。于是 $pre$ 停在上一段保留节点，$cur$ 滑过当前值段：若 $pre.next$ 仍是 $cur$，说明这段只有一个，前驱前进；否则整段扔掉。
