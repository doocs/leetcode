...
最容易想到的是枚举全部排列再取当前序列的下一个。正确，但 $n!$ 在 $n \le 100$ 时不可行，题目还要求原地、只用常数额外空间。

慢在我们并不需要全集，只要字典序里紧挨着的那一个。下一个排列必须比当前大，且改动尽量靠右、幅度尽量小。

从右看，最长降序后缀已经是这段能排的最大形态；它左边的 $nums[i]$ 才是要抬高的枢轴。后缀里刚好比 $nums[i]$ 大的数在最右侧，换完再把后缀翻成升序，改动就落在最右边。整段都降序则不存在更大排列，反转即回到最小排列。
