...
> **思考**
>
> 若枚举 $s$ 的所有子串再检查是否覆盖 $t$，窗口有 $O(m^2)$ 个，$m \le 10^5$ 无法通过。进阶要求 $O(m+n)$。
>
> 覆盖一旦成立，左端多余字符可以移出窗口；尚未覆盖则只能继续右扩。因此维护 $t$ 的字符配额 $\textit{need}$、窗口计数 $\textit{window}$，并用 $\textit{cnt}$ 记录已经凑齐的必要出现次数，判断覆盖只需常数时间。右指针扫描 $s$，窗口合法时尽量收缩左端并记录最短区间。
