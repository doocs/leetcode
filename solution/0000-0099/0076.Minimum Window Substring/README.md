...
枚举 $s$ 的所有子串再检查是否盖住 $t$，窗口有 $O(m^2)$ 个，$m \le 10^5$ 必超时。进阶要 $O(m+n)$。

覆盖一旦成立，左端多出来的字符可以丢掉；还不覆盖就只能右扩。用 $\textit{need}$ 记下 $t$ 的配额，用 $\textit{window}$ 记窗口，再用 $\textit{cnt}$ 统计已经凑齐的必要出现次数，判断覆盖就是 $O(1)$。右指针扫 $s$，窗口合法就尽量缩左并记下最短区间。
