...
子集就是每个元素选或不选，共 $2^n$ 个。$n \le 10$，全部列出即可。朴素做法是为每个掩码再建一次数组，或递归做同样的事。

递归更直观：从下标 $i$ 出发，先不选 $nums[i]$ 再选，回溯弹出。扫到 $i=n$ 时把当前路径拷进答案。元素互不相同，不会产生重复子集。
...
方法一靠递归栈把「选/不选」展开。$n$ 很小，也可以把每个子集对应成 $[0,2^n)$ 里的一个掩码：第 $i$ 位为 $1$ 就收 $nums[i]$。少一层函数调用，复杂度同为 $O(n \times 2^n)$。
