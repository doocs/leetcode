...
整段反转可以先切出 $[left,right]$、反转、再接回去，但要处理 $left=1$ 时头结点被换掉，还要记四五个断点，容易写漏。进阶要求一趟扫描。

关键观察：把区间内的结点一个个「头插」到区间前驱后面，就地完成反转，不必单独再接。虚拟头挡住 $left=1$ 的边界，先走到 $left-1$，再把头插做 $right-left+1$ 次。
