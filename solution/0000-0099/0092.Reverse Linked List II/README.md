...
> **思考**
>
> 可以把 $[left, right]$ 整段切出、反转后再接回去，但 $left = 1$ 时头结点会被换掉，还要同时记住多个断点，容易写漏。进阶要求一趟扫描完成。
>
> 若把区间内的结点依次头插到区间前驱之后，即可就地反转，不必单独再拼接。虚拟头挡住 $left = 1$ 的边界：先走到第 $left-1$ 个结点，再把头插做 $right - left + 1$ 次。
