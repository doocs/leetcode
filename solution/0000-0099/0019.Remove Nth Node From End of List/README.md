...
先遍历一遍得到长度 $L$，再走 $L - n$ 步删除结点，时间复杂度为 $O(n)$。在 $sz \le 30$ 的范围内可以接受，但题目进阶要求一趟扫描。

不到末尾就无法确定倒数第 $n$ 个结点的位置，而删除还需要它的前驱。我们可以让快指针先走出 $n$ 步的间距，再与慢指针一起前进：快指针到达表尾时，慢指针恰好停在待删结点的前驱。设置虚拟头结点，可使删除头结点与删除中间结点共用同一套逻辑。
