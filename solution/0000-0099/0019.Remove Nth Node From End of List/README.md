...
最容易想到的是先走一遍数出长度 $L$，再走 $L-n$ 步删结点。$sz\le 30$ 能过，但题目进阶要求一趟扫描。

难点是：不到末尾就不知道倒数第 $n$ 个在哪，而删除还需要它的前驱。让快指针先走出 $n$ 步的间距，再与慢指针一起走；快指针到尾时，慢指针恰好停在待删结点的前驱。虚拟头结点让删头与删中间同一套逻辑。

于是我们用快慢指针一次遍历即可定位并删除。
