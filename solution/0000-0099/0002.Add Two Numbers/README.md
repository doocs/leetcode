...
最容易想到的是把两条链表转成整数再相加。链表最长只有 $100$，大整数能过，但题目已经按逆序把个位放在链头，本质就是竖式加法，转成整数反而绕开了结构。

难点不在运算次数，而在两条链长度可能不同、最高位还可能多出进位。每一位都是「当前两位数字（缺了当 $0$）加上进位 $carry$」，再拆出新的个位和新的进位。

因此我们从哑节点往后接答案，两指针同步前进；循环条件必须是 $l_1$ 或 $l_2$ 或 $carry$ 仍在，否则像 $[9]+[1]$ 会丢掉最后的 $1$。
