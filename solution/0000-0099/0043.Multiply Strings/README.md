...
> **思考**
>
> 题目不允许将输入直接转为整数，也不允许使用内置大整数库。两串长度可达 $200$，普通整型无法容纳乘积。
>
> 竖式乘法给出位置关系：$\textit{num1}$ 的第 $i$ 位与 $\textit{num2}$ 的第 $j$ 位之积只贡献到固定的数位，各位的进位可以在全部累加之后再统一处理。
>
> 乘积长度至多为 $m+n$。我们可以申请长度为 $m+n$ 的数组作为草稿纸，按位累加后再从低位向高位进位，最后去掉可能的前导零。
