...
皇后不能同行、同列或同对角线。若枚举列的全排列再检查对角线，$n \le 9$ 时至多 $9!$ 种排列可以搜完，但冲突往往在中途已经出现，等到整盘放完才验证会浪费大量搜索，棋盘字符串也只能在叶子处构造。

一行恰好一个皇后之后，冲突只可能来自同列或同对角线。同一列、同一正对角线 $i+j$、同一反对角线 $n-i+j$ 都可以用数组在 $O(1)$ 时间内查询与更新。

为此我们按行搜索，只在三个标记均空闲时落子，并在棋盘上同步写入，回溯时撤销标记。
