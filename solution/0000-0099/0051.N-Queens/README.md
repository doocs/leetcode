...
最容易想到的是枚举所有列排列：第 $i$ 行皇后放在第 $p_i$ 列，再检查是否共对角线。$n \le 9$，全排列最多 $9!$，能搜完，但要等整盘放完才发现冲突，还要额外构造棋盘字符串。

慢在「放完再验」。一行只能放一个皇后，冲突只可能来自同列或同对角线；列、$i+j$、$n-i+j$ 都可以 $O(1)$ 标记。

于是按行 DFS：合法才落子并标记，搜完回溯。棋盘 $g$ 同步改 `'Q'`/`'.'`，搜到第 $n$ 行时拷贝一份答案。
