...
最容易想到的是枚举左右端点再求和，时间 $O(n^2)$ 或 $O(n^3)$。$n \le 10^5$，必超时。

慢在子数组互相重叠却每次重算。固定右端点 $i$ 时，最优左端要么接着上一段（若上一段和为正），要么从 $i$ 重新开始。

于是令 $f[i]$ 为以 $i$ 结尾的最大和，$f[i]$ 只依赖 $f[i-1]$，一个变量滚动即可，答案取全程最大。
...
方法一已经 $O(n)$ 时间和 $O(1)$ 空间，扫描一遍就够用。题目进阶要求分治，而 DP 依赖「以右端点结尾」的线性递推，切不成互不重叠的子问题。

缺的是另一条观察：最优段要么全在左半、全在右半，要么跨过中点。跨中点时左右各扫一遍最大后缀/前缀再相加。时间变成 $O(n \log n)$，用来回应进阶，不是为了更快。
