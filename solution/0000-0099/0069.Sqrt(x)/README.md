...
最容易想到的是从 $0$ 枚举到 $x$，找最大的 $i$ 使 $i^2 \le x$。$x$ 最大约 $2^{31}-1$，线性扫太慢，且 $i^2$ 容易溢出；题目也不允许用 `pow`。

答案具有单调性：$mid^2 \le x$ 则更大的候选仍可能合法，否则只能往左。于是在 $[0, x]$ 上二分。

比较时写成 $mid > x / mid$，避免乘法溢出；取上中位数并收缩成「最大的可行值」，结束时左端点就是整数平方根。
