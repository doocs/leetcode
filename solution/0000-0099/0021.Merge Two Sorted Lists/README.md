...
最容易想到的是把两个链表的值取出来排序再重建。节点总数不超过 $100$，能过，但浪费了「已经有序」这一条件。

慢在不该重排：每一步只需在两个头结点里选更小的那个。选完之后，剩下的仍是两条有序链表，问题规模减一，结构不变。

于是比较 $l_1$、$l_2$ 的头：较小者留下，把它的 $\textit{next}$ 接到「合并剩余部分」的结果上。某一边为空时，另一边整段接上即可。

递归深度等于节点总数，本题长度很小，空间 $O(m+n)$ 可接受。
...
方法一正确，但递归栈与节点数同阶。本题虽短，更稳妥的写法是用常数额外空间完成同一选择。

观察没变：每一步仍是比较两个头，接上较小者。用虚拟头 $dummy$ 和尾指针 $curr$ 在循环里做这件事，某条链表耗尽后把另一条整段接上。
