...
按规则从左到右扫一遍即可，$n \le 200$ 没有复杂度压力。真正容易写错的是顺序：先丢掉前导空格，再吃至多一个正负号，然后只收连续数字，遇非数字就停，最后夹到 $32$ 位。

不允许用 $64$ 位时，不能先算完再裁。我们累加绝对值，一边乘 $10$ 一边和 $\lfloor (2^{31}-1)/10 \rfloor$ 比较，越界就按符号饱和成 $2^{31}-1$ 或 $-2^{31}$。

正负共用同一套上界，最后乘 $\textit{sign}$；空串或全空格没有数字，返回 $0$。
