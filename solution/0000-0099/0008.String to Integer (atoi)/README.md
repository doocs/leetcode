...
> **思考**
>
> 转换规则只需从左到右扫描一遍，时间复杂度为 $O(n)$，$n \le 200$ 可以接受。顺序不可颠倒：符号出现在数字之前，数字必须连续，一旦遇到非数字就应停止；最后还要把结果限制在 $32$ 位整数范围内。
>
> 若不允许使用 $64$ 位整数，则不能先算出完整结果再截断。我们累加绝对值，每次乘 $10$ 之前与 $\lfloor (2^{31}-1)/10 \rfloor$ 比较，一旦越界则按符号分别返回 $2^{31}-1$ 或 $-2^{31}$。正负情形共用同一上界，最后再乘以 $\textit{sign}$；空串或全空格没有数字，返回 $0$。
