...
> **思考**
>
> 扫描一遍记下 $target$ 的首次与末次出现，时间复杂度为 $O(n)$。$n \le 10^5$ 且题目要求 $O(\log n)$；即便先找到一个命中再向两侧扩展，最坏仍是线性。
>
> 数组非递减，左右边界各自是一个二分问题，不必在同一轮里同时寻找两端。左边界即第一个不小于 $target$ 的位置，右边界即第一个不小于 $target+1$ 的位置减一。两次下界查找即可；若两次结果相同，说明 $target$ 不存在。
