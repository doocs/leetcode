...
最容易想到的是扫一遍记下 $target$ 首次和末次出现。正确，但 $n \le 10^5$ 且题目要求 $O(\log n)$；找到一个后再向两边扩，最坏仍是 $O(n)$。

数组非递减，左右边界各自是一个二分问题，不必在同一轮里同时找两端。

左边界就是第一个 $\ge target$ 的位置，右边界是第一个 $\ge target+1$ 的位置减一。两次下界查找即可；若两次结果相同，说明 $target$ 不存在。
