...
最容易想到的是回溯：从 $0$ 出发，每次翻一个还没翻过的比特，走到 $2^n$ 个数。正确，$n \le 16$ 能过，但要显式判重，还要保证首尾也只差一位。

慢在搜索空间里手工维护「相邻只差一位」。其实二进制反射格雷码有闭式：$i$ 对应 $i \oplus (i \gg 1)$。

这个公式保证相邻整数的码只差一位，且 $0$ 与 $2^n - 1$ 也只差最高位。于是直接把 $[0, 2^n)$ 映射过去即可，不必搜索。
