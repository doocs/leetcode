...
> **思考**
>
> 从 $0$ 出发回溯，每次翻转一个尚未使用的比特，直到得到 $2^n$ 个数，可以构造出一条格雷码序列。$n \le 16$ 搜索能完成，但要显式判重，还要保证首尾也只差一位。
>
> 其实不必搜索。二进制反射格雷码有闭式：$i$ 对应 $i \oplus (i \gg 1)$。该公式保证相邻整数的码只差一位，且 $0$ 与 $2^n - 1$ 也只差最高位。
>
> 因此直接把 $[0, 2^n)$ 映射过去即可得到答案。
