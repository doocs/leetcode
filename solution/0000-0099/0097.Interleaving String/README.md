...
交错串要求 $s_3$ 由 $s_1$、$s_2$ 保序穿插而成，长度必须先相等。暴力是每一步选 $s_1$ 或 $s_2$ 的下一个字符，指数爆炸。

状态其实只有「$s_1$ 用了几个、$s_2$ 用了几个」：当前位置 $k=i+j$ 对 $s_3$ 是确定的。能选谁，就看 $s_1[i]$ 或 $s_2[j]$ 是否等于 $s_3[k]$。对 $(i,j)$ 记忆化，$O(mn)$ 个子问题。
...
进阶只要 $O(|s_2|)$ 额外空间。$f[i][j]$ 只依赖上一行和本行左侧，一行滚动数组就能放下，对应题目要求的 $O(n)$ 空间。
