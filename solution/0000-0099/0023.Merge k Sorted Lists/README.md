...
最容易想到的是两两合并：先并第 $1$、$2$ 条，再并进第 $3$ 条，依此类推。总节点 $N \le 10^4$、$k \le 10^4$ 时最坏 $O(Nk)$，容易超时。也可以全抽出来排序再建表，但丢掉了每条链表已经有序的事实。

慢在每次都要在 $k$ 个头里找最小。线性扫头是 $O(k)$，乘上 $N$ 个节点就过不去。

任意时刻「下一步该接谁」只取决于各链表当前头。小根堆维护这 $k$ 个头，取出最小后把它的后继入堆。

这样每个节点进出堆各一次，时间 $O(N\log k)$，堆里最多 $k$ 个指针。
