...
最容易想到的是按字符递归匹配：普通字符和 `?` 一对一，`*` 可以吞空串、一个或多个字符。正确，但不记忆化时 `*` 会指数级分叉。$|s|, |p| \le 2000$，必超时。

慢在同一对后缀 $(i, j)$ 被反复问。状态只取决于「$s$ 从 $i$ 起、$p$ 从 $j$ 起能否配上」，子问题大量重叠。

于是把 $dfs(i, j)$ 记下来。`*` 的三种走法（吞一个、同时推进、跳过 `*`）都是这个状态上的转移；配完或配空是边界。记忆化把指数搜索压成 $O(mn)$。
...
方法一已经是 $O(mn)$，但递归有调用栈，缓存也有常数开销。$|s|, |p|$ 到 $2000$ 时，深度和哈希都偏贵。

缺的是「同样的转移、改成自底向上」。$f[i][j]$ 就是 $dfs(i, j)$ 的表格版，先处理空串对 `*` 的边界，再按序填表。时间空间同阶，实现更稳。
