...
合法数字的形态很多：可选符号、整数或小数、再跟可选的指数。$n \le 20$，套正则或语言自带的解析函数能过，但题目要我们自己判定。

慢在规则交叉：小数点不能出现在指数里，`e` 前后都要有数，指数可以再带符号，单独一个 `.` 或 `+` 都不行。

从左扫一遍即可：先吃掉开头的符号，再用 $dot$、$e$ 计数约束「点」和「指数」各至多一次，并在遇到非法字符或位置时立刻拒绝。一遍扫描、常数额外空间。
