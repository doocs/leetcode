...
> **思考**
>
> 有效数字由可选符号、整数或小数，以及可选指数组成，形态较多。字符串长度 $n \le 20$，用正则或语言自带的解析函数即可判定，但题目要求自行实现。
>
> 规则彼此约束：小数点不能出现在指数部分，`e` 或 `E` 前后均须有数字，指数仍可带符号，单独的 `.` 或 `+` 均不合法。因此从左至右扫描一遍即可：先处理开头的符号，再用 $dot$ 与 $e$ 分别限制小数点与指数至多出现一次，遇非法字符或位置立即返回。额外空间为常数。
