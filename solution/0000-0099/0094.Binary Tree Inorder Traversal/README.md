...
中序就是「左、根、右」。树的定义已经是递归的，所以第一反应就是递归：先走完左子树，再记下根，再走右子树。$n \le 100$，深度最坏 $O(n)$ 的栈完全够用。进阶才要求改成迭代。
...
方法一已经对了，进阶要去掉系统递归。递归本质是「一路向左，走不动再弹出处理、转向右子树」。用显式栈模拟这一过程：有左就压栈下探，没有就弹出访问并改走右孩子。顺序与中序相同，空间仍是 $O(n)$。
...
栈把空间留在 $O(h)$。还能不能 $O(1)$？Morris 的观察是：左子树里最右结点的后继本来就是当前根，只是指针空着。临时把这条空右指针接到根上，沿左链走下去；回来时再拆掉。这样用树自己的空指针当栈，中序顺序不变，空间 $O(1)$。
