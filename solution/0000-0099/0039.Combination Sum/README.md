...
最容易想到的是对每个数无限次选用或不用，搜完所有和为 $target$ 的序列。$n \le 30$、$target \le 40$，不剪枝会搜大量超过目标和的路径，且不同顺序会被当成不同组合。

组合与顺序无关，同一组数只应出现一次。先排序，搜索时只从当前下标向右取，避免排列重复。

排序后若剩余 $s$ 已小于 $candidates[i]$，后面更大，整支可以停。$dfs(i,s)$ 从 $i$ 起枚举 $j$，递归仍从 $j$ 开始，表示这个数还能再用。
...
方法一用循环枚举「下一个选谁」，已经剪枝正确。另一种写法是对当前位置做选或不选：不选则 $i+1$，选则留在 $i$（允许重复用当前数）。搜索树形状不同，答案相同，只是实现换成二叉树展开。
