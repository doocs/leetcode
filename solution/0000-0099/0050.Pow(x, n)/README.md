...
最容易想到的是把 $x$ 连乘 $|n|$ 次。$|n|$ 最大约 $2^{31}$，线性乘法必超时；还要单独处理 $n < 0$ 时取倒数。

慢在一次只乘一个 $x$。观察：$x^{2k} = (x^k)^2$，$x^{2k+1} = x \cdot (x^k)^2$，指数每次折半。

于是按 $n$ 的二进制位累乘：当前底平方，遇到 $1$ 位就乘进答案。负数指数先算正幂再取倒数。快速幂把 $O(|n|)$ 压成 $O(\log |n|)$。
