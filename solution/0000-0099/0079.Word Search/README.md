...
从每个格子出发走出长度 $|word|$ 的路径再比对，格子最多 $36$、$|word| \le 15$，不剪枝也会爆。必须边走边比，失配立刻返回，且同一格不能用两次。

于是枚举起点，用 $dfs(i,j,k)$ 表示在 $(i,j)$ 对齐 $word[k]$。匹配则把格子改成 `'0'` 防重入，再试四邻的 $k+1$，回来还原。字母不是 `'0'`，不必另开 vis。任一起点成功即可。
