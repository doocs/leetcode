...
数字长度 $n\le 4$，至多 $4^n$ 种组合，怎么枚举都能过。本质是若干字母集合的笛卡尔积。若手写 $n$ 层循环，$n$ 不固定就写不下去。

观察：从空串出发，每读入一位数字，就把当前所有前缀与该位对应字母再拼一遍，得到新前缀表。这就是按层展开的笛卡尔积。

于是我们用数组存映射，从只含空串的列表出发逐位扩展。空输入单独返回空列表。
...
方法一已经能枚举全部答案，但每一层都新开一张前缀表，中间字符串的空间始终是 $O(4^n)$。

若我们只想用一条路径往下走、走完再回溯，额外空间可以压到 $O(n)$（递归深度与路径长度）。

于是按下标 DFS：选当前数字的一个字母、推进、回溯。答案仍是那些组合，只是生成方式从「按层扩展」换成「一条路径走到底」。
