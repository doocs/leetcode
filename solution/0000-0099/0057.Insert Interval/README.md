...
最容易想到的是把 $\textit{newInterval}$ 塞进列表，再当普通「合并区间」做。$n \le 10^4$，排序 $O(n \log n)$ 能过。

区间已经按左端排好且互不重叠，但先复用合并区间最省事：插入后排序，再扫一遍合并。正确，只是没用上「已排序」这个条件。
...
方法一重新排序，把已有的有序性丢掉了，时间多一个 $\log n$。

缺的是一次遍历：当前区间整段在新区间左边则直接写入；整段在右边则先插入新区间再写入；否则把新区间的两端拉大。扫完还没插入就把新区间补上。线性，且不用再排序。
