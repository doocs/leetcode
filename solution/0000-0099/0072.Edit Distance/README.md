...
最容易想到的是递归：在 $word1$ 的每个位置试插入、删除、替换，再取三种方案的最小代价。正确，但分支指数级。$m,n \le 500$，必须多项式。

慢在大量子问题重复——「把前缀 $i$ 变成前缀 $j$」会被算很多次。定义 $f[i][j]$ 为这个最少操作数。末尾字符相同则继承对角线；否则插入、删除、替换各对应一格邻居，取 $\min$ 再加一。空串边界是全删或全插。填完表后 $f[m][n]$ 就是答案。
