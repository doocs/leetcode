...
枚举所有 $s[i..j]$ 再逐个判断回文，时间 $O(n^3)$。$n \le 10^3$ 非常紧，容易超时。

慢在每个子串都从两端重新比较。回文有最优子结构：$s[i..j]$ 是回文，当且仅当两端字符相同且内层 $s[i+1..j-1]$ 已是回文。单字符天然为真。

所以问题变成按依赖填一张区间表，而不是反复扫描。因为 $f[i][j]$ 读的是更短的内层，枚举顺序必须让内层先算完；填表时用 $k$ 和 $mx$ 记下当前最长回文的起点和长度。
...
方法一已经是 $O(n^2)$ 时间，但还要 $O(n^2)$ 的表。每次判断其实只读内一层是否回文，并不需要整张表。

回文由中心对称展开：固定中心向两边扩，直到字符不等。必须同时试奇数中心 $(i,i)$ 和偶数中心 $(i,i+1)$，否则会漏掉 `"bb"` 这类。

扩完得到长度后，用中心下标 $i$ 和长度 $t$ 反推起点，额外空间降到 $O(1)$。
