...
最容易想到的是三重循环再丢进集合去重。正确，但 $O(n^3)$。$n\le 3000$ 过不了。对每个 $i$ 用哈希做两数之和能到 $O(n^2)$，但去重和额外空间都别扭。

瓶颈是：固定一个数后，在剩余数组里找两数之和为相反数，同时不能产出重复三元组。先排序，重复元素挨在一起便于跳过；有序后两数之和可以用左右指针 $O(n)$ 完成。若 $nums[i]>0$，后面全是正数，和不可能再为 $0$。

于是我们排序后枚举第一个数，后半段双指针夹逼。
