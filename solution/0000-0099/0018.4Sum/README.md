...
最容易想到的是四重循环再丢集合去重。$n\le 200$ 时 $O(n^4)$ 偏紧，去重也烦。

瓶颈与三数之和相同：固定前两个数之后，剩下仍是两数之和，暴力是 $O(n^2)$，有序后可以 $O(n)$。排序后重复值相邻，枚举 $i,j$ 时跳过相同元素即可保证四元组不重复。

于是我们两层枚举加一对指针，时间 $O(n^3)$，不必再开哈希。
