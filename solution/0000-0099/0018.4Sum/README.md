...
四重循环枚举后再用集合去重，时间复杂度为 $O(n^4)$。$n \le 200$，难以通过。固定前两个数之后，问题退化为两数之和，哈希表可以将内层降至线性，但去重较繁琐，也需要额外空间。

题目要求四元组不重复，并不限定原顺序。先将数组排序，重复元素会相邻，枚举 $i$、$j$ 时跳过相同值即可；有序之后，剩下的两数之和可以用左右指针在线性时间内完成。因此排序后两层枚举前两个数，在右侧用双指针夹逼，时间降至 $O(n^3)$。
