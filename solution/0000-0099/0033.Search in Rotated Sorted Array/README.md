...
最容易想到的是从左到右扫一遍。正确，但 $n \le 5000$ 时题目明确要求 $O(\log n)$。

数组被旋转后整体不再有序，普通二分无法直接判断 $target$ 落在哪一半。

观察：任意切一刀，至少有一半是单调的。用 $nums[0]$ 和 $nums[mid]$ 比较就能认出左半是否有序，再看 $target$ 是否落在那一段有序区间里，从而丢掉另一半。

因此仍可以二分：有序半段用来判定去留，无序半段留给下一轮继续切。
