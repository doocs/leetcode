...
最容易想到的是枚举 $\{1,\ldots,n\}$ 的全部 $2^n$ 子集，留下长度为 $k$ 的。$n \le 20$ 勉强能过，但大量长度不对的子集是浪费。

组合与顺序无关，每个数只决定选或不选，路径长度到 $k$ 就可以停。于是 $dfs(i)$ 处理数字 $i$：先放入再递归 $i+1$，回溯后再走「不选」。另一种写法是在剩余数里枚举下一个 $j$，两种搜索树不同，答案相同；下面代码是前一种。
...
方法一按「选或不选」走到深度 $n$，不选的那一支仍会把后面的数字走完。方法二改成枚举下一个被选中的 $j \in [i,n]$，递归深度等于已经选出的个数，搜索树的形状更贴近「长度为 $k$ 的组合」。答案不变，只是实现换成循环展开剩余候选。
