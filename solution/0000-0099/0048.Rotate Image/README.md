...
最容易想到的是新开一个矩阵，令 $\textit{matrix}[j][n-1-i] \gets \textit{matrix}[i][j]$。正确，时间和空间都是 $O(n^2)$。$n \le 20$ 空间够用，但题目要求原地修改。

慢在额外矩阵。$90^\circ$ 顺时针可以拆成两个可原地完成的操作：先上下翻转，再沿主对角线翻转（转置）。

$(i, j)$ 先到 $(n-1-i, j)$，再转到 $(j, n-1-i)$，正好是目标位置。两次交换，空间 $O(1)$。
