...
最容易想到的是搜索所有从左上到右下的路径，取数字和最小的那条。$m, n \le 200$，路径条数指数级，枚举过不了。

慢在大量子路径被重复计算。只能向右或向下，走到 $(i, j)$ 的最优一定来自上方或左方的最优再加当前格。

于是 $f[i][j]$ 记最小路径和：边界只能沿一条边累加，内部取 $\min$ 后加上 $\textit{grid}[i][j]$。按行填表，右下角即答案。
