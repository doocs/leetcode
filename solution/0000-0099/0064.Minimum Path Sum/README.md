...
> **思考**
>
> 从左上走到右下、每步只能向右或向下时，若枚举全部路径再取数字和最小者，$m, n \le 200$，路径条数随格子增长而迅速增加，无法在时限内完成。同一格子会被多条路径反复走到，子问题大量重叠。
>
> 因此走到 $(i, j)$ 的最小路径和，必来自其上方或左方的最优值再加上当前格。用 $f[i][j]$ 记录该值，边界沿唯一方向累加，内部取两侧最小值，按行填表即可。
