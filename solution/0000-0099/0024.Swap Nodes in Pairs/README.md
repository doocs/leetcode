...
最容易想到的是交换相邻节点的值。题目禁止改值，必须动指针。节点数 $\le 100$，任何正确接法都能过。

慢在连接顺序：先把第一对的两个指针拧过来，再处理后面，很容易把后继弄丢。

一对交换之后，剩下的从第三个节点起，又是同一问题。新头一定是原来的第二个节点。

于是先递归处理好 $head.next.next$，再把当前这对反过来接到那段结果上。空表或单节点无法成对，直接返回。
...
方法一正确，但递归深度与对数成正比，额外空间 $O(n)$。同一对内的三次指针改写可以在循环里完成。

用虚拟头钉住链表起点，指针 $pre$、$cur$ 每次处理 $pre$ 后面的两个节点，处理完后前进一对。
