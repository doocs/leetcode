...
> **思考**
>
> 题目要求不能修改节点内部的值，必须实际交换指针。节点数不超过 $100$，任何正确的指针接法在时间上都可以接受。
>
> 一对节点交换完成后，从原来的第三个节点起与原问题同构，且新头一定是原来的第二个节点。若先处理后面再回头改当前这对，便不必在一次操作里同时维护整条链的全部断点。
>
> 因此可以递归：先处理 $head.\textit{next}.\textit{next}$，再把当前这对反过来接到那段结果上。空表或仅有一个节点无法成对，作为递归出口直接返回。
...
> **思考**
>
> 方法一的递归深度与节点对数成正比，额外空间为 $O(n)$。同一对内的指针改写可以在循环中完成，从而将额外空间降至 $O(1)$。
>
> 为此设置虚拟头钉住链表起点，用指针 $pre$、$cur$ 每次处理 $pre$ 后面的两个节点，处理完后前进一对。时间复杂度仍为 $O(n)$。
