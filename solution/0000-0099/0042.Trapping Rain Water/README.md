...
最容易想到的是对每个位置 $i$ 向左、向右各扫一遍最高柱，能接的水是 $\min(\textit{leftMax}, \textit{rightMax}) - \textit{height}[i]$。正确，但时间 $O(n^2)$。$n \le 2 \times 10^4$ 时容易超时。

慢在每个 $i$ 都重新找左右最高。其实左边最高只依赖 $i$ 左边，右边最高只依赖 $i$ 右边，都可以线性预处理。

于是用两次数组记下每个位置「含自身」的左最高、右最高，再一次求和。水位由两侧较矮的墙决定，这就是动态规划的状态。
