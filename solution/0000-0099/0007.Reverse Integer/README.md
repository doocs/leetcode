...
转成字符串再反转最直接，但题目假定环境存不下 $64$ 位整数，不能「先用更宽的类型算完，再看有没有爆」。

逐位剥个位、往 $\textit{ans}$ 末尾贴，等价于 $\textit{ans} \leftarrow 10\cdot\textit{ans}+y$。乘 $10$ 之前就要判断会不会越出 $[-2^{31}, 2^{31}-1]$，越界返回 $0$。

判断可以收成：当前 $\textit{ans}$ 是否仍落在 $[\lfloor mi/10 \rfloor, \lfloor mx/10 \rfloor]$。在这个范围内再贴一位一定合法；越出则已经溢出。
