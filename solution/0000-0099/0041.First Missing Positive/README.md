...
最容易想到的是开一个哈希集合记下出现过的数，再从 $1$ 开始逐个查缺哪个。正确，时间 $O(n)$，但额外空间也是 $O(n)$。题目要求 $O(n)$ 时间且只用常数额外空间，$n \le 10^5$，哈希集过不了空间这一关。

慢在「用额外结构记录 $1..n$ 是否出现」。关键观察：缺失的最小正整数一定落在 $[1, n+1]$，所以我们只关心 $1..n$ 谁在、谁不在——数组下标本身就能当这张表。

于是把值 $x$ 交换到下标 $x-1$（仅当 $x \in [1, n]$）。原地交换用数组当哈希，再扫一遍就能找到第一个空洞。
