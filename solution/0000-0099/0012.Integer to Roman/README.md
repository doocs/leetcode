...
最容易想到的是按千、百、十、个分别查表，再单独处理 $4$、$9$ 的减法写法。$1 \le num \le 3999$，能过，但分支多、容易漏。

麻烦在「普通累加」和「减法形式」两套规则要分开写。若把 $\textit{CM}=900$、$\textit{CD}=400$、$\textit{XC}=90$ 等也当成独立面额，罗马数字就变成一组从大到小的固定面值。

题目保证表示唯一，每次选还能放下的最大面额一定合法。于是我们从大到小扫面额表，能减就减并拼上对应符号。
