...
最容易想到的是枚举所有下标对 $(i,j)$，面积为 $\min(height[i],height[j])\times(j-i)$。正确，但时间 $O(n^2)$。$n \le 10^5$ 时必然超时。

慢在把每一对都独立算一遍，没有用上「高度由较短的那根线决定」。从最宽区间 $[0,n-1]$ 出发：若内移较高的一端，宽度变小、高度仍被较短边卡住，面积不可能变大，因此只能丢掉较短的那根去碰更高的边。

于是我们用左右指针向中间收缩，每次移动较矮一侧。每个下标最多访问一次。
