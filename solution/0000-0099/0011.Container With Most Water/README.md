...
> **思考**
>
> 若枚举所有下标对 $(i, j)$，面积为 $\min(height[i], height[j]) \times (j - i)$，时间复杂度为 $O(n^2)$。$n \le 10^5$，无法通过。
>
> 容器的高度由较短的那根线决定。从最宽的区间 $[0, n-1]$ 出发：若向内移动较高的一端，宽度减小，高度仍受较短边限制，面积不会变大。因此每次应当舍弃较短的那根，尝试碰到更高的边。用左右指针向中间收缩，每次移动较矮的一侧，每个下标至多访问一次。
