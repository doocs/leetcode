...
> **思考**
>
> 对每个已填数字再扫描它所在的行、列与宫，棋盘仅为 $9 \times 9$，时间可以接受。同一行、列、宫却会被反复访问。
>
> 判定有效性只需知道「这一行 / 列 / 宫是否已经出现过数字 $v$」。我们可以用三张布尔表 $row[i][v]$、$col[j][v]$、$sub[k][v]$ 在一次遍历中记录，其中宫号 $k = \lfloor i/3 \rfloor \times 3 + \lfloor j/3 \rfloor$。若碰到已经出现过的数字，立即返回假。
