...
最容易想到的是对每个已填数字再扫一遍它所在的行、列、宫。棋盘只有 $9 \times 9$，这样也能过。

慢在同一行、列、宫被反复扫描。其实只要记下「这一行 / 列 / 宫是否已经出现过数字 $v$」。

一次遍历即可：用三张布尔表 $row[i][v]$、$col[j][v]$、$sub[k][v]$，其中宫号 $k = \lfloor i/3 \rfloor \times 3 + \lfloor j/3 \rfloor$。碰到已出现的数字立即返回假。
