...
最容易想到的是新开数组，把不等于 $val$ 的元素拷过去。$n \le 100$，能过，但题目要求原地，多一次分配没有必要。

慢在「删元素」若用 $erase$ 一类操作，后面的元素会反复前移，最坏 $O(n^2)$。

评测只看前 $k$ 个是不是所有非 $val$ 元素，顺序可变。于是把保留的值依次覆盖到数组前部即可。

$k$ 记已经写下的个数：遇到 $x \neq val$ 就写到 $nums[k]$ 并右移 $k$。一遍扫描，额外空间 $O(1)$。
