...
最容易想到的是枚举左右边界 $[l, r]$，高度取区间最小，面积 $(r - l + 1) \times \min$。正确，但时间 $O(n^2)$。$n \le 10^5$，必超时。

慢在每个区间都重算最小高度。换个枚举：以每根柱 $i$ 为矩形的「最矮柱」，宽度就是它向左右伸到第一根更矮柱之间。

找「左右最近的更小」正是单调栈模型。栈里下标对应高度递增，遇到更矮的柱就弹出，同时得到被弹柱的右边界；栈顶自然是左边界。每根柱进出各一次，线性扫完。
