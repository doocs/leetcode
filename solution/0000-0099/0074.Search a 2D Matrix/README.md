...
逐格扫描是 $O(mn)$。$m,n \le 100$ 能过，但题目要求 $O(\log(mn))$。

二维上看不出直接二分，卡在下标。观察：每一行递增，且上一行末尾小于下一行开头，整表按行拼接后是长度为 $mn$ 的有序数组。逻辑下标 $mid$ 对应 $matrix[\lfloor mid/n\rfloor][mid\bmod n]$，就能在 $[0,mn-1]$ 上做一次二分，不必真的展开。
...
方法一把矩阵压成一维再二分，正确且达到 $O(\log(mn))$，但要把 $mid$ 换成行列坐标，也强依赖「上一行末尾小于下一行开头」。如果不做下标映射，利用行、列各自单调，从角落出发一次比较就能排除一行或一列。实现更直，代价是时间退成 $O(m+n)$。
