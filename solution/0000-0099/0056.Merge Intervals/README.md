...
最容易想到的是任取一个区间，再扫其余区间看谁重叠就合并，反复直到不能再合。正确，但最坏 $O(n^2)$，且合并顺序乱。$n \le 10^4$ 偏紧。

慢在无序时重叠关系不好定位。按左端点排序后，一个区间只会和「当前还未闭合的那段」重叠——后面的起点更大，不可能跳过中间再叠回来。

于是排序后扫一遍，用 $\textit{st}, \textit{ed}$ 维护正在合并的区间，断开就写入答案。
...
方法一已经 $O(n \log n)$，逻辑也对。还多维护一对 $\textit{st}, \textit{ed}$，闭合时才写入，最后还要补一次。

缺的是把「当前区间」直接放进答案：一开始就把第一个区间放进 $\textit{ans}$，之后只改 $\textit{ans}[-1]$ 的右端，或追加新区间。少一组变量，少一次收尾。
...
方法二已经在答案末尾原地改右端。实现上还是「一个个看、碰到就改」。

缺的是按组合并：固定左端 $l$，用内层循环把所有与当前段相交的区间吃掉、拉大 $r$，再一次性压入 $[l, r]$。不必回头改已经写入的答案。
