...
重叠关系在无序时难以定位。若反复任取区间与其余区间两两合并，最坏时间 $O(n^2)$，且合并顺序杂乱。$n \le 10^4$ 偏紧。

按左端点排序后，一个区间只会与「当前尚未闭合的那段」重叠：后续区间的起点更大，不可能跳过中间再叠回来。

因此排序后扫一遍，用一对端点维护正在合并的区间，一旦与下一个断开就写入答案。
...
方法一已按左端排序并一次扫描完成合并，但额外维护了一对端点，区间闭合时才写入，结束时还要再补一次。

可以将「当前区间」直接放在答案末尾：先将第一个区间放入 $\textit{ans}$，之后只更新末尾右端点或追加新区间，从而少一组变量、少一次收尾。
...
方法二已在答案末尾原地更新右端点，但仍是逐个区间判断、重叠则改写。

还可以按组合并：固定当前段的左端，用内层循环吞并所有与之相交的区间并拉大右端，再一次性写入，无需回头修改已写入的答案。
