...
最容易想到的是按定义递归：枚举分割点，尝试交换或不交换，再递归两边。正确，但同一对子串会被反复问。$n \le 30$，不记忆化会爆。

慢在子问题重叠。状态其实只取决于「$s_1$ 从 $i$ 起长 $k$ 的段，能否扰成 $s_2$ 从 $j$ 起长 $k$ 的段」。

扰乱只是在某次切分后选择交换或不交换。枚举切分长度 $h$，要么两段各自对应，要么交叉对应。把 $dfs(i, j, k)$ 记下来，状态 $O(n^3)$、每层枚举切分 $O(n)$，总 $O(n^4)$。
...
方法一已经是 $O(n^4)$，但递归有调用栈，缓存也有常数。$n = 30$ 时深度和哈希都偏贵。

缺的是「同样的转移、改成自底向上」。$f[i][j][k]$ 就是 $dfs(i, j, k)$ 的表格版：先填长度 $1$，再按长度递增填。时间空间同阶，实现更稳。
