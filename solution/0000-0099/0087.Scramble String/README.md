...
> **思考**
>
> 按定义递归：枚举分割点，尝试交换或不交换后再递归两边，语义正确。同一对子串却会被反复询问。$n \le 30$，若不记忆化，搜索树规模过大。
>
> 子问题重叠，状态只取决于「$s_1$ 从 $i$ 起长 $k$ 的段，能否扰成 $s_2$ 从 $j$ 起长 $k$ 的段」。扰乱对应某次切分后选择交换或不交换：枚举切分长度 $h$，要么两段各自对应，要么交叉对应。
>
> 将 $dfs(i, j, k)$ 记忆化后，状态数为 $O(n^3)$，每层再枚举切分 $O(n)$，总时间 $O(n^4)$。
...
> **思考**
>
> 方法一的转移已经是 $O(n^4)$，但递归带调用栈，哈希缓存也有常数开销。$n = 30$ 时深度与哈希都偏贵。
>
> 可以把同一套转移写成自底向上。$f[i][j][k]$ 即 $dfs(i, j, k)$ 的表格：先填长度 $1$，再按长度递增填满。时间与空间同阶，实现更稳。
