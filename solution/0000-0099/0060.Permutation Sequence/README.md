...
> **思考**
>
> 第 $k$ 个排列不必先列出前面的 $k-1$ 个。若生成全部 $n!$ 种排列再按下标取值，$n \le 9$ 时可以枚举完，但其余排列都用不上。
>
> 确定某一位后，剩余数字的排列恰好占 $(n-i-1)!$ 种。用 $k$ 与这个块大小比较，即可知道当前位应取第几个尚未使用的数字，并一次性跳过整块。
>
> 为此从左到右填每一位，用阶乘定位、用 $\textit{vis}$ 记录已用数字，时间 $O(n^2)$。
