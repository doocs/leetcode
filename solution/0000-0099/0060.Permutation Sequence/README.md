...
最容易想到的是生成全部 $n!$ 个排列再取第 $k$ 个。$n \le 9$，$9!$ 能过，但其余排列都用不上。

慢在「先列出再挑选」。确定第一位后，剩余排列恰好 $(n-1)!$ 个；用 $k$ 对照这个块大小就能知道当前位该用第几个还没用过的数。

于是从左到右枚举每一位，用阶乘跳过整块，用 $\textit{vis}$ 记下已用数字。时间 $O(n^2)$。
