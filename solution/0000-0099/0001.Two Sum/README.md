...
最容易想到的是两重循环：枚举第一个数 $x$，再扫后面有没有 $target-x$。正确，但时间 $O(n^2)$。$n \le 10^4$ 时勉强能过，题目进阶却要求更快。

慢在反复「在已经看过的数里查找补数」。查找交给哈希表，期望 $O(1)$。于是一边遍历一边把「值 $\to$ 下标」放进表：看到 $x$ 时先问 $target-x$ 出现过没有，有则返回两个下标；没有再把 $x$ 存进去。

必须先查再存。若先存再查，$[3,3]$、$target=6$ 会把同一个下标用两次。先查再存，配到的一定是更早的下标。
