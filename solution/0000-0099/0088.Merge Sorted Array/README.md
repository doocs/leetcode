...
最容易想到的是另开数组，双指针从前往后合并再写回 $\textit{nums1}$。正确，$m + n \le 200$ 完全够，但要 $O(m + n)$ 额外空间，也不满足原地。

慢在从前往后写会覆盖 $\textit{nums1}$ 里还没读的数。空位却在尾部。

于是从后往前：$\textit{nums1}$ 的尾巴是空的，先放当前较大者不会踩到未处理元素。指针 $i$、$j$ 扫两段末尾，$k$ 写合并末尾，直到 $\textit{nums2}$ 耗尽。
