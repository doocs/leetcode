...
最容易想到的是枚举 $haystack$ 的每个起点 $i$，看长度为 $m$ 的切片是否等于 $needle$。$n,m \le 10^4$，最坏 $O((n-m)m)$ 约 $10^8$，通常能过。

慢在大量近乎匹配的位置上反复比完 $m$ 个字符。更精巧的匹配能均摊到接近线性，但本题规模还不至于逼我们上那些算法。

只关心第一次命中，失配即可换下一个 $i$，不必收集全部出现位置。

于是 $i$ 从 $0$ 扫到 $n-m$，相等则返回 $i$，扫完返回 $-1$。额外空间 $O(1)$。
...
方法一最坏每个起点都比满 $m$ 个字符，接近 $O(nm)$。我们希望窗口滑动时复用上次的比较结果。

固定长度的子串可以滚哈希：右端纳入一个字符、左端丢掉一个字符，哈希 $O(1)$ 更新。窗口哈希等于 $needle$ 的哈希时再核对原串，避免冲突误报。

这样扫描一遍 $haystack$ 即可，期望时间 $O(n+m)$。
