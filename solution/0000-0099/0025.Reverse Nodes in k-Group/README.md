...
最容易想到的是按组递归：每 $k$ 个一组翻转，再接到后面。$n \le 5000$ 能过，但进阶要求额外空间 $O(1)$，递归栈按组数增长。

慢在两处：先确认这一组够 $k$ 个（不够则原样留下），再把翻转后的头尾接回原链，断点一多就容易接错。

一组内部就是普通的链表翻转；组与组之间只需记住上一组的尾。从虚拟头出发，每次先走 $k$ 步探测，够则摘下、翻转、接回，不够则整段结束。

探测、翻转、重接都是指针操作，额外空间 $O(1)$。
