...
最容易想到的是搜索：每步只向右或向下，枚举全部路径。格点 $m, n \le 100$，路径数可到组合数级别，暴力搜索会指数爆炸。

慢在同一格子会被反复走到。到达 $(i, j)$ 的路径只可能来自上方或左方，互不重叠，子问题可以直接相加。

因此用 $f[i][j]$ 记走到该格的路径数，按行列扫描即可保证依赖已经算完。起点置 $1$，右下角就是答案。
...
方法一对每个格子都要判断「有没有上方 / 左方」，边界上反复走分支，实现偏碎。

第一行只能从左来、第一列只能从上来，路径数都是 $1$。先铺好边界，内部格子无条件相加即可，逻辑更干净，复杂度不变。
...
前两法都要整张 $m \times n$ 表。但 $f[i][j]$ 只用到上一行的 $f[i-1][j]$ 和同行左边的 $f[i][j-1]$，第一维可以丢掉。

压成一维后，$f[j]$ 更新前仍是上一行的值，加上 $f[j-1]$ 就是当前行。空间降到 $O(n)$，时间不变。
