...
> **思考**
>
> 合法 IP 必须切成恰好四段，每段取值在 $0$–$255$ 且不能有前导零。$n \le 20$，枚举三个切点在时间上可行，但前导零、越界、段数不足等边界分散在三重循环里，实现容易疏漏。
>
> 更自然的结构是回溯：从位置 $i$ 起尝试取 $1$–$3$ 位作为下一段，合法再继续往下切。段数恰为 $4$ 且字符串用尽才收入答案；段数已满或下标越界则停止。非法前缀尽早剪掉。
