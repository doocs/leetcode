...
IP 必须切成恰好四段，每段 $0$–$255$ 且不能有前导零。$n \le 20$，暴力枚举三个切点可行，但边界（前导零、超 $255$、段数不够）分散在三重循环里不好写。

自然结构是回溯：从位置 $i$ 起试 $1$–$3$ 位作为下一段，合法再往下切。段数到 $4$ 且字符串用尽才收入答案；段数满了或越界就停。剪枝把非法前缀尽早丢掉。
