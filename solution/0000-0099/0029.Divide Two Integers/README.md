...
最容易想到的是反复减：被除数能减一次除数，商就加 $1$。$|a|$ 最大约 $2^{31}$，减到结束会超时，且题目禁止乘、除、取余。

慢在每次只扣掉 $1$ 倍 $b$。若某次能扣 $2^k$ 倍，商就能按二进制往上跳。

这和快速幂同一思路：在剩余量仍够的前提下把 $b$ 不断左移加倍，取出当前能减的最大一块，再对余数重复。

符号单独记，运算全程用负数，避免 $a= -2^{31}$ 取相反数时溢出；再特判 $b=1$ 以及 $a= -2^{31},\, b=-1$ 封顶 $2^{31}-1$。
