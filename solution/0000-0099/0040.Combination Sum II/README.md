...
和上一题不同，每个数只能用一次，且 $candidates$ 里有重复值。若不处理重复，同一组数会按不同下标组合被搜出来多次。$n \le 100$、$target \le 30$，更需要剪枝。

先排序，相同数字挨在一起。同一层里若 $j \gt i$ 且值与前一个相同，说明这个值已经作为「本层选择」搜过，直接跳过。

递归改为 $dfs(j+1,\ldots)$，表示这个下标用过就不能再用；剩余 $s$ 小于当前值则整支停止。
...
方法一在循环里跳过同层重复值。也可以改成选 / 不选：选当前 $x$ 则下标加一；不选则一次跳过整段等于 $x$ 的数，避免同一组合被不同下标复现。复杂度不变，只是把去重写到「不选」那一支。
