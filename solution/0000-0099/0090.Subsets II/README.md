...
最容易想到的是和「子集」一样 DFS 选或不选，直接收集。正确，但有重复元素时 $[1, 2]$ 会出现两次。$n \le 10$，子集个数不多，却过不了「解集不能重复」。

慢在相同值的不同下标被当成不同选择。先排序，相同值连在一起；不选的那条分支上跳过相同值，每种多重集只生成一次。

于是排序后 DFS：先选 $i$，再在不选时跳过后面所有等于 $\textit{nums}[i]$ 的位置。「选几个相同值」由递归深度决定，不会并列出两份相同子集。
...
方法一已经能去重，但递归有调用栈，跳过逻辑也更绕。$n \le 10$，完全可以枚举 $2^n$ 个掩码。

缺的是「选或不选改成二进制位」。$\textit{mask}$ 的第 $i$ 位表示是否选 $\textit{nums}[i]$；若选了 $i$ 却没选 $i - 1$、且两值相同，说明跳过了靠前的相同值，是重复方案，丢掉。时间同阶，写成一层循环即可。
