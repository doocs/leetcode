...
转成字符串再与反转比较最直接，题目进阶却要求不转字符串。把整个 $x$ 反转再比较，又会撞上 $32$ 位溢出。

其实只需反转一半：不断把 $x$ 的个位接到 $y$ 上，直到 $y$ 不再小于剩下的前半。负数带负号不可能回文；非零却以 $0$ 结尾的数反转会丢零，也可以直接判否。

循环停在两半长度对齐处。偶数位时比较 $x$ 与 $y$；奇数位时中间那一位落在 $y$ 上，比较 $x$ 与 $y/10$。
