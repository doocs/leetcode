...
最容易想到的是把二进制串转成整数再相加。串长可达 $10^4$，普通整型放不下。

慢在两个串长度可能不同、以及最高位还可能再进一位。和十进制竖式一样，从低位对齐往高位加，用 $\textit{carry}$ 记住进位。

两个指针从末尾同步前移，缺位当 $0$，循环条件带上 $\textit{carry}$，这样最后多出来的进位也会写入答案。
