...
最容易想到的是放进集合再重建。正确，$n \le 300$ 完全够，但浪费了有序、重复相邻，还要额外空间。

慢在没原地改指针。同一值挤在一起，只要把 $cur.next$ 跳过相同值即可，不必另开链表。

和上一题不同，这里要保留第一次出现的节点。于是单指针走一遍：相等就丢掉下一个，不等才前进。头节点一定留下，所以不用哑节点。
