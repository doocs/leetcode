...
另开数组、每个值最多抄两次，正确，但题目要求原地、$O(1)$ 额外空间。$n \le 3\times 10^4$，必须一趟。

有序则相同值连在一起，问题 26 是保留 $1$ 次，这里保留 $2$ 次。用 $k$ 表示已保留前缀的长度：当前 $x$ 能写下，当且仅当还不到两个，或它和倒数第二个保留值不同——否则就是第三次。写到 $nums[k]$ 再让 $k$ 加一，扫完返回 $k$。
