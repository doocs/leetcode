...
> **思考**
>
> 另开数组、每个值最多抄写两次可以完成，但题目要求原地修改且额外空间为 $O(1)$。$n \le 3 \times 10^4$，应当一趟完成。
>
> 由于数组有序，相同值必然相邻。问题 26 保留一次，本题保留至多两次。用 $k$ 表示已保留前缀的长度：当前值 $x$ 可以写入，当且仅当已保留不足两个，或它与倒数第二个保留值不同——否则即为第三次出现。写入 $nums[k]$ 后令 $k$ 加一，扫完返回 $k$。
