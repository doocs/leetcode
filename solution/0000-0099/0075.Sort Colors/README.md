...
计数 $0,1,2$ 再覆写是两趟；直接排序也行，$n \le 300$。题目不许用库排序，进阶还要一趟、$O(1)$ 空间。

三个取值只要把数组分成「全 $0$ / 全 $1$ / 全 $2$」三段。用 $i$、$j$ 框住已经归位的 $0$ 和 $2$，$k$ 扫未知区。换到 $2$ 那一侧时，换回来的数还没看过，所以 $k$ 不能加；换到 $0$ 那一侧时，换回来的一定来自已扫描区间，所以 $k$ 可以一起走。一遍结束后三段自然排好。
