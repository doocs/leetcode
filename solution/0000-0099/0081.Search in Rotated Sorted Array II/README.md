...
线性扫描每个元素并与 $\textit{target}$ 比较，$n \le 5000$ 可以接受，但题目希望尽量减少操作，且旋转后的数组两段各自有序。

普通二分依赖全程有序；本题用 $\textit{nums}[\textit{mid}]$ 与 $\textit{nums}[r]$ 比较，即可判断哪一侧有序，再看 $\textit{target}$ 是否落在有序段内。与「搜索旋转排序数组」的差别在于允许重复：当 $\textit{nums}[\textit{mid}] = \textit{nums}[r]$ 时无法判断哪边有序，只能将 $r$ 减一。最坏情况下整段相等，时间退化为 $O(n)$，这是重复元素带来的代价。
