...
最容易想到的是线性扫一遍，每个元素和 $\textit{target}$ 比。正确，但题目要求尽量少操作，且数组旋转后两段各自有序。$n \le 5000$ 线性能过，却没用上有序性。

慢在没利用「至少一半区间是有序的」。普通二分靠全程有序；这里用 $\textit{nums}[\textit{mid}]$ 和 $\textit{nums}[r]$ 比较，就能判断哪一侧有序，再看 $\textit{target}$ 落不落在有序段里。

和「搜索旋转排序数组」的差别是可以重复。$\textit{nums}[\textit{mid}] = \textit{nums}[r]$ 时无法判断哪边有序，只能把 $r$ 减 $1$。最坏整段相等，退化成 $O(n)$，这是重复元素带来的必然代价。
