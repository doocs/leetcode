...
最容易想到的是拿第一个串当前缀，再挨个用后面的串去截短它。$n,m\le 200$，三重循环也过。字典树能做，但对这题过重。

真正能提前停的是：公共前缀只会变短，某一列一旦对不上，更长的前缀不可能存在。把各串竖着对齐，第 $i$ 列全相同才可能把前缀延长到 $i$。

于是我们以 $strs[0]$ 为基准按列比较，遇到长度不够或字符不同就立刻返回，不必先建树。
