...
拿第一个字符串作为候选前缀，再依次用其余字符串去截短它，时间复杂度为 $O(n \times m)$。$n, m \le 200$，可以接受。字典树也能维护公共前缀，但对这道题而言结构过重。

公共前缀只会变短：某一列一旦对不上，更长的前缀不可能存在。把各串按列对齐，第 $i$ 列全部相同，才可能把前缀延长到位置 $i$。因此以 $strs[0]$ 为基准按列比较，遇到长度不够或字符不同就立即返回，不必先建树。
