...
最容易想到的是枚举所有子串，再判断有没有重复字符。正确，但时间至少 $O(n^2)$。$n \le 10^5$ 会超时。

慢在子串之间大量重叠：一旦 $[l,r]$ 已经无重复，右端再纳入一个字符 $c$，只需处理 $c$ 是否把窗口弄脏，而不必重扫整段。

窗口一旦出现重复，合法答案不可能再包含左边多出来的那个 $c$，只能右移 $l$。用计数表看 $\textit{cnt}[c]$ 是否大于 $1$ 即可决定要不要缩；缩到 $c$ 只剩一次后，当前窗口就是以 $r$ 结尾的最长无重复子串。
