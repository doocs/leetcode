...
最容易想到的是对整段路径反复替换 `'//'`、`'/./'`、`'/../'`。$n \le 3000$ 也许能过，但连续斜杠和连续 `'..'` 叠在一起时规则交叉，替换顺序很难一次写对。

本质是目录的进退：有效名字要能被 `'..'` 撤销，空段和 `'.'` 不影响位置。后进先出，所以用栈保存从根到当前的目录序列。按 `'/'` 切开后逐段处理，最后拼成规范路径即可。根之上不能再退，弹出前要看栈是否为空。
