...
Unix 风格路径由目录名与斜杠组成，规范过程需要消除多余斜杠、当前目录 `'.'` 以及返回上级 `'..'`。反复字符串替换在 $n \le 3000$ 时时间上可以接受，但连续斜杠与连续 `'..'` 叠加时规则交叉，顺序不易一次写对。

路径的本质是目录的进退：有效名字应能被 `'..'` 撤销，空段与 `'.'` 不改变当前位置。这是后进先出的结构，因此用栈保存从根到当前的目录序列。按 `'/'` 切分后逐段处理，根之上不能再退，最后将栈中目录拼成规范路径。
