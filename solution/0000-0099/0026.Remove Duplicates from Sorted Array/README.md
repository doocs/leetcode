...
> **思考**
>
> 数组已经按非递减排列，重复元素必然相邻。若另开哈希表记录已经出现过的值再写回数组，时间 $O(n)$。在 $n \le 3\times 10^4$ 的数据范围下可以接受，但题目要求原地修改，额外的线性空间并不必要。
>
> 因此判断当前值是否该保留，只需看它是否等于已经写下的最后一个数，而不必全局查表。为此用 $k$ 同时充当写指针与去重后的长度：仅当尚未写下任何元素，或当前值与 $nums[k-1]$ 不同时，才覆盖到位置 $k$。一遍扫描，额外空间 $O(1)$。
