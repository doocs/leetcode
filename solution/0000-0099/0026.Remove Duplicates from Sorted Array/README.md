...
最容易想到的是用哈希表记下出现过的值，再写回数组。$n \le 3\times 10^4$，能过，但题目要求原地，额外空间不划算。

慢在「值是否见过」这件事：数组已非递减，相同值一定挨在一起，不必全局查表。

只要看当前 $x$ 是否等于已写下的最后一个数。不等（或还什么都没写）就写到位置 $k$，否则跳过。

$k$ 既是写指针，也是去重后的长度。一遍扫描，额外空间 $O(1)$。
