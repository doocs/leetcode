...
按题意画出二维网格再按行读，正确，但多数格子是空的。$n \le 10^3$ 能过，却浪费在「列」这个并不需要输出的维度上。

真正决定答案的只有每个字符落在哪一行，以及同一行内的先后。行号沿 $0,1,\ldots,\textit{numRows}-1$ 再折返，到顶或到底就掉头。

因此只开 $\textit{numRows}$ 个列表，用方向 $k$ 在边界取反。$k$ 初值设为 $-1$，遇到第一行立刻翻成向下。$\textit{numRows}=1$ 时顶和底是同一行，必须直接返回，否则会在原地反复掉头。
