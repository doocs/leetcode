...
最容易想到的是枚举所有三元组，记下与 $target$ 最接近的和。$O(n^3)$。$n\le 1000$ 大约 $10^9$ 次运算，撑不住。

慢在固定一个数后仍暴力扫另外两个。我们并不需要全部三元组，只要最接近 $target$ 的那个和。排序后，和偏大只能让右端变小，和偏小只能让左端变大；若恰好等于 $target$，已经是最优。

于是我们枚举第一个数，其余用双指针按与 $target$ 的大小夹逼，并维护最小偏差。题目保证唯一解，不必处理重复。
