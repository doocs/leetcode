...
最容易想到的是两两判断是否字母异位：对每个串排序再比。正确，但 $n \le 10^4$、$k \le 100$ 时 $O(n^2 \cdot k \log k)$ 过不了。

慢在成对比较。异位词排序后是同一个串，这个串就是分组的「身份证」。

于是以排序后的字符串为键、原串列表为值，扫一遍放进哈希表。同一键自然聚到一起，不必再两两配对。
...
方法一对每个串排序，单串 $O(k \log k)$。字母只有 $26$ 个小写，$k \le 100$ 时排序的对数因子可以去掉。

缺的是更便宜的键。统计每个字母出现次数，用长度为 $26$ 的计数元组当键，哈希分组方式不变，单串变成线性。
