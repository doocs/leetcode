...
从左到右扫描，遇到 $\textit{IV}$、$\textit{IX}$ 等六种减法组合再特殊处理，其余字符直接累加，在 $|s| \le 15$ 的范围内可以接受，但特例容易遗漏。

一个字符应当减去，当且仅当它比右侧相邻字符小；末位永远是加。因此不必枚举那六种组合。我们可以用哈希表记录每个字符的数值，从左到右比较相邻两位以决定正负，最后补上末位即可。
