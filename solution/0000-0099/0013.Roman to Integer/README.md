...
最容易想到的是扫一遍，遇到 $\textit{IV}$、$\textit{IX}$ 等六种减法对就特殊处理，其余直接累加。$|s|\le 15$，能过，但特例容易漏。

烦在判断「当前字符要加还是减」。观察：一个字符被减，当且仅当它比右边那个字符小；最后一个字符永远是加。不必枚举那六种组合。

于是我们用哈希表记下每个字符的值，相邻比较决定正负，最后补上末位。
