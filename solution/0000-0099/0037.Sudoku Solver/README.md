...
空格最多八十多个，每个 $9$ 种填法，暴力枚举不可行。必须在违反行、列、宫约束时立刻停。

和「有效的数独」一样，瓶颈是反复检查某数字是否已用。先把已填数字记进 $row$、$col$、$block$，再只对空格列表 $t$ 做搜索。

$dfs(k)$ 处理第 $k$ 个空格，枚举尚未占用的 $v$，写入后递归；找到解就用 $ok$ 截断，不再继续换数。回溯时恢复占用标记，成功路径上棋盘里的数字予以保留。
