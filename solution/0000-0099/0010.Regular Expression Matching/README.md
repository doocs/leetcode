...
按模式从左到右匹配，遇到 `*` 就枚举它吃掉 $0,1,2,\ldots$ 个字符。$s,p$ 最长 $20$，指数级有时能过，但同一对位置会被反复算到。

`*` 只作用在它前一个元素：要么匹配 $0$ 次（模式一次跳两格），要么当前字符与该元素相符（含 `.`），再让同一个 `*` 继续吃 $s$ 的下一个。普通字符则必须一对一消耗。

所以状态是「$s$ 的后缀 $i$ 与 $p$ 的后缀 $j$ 能否匹配」。先搜再记，避免指数分支撞上同一对 $(i,j)$。
...
方法一记忆化后已是 $O(mn)$，但仍是递归，隐式栈深、常系数更大。转移本身没有后效，完全可以按 $i,j$ 从小到大填表。

$f[i][j]$ 表示 $s$ 前 $i$ 个与 $p$ 前 $j$ 个是否匹配。空对空为真；还要允许 $i=0$ 去匹配 `a*` 这类可吃光的模式，所以外层从空串开始。

`*` 与方法一同一套：跳过两个模式字符，或在对得上时沿 $s$ 再吃一个。迭代填完直接读 $f[m][n]$。
