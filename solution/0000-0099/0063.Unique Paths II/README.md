...
和「不同路径」一样，暴力搜索每一步向下或向右。$m, n \le 100$，无记忆会指数爆炸；障碍物还让第一行、第一列不再处处为 $1$。

慢在同一格子 $(i, j)$ 会被重复展开。从当前位置出发，越界或踩到障碍就是 $0$，走到终点就是 $1$，否则两条分支相加。

给 $\textit{dfs}(i, j)$ 加上记忆化，每个格子只算一次，时间和空间都是 $O(mn)$。
...
方法一记忆化后已是 $O(mn)$，但仍是递归，隐式栈深、常系数更大。障碍切断后，第一行、第一列不再整段为 $1$，自底向上填表更直接。

$f[i][j]$ 表示从起点走到该格的路径数：障碍格为 $0$，否则来自上方与左方。先顺着第一行、第一列铺到第一块障碍为止，再填内部，避免递归。
