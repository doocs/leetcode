...
最容易想到的是按层剥：每层四次循环走上、右、下、左，再收缩边界。$m, n \le 10$ 能过，但剩下一行或一列时边界很容易写错。

慢在「手动维护四条边」。螺旋本质是：沿当前方向走，下一步越界或已访问就右转。

于是用方向数组循环四种走法，再用 $\textit{vis}$ 标记已走格子，每走 $m \times n$ 步即可。
...
方法一已经线性扫完矩阵，但 $\textit{vis}$ 还要 $O(mn)$ 额外空间。元素范围只有 $[-100, 100]$，访问标记可以写回格子本身。

缺的是原地标记：走过的格子加 $300$，下一步只要看是否 $> 100$。遍历后再减回去，空间降到 $O(1)$。
