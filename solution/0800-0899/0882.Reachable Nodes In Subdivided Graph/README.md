...
> **思考**
>
> 原图每条边被细分成 $\textit{cnt}$ 个新点，问从 $0$ 出发在 $\textit{maxMoves}$ 步内能到达多少点。细分点极多，不能建出显式图。
>
> 把边权当作 $\textit{cnt}+1$，对原图结点跑 Dijkstra，得到到达各原结点的最短路。原结点可达则计数；边上的细分点由两端剩余步数夹逼，不超过 $\textit{cnt}$。
