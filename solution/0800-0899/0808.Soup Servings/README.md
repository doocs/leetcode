...
> **思考**
>
> 四种操作等概率减少汤量，求 A 先倒完的概率（同时倒完计 $1/2$）。$n$ 可达 $10^9$，按毫升逐步模拟不可行；注意到每次至少减少 $25\,\mathrm{ml}$，可把状态缩到 $\lceil n/25\rceil$ 的格点。
>
> 记忆化搜索 $dfs(i,j)$ 表示剩余 $(i,j)$ 份时的概率，边界按谁先耗尽返回 $1$、$0$ 或 $1/2$。$n$ 很大时 A 几乎必然先空，代码在 $n>4800$ 时直接返回 $1$，避免状态爆炸。
