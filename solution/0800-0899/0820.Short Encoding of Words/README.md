...
> **思考**
>
> 编码是若干单词加 `#` 的拼接，较短词若是较长词的后缀则可被其编码覆盖。直接对每对单词做后缀判断在规模上偏冗余。
>
> 把单词倒序插入 Trie，公共后缀会落在同一条路径上。只有叶子对应的单词需要单独编码，长度贡献为单词长加一；非叶子已被更长单词覆盖。
...
> **思考**
>
> 方法一先建完整 Trie，再 DFS 统计叶子。若按长度降序插入倒序单词，更长的词先占据路径；后插入的词若整条路径已存在，说明它是某个更长词的后缀，贡献为 $0$，否则贡献长度加一。
>
> 这样在插入时即可累加答案，省去第二遍 DFS。
