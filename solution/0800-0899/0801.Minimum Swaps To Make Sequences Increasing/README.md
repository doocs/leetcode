...
> **思考**
>
> 每个下标可选择交换或不交换，穷举全部 $2^n$ 方案在 $n\le 10^5$ 时不可行。某一位置能否保持两条序列严格递增，只取决于它与前一位置在交换后的相对大小。
>
> 为此记录做到当前位置且「本位置不换 / 本位置交换」的最少次数。转移按与前一位是否已经递增、以及交叉后是否仍递增分情况讨论；转移只依赖上一位置的两个状态，滚动两个变量即可。
