...
> **思考**
>
> 把数组分成至多 $k$ 段、最大化各段平均值之和。段的分界未知，朴素子集划分过多；$n,k\le 100$，按「从 $i$ 起还剩 $k$ 段」搜索并记忆化即可。
>
> 前缀和使一段 $[i,j)$ 的平均值 $O(1)$ 可得。$k=1$ 时整段只能作为最后一组；否则枚举第一段右端，递归剩余。
...
> **思考**
>
> 记忆化搜索的递归深度与枚举顺序可以改写成递推，去掉递归常数。$f[i][j]$ 表示前 $i$ 个数分成 $j$ 组的最大平均值和，内层枚举上一组分界 $h$。
>
