...
> **思考**
>
> 山形子数组先严格上升再严格下降，长度至少为 $3$。$n\le 10^4$，枚举峰并分别向两侧扩展可行，但会重复走同一段坡。
>
> 预处理 $f[i]$ 为以 $i$ 结尾的最长上升长度，$g[i]$ 为以 $i$ 开头的最长下降长度。仅当两侧都大于 $1$ 时，$f[i]+g[i]-1$ 才是一座山。
...
> **思考**
>
> 两个辅助数组可以省掉。从左山脚出发一次扫描：先爬升到峰，再下降到右山脚，用区间长度更新答案，然后把左指针移到当前右端。
>
> 无法形成峰的位置直接前进。整段数组每个元素至多被左右指针各经过常数次。
