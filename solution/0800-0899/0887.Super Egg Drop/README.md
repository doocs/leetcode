...
> **思考**
>
> 有 $k$ 个鸡蛋、$n$ 层楼，求最坏情况下最少扔几次。朴素枚举扔的楼层是 $O(kn^2)$，在约束下偏慢。对固定鸡蛋数，最坏次数关于楼层单调，存在使「碎 / 不碎」两次子问题尽量均衡的楼层。
>
> 记忆化 $dfs(i,j)$ 表示 $i$ 层 $j$ 个蛋。对扔的楼层二分，比较碎与不碎的代价，取平衡点后再加一次。一层或一蛋有闭式。
...
> **思考**
>
> 同一转移可改成表 $f[i][j]$，按层数与蛋数填，避免递归。一蛋时 $f[i][1]=i$。
>
