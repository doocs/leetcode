...
> **思考**
>
> 弹出当前频率最高且最近压入的值。操作达 $2\cdot 10^4$，每次扫描栈不可行。需要同时知道频率与时间戳。
>
> 哈希表记频率，优先队列存 $(-\textit{freq},-\textit{ts},val)$。弹出堆顶后把该值频率减一；堆中旧记录因频率过时不会再成为堆顶的有效选择。
...
> **思考**
>
> 堆里会留下过期的频率记录。若按频率分桶，第 $f$ 层是一个栈，专放当前频率恰为 $f$ 的值。
>
> 压入时把值推进 $f+1$ 层并更新最大频率；弹出只取最高层栈顶，该层空了再降低最大频率。每次操作均摊 $O(1)$。
