...
> **思考**
>
> 求矩形并的面积，坐标达 $10^9$、矩形至多 $200$ 个。两两容斥项数随交集增多而难以控制，沿 $x$ 扫描更稳：把每条左右边界当成事件，中间夹着的是当前被覆盖的 $y$ 向长度。
>
> 将 $y$ 坐标离散化后，用线段树维护「覆盖次数 $>0$ 的纵坐标长度」。按 $x$ 排序扫描，相邻事件的 $\Delta x$ 乘树中覆盖长度即这一竖条的面积。
