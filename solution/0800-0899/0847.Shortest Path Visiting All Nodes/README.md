...
> **思考**
>
> 求访问所有结点的最短路，边可重复走。$n\le 12$，路径本身不能当状态，但「当前点 + 已访问集合」只有 $n\cdot 2^n$ 个。
>
> 从每个点出发做状态压缩 BFS，状态 $(i,\textit{mask})$ 表示位于 $i$ 且访问集合为 $\textit{mask}$。首次使 $\textit{mask}$ 为全集的层数即最短长度。
...
> **思考**
>
> 普通 BFS 按层扩展，启发式可优先扩展「未访问点更少」的状态。以未访问点数为估计，用优先队列按 $dist+h$ 出队。
>
> 松弛条件仍是边权 $1$ 的最短路。启发可采纳，不会漏掉最优；在稠密访问后期往往更早碰到全集。
