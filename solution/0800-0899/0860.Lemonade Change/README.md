...
> **思考**
>
> 只有 $5$、$10$、$20$，找零必须用手头已有的钞票。$n\le 10^5$，应贪心：收 $20$ 时优先配一张 $10$ 加一张 $5$，否则三张 $5$。
>
> 维护 $5$ 与 $10$ 的张数，任一步 $5$ 变为负数即失败。无需保存整段交易历史。
...
> **思考**
>
> 方法一的分支已经覆盖全部找零规则。方法二把同一对计数器压进一次 $\textit{every}$ 遍历，用位运算区分面额。
>
> 语义不变，只是实现压成一行，仍在 $5$ 张数变负时失败。
