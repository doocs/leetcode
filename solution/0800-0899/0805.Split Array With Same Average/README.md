...
> **思考**
>
> 将数组分成两个非空子集且平均值相等，等价于存在大小为 $k$ 的子集其和为 $k\cdot \mathrm{avg}$。$n\le 30$ 时 $2^n$ 枚举偏慢，但折半后每半约 $15$ 个数，二进制枚举可接受。
>
> 先把每个数变为 $n\cdot a_i-S$，则目标转化为「存在非空真子集其和为 $0$」。左半子集和放入集合，右半枚举时若自身为 $0$ 或相反数已在左半出现（且不是全集对全集），即可判定。
