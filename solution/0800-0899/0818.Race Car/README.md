...
> **思考**
>
> 指令序列对应在数轴上加速或掉头，目标位置可达 $10^4$，直接 BFS 状态（位置、速度）偏多。注意到连续 $k$ 次 A 到达 $2^k-1$，任何目标都可由「越过再回头」或「中途回头再加速」拼成。
>
> 令 $dp[i]$ 为到达位置 $i$ 的最少指令。若 $i=2^k-1$ 则恰好 $k$ 次 A；否则比较「冲到 $2^k-1$ 再回头」以及在第 $k-1$ 次加速后提前回头走 $j$ 步再折返的代价，取最小。
