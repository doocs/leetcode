...
> **思考**
>
> 每个犯罪可选或不可选，受人数与最低利润约束，方案数取模。犯罪与人数规模有限，状态可记「考虑到第 $i$ 项、已用 $j$ 人、已获利润（封顶到 $\textit{minProfit}$）」。
>
> 记忆化 $dfs(i,j,k)$：不选则前进，人数够则再加选中的一支。利润封顶是为了让达到门槛后的状态合并。边界在物品用尽时看 $k$ 是否已达门槛。
