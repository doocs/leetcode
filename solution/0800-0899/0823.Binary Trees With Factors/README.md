...
> **思考**
>
> 以数组中的数为结点、且孩子之积等于父结点，统计可构成的带标号二叉树。数组长度 $\le 1000$，值互不相同，应按值从小到大构造，以便孩子先于父亲算完。
>
> 令 $f[i]$ 为以 $arr[i]$ 为根的方案数，枚举左孩子 $b$，若 $a/b$ 也在数组中则累加 $f[b]\cdot f[c]$。每个结点至少有一种单结点树，最后对 $f$ 求和取模。
