...
> **思考**
>
> 在二叉树中找与 $\textit{target}$ 距离为 $k$ 的结点。只有向下的孩子指针时，无法走向父结点。结点至多 $500$ 个，可先建父指针再从 $\textit{target}$ 向外走 $k$ 步。
>
> 第一次 DFS 记下每个结点的父亲，第二次从 $\textit{target}$ 向左、右、父三向扩展，避开来路，深度为 $0$ 时收集结点。
