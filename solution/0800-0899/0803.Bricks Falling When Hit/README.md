...
> **思考**
>
> 每次打击后正向洪水填充统计掉落，$hits$ 达 $4\cdot 10^4$、$m,n\le 200$，重复搜索过慢。掉落的本质是「与顶行的连通被切断」。
>
> 为此先打掉全部打击位置，用并查集把剩余砖块连到一个虚拟屋顶，再按相反顺序补回砖块。补回后屋顶连通块的增量减一，即该次打击实际掉落的砖数；原先就是空格的打击贡献为零。
