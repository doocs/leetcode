...
> **思考**
>
> 同构要求 $s$ 与 $t$ 的字符一一对应；仅记录单向映射无法发现多对一。两串等长且字符集有限，一遍扫描即可判定。
>
> 为此维护 $d_1,d_2$ 分别记录 $s\to t$ 与 $t\to s$，一旦与已有映射冲突则返回否，否则更新后继续。
...
> **思考**
>
> 方法一的两张表语义正确，哈希实现带常数开销。字符集大小固定为 $256$，可用数组代替。
>
> 把“上次出现的位置”写成同一时间戳：若两字符已记录的下标不同，则映射不一致。
