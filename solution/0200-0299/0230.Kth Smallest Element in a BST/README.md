...
> **思考**
>
> BST 的中序遍历即为升序。只需走到第 $k$ 个被访问的结点，不必列出整棵树。
>
> 用栈模拟一路向左，弹出时将 $k$ 减一，减到 $0$ 即答案。
...
> **思考**
>
> 单次中序已够，但若反复查询而树不变，每次 $O(h+k)$ 仍重复走左链。预处理每棵子树的结点数后，可比较左子树规模与 $k$ 决定向左或向右。
>
> 一次查询变为沿根下行，适合静态树上的多次询问。
