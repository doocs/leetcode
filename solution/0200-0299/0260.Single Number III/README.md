...
> **思考**
>
> 除两个数各出现一次外其余均出现两次，全部异或得到的是这两个数之异或 $xs$。二者不等，故 $xs$ 至少有一位为 $1$。
>
> 取 $xs$ 的 $\mathrm{lowbit}$ 按该位分组，两个目标数落入不同组；对含该位的一组再异或得到 $a$，则 $b=xs\oplus a$。
...
> **思考**
>
> 位运算把空间压到常数。若允许线性额外空间，用集合：第一次出现则加入，第二次则删去，最后剩下的即两个单次出现的数。
