...
> **思考**
>
> 在 $s$ 前面补最少字符使其回文，等价于找出 $s$ 的最长回文前缀，再把剩余后缀反转接到前面。枚举前缀并判回文偏慢。
>
> 滚动维护前缀哈希与其反转哈希，二者相等则更新最长回文前缀长度；最后把未覆盖的后缀反转拼到 $s$ 前。
...
> **思考**
>
> 字符串哈希存在冲突可能。把 $s$ 与其反串拼接后求 KMP 的 $next$，末位值即为最长回文前缀长度。
>
> 从而在线性时间内得到同一构造，无需取模哈希。
