...
> **思考**
>
> 完全二叉树仍可用普通递归统计左右子树规模。结点总数为 $1$ 加上两棵子树的结点数，遍历每个结点一次即可。
...
> **思考**
>
> 方法一未利用“最后一层靠左填满”的结构，仍是线性扫描。左右子树高度相等则左子树为满二叉树，结点数可直接写成 $2^{left}$，只需递归右子树；否则右子树为满，递归左子树。
>
> 每次沿最左链求高为 $O(\log n)$，递归层数亦为 $O(\log n)$，总时间为 $O(\log^2 n)$。
