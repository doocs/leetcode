...
> **思考**
>
> 从 $1$ 到 $9$ 中选 $k$ 个数使其和为 $n$，全集很小，可以全部枚举。按数字递增决策“选或不选”，并在剩余和、已选个数或数字越界时剪枝。
>
> 函数 $dfs(i,s)$ 表示当前考虑到 $i$、剩余和为 $s$，选 $i$ 则递归 $dfs(i+1,s-i)$，否则 $dfs(i+1,s)$。
...
> **思考**
>
> 回溯依赖递归栈。$1$ 到 $9$ 只有 $2^9$ 个子集，可用长度为 $9$ 的二进制掩码枚举。
>
> 若掩码中 $1$ 的个数为 $k$ 且对应数字之和为 $n$，则收集该子集。
