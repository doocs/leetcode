...
> **思考**
>
> 每个窗口内再扫一遍求最大，总时间为 $O(nk)$。大根堆可在对数时间内给出当前最大，过期下标在堆顶时再弹出。
>
> 先放入前 $k-1$ 个，随后每纳入一个新元素并清理过期堆顶，堆顶即为该窗口最大值。
...
> **思考**
>
> 堆中会残留过期元素，最坏仍带对数。窗口最大值只关心队头到队尾递减的候选。
>
> 单调队列存下标：队头滑出则弹出，队尾不大于当前值则弹出，队头即为窗口最大，线性完成。
