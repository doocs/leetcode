...
> **思考**
>
> $h$ 指数是最大的 $h$ 使得至少 $h$ 篇论文引用不少于 $h$。降序排序后，$citations[h-1]\ge h$ 即满足，从大到小找第一个即可。
...
> **思考**
>
> 排序为 $O(n\log n)$。$h$ 不超过 $n$，可把引用次数封顶为 $n$ 后计数，从大到小累加篇数，第一次 $s\ge h$ 即答案。
...
> **思考**
>
> “至少 $h$ 篇引用 $\ge h$”对 $h$ 单调，可二分最大可行 $h$。每次统计 $\ge mid$ 的篇数以决定收缩哪一侧。
