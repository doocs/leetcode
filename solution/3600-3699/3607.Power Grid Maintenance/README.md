...
> **思考**
>
> 连接关系在查询前已经固定，之后只会离线、不会加边。查询达 $2\times 10^5$，每次在电网内线性寻找最小在线编号不可行。
>
> 并查集预处理每个电站所属连通块。块内在线编号需要支持删除与查最小，有序集合恰好提供这两种操作。
>
> 初始将 $1\ldots c$ 插入各自根的有序集合。查询 $[1,x]$ 时若 $x$ 仍在集合中则返回 $x$，否则返回集合最小元（空则 $-1$）；查询 $[2,x]$ 将其从根集合中删除。
