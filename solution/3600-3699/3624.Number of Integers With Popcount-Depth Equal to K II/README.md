...
> **思考**
>
> 单点修改与区间查询 depth 恰为 $k$ 的个数，$n,q\le 10^5$，且 $k\le 5$。每次暴力重算区间不可行。
>
> depth 的值域极小，可对每个 $k$ 建一棵树状数组或线段树，维护该 depth 的出现次数。查询 $[l,r]$ 对第 $k$ 棵结构做前缀差。
>
> 修改时先扣去旧值的 depth，再把新值加到对应结构。$\textit{nums}[i]\le 10^{15}$，depth 可预处理或按位运算即时求出。
