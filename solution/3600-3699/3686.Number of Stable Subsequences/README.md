...
> **思考**
>
> 稳定子序列限制奇偶（或相邻差）不能连续过长。$n\le 10^5$，需线性 DP。
>
> 按奇偶分类：令 $f_0,f_1$ 为以偶数/奇数结尾、且末尾连续奇数个同类的方案，再细分连续 $1$ 个与 $2$ 个，禁止连续 $3$ 个相同奇偶。
>
> 遇到 $x$ 时，只能接在另一奇偶的状态后，或接在同一奇偶但连续长度尚小于 $2$ 的状态后。对 $10^9+7$ 取模，答案为所有合法状态之和。
