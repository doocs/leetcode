...
> **思考**
>
> 一般梯形由两对平行但长度不同的边确定，$n\le 500$，枚举四点为 $O(n^4)$，而枚举点对为 $O(n^2)$ 可接受。
>
> 同一直线由斜率 $k$ 与截距 $b$ 标识。斜率相同、截距不同的线段两两可组成梯形；平行四边形的两组对边会被各计一次，须再减去。
>
> 平行四边形的对角线中点重合。用 $\textit{cnt1}[k][b]$ 统计点对所在直线，用 $\textit{cnt2}[p][k]$ 统计中点与斜率。对同一 $k$ 下不同 $b$ 的计数两两相乘累加，再对同一中点下不同斜率的计数两两相乘从答案中扣除。
