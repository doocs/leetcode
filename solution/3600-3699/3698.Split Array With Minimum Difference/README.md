...
> **思考**
>
> 在 $i$ 处切开，要求左段严格递增、右段严格递减，最小化两段和之差的绝对值。$n\le 10^5$，不能对每个切口再检查单调性。
>
> 前缀和给出两段和；$f[i]$ 表示 $[0,i]$ 是否严格递增，$g[i]$ 表示 $[i,n-1]$ 是否严格递减，均可各扫一次得到。
>
> 枚举切口，仅当 $f[i]$ 与 $g[i+1]$ 同时为真时更新 $|s[i]-(s[n-1]-s[i])|$。无合法切口则返回 $-1$。
