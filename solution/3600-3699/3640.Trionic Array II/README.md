...
> **思考**
>
> 求权和最大的三段式子数组。枚举所有峰谷为平方级。相邻三段式会共享上升段，可用分组循环在线性时间内枚举极大三段。
>
> 指针依次切出升、降、升三段；中段或末段退化则跳过。极大段的和由中部固定部分、左升段向左的最大后缀、右升段向右的最大前缀组成。
>
> 第三段上升可作为下一段的第一段，故将指针回退到谷底 $q$。全程每个下标进出常数次。
