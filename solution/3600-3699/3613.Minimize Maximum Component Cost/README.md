...
> **思考**
>
> 连通块的成本是块内最大边权，要在至多 $k$ 个连通块下最小化该最大值。若 $k=n$，全部边可删，答案为 $0$。
>
> 最大值最小具有单调性：权不超过 $w$ 的边若能把图连成至多 $k$ 块，则更大的 $w$ 亦可。按权排序后从小到大加边，正是 Kruskal 的过程。
>
> 初始 $n$ 个分量，并查集合并；当分量数降至不超过 $k$ 时，当前边权即为所求最大成本。输入图连通，算法总会成功或在 $k=n$ 处提前返回。
