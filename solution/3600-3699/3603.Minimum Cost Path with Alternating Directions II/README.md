...
> **思考**
>
> 网格仅能向右或向下，且奇数秒必须移动、偶数秒必须原地等待，因此一条从 $(0,0)$ 到 $(m-1,n-1)$ 的路径由移动序列唯一确定，等待穿插在相邻两次移动之间。
>
> $m\cdot n\le 10^5$，枚举路径不可行，但格子数允许线性动态规划。到达 $(i,j)$ 恰好经过 $i+j$ 次移动；除起点外，每次移动前都在上一格等待（终点到达后不再等待）。
>
> 记入格代价 $(i+1)(j+1)$。从上方或左方转移时累加入格费，并在非终点格子加上 $\textit{waitCost}[i][j]$。起点单独计入入场费。最优子结构成立，因为到达同一格的等待次数与奇偶性已由坐标固定。
