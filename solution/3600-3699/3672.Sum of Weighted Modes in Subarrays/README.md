...
> **思考**
>
> 每个长 $k$ 的窗口贡献 $\textit{mode}\times\textit{freq}$，并列取较小的 mode。窗口滑动时若每次重扫，总代价为 $O(nk)$。
>
> 哈希表维护频次，堆按 $(-\textit{freq},\textit{val})$ 取顶。懒删除：堆顶频次与表不一致则弹出。
>
> 滑窗时对新旧值分别入堆。$\textit{get\_mode}$ 在堆顶稳定后返回乘积。每个下标引发常数次堆操作。
