...
> **思考**
>
> 特殊回文要求每个数字 $d$ 的出现次数为 $d$（或满足题目给定的次数约束），且整体为回文。从 $n+1$ 起逐个检验在 $n$ 很大时不可行。
>
> 满足次数约束的多重集合有限，可枚举半边排列并镜像生成全部特殊回文，排序后二分后继。
>
> 奇数位数字至多一个放在中心，其余成对出现。预处理全部候选后，对每个询问取严格大于 $n$ 的最小者。
