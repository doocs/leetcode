...
> **思考**
>
> 剩余数组平衡当且仅当最大值不超过最小值的 $k$ 倍。删除等价于保留一段在排序后连续的子数组。$n\le 10^5$，枚举保留集合不可行。
>
> 排序后，枚举左端 $i$ 作为最小值，则右端不能超过 $k\cdot \textit{nums}[i]$。二分找出第一个越界位置 $j$，窗口 $[i,j)$ 即可保留。
>
> 记录最长窗口，答案为 $n$ 减该长度。排序保证窗口内最值就是两端。
...
> **思考**
>
> 上一方法对每个左端二分，多出 $\log n$ 的常数。右端随左端右移而单调不减，可用双指针一次扫完。
>
> 维护 $r$ 使得 $\textit{nums}[r]\le \textit{nums}[l]\cdot k$，左端每进一步只把 $r$ 往右推。答案取 $n-(r-l)$ 的最小值。
>
