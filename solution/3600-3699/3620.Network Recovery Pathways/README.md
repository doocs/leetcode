...
> **思考**
>
> 路径分数是路径上的最小边权，要在总代价不超过 $k$、且只走在线节点的前提下最大化该最小值。枚举路径不可行。
>
> 阈值越大，可用边越少，可行性单调，因此对最小边权二分。判定时丢掉权小于 $\textit{mid}$ 的边，在剩余图上用堆优化 Dijkstra 求 $0$ 到 $n-1$ 的最短路，并与 $k$ 比较。
>
> 建图时跳过任一端离线的边。若最小候选仍不可行则返回 $-1$。
