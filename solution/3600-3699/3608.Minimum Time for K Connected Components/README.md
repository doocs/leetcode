...
> **思考**
>
> 时刻 $t$ 之后，权大于 $t$ 的边消失。对每个候选 $t$ 重建图求连通分量，$m\le 10^5$ 且权值达 $10^9$，不可行。
>
> 边只随时间删去、不会增加，连通分量数单调不减。从权值从大到小加边，等价于时间倒流。
>
> 初始 $n$ 个分量，每合并一次分量减一；当分量即将小于 $k$ 时，当前边的时间即为使分量数仍至少为 $k$ 的最小 $t$。若始终不少于 $k$ 个分量，答案为 $0$。
