...
> **思考**
>
> 与上一题不同，这里只需结果串的第 $k$ 个字符，而 `#` 会使长度倍增，物化整串不可行。
>
> 先正演一遍只维护长度 $m$：字母加一，`*` 减一（下界 $0$），`#` 左移一位。若 $k\ge m$ 则越界，返回 `'.'`。
>
> 再倒序把 $k$ 映射回源字符。遇到 `#` 时长度减半，若 $k$ 落在后半段则减掉前半长度；`%` 将 $k$ 对称到 $m-1-k$；字母使长度减一，当 $k$ 恰好等于新的 $m$ 时即找到该字符。
