...
> **思考**
>
> 若枚举生成树并再枚举至多 $k$ 次升级，方案数为指数级，在 $n,m\le 10^5$ 下不可行。稳定性由树上最小边权决定，因此「能否达到稳定性 $x$」对 $x$ 单调，可将最大值转化为判定。
>
> 必选边不能升级，其最小强度 $mn$ 构成上界；若必选边成环，或将全部边并入后仍不连通，则无解。
>
> 为此，对阈值 $\textit{lim}$ 先并入强度不小于 $\textit{lim}$ 的边，再至多用 $k$ 次升级并入满足 $2s\ge \textit{lim}$ 的边。并查集维护连通性，即可在对数次判定中求出最大可行稳定性。
