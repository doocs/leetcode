...
> **思考**
>
> 每次取三个数，贡献其中位数。要使中位数之和最大，应让尽可能大的数成为中位数，同时用更小的数去「陪衬」。$n\le 5\times 10^5$，不能搜索划分。
>
> 排序后，最小的 $n/3$ 个数注定只能充当较小的陪衬。剩余 $2n/3$ 个数中，每隔一个取较小者作为中位数，等价于从下标 $n/3$ 起步长 $2$ 求和。
>
> 该选取保证每个三元组都能配到一个更小的数，且中位数取到剩余序列中尽可能大的那一半。
