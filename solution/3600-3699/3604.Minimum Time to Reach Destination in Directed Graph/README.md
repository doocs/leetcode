...
> **思考**
>
> 边仅在时间窗 $[\textit{start},\textit{end}]$ 内可走，忽略窗口的 BFS 会得到错误到达时刻。需要的是到达各点的最早时间。
>
> 若在时刻 $t$ 到达 $u$，沿边 $(u,v,s,e)$ 须在 $\max(t,s)$ 出发且该时刻不超过 $e$，于是到达 $v$ 的时间为 $\max(t,s)+1$；若已错过窗口则该边不可用。
>
> 这是带时间约束的最短路。用堆按到达时间弹出节点，松弛合法出边，首次到达 $n-1$ 即为答案；若堆空则返回 $-1$。
