...
> **思考**
>
> $n,q\le 10^5$，再按 I 的方式模拟，小步长询问会退化成平方。步长大于 $\sqrt{n}$ 的询问次数少，仍可直接乘；小步长须批量处理。
>
> 对 $k\le B$ 的询问，按余数 $l\bmod k$ 落到一条等差数列上，在下标 $t=(i-\textit{res})/k$ 处做乘 $v$，并在区间右端之后乘逆元，形成差分。
>
> 每个 $(k,\textit{res})$ 上排序合并同位置的因子，再扫一遍该等差数列把前缀乘积施加到 $\textit{nums}$。最后对数组求异或。
