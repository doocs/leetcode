...
> **思考**
>
> 把 $n$ 拆成 $k$ 个正因数之积，使最大数与最小数之差最小。$k\le 5$、$n\le 10^5$，可预处理每个数的因数表后搜索。
>
> $\textit{dfs}(i,x,\textit{mi},\textit{mx})$ 表示还要选 $i$ 个数、当前剩余为 $x$。枚举 $x$ 的因数 $y$ 写入路径，递归 $x/y$。
>
> 当 $i=0$ 时用最后的 $x$ 更新极差。记录极差最小的一条路径。因数预筛使每个剩余值的分支为 $O(\sigma(x))$。
