...
> **思考**
>
> 每个机器人射向左或向右，射程受自身 $\textit{distance}$ 与相邻机器人阻挡。枚举方向为 $2^n$，不可行。
>
> 按位置排序后，第 $i$ 个的选择只与第 $i+1$ 个的方向有关。令 $\textit{dfs}(i,j)$ 为考虑完 $i$ 且下一个方向为 $j$ 时的最大摧毁数。
>
> 向左时左端被上一机器人挡住；向右时右端被下一机器人（及其若向左射出的射程）挡住。墙上二分统计区间内的墙数，记忆化避免重复状态。
