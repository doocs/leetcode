...
> **思考**
>
> 不平衡定义为：商品种类不少于 $3$，且最贵商品的库存严格少于最便宜商品。逐店手工比较价格会在并列时丢失「数量较大者优先」的规则。
>
> 先筛出种类足够的商店。对库存按 $(\textit{store\_id},\textit{price},\textit{quantity})$ 排序后取每店第一条，分别得到最贵与最便宜商品；并列时数量降序保证选取唯一。
>
> 连接两张结果，保留最贵库存小于最便宜库存的商店，比率四舍五入到两位，再与商店表连接并按比率、店名排序。
