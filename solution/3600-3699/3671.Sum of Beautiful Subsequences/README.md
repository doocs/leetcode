...
> **思考**
>
> 子序列美丽值通常与 $\gcd$ 有关。$n\le 10^4$、值域 $7\times 10^4$，按 $\gcd=d$ 统计再容斥。
>
> 对每个 $d$，在所有 $d$ 的倍数上做关于「以该值为结尾的递增/合法子序列个数」的树状数组 DP，得到 $\gcd$ 为 $d$ 的倍数的子序列和 $g[d]$。
>
> 再令 $f[d]=g[d]-\sum_{t>1}f[td]$，则 $f[d]$ 对应 $\gcd$ 恰为 $d$ 的贡献。答案为 $\sum d\cdot f[d]$。
