...
> **思考**
>
> 「会议密集周」依赖 ISO 年与周号上的时长合计，而不是单日记录。先把日期转为年、周，再按 $(\textit{employee\_id},\textit{year},\textit{week})$ 对 $\textit{duration}$ 求和。
>
> 筛出合计至少 $20$ 小时的周，再按员工计数。次数不少于 $2$ 者与员工表连接，按密集周数降序、姓名升序输出。
>
> 聚合键必须包含年，否则跨年的同一周号会被错误合并。
