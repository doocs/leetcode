...
> **思考**
>
> 沿原方向走边权为 $w$，反向走代价为 $2w$。求 $0$ 到 $n-1$ 的最小代价。若对每条路径枚举翻转集合，边数会使方案爆炸。
>
> 把反向边显式建进图：$(u,v,w)$ 保留正向，并加 $(v,u,2w)$。问题化为普通非负权最短路。
>
> 堆优化 Dijkstra 从 $0$ 出发，首次弹出 $n-1$ 即为答案；堆空则不可达。每条原边对应两条有向边，复杂度与 $m\log m$ 同阶。
