...
> **思考**
>
> 每张牌是两个字母，一次出牌消耗两张共享恰好一个给定字母 $x$ 的牌。求最多回合数。按是否含 $x$ 以及另一字母分类计数。
>
> 含 $x$ 的牌可写成 $x?$ 或 $?x$。两张能配对当且仅当另一字母不同或位置不同。将各类张数贪心配对，剩余再与不含 $x$ 的牌无法配对。
>
> 枚举作为关键字母的 $x$（至多 $26$），对每类计数做有上限的配对，取回合数最大者。
