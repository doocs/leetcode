...
> **思考**
>
> 与 I 不同，这里选出的 $k$ 个子数组必须互不相同，不能反复取同一段。固定左端 $l$ 后，值随右端增大而单调不减。
>
> 于是有 $n$ 条单调序列，要求它们之中前 $k$ 大之和。每条序列的最大项是 $[l,n-1]$，放入最大堆；弹出后若还能缩短右端，则把次大项入堆。
>
> 任意区间最值用 ST 表 $O(1)$ 查询。一共弹出 $k$ 次，每次 $O(\log n)$。
