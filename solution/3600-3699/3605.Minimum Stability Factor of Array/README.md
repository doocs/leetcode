...
> **思考**
>
> 稳定性因子是最长「区间 $\gcd\ge 2$」的长度。至多修改 $\textit{maxC}$ 个位置后要最小化该长度。枚举修改方案不可行。
>
> 若长度 $L$ 可通过不超过 $\textit{maxC}$ 次修改使所有长为 $L+1$ 的窗口不再稳定，则更短的目标也可行，因此对答案二分。
>
> 区间 $\gcd$ 可用稀疏表在常数时间求出。判定时扫描每个长度为 $\textit{mid}+1$ 的窗口，若 $\gcd\ge 2$ 则必须在窗口内修改一点；将修改点尽量靠右，以便覆盖后续重叠窗口，所需修改次数与 $\textit{maxC}$ 比较即可。
