...
> **思考**
>
> 需要同时给出 $n^2$ 的十六进制与 $n^3$ 的三十六进制。语言内置转换通常只覆盖十六进制，两套逻辑并不统一。
>
> 任意进制表示都可由反复取模与整除得到。为此抽象 $f(x,k)$：余数不大于 $9$ 时写数字，否则映射为字母，再将低位序列反转。
>
> 计算 $x=n^2$、$y=n^3$ 后返回 $f(x,16)+f(y,36)$。$n$ 的量级使循环次数为对数级。
