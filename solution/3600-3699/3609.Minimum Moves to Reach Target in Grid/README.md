...
> **思考**
>
> 坐标可达 $10^9$，每步把某一维加上 $\max(x,y)$，正向搜索的状态空间无法承受。操作可逆：较大维若严格大于另一维，则上一步必然加在较大维上。
>
> 从 $(t_x,t_y)$ 向 $(s_x,s_y)$ 逆推。设 $t_x\ge t_y$，若 $t_x\ge 2t_y$，则可一次性减去若干个 $t_y$（对应连续多次加在 $x$ 上）；否则减去 $t_y$ 一次。对称处理 $t_y>t_x$。
>
> 若某一步会使坐标低于起点，或两维相等却尚未到达，则不可达。逆推保证每一步唯一，步数即为最少移动次数。
