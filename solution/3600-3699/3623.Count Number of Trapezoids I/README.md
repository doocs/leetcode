...
> **思考**
>
> 题目只统计两腰为水平边的梯形，因此一对水平边由同一 $y$ 上至少两点确定。$n\le 10^5$，枚举四点不可行。
>
> 按 $y$ 分组后，一组 $v$ 个点贡献 $\binom{v}{2}$ 条水平边。两条位于不同 $y$ 的水平边恰确定一个梯形。
>
> 扫描各组，用 $s$ 记录此前水平边总数，当前组贡献 $t$ 条边时累加 $s\cdot t$，再把 $t$ 加入 $s$。对 $10^9+7$ 取模。
