...
> **思考**
>
> 「LCT」子序列可在扫描时用左侧 $L$ 数与右侧 $T$ 数相乘，于每个 $C$ 处累加。插入至多一个字母，$n\le 10^5$，不能对每个插入位置重算。
>
> 插入 $L$ 增加原串中「CT」的个数，插入 $T$ 增加「LC」，插入 $C$ 增加某个位置左侧 $L$ 与右侧 $T$ 之积。后两者可一次扫描，前者用二元子序列计数。
>
> 扫描维护 $l,r$ 并记录最大的 $l\cdot r$，再与 $\textit{calc}(\text{LC})$、$\textit{calc}(\text{CT})$ 取最大，加到原串的「LCT」个数上。
