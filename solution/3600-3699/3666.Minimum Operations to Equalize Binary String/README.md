...
> **思考**
>
> 一次翻转恰好 $k$ 个位置，目标是全 $1$。状态可用「当前 $0$ 的个数」表示，因为具体哪些位为 $0$ 不影响可翻转范围。
>
> 从 $\textit{cur}$ 个 $0$ 出发，可翻 $x$ 个 $0$（其余翻 $1$），新的 $0$ 个数为 $\textit{cur}+k-2x$，且 $x$ 落在 $[\max(k-n+\textit{cur},0),\min(\textit{cur},k)]$。到达的 $0$ 个数构成一段同奇偶区间。
>
> 按奇偶分两个有序集合存放尚未访问的计数。BFS 弹出 $\textit{cur}$，把 $[l,r]$ 内同奇偶的值全部入队并删除。到达 $0$ 的层数即为最少操作；集合空则无解。
