...
> **思考**
>
> 从 $i$ 可走到相邻下标，或传送到所有满足 $p\mid \textit{nums}[j]$ 的 $j$（$p$ 为 $\textit{nums}[i]$ 的质因子且 $p$ 为质数）。最短步数是无权图 BFS。
>
> 若每次按值扫描全数组，$n$ 与值域都会使边数爆炸。预处理每个数的质因子，按质数 $p$ 收集可被传送到的下标列表 $g[p]$。
>
> BFS 扩展 $i\pm 1$ 以及 $g[\textit{nums}[i]]$，并在首次使用后清空该列表，同一质数只扩展一次。到达 $n-1$ 的层数即为答案。
