...
> **思考**
>
> 除向右、向下付格子代价外，还可从值较大的格子传送到值较小的格子，至多 $k$ 次。状态须带剩余传送次数。
>
> $f[t][i][j]$ 为使用 $t$ 次传送到达 $(i,j)$ 的最小代价。$t=0$ 时只做网格转移。传送要求起点值不小于终点，按值从大到小扫描，把上一层到达同值或更大值格子的最小代价赋给本层。
>
> 赋值后再做一次右下转移，使传送后仍可继续走路。答案取各 $t$ 下终点代价的最小者。
