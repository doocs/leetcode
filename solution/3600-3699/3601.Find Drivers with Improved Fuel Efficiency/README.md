...
> **思考**
>
> 若按司机逐条扫描行程再手工划分上下半年，既难保证「两个半年都必须有记录」，也容易把平均做成总和之比。题目比较的是同一司机两半年的平均单次效率。
>
> 因此先将 $\textit{distance}/\textit{fuel}$ 记为单次效率，再按 $\textit{driver\_id}$ 与半年分组求均值。透视后丢弃缺半年的司机，并保留下半年均值严格更大者。
>
> 最后与 $\textit{drivers}$ 连接得到姓名，按提升幅度降序、姓名升序输出。分组与透视把「半年」从过滤条件变成了明确的聚合键。
