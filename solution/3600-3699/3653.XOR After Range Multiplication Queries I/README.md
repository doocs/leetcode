...
> **思考**
>
> $n,q\le 10^3$，每个询问沿步长 $k$ 把 $[l,r]$ 上的元素乘 $v$。直接模拟的代价约为 $O(q\,n/k)$，最坏可接受。
>
> 按询问逐个更新 $\textit{nums}[\textit{idx}]=\textit{nums}[\textit{idx}]\cdot v\bmod (10^9+7)$，最后对全数组求异或。
>
> 规模更小的版本无需分块；同系列 II 才需要按步长分类。
