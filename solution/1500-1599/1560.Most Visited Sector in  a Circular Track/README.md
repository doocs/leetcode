...
> **思考**
>
> 圆环上按给定路段连续走过，求经过次数最多的扇区。完整圈对每个扇区贡献相同，真正拉开差距的只有第一起点到最后终点之间多走的那一段。
>
> 若起点编号不超过终点，多走的是闭区间 $[rounds[0],rounds[-1]]$；否则这段跨过 $n$，等于 $[1,rounds[-1]]$ 与 $[rounds[0],n]$ 的并。列出该并集即可，不必模拟每一圈。
