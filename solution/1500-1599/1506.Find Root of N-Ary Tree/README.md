...
> **思考**
>
> 给定一棵 $N$ 叉树的全部节点，求根。根在列表中只出现一次，其余节点还会作为某个父亲的孩子再出现一次。用哈希表记录所有孩子后，列表中未出现在孩子集合里的节点即为根，但需要额外线性空间。
>
> 每个非根值出现偶数次、根值出现奇数次，异或恰好消去成对出现的值。遍历每个节点及其孩子，将全部值异或到 $x$，再在列表中找回值为 $x$ 的节点即可，额外空间为常数。
