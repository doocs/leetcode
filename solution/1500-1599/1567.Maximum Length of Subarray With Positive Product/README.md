...
> **思考**
>
> 求乘积为正的最长子数组。$n\le 10^5$，枚举两端再求积会溢出且超时。零把数组切开，负号只改变符号，适合按结尾位置记录正、负两种长度。
>
> 令 $f[i]$、$g[i]$ 分别为以 $i$ 结尾的最长正积、负积长度。正数延续原符号；负数把正负对调；零则两长度皆断。答案取所有 $f[i]$ 的最大者。
...
> **思考**
>
> 每个状态只依赖前一位置的 $f,g$，数组可以压成两个变量。时间仍是线性，额外空间降为常数。
