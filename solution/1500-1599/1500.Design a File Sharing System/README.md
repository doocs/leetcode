...
> **思考**
>
> 系统需要为每位用户分配未被占用的最小正整数编号，用户离开后该编号应可立即复用；同时要维护每名用户持有的文件块，并在请求时返回当前持有该块的全部用户。若每次加入都从 $1$ 起扫描空闲编号，则单次代价随历史用户数线性增长，在调用次数达 $10^4$、文件块数达 $10^5$ 时并不划算。
>
> 注意到尚未复用时编号单调递增，离开的编号构成一个可回收集合。为此用递增计数器发放新编号，用小根堆保存已释放编号，即可在对数时间内取出当前最小空闲值；再用哈希表记录「用户 $\to$ 文件块集合」。请求时遍历在线用户即可，在线人数与调用次数同阶，足以满足约束。若返回列表非空，请求方同时获得该文件块。
