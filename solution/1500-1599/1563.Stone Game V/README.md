...
> **思考**
>
> 每次把一段石子分成左右两部分，得分取较小一侧的和，并只在较小（或相等时任选）一侧继续。区间个数 $O(n^2)$，再枚举切点是 $O(n^3)$，石子数通常不超过 $500$，可以接受，但朴素搜索会重复计算同一区间。
>
> 令 $dfs(i,j)$ 为区间 $[i,j]$ 上 Alice 的最大得分，用前缀和在 $O(1)$ 内得到左右两段之和。左段更小则只递归左侧，并在 $ans\ge 2\times l$ 时跳过（再搜也不会更优）；右段更小且 $ans\ge 2\times r$ 时可直接结束后续切点。记忆化避免重复区间。
