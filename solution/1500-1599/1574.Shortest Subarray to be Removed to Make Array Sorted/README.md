...
> **思考**
>
> 删除一段连续子数组后使剩余部分非递减，求最短删除长度。$n\le 10^5$，不能枚举删除区间。剩余数组只能是「一段前缀加一段后缀」，且前缀的末元不超过后缀的首元。
>
> 先找出最长非递减前缀 $[0,i]$ 与后缀 $[j,n)$。整段有序则答案为 $0$；否则至少可删掉整段后缀或整段前缀。再枚举前缀右端 $l$，在后缀中二分第一个 $\ge arr[l]$ 的位置 $r$，删除 $(l,r)$ 并取最短。
