...
> **思考**
>
> 在篮子上放 $m$ 个球，最大化任意两球间距的最小值。位置坐标达 $10^9$，不能按间距枚举所有放法；$n\le 10^5$，需要更快的判定。
>
> 间距越大能放下的球越少，存在单调性。排序后二分间距 $f$，从左到右贪心：与上一球距离至少为 $f$ 才放入。用「无法放下 $m$ 个」作为二分键，第一个失败的 $f$ 的前驱即为最大可行间距。
