...
> **思考**
>
> 删除尽量多的边，使 Alice 与 Bob 仍都能走遍全图。$n$ 与边数达 $10^5$，不能枚举子集。公共边两人都能用，应优先保留；独占边只在各自图上补连通。
>
> 两个并查集分别维护两人的连通性。先加类型 $3$：若两端已连通则这条公共边可删。再分别加类型 $1$、$2$，同样把成环的边计入删除数。最后两人的连通块数都为 $1$ 才有解。
