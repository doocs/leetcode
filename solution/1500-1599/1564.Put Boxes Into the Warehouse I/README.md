...
> **思考**
>
> 箱子只能从仓库左侧依次推入，且不能翻越更矮的房间。$n\le 10^5$，不能枚举装箱排列。房间 $i$ 能容纳的高度受其左侧最矮房间限制。
>
> 预处理 $left[i]=\min(warehouse[0..i])$，再把箱子升序排列。小箱子优先放入尽量靠里、仍够高的房间：右指针从最深处左移到第一个 $left[j]\ge boxes[i]$ 的位置，放入后两指针各进一步。这样大箱子留在外侧更高的房间。
