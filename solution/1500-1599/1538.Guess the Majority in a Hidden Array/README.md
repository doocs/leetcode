...
> **思考**
>
> 隐藏数组只含 $0/1$，每次查询四个互异下标返回其中多数情况的编码，要找出多数值的一个下标，查询次数还受限制。不能读出全部元素。
>
> 以 $query(0,1,2,3)$ 为基准 $x$。对 $i\ge 4$，若 $query(0,1,2,i)=x$，则 $i$ 与 $3$ 同类，否则与 $3$ 异类。再用三次查询把 $0,1,2$ 分别与 $3$ 比较。较大的一类即多数；两类相等则返回 $-1$。除下标 $3$ 外，异类下标记在 $k$ 中以便输出。
