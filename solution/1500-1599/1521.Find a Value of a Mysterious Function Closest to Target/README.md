...
> **思考**
>
> 函数 $func$ 是子数组的按位与，要求它与 $target$ 的绝对差最小。子数组有 $O(n^2)$ 个，$n\le 10^5$，全部求出不可行。
>
> 从固定右端向左延伸时，按位与只减不增，且不同值至多 $O(\log A)$ 个，因为每次变化至少少掉一个 $1$ 比特。用集合维护「以当前下标为右端的所有与值」，由上一列的与值再与 $arr[i]$ 得到，同时更新与 $target$ 的最小偏差。
