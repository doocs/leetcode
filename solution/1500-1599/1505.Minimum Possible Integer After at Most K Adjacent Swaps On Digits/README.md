...
> **思考**
>
> 相邻交换至多 $k$ 次后要得到字典序最小的数字串。$k$ 可达 $10^9$，而 $n\le 3\times 10^4$，逐次模拟交换既不可能按 $k$ 计步，也不能对每个位置暴力搬移。
>
> 从左到右构造答案时，当前位应换成能够在剩余次数内到达该位的最小数字。将某一尚未使用的数位 $j$ 搬到位置 $i$，代价等于它与 $i$ 之间尚未被取走的数位个数。
>
> 为此按数字 $0$–$9$ 分别记下原始下标队列；树状数组维护「哪些原位置已被取走」。枚举候选数字时，用树状数组在 $O(\log n)$ 内算出真实距离，若不超过剩余 $k$ 则选用并更新。这样每位只检查常数个数字，总时间 $O(n\log n)$。
