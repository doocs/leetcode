...
> **思考**
>
> 统计叶到叶距离不超过 $distance$ 的对数。若对每对叶子求 LCA，叶子数平方再乘树高，在最坏链状树上偏大。一对好叶子要么同在一侧子树，要么分属左右子树。
>
> 答案拆成左子树内部、右子树内部、以及跨越根的三部分。跨越部分只需左右两侧「到当前根距离为 $d$」的叶子计数，对距离之和不超过限制的桶做乘积。$distance\le 10$，桶很小，递归统计即可。
