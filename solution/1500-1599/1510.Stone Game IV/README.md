...
> **思考**
>
> 双方轮流从 $n$ 个石子中拿走一个平方数，取走最后一枚者胜。$n\le 10^5$，若对每个局面展开整棵博弈树，同一剩余量会被反复计算。
>
> 局面只由剩余石子数决定：若存在一种取法使对手面对必败态，则当前必胜。记忆化搜索 $dfs(i)$ 表示剩 $i$ 枚时先手是否必胜，枚举 $j^2\le i$ 并查询 $dfs(i-j^2)$ 即可，每个状态至多处理 $O(\sqrt{i})$ 次转移。
