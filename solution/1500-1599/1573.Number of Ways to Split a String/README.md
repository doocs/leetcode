...
> **思考**
>
> 把字符串切成三段，使每段中 $1$ 的个数相同。$n\le 10^5$，不能枚举两个切点。三段的 $1$ 数必须都等于总数的三分之一。
>
> 总数不能被 $3$ 整除则无解；全 $0$ 时任意两个切点都合法，方案为 $C_{n-1}^{2}$。否则找到第 $cnt$ 与第 $cnt+1$ 个 $1$ 的位置作为第一刀的可选区间，第 $2cnt$ 与第 $2cnt+1$ 个 $1$ 作为第二刀的可选区间，两段长度相乘即方案数。
