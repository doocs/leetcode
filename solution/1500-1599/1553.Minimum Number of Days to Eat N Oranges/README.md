...
> **思考**
>
> 每天可吃 $1$ 个，或在能整除时吃掉一半或三分之二。$n$ 达 $2\times 10^9$，按剩余量递推的数组放不下，一路减一更不可行。
>
> 最优策略总是先吃掉 $n\bmod 2$ 或 $n\bmod 3$ 个，以便立刻除以 $2$ 或 $3$。因此 $dfs(n)=1+\min(n\bmod 2+dfs(\lfloor n/2\rfloor), n\bmod 3+dfs(\lfloor n/3\rfloor))$。除法使状态沿 $n$ 迅速下降，记忆化后规模约 $O(\log^2 n)$。
