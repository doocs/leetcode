...
> **思考**
>
> 几何中位（费马—韦伯点）使到各客户的欧氏距离之和最小，目标函数对平面坐标可微但没有简单闭式。客户数不大，允许迭代逼近，误差不超过 $10^{-5}$。
>
> 从质心出发做梯度下降：梯度是各点方向单位向量之和。学习率按 $0.999$ 衰减，并在距离为零处加上微小分母以免除零。步长分量均小于 $10^{-6}$ 时停止，返回当前距离和。
