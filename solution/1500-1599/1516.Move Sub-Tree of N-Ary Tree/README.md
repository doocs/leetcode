...
> **思考**
>
> 要把以 $p$ 为根的子树挪到 $q$ 之下。若直接改指针而不处理祖先关系，当 $q$ 落在 $p$ 的子树内时，$p$ 会被从原树断开，$q$ 也随之失去与其余部分的连接。
>
> 先定位 $p$、$q$ 及其父节点，并判断二者的祖先关系。若 $q$ 在 $p$ 的子树中，须先把 $q$ 接到 $p$ 原来的父节点上，再把 $p$ 接到 $q$ 之下；否则只需从 $p$ 的父节点断开并挂到 $q$。整树规模有限，两次遍历足以完成定位与改接。
