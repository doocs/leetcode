...
> **思考**
>
> 按 $arr$ 的顺序把 $0$ 变成 $1$，求最后一次出现长度为 $m$ 的连续 $1$ 段的时刻。$n\le 10^5$，每步重建全串不可行。$m=n$ 时整段一次填满，答案为 $n$。
>
> 并查集维护当前 $1$ 连通块的大小。新点与左或右已填邻居合并前，若邻居块长度恰为 $m$，则该时刻仍有一段长度为 $m$（即将被合并破坏），记下步骤。合并后更新块大小。
...
> **思考**
>
> 并查集查找带来额外对数因子，而合并只发生在区间端点。在每个 $1$ 段的左右端记下长度，新点根据左右邻居的端点长度拼出新段，并写回新端点。判断与更新都是 $O(1)$，总时间线性。
