...
> **思考**
>
> 在至多删除 $k$ 个字符后，使行程长度编码尽量短。$n$ 与 $k$ 均不超过 $100$，状态空间允许按「从 $i$ 出发、还剩 $k$ 次删除」记忆化，但不能枚举全部删除子集。
>
> 一段编码对应把 $s[i..j]$ 压成单一字母：保留其中出现最多的字母，删掉其余 $j-i+1-maxFreq$ 个，编码长度由该字母的频数决定。枚举右端 $j$，把这段代价加上 $compression(j+1, k')$ 取最小。删除额度不足或剩余长度可全部删光时直接返回边界值。
