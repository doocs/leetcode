...
> **思考**
>
> 超赞子串重排后能成回文，即最多一个数字出现奇数次。$n\le 10^5$，不能枚举子串再统计。数字只有 $0$–$9$，奇偶可用 $10$ 位掩码表示。
>
> 前缀掩码 $st$ 记录每种数字的奇偶。子串 $s[j+1..i]$ 合法，当且仅当 $st_i$ 与 $st_j$ 至多一位不同。哈希表记下每种掩码首次出现的位置：相等掩码给出全偶数段，翻转一位后命中则给出恰一个奇数。取最长即可。
