...
> **思考**
>
> 数组为 $1,3,5,\ldots,2n-1$，每次把一个数加一、另一个减一，要使所有值相等。模拟每次操作在 $n\le 10^4$ 时偏慢，且总和守恒给出了目标值。
>
> 元素之和为 $n^2$，平均值为 $n$。只需把小于 $n$ 的前一半元素补到 $n$，第 $i$ 个奇数 $2i+1$ 需要 $n-(2i+1)$ 次。对这些缺口求和即得最少操作数。
