...
> **思考**
>
> 每次可将一块陆地变成水，求使网格不再恰有一座岛的最少天数。网格很小，但答案有更强的结构：任意岛屿至多挖两块陆地就能拆开（例如挖掉一个角上相邻的两格）。
>
> 因此答案只可能是 $0$、$1$ 或 $2$。先 DFS 计数，若不是恰好一座岛则返回 $0$。再依次把每一块陆地临时挖掉并重新计数，一旦不再是一座岛则返回 $1$。否则必须挖两块，返回 $2$。
