...
> **思考**
>
> 求所有奇数长度子数组的元素和。$n$ 通常不超过 $100$，三重循环可过，但会重复累加同一元素。以 $i$ 结尾的奇数长、偶数长子数组个数有闭式。
>
> 令 $f[i]$、$g[i]$ 分别为以 $i$ 结尾的奇数长、偶数长子数组之和。奇数段由上一位置的偶数段接上 $arr[i]$ 得到，个数为 $i/2+1$；偶数段对称。答案累加全部 $f[i]$。
...
> **思考**
>
> 每个状态只读前一位的 $f,g$，两个滚动变量即可。时间仍线性，额外数组可以去掉。
