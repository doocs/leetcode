...
> **思考**
>
> 统计能建出与 $nums$ 相同 BST 的重排个数（不含原序列本身）。$n\le 1000$，枚举排列不可行。BST 的根必须是序列首位，左右子树元素集合由大小关系唯一确定，内部次序可递归计数。
>
> 左右子树的相对顺序可任意交错，方案数为 $C_{m+n}^{m}$ 乘上两侧各自的方案数。预处理组合数后递归求解，最后减一去掉原序列。
