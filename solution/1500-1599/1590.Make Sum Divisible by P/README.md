...
> **思考**
>
> 删除最短子数组，使剩余元素之和能被 $p$ 整除。$n\le 10^5$，不能枚举子数组。设总和模 $p$ 为 $k$，删除段的和也必须模 $p$ 等于 $k$。
>
> 扫描前缀和模 $p$，用哈希表记下每种余数最近一次出现的位置。当前余数为 $cur$ 时，查找余数 $(cur-k)\bmod p$ 的最近下标，两者之间即为候选删除段。全程删除则无解。
