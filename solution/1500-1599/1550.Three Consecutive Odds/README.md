...
> **思考**
>
> 判断是否存在连续三个奇数。$n$ 不大，一次扫描即可，无需额外下标结构。
>
> 用计数器累加当前奇数游程：遇到奇数加一，到 $3$ 便成功；遇到偶数清零。这样只依赖相邻关系，额外空间为常数。
...
> **思考**
>
> 方法一单独判断每个数的奇偶，实现清晰但分支稍多。三个数均为奇数，当且仅当它们按位与的最低位为 $1$。对每个三元组做一次与运算即可，省去显式计数。
