...
> **思考**
>
> 统计全为 $1$ 的子串个数，$n\le 10^5$，不能枚举全部 $O(n^2)$ 个子串。被 $0$ 隔开的连续 $1$ 段彼此独立，只需对每段计数。
>
> 长度为 $k$ 的全 $1$ 段贡献 $k(k+1)/2$ 个子串，也等价于扫描时维护当前连续 $1$ 长度 $cur$：每多一个 $1$，$cur$ 加一并把 $cur$ 累入答案；遇到 $0$ 则清零。这样不必预先切段。
