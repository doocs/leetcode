...
> **思考**
>
> 需要把所有子数组和排序后，求第 $\textit{left}$ 到第 $\textit{right}$ 项之和。子数组有 $O(n^2)$ 个，$n\le 10^3$，若再对每个询问区间重新求和会重复劳动，但本题只询问一次。
>
> 枚举左端点并维护向右延伸的前缀和，即可在 $O(n^2)$ 内列出全部子数组和；排序后对指定闭区间求和并对 $10^9+7$ 取模。$n^2\log n$ 的量级可以接受。
