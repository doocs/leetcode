...
> **思考**
>
> 判断网格中是否存在长度至少为 $4$、字符相同的环。$m,n$ 不大，一次图搜索即可，但必须排除立刻走回父格造成的假环。
>
> 对每个未访问格做广度优先：只走向同字符邻居，并记下父坐标。若邻居已被访问且不是父格，则找到环。同一连通块只搜一次。
...
> **思考**
>
> 广度优先需要显式队列，深度优先用递归栈表达同一父指针判定，代码更短。访问标记与「非父且已访问则为环」的规则不变，只是遍历次序不同。
