...
> **思考**
>
> 把 $s$ 变成 $t$，第 $i$ 次操作只能把某个字符沿字母表循环前进 $i$ 位，且每个 $i$ 至多用一次。$n\le 10^5$、$k\le 10^9$，不能模拟每一次操作。
>
> 长度不同则无解。每位的最小移位 $x\in[1,25]$ 确定，相同 $x$ 必须错开使用 $x,x+26,x+52,\ldots$。第 $cnt[x]$ 个同类需要 $x+26(cnt[x]-1)$ 步，只要该值不超过 $k$ 即可。移位 $0$ 无需操作。
