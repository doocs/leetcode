...
> **思考**
>
> 两组点之间连边，要求每点至少连一条，总费用最小。两侧至多 $12$ 个点，边的子集有 $2^{mn}$ 种，不能枚举；第二组可用 $n$ 位掩码表示「哪些点已被连上」。
>
> 令 $f[i][j]$ 为处理完第一组前 $i$ 个点、第二组连通状态为 $j$ 的最小费用。枚举与第 $i$ 个点相连的 $k$（$j$ 含该位），可从「本轮刚连上 $k$」「上一轮已是 $j$」「上一轮尚未含 $k$」三种前驱转移。答案为 $f[m][2^n-1]$。
...
> **思考**
>
> 第 $i$ 层只依赖第 $i-1$ 层以及本层已算出的状态，两层滚动数组即可把空间从 $O(m\cdot 2^n)$ 降到 $O(2^n)$。转移式不变。
