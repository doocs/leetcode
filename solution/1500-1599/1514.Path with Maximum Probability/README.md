...
> **思考**
>
> 在无向带权图上求从起点到终点的最大成功概率，边权为乘法意义下的成功率。$n\le 10^4$、$m\le 2\times 10^4$，枚举全部简单路径不可行。
>
> 若把边权看成代价的相反数，问题与最短路同构：概率沿路径相乘，且均在 $(0,1]$ 内，越长通常越小，满足最优子结构。用最大堆按当前概率松弛邻接边，即 Dijkstra 的最大化版本；堆中以相反数模拟大根堆，首次确定终点时即为答案。
