...
> **思考**
>
> 最多改三个数，使最大值与最小值之差尽量小。$n\le 10^5$，不能枚举被改的三元组。长度不足 $5$ 时改三个数后只剩至多一个有效值，差为 $0$。
>
> 排序之后，未改动的值构成一段连续区间，三次修改等价于从两端一共丢掉三个极端值。枚举左边丢掉 $l\in\{0,1,2,3\}$ 个、右边丢掉 $3-l$ 个，取 $nums[n-1-r]-nums[l]$ 的最小值即可。
