...
> **思考**
>
> 用曼哈顿距离连接全部点，求最小生成树。点数 $n\le 1000$，完全图有 $O(n^2)$ 条边，适合稠密图算法。
>
> 朴素 Prim：维护每个点到当前树的距离，每次选未访问的最近顶点加入，并用它更新其余点。$n$ 轮、每轮扫 $n$ 个点，时间为 $O(n^2)$。
...
> **思考**
>
> Prim 始终维护切边，实现依赖邻接矩阵。Kruskal 则把全部边按权排序，用并查集按序加边，连通时即停。边数为 $O(n^2)$，排序占主导，适合希望按边处理的写法。
