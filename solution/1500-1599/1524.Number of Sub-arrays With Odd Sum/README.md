...
> **思考**
>
> 统计子数组和为奇数的个数。子数组有 $O(n^2)$ 个，$n\le 10^5$，直接求和不可行。子数组和的奇偶等于两端前缀和之差的奇偶。
>
> 前缀和为奇当且仅当此前偶数前缀的个数可与之配对（奇减偶为奇），偶前缀则与已有奇前缀配对。扫描时维护奇、偶前缀计数，每走一步按当前前缀奇偶累加配对并更新计数，最后对 $10^9+7$ 取模。
