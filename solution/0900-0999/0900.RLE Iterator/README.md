...
> **思考**
>
> 游程次数可达 $10^9$，若将编码展开成完整序列，空间无法承受；每次 `next` 的 $n$ 同样可达 $10^9$，按元素逐个消耗也不现实。
>
> 因此必须直接在压缩表示上消耗。当前游程剩余量为 $\textit{encoding}[i]-j$，若小于待消耗的 $n$，则整段跳过并把 $i$ 前进 $2$；否则只在本段内推进 $j$。两个指针只向前移动，全部查询合计线性扫描一遍编码。
