...
> **思考**
>
> 求元素为 $0/1$ 的数组中和为 $\textit{goal}$ 的子数组个数。$n$ 较大，不能枚举区间。前缀和 $s$ 出现时，左端满足 $s-\textit{goal}$ 的前缀个数即为以当前为右端的答案。用计数表维护前缀出现次数，初始 $cnt[0]=1$。
...
> **思考**
>
> 方法一需要线性额外空间。数组非负，窗口和随右端单调，可用两条左指针分别维护和大于 $\textit{goal}$ 与和至少为 $\textit{goal}$ 的边界，两者之差即和恰为 $\textit{goal}$ 的子数组个数，额外空间为常数。
