...
> **思考**
>
> 出行日必须被 $1/7/30$ 日票覆盖，求最少花费。在第 $i$ 次出行买哪种票，下一次未覆盖的出行可用二分定位。$dfs(i)$ 对三种票取最小，出行次数 $\le 365$，记忆化即可。
...
> **思考**
>
> 按出行下标搜索需要二分。也可按日历日递推：$f[i]$ 为到第 $i$ 天的最少花费，非出行日等于 $f[i-1]$，出行日则在三种票的起点取 $\min$。日期上界为 $365$。
