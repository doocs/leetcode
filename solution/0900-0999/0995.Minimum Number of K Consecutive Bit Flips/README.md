...
> **思考**
>
> 每次翻转长度为 $k$ 的连续段，使全数组变为 $1$，次数最少。$n\le 10^5$，不能模拟每次翻转。从左到右贪心：当前位在此前翻转次数后仍为 $0$，则必须在此开一段。差分数组 $O(1)$ 标记 $[i,i+k)$ 的翻转。
...
> **思考**
>
> 差分数组需要线性额外空间。用变量记录当前奇偶，并把翻转起点标成 $-1$，窗口滑出时异或回去，即可在原数组上 $O(1)$ 空间完成同样贪心。
