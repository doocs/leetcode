...
> **思考**
>
> 三分配钢筋：左柱、右柱或丢弃，使两柱等高且尽量高。$n$ 不大但子集和较大，枚举 $3^n$ 不可行。状态只需“用到第 $i$ 根、当前高度差 $j$”的最大共同高度。三选一转移，差为 $0$ 时即为可行高度。记忆化避免重复。
...
> **思考**
>
> 搜索可改成自底向上：$f[i][j]$ 为前 $i$ 根、高度差 $j$ 的最大共同高度。转移与搜索相同，避免递归常数，答案为 $f[n][0]$。
