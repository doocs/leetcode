...
> **思考**
>
> 同行或同列的石头可互相到达，一个连通块最后只能留一块，故答案为 $n$ 减去连通块数。两两检查是否同行同列并入并查集，成功合并的次数即为可移除数。
...
> **思考**
>
> 方法一对石头两两连边，为平方级。行与列本身可作为并查集元素：把列加上偏移后与行合并，同一行或同一列自动连通。剩余块数等于不同行根的个数。
...
> **思考**
>
> 并查集之外，也可按同行同列建图后 BFS 统计连通块。实现更直接，边数在最坏情况下仍为平方级。
