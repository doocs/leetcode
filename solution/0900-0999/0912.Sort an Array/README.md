...
> **思考**
>
> $n\le 5\times 10^4$，需要平均 $O(n\log n)$ 的排序，且不能依赖会超时或超空间的朴素算法。三路快排按随机主元划分小于、等于、大于三段，避免大量重复值导致退化，再递归两侧。
...
> **思考**
>
> 快排平均很快，最坏仍可能不平衡，且不是稳定排序。归并每次折半再线性合并，时间稳定为 $O(n\log n)$，需要线性辅助数组。
