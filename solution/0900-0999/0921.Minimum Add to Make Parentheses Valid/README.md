...
> **思考**
>
> 添加最少括号使序列合法。栈匹配后剩余的左右括号都必须各补一个配对。左括号入栈，右括号优先与栈顶左括号抵消，否则入栈；结束时栈长即为答案。
...
> **思考**
>
> 栈中实际只需记录未匹配的左括号个数，不必存字符。用 $cnt$ 计待匹配左括号，无法匹配的右括号计入 $ans$，最后再把剩余 $cnt$ 加上，空间降为常数。
...
> **思考**
>
> 已经配对的 `()` 对答案无贡献，删去后问题规模下降。反复替换掉一对 `()`，长度不变说明已无配对，剩余长度即需添加的数量；否则继续递归。$n\le 1000$，替换次数可接受。
