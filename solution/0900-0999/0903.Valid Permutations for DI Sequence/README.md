...
> **思考**
>
> 长度为 $n$ 的 DI 串对应 $0..n$ 的排列，$n$ 可达 $200$，枚举排列不可行。关键在于相对名次：放完前 $i$ 个数后，下一个数在剩余数中的名次 $j$ 决定与上一位置的升降关系。
>
> 定义 $f[i][j]$ 为前 $i$ 个字符、以名次 $j$ 结尾的方案数。遇 `'D'` 时从更大的名次转移，遇 `'I'` 时从更小的名次转移；已用名次压缩到 $[0,i]$，得到立方级递推。
...
> **思考**
>
> 方法一每次转移要扫一段 $k$，同一行上相邻的 $j$ 共享几乎相同的区间和。用按方向滚动的前缀和在常数时间内得到 $\sum f[i-1][k]$，时间降为平方级，状态定义不变。
...
> **思考**
>
> $f[i]$ 只依赖 $f[i-1]$，二维表中多数空间可以丢掉。用一维数组滚动，按 `'D'`/`'I'` 的方向维护累加和写入新一层，空间降至线性。
