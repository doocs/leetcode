...
> **思考**
>
> 用给定数位拼出不超过 $n$ 的正整数，按长度枚举全部排列会在接近 $n$ 的位数上重复展开相同前缀。方案数只取决于当前位、是否仍为前导零、以及是否贴着上界。
>
> 将 $n$ 写成字符串 $s$，记忆化搜索 $\textit{dfs}(i, \textit{lead}, \textit{limit})$：前导零可继续填 $0$，否则只能填 $\textit{digits}$ 中的数字，并按 $\textit{limit}$ 限制本界。处理完所有位后，非前导零的构造计为 $1$。
