...
> **思考**
>
> 区间数量与时间上界均不超过 $100$，枚举子集可以覆盖 $[0,\textit{time}]$，但子集数为指数。同一起点只需保留右端最远的片段，问题化成「从当前位置能跳到的最远覆盖」。
>
> 这与跳跃游戏相同：记下每个起点的最远终点 $\textit{last}[i]$，从左扫描维护当前可达 $\textit{mx}$；若 $\textit{mx}$ 停在 $i$，则无法前进。每越过上一段的右端 $\textit{pre}$ 就增加一段。
>
> 一遍扫描得到最少片段数，或在断开时返回 $-1$。
