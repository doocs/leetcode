...
> **思考**
>
> 枚举所有合并顺序可以给出答案，但 $n \le 30$ 时合法划分的数量随区间长度迅速增长，无法在时限内穷尽。任意一次操作只作用于连续 $K$ 堆，因此真正决定代价的是「某一段最终并成若干堆」这一子问题，而同一区间会被许多顺序重复计算。
>
> 注意到每次把 $K$ 堆合成一堆，堆数减少 $K-1$，故仅当 $(n-1)\bmod (K-1)=0$ 时才能并成一堆，否则直接返回 $-1$。可行时，将 $[i,j]$ 并成 $k$ 堆，等价于先把某一前缀并成 $1$ 堆、后缀并成 $k-1$ 堆；若要并成 $1$ 堆，则须先得到 $K$ 堆再付出该区间石子总和。
>
> 为此按区间长度递推 $f[i][j][k]$，并用前缀和 $s$ 在 $O(1)$ 内取出区间和。枚举断点 $h$ 时只取左段的 $f[i][h][1]$，与「一次只能合并连续堆」的约束一致，最终答案为 $f[1][n][1]$。
