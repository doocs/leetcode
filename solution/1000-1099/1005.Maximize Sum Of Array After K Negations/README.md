...
> **思考**
>
> 每次取当前最小元素取反并重复 $k$ 次可以得到最优解，$n$ 与 $k$ 均不超过 $10^4$，但反复找最小值需要额外的选择结构。元素范围只有 $[-100,100]$，不必在原数组上排序。
>
> 为使总和最大，应优先把绝对值最大的负数翻成正数；若次数用尽后仍剩奇数次且没有 $0$，则必须再翻一次最小的正数。
>
> 用计数数组按 $-100$ 到 $-1$ 依次消耗翻转次数，再在必要时对最小正数补一次翻转，最后按频次加权求和。
