...
> **思考**
>
> 不在位的学生数等于原序列与非降序排列在对应位置上不同的个数。$n\le 100$，排序后逐位比较即可。
>
> 复制一份排序得到 $\textit{expected}$，统计与 $\textit{heights}$ 不等的下标个数。
...
> **思考**
>
> 比较排序需要 $O(n\log n)$ 与额外数组。高度只有 $1$ 到 $100$，计数排序能在线性时间内生成应有序列。
>
> 统计每个高度的次数，再按高度从小到大展开，与原数组并行扫描，遇不同则累加。
