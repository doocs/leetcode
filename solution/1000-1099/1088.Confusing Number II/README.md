...
> **思考**
>
> 统计 $[1,n]$ 中旋转后变成不同数的个数，$n\le 10^9$，不能枚举。可旋转的数位只有 $0,1,6,8,9$，应按位构造合法数字再判断是否易混淆。
>
> 数位 DFS 从高到低填写，仅枚举映射表非 $-1$ 的数位，并受 $n$ 的上界约束。到达末位后用与 $1056$ 相同的旋转比较判定。
>
> $d$ 同时用于填写与 `check`，前导零作为数值 $0$ 自然处理。
