...
> **思考**
>
> 按曼哈顿距离对 $rows\times cols$ 个格子排序即可，$r,c\le 100$ 时 $O(rc\log(rc))$ 可过。距离为 $d$ 的格子恰好组成以中心为原点的菱形，按层扩展不必比较距离。
>
> 从 $(rCenter,cCenter)$ 做广度优先，每一层的格子距离比上一层恰好多 $1$，出队顺序就是题目要求的顺序。
>
> 用队列与访问标记避免重复入队，扫完所有格子即得答案。
