...
> **思考**
>
> 网格为 $10^6\times 10^6$，不能建图。障碍至多 $200$ 个，它们围住的区域面积不超过 $|blocked|^2/2$，因此从一点出发只要走到足够多的格子，就说明没有被围死。
>
> 分别从起点与终点做深度优先：碰到对方则通路存在；访问数超过该上界则已绕出包围。两侧都能走出去（或相遇）才算可逃。
>
> 障碍放入集合以便 $O(1)$ 判断，搜索空间与障碍规模的平方同阶。
