...
> **思考**
>
> 点数只有 $1$ 到 $6$，对每个候选值检查能否通过旋转铺满一行即可，长度为 $2\times 10^4$ 时线性扫描足够。六个候选里多数会因某一列上下都不是该值而失败。
>
> 若存在一种统一的点数，它必须出现在第一列的 $tops[0]$ 或 $bottoms[0]$ 上，否则该列无法变成这个值。
>
> 因此只需对这两个候选计算 $f(x)$：任一列都不含 $x$ 则不可能；否则旋转次数为 $n$ 减去该值在上或下出现次数的较大者，取两次结果的较小值。
