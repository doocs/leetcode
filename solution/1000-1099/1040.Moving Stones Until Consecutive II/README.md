...
> **思考**
>
> 每次只能移动端点石子到非端点空位，$n\le 10^4$ 且坐标达 $10^9$，不能逐步模拟。最大步数来自「尽量不填中间、每次只挪一格」；最小步数则是用尽量少的移动把石子装进某个长为 $n$ 的窗口。
>
> 排序后，第一次若动左端则跳过 $stones[0]$ 到 $stones[1]$ 的空隙，最多 $stones[n-1]-stones[1]+1-(n-1)$ 步，对称地动右端；二者取大。最小步用双指针找含 $n$ 个位置的窗口里已有石子数，缺口即需移动次数；特判「差一个且窗口几乎连满」时必须挪两次。
>
> 一次排序加一次滑动窗口同时得到 $[\textit{mi},\textit{mx}]$。
