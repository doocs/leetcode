...
> **思考**
>
> 先按深度把序列拆成节点再递归挂左右孩子可以得到树，$n\le 1000$ 可行，但要反复切分字符串。先序加上深度意味着：新节点的父结点是深度恰好比它小 $1$ 的最近节点，也就是当前栈中深度为 $d-1$ 的栈顶。
>
> 从左到右解析连续的 `-` 作为深度、数字作为权值；栈中节点深度等于其下标，新节点入栈前弹出所有深度不小于它的节点，再挂到新的栈顶左侧或右侧。
>
> 扫完后栈底即为根。实现里用栈大小表示深度，与序列中的短横线计数对齐。
