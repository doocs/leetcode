...
> **思考**
>
> 运载能力的下界是最重包裹，上界是全部重量之和，在该范围内逐一验证所需天数，$n\le 5\times 10^4$ 且总和可达千万级，线性枚举不可行。
>
> 能力增大时所需天数不会增加，因此「能否在 $\textit{days}$ 天内运完」对能力值单调，最小可行能力可以用二分定位。
>
> 对给定上限 $x$，按顺序把包裹装入当前船，超重则换一天。在 $[\max w_i,\sum w_i]$ 上二分第一次使 $\textit{check}$ 为真的位置。
