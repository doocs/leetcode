...
> **思考**
>
> 枚举两段的左右端点需要 $O(n^2)$，长度 $n\le 1000$ 偏紧。固定其中一段后，另一段的最优位置只取决于它左侧（或右侧）定长窗口的最大和。
>
> 前缀和使任意定长窗口的和可在 $O(1)$ 取出。分「长为 $\textit{firstLen}$ 的段在左」与「长为 $\textit{secondLen}$ 的段在左」两种情形，扫右段左端点时用变量 $t$ 维护左侧窗口的历史最大和。
>
> 两种情形各扫一遍，取较大者。
