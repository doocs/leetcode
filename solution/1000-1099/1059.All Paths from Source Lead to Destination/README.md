...
> **思考**
>
> 要确认从 $source$ 出发的每条路径都停在 $destination$ 且图中无环通向别处。$n,m\le 10^4$，应对每个点记忆「正在访问 / 已确认可达终点」，以免重复搜索。
>
> 终点若仍有出边则不能作为终止。DFS 遇状态 $1$ 为环；无出边则必须等于终点；否则先标 $1$ 再要求所有后继成功，最后标 $2$。
>
> 从 $source$ 出发的搜索结果即为答案。
