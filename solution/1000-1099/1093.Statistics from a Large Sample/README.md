...
> **思考**
>
> 样本以 $0..255$ 的频次给出，总数可达 $10^9$，不能展开。最小、最大、和、众数都可以在扫频次数组时得到；中位数则要按累计频次定位第 $k$ 个元素。
>
> 一遍扫描更新 $mi,mx,s,cnt$ 以及频次最大的 $\textit{mode}$。`find(i)` 沿 `count` 累加直到达到第 $i$ 个样本。
>
> 总数为奇数取正中一个，偶数取中间两个的平均，再与均值 $s/cnt$ 一起返回。
