...
> **思考**
>
> 普通正基转换对 $-2$ 会得到负余数，不能直接套用「除基取余」。$n\le 10^9$，位数约为 $O(\log n)$，按位模拟足够。
>
> 当前最低位为 $n\bmod 2$。若该位为 $1$，减去这一位的权值 $k$（在奇数位为 $-1$ 的幂）才能使剩余部分仍能被 $2$ 整除，然后再除以 $2$ 并翻转 $k$ 的符号。
>
> 从低到高收集 $0/1$，最后反转；若 $n$ 一开始就是 $0$，则答案为 $\texttt{0}$。
