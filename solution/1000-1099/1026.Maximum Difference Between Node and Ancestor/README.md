...
> **思考**
>
> 对每个节点向上收集全部祖先再求最大差，$n\le 5000$ 时路径重复、时间偏大。路径上真正用到的只有祖先的最小值与最大值。
>
> 下行时把当前路径的 $\textit{mi},\textit{mx}$ 传给子节点，在每个节点用 $|\textit{val}-\textit{mi}|$ 与 $|\textit{val}-\textit{mx}|$ 更新全局答案，再把本节点纳入区间后继续。
>
> 一次 DFS 遍历整棵树即可。
