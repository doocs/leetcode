...
> **思考**
>
> 四重枚举子矩阵在 $100\times 100$ 上为 $O(n^4)$，偏紧。固定上下边界后，每列压缩成一个数，问题变成「一维数组中和为 $\textit{target}$ 的子数组个数」。
>
> 一维可用前缀和哈希：每加入 $x$ 就查询 $s-\textit{target}$ 出现次数。上下边界 $O(n^2)$ 对，每对扫描一列。
>
> 累加所有边界对上 $f(\textit{col})$ 的返回值。
