...
> **思考**
>
> 不生气的顾客始终计入；技巧只能覆盖连续 $\textit{minutes}$ 分钟的生气顾客。$n\le 2\times 10^4$，应一次扫清所有窗口。
>
> 先把不生气时段的客流求和，再在生气客流上维护长为 $\textit{minutes}$ 的滑动窗口最大值。
>
> 窗口右移时加上新分钟的 $customers[i]\cdot grumpy[i]$、减去滑出的对应项，答案为固定客流加上该最大值。
