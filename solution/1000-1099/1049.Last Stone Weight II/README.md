...
> **思考**
>
> 每次粉碎两块石头等价于给它们相反的符号后求和，最终剩余是把石子分成两堆后的差的绝对值。$n\le 30$ 但总和只有几千，可转化为容量 $\lfloor s/2\rfloor$ 的 $0$-$1$ 背包：一堆尽量接近一半。
>
> $\textit{dp}[i][j]$ 表示前 $i$ 块能否（或最大地）凑出不超过 $j$ 的重量。转移选或不选第 $i$ 块。
>
> 答案为 $s-2\cdot\textit{dp}[n][\lfloor s/2\rfloor]$。
...
> **思考**
>
> 二维表中第 $i$ 行只依赖第 $i-1$ 行。滚动到一维数组并从大到小枚举容量，即可避免同一块石头被用两次，空间降为 $O(s)$。
