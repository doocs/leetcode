...
> **思考**
>
> 每个节点应换成不小于它的所有键之和。BST 的反序中序（右、根、左）恰按键从大到小访问，沿途累加即可就地改写。$n\le 100$，递归深度可接受。
>
> 先递归右子树，把当前值加入总和 $s$ 并写回，再递归左子树。
>
> 一遍遍历完成转换。
...
> **思考**
>
> 递归栈随树高增长，进阶希望额外空间 $O(1)$。Morris 遍历用空的右子树最左指针暂存后继，可在不使用栈的情况下完成同一「右根左」顺序。
>
> 当前节点无右子则结算并走向左；有右子则找到后继：尚未穿线则先穿到自己再向右，已穿线则结算、拆线并走向左。累加规则与方法一相同。
