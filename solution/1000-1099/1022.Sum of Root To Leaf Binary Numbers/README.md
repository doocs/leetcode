...
> **思考**
>
> 先列出根到叶的每条路径再把二进制转成整数相加，$n\le 1000$ 可行，但路径要额外存储。从根走下时，路径值满足 $t\leftarrow 2t+\textit{val}$，到达叶子即可结算。
>
> 空节点贡献 $0$；左右皆空视为叶子，返回当前 $t$；否则把同一 $t$ 交给左右子树相加。
>
> 深度优先带着路径值下行，每个节点访问一次。
