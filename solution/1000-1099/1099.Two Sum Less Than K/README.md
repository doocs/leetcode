...
> **思考**
>
> 小于 $k$ 的最大两数之和，枚举一对是平方级。排序后对每个 $x$ 找小于 $k-x$ 的最大右端，可用二分。
>
> 对下标 $i$，在 $(i,n)$ 上 `bisect_left` 定位 $k-x$，前一个位置若仍大于 $i$ 则更新答案。
>
> 无合法对时保持 $-1$。
...
> **思考**
>
> 每个 $x$ 一次二分共 $O(n\log n)$。两端相向是同一单调性：和偏小则左端右移以增大，和达到 $k$ 则右端左移。
>
> 双指针一次扫完所有互异下标对的帕累托前沿，常数更小。
