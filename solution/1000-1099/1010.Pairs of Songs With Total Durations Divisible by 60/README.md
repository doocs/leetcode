...
> **思考**
>
> 枚举所有无序对判断时长之和是否整除 $60$，在 $n\le 6\times 10^4$ 时为平方级。配对只依赖对 $60$ 取模后的余数。
>
> 若 $x=a\bmod 60$，则与之匹配的余数是 $y=(60-x)\bmod 60$。对每个已出现的 $y$ 计数，即可在读入当前歌时累加合法对数。
>
> 用长度为 $60$ 的计数表边遍历边查询再更新，避免把同一首歌与自己配对，时间和余数域均为线性。
