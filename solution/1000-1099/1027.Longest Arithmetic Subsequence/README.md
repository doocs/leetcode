...
> **思考**
>
> 枚举公差再对每个公差求最长子序列，$n\le 1000$ 且值域在 $[0,500]$，公差种类约 $10^3$，双重枚举下标即可在时限内完成。子序列不必连续，状态应落在「以某下标结尾、某公差」。
>
> 定义 $f[i][j]$ 为以 $i$ 结尾、公差为 $j$ 的最长长度。因公差可负，将 $j$ 平移 $500$ 后落在 $[0,1000]$。转移枚举 $k<i$，用 $f[k][j]+1$ 更新。
>
> 每个下标对更新一次，答案取所有 $f[i][j]$ 的最大者。
