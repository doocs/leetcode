...
> **思考**
>
> 直接枚举 $[1,n]$ 并检查是否有重复数位，$n\le 10^9$ 不可行。改统计没有重复数位的个数 $f(n)$，答案即为 $n-f(n)$。
>
> 数位逐位填写时，已用过的数字集合决定下一步还能选什么，前导零不应占用集合，且上界受 $n$ 的数位约束。集合大小为 $10$，可用二进制掩码表示。
>
> 记忆化 $\textit{dfs}(i,\textit{mask},\textit{lead},\textit{limit})$ 按第 $i$ 位枚举合法数字：仍为前导零则掩码不变；否则仅当该位未出现过才继续。终点在非前导零时计 $1$。
