...
> **思考**
>
> 划分方案数为指数，$n\le 500$、$k\le n$ 不能枚举全部切法。最后一段长度至多为 $k$，其贡献等于该段最大值乘长度，之前的部分是规模更小的同类问题。
>
> 令 $f[i]$ 为前 $i$ 个元素的最大和。从 $i$ 往左枚举最后一段起点 $j$，同时维护段内最大值 $\textit{mx}$，用 $f[j-1]+\textit{mx}\cdot(i-j+1)$ 更新。
>
> 按 $i$ 递增填表，答案为 $f[n]$。
