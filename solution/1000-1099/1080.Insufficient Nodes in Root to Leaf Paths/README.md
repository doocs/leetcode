...
> **思考**
>
> 要删掉所有根到叶路径和小于 $\textit{limit}$ 的节点。从叶往上：一条路径的合法性在叶子才能判定，父节点仅当左右都删光时才失去存在意义。$n\le 5000$，一次后序即可。
>
> 下行时把 $\textit{limit}$ 减去当前值。叶子在剩余 $\textit{limit}>0$ 时删除。内部节点先处理左右孩子，若两边都空则自身也删。
>
> 返回新的子树根，可能为空。
