...
> **思考**
>
> 按定义从 $n$ 递减套用 $\times,\div,+, -$ 即可，但乘除优先于加减，直接从左到右累加会算错。$N\le 10^4$，一次线性扫描足够，关键是把不同优先级拆开。
>
> 乘、除必须立刻与前一个操作数结合，加、减只是向结果放入正负项，可延后到最后再求和。
>
> 用栈保存当前待结算的项，并以 $k\bmod 4$ 轮转四种运算：乘除时弹出栈顶运算后再压回，加减则压入 $x$ 或 $-x$，最后把栈中元素相加。
