...
> **思考**
>
> 最短公共超序列长度等于 $|s_1|+|s_2|-\mathrm{LCS}$。$m,n\le 1000$，先求 LCS 表，再沿转移回溯把非公共字符按序插入。
>
> $f[i][j]$ 为两前缀的 LCS 长。从 $(m,n)$ 往回走：若来自上方或左方则输出被丢掉的那个字符；否则输出公共字符并对角后退。
>
> 路径是逆序收集的，最后反转。某一侧耗尽时把另一侧剩余字符全部接上。
