...
> **思考**
>
> 每次查询都对历史字符流做一次多模式匹配，$q\le 4\times 10^4$ 且模式总长可观，朴素扫描会重复读同一后缀。询问的是「当前流是否以某个单词结尾」，因此应按后缀匹配。
>
> 把每个单词反转后插入前缀树，查询时从流的末尾往前走树。单词最长 $200$，故只需回看有限界长。
>
> 构造时建反向前缀树，查询把新字母追加到缓冲并在树上走至多 $201$ 步，遇到结束标记即成功。
