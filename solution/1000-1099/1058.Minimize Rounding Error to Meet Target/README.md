...
> **思考**
>
> 每个价格可下取整或（小数非零时）上取整，误差为与原价的距离。全部下取整之和为 $\textit{mi}$，可上取整的个数为小数非零的个数，目标必须落在该闭区间内。
>
> 需要上取整 $d=\textit{target}-\textit{mi}$ 个。上取整误差为 $1-\{p\}$，下取整误差为 $\{p\}$，因此应优先给小数部分最大的 $d$ 个做上取整。
>
> 把小数部分降序排序后按此计算总误差，保留三位小数。
