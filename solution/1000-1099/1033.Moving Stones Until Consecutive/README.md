...
> **思考**
>
> 三枚石子位置互异，状态空间虽可搜索，但每一步的最优选择只取决于排序后的间距，不必模拟过程。
>
> 记 $x<y<z$。三点已相邻则无需移动。否则若 $y$ 与某一端间隔不超过 $2$，一次即可填入空隙；其余情况两端各靠一次，最少为 $2$。最多步数是两端逐步向中间空位移动，共 $z-x-2$ 次。
>
> 排序后按上述分类直接给出 $[\textit{mi},\textit{mx}]$。
