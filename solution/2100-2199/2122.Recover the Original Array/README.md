...
> **思考**
>
> $\textit{nums}$ 是原数组每个元素加减 $k$ 后的多重集合。$k$ 未知，若枚举所有可能的配对，状态过多。较低的一半与较高的一半应能按同一 $k$ 一一匹配。
>
> 排序后最小元素必是某个 $a_i-k$，因而 $k$ 只能是 $(\textit{nums}[i]-\textit{nums}[0])/2$，其中 $i$ 取使差为正偶数的位置。对每个候选 $k$，用双指针在有序数组上贪心配对 $x$ 与 $x+2k$。
>
> 一旦配出 $n/2$ 对，均值序列即为原数组。
