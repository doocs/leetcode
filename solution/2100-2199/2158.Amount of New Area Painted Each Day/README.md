...
> **思考**
>
> 每天涂一个区间，求当天新涂到的长度。区间端点到 $10^5$，天数到 $10^5$，若每天扫描格子则为二次。需要能对区间做覆盖并查询已涂长度。
>
> 线段树维护「区间内已涂格子数」，修改时把整段标记为已涂并下推懒标记。当天答案为区间长度减去查询到的已涂数，再把该区间置为已涂。
>
> 坐标平移到从 $1$ 开始，对每天先 $\texttt{query}$ 再 $\texttt{modify}$。
