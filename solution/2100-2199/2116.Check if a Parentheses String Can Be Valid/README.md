...
> **思考**
>
> 可变括号可改成任一方向，锁定括号则不能改。奇数长度无法两两匹配，直接判否。若枚举每个可变位置的方向，状态数为指数级，不可行。
>
> 从左向右看，每个前缀中「可当作左括号的位置」不得少于右括号；从右向左对称。锁定的左括号必须向右匹配，锁定的右括号必须向左匹配，可变位置在对应方向上均可充当匹配方。
>
> 为此先排除奇数长度，再做两次扫描：向右时将 `'('` 或未锁定位置计入余额，遇到锁定的 `')'` 则消耗；向左时角色对调。任一次余额不足即无效。
