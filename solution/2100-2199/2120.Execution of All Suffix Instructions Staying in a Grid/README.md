...
> **思考**
>
> 对每个起始下标 $i$，需要从 $\textit{startPos}$ 执行指令后缀 $s[i:]$，直到越界或指令用尽，统计步数。指令长 $m\le 500$，对每个 $i$ 模拟剩余指令的总代价为 $O(m^2)$，可以接受。
>
> 格子有界，每一步由字符决定位移，越界即停。不同后缀的起点相同但路径不同，难以共享状态，直接双重循环即可。
>
> 外层枚举起点 $i$，内层从 $i$ 走到末尾并更新坐标，累加合法步数。
