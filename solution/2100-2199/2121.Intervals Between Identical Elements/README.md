...
> **思考**
>
> 对每个下标 $i$，需要所有 $j$ 满足 $\textit{arr}[j]=\textit{arr}[i]$ 的 $|i-j|$ 之和。按值分组后，朴素对组内两两求和是 $O(n^2)$，在 $n\le 10^5$ 时不可行。
>
> 同一值的下标有序，记为 $v_0,\ldots,v_{m-1}$。相邻两项的差 $\Delta$ 会使左侧 $i$ 个点各增加 $\Delta$、右侧 $m-i$ 个点各减少 $\Delta$，因而可以从「全部相对第一个下标的距离和」递推到组内每一个位置。
>
> 按值收集下标，先算 $\sum v-v_0\cdot m$，再沿组扫一遍按上式更新并写入答案。
