...
> **思考**
>
> 矩阵应在每一行、每一列都恰好包含 $1\ldots n$ 各一次。$n\le 100$，逐行逐列检查集合大小是否为 $n$ 即可，无需更重的结构。
>
> 行直接取 $\textit{matrix}$，列用转置；集合去重后长度为 $n$ 当且仅当该行（列）无重复，从而必是 $1\ldots n$ 的排列（题目保证元素在该范围内）。
>
> 对 $\texttt{chain}(\textit{matrix},\texttt{zip}(*\textit{matrix}))$ 中每一序列验证即可。
