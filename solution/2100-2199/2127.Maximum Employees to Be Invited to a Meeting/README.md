...
> **思考**
>
> 每人恰好喜欢一人，图由若干基环内向树组成。圆桌要求相邻都互相喜欢，因而要么坐在同一个环上，要么利用长度为 $2$ 的互喜欢对把两侧的链拼在一起。不同环互不连通，不能混坐。
>
> 环长至少为 $3$ 时，只能取该环本身，答案候选为全局最长环。所有长度为 $2$ 的环彼此可以同时入席，各自再接上指向该点的最长入链，这些链的长度可在拓扑消点后得到。
>
> 为此分别求最大环长，以及所有二元环上 $\textit{dist}$ 之和，取较大者。
