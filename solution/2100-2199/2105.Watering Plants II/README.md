...
> **思考**
>
> Alice 与 Bob 分别从两端走向中间，每人浇到一株植物前必须保证罐中水量足够，否则先灌满再浇。过程由位置唯一确定，没有分枝决策，直接模拟即可。
>
> $n\le 10^5$，每人至多经过 $O(n)$ 株植物，用双指针相向移动并维护当前水量 $a$、$b$，即可在线性时间内统计灌满次数。
>
> 当两指针相遇时，剩余的一株由当前水量较多者浇；若二者都不足则再灌满一次。相遇之前各自独立判断并扣减水量。
