...
> **思考**
>
> 好三元组在两个排列中相对顺序一致。枚举三元组为 $O(n^3)$，$n\le 10^5$ 不可行。固定中间值后，左侧应是「在 $\textit{nums1}$ 中已出现、且在 $\textit{nums2}$ 中位置更前」的个数，右侧对称。
>
> 按 $\textit{nums1}$ 顺序插入 $\textit{nums2}$ 中的位置，树状数组查询该位置前缀已插入数，以及后缀尚未插入数，乘积即该中间值的贡献。
>
> 位置从 $1$ 编号，每处理完一个值就 $\texttt{update}$。
...
> **思考**
>
> 方法一用树状数组维护前缀插入数。同一前缀和也可用线段树单点加、区间求和完成，渐近复杂度相同，常数与实现不同。
>
> 结点存储区间内已出现的个数，查询 $[1,p]$ 与 $(p,n]$ 的未出现数，乘积累加后将该点加一。
>
> 题解给出线段树作为对照实现。
