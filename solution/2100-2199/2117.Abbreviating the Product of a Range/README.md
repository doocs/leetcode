...
> **思考**
>
> 区间乘积的位数可达数万，直接高精度乘法再截取前后五位代价过大。科学计数法要求去掉尾随零，并在位数超过十时输出前五位、后五位与指数。
>
> 尾随零的个数等于因子 $2$ 与 $5$ 的对数的较小值。去掉这些因子后，后缀可用对 $10^{10}$ 取模维护最后若干位；前缀则用浮点（或反复除以 $10$）把有效数字压回五位以上的窗口。
>
> 为此先统计并扣除 $\min(\textit{cnt}_2,\textit{cnt}_5)$ 个 $2$ 与 $5$，再扫一遍乘积维护 $\textit{pre}$ 与 $\textit{suf}$；若后缀曾超过 $10^{10}$ 则按缩写格式输出，否则输出完整去零结果。
