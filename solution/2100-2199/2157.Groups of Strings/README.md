...
> **思考**
>
> 两个单词若能通过加、删或替换一个字母互相得到，则属于同一组。单词可看成 $26$ 位掩码，枚举邻居若按字符串比较则过慢。$n\le 2\times 10^4$，需把掩码放进并查集。
>
> 每个掩码与「翻转一位」（加或删）以及「先删一位再加另一位」（替换）的掩码合并。相同掩码本身已是一组，计入该组大小。
>
> 扫描全部已出现掩码做合并，维护连通块个数与最大块大小。
