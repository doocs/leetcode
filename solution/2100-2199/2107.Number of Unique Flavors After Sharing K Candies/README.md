...
> **思考**
>
> 分出的 $k$ 颗糖必须是连续一段，自己留下的是这段的补集。若对每个窗口重建口味集合，每次 $O(n)$，总时间 $O(n^2)$，在 $n\le 10^5$ 时不可行。
>
> 窗口长度固定为 $k$，右移一格时只有两端各一颗糖进出补集。用计数表维护窗口外每种口味的个数，种类数即表的大小。
>
> 初始将 $\textit{candies}[k:]$ 放入计数表，再滑过每个长度为 $k$ 的窗口，更新两端并记录种类数的最大值。
