...
> **思考**
>
> 环形数组中把全部 $1$ 换到一起，等价于找一个长度为 $k=\textit{count}(1)$ 的窗口，使其中 $1$ 最多，答案为 $k$ 减该最大值。枚举所有窗口若每次重算计数则为二次。
>
> 窗口长度固定，环形只要把下标对 $n$ 取模继续滑 $k$ 步。先统计前 $k$ 个 $1$ 的个数，再每次加右端、减左端。
>
> 扫完一圈后返回 $k-\textit{mx}$。
...
> **思考**
>
> 方法一用取模模拟环形。也可用前缀和在线性数组上统计窗口内某值的个数：对值为 $x$ 的窗口，交换次数为「窗口长度减去窗口内 $x$ 的个数」。
>
> 在环上把全部 $1$ 聚拢与把全部 $0$ 聚拢是同一类构造的两端描述，前缀和分别对 $0$ 与 $1$ 求最小交换次数后取较小值。
>
> 本题用前缀和枚举长度为「该值全局个数」的区间，作为滑动窗口的另一种实现。
