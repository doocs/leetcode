...
> **思考**
>
> 若能同时运行 $t$ 分钟，则也能运行更短时间，具有单调性。$t$ 的上界为电量总和，枚举 $t$ 不可行。
>
> 一块电池对时长 $t$ 的有效贡献至多为 $t$（同一时刻只能供一台），因此当且仅当 $\sum_i \min(b_i,t)\ge n\cdot t$ 时可行。在 $[0,\sum b_i]$ 上二分 $t$。
>
> 每次用线性扫描验证中间值，取最大可行 $t$。
