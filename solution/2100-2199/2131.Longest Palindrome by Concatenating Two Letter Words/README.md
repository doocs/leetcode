...
> **思考**
>
> 回文由若干单词及其反串对称拼接，中心至多放一个形如 `aa` 的单词。若搜索拼接顺序，状态过多。每种单词只关心出现次数与其反串是否可配对。
>
> 哈希表统计频次后，`ab` 与 `ba` 可配 $\min$ 次，各贡献 $4$ 个字符；`aa` 类可先按对用掉偶数个，余下一个奇数留给中心。
>
> 遍历计数表累加配对长度，若存在未用的对称单词再加 $2$。为避免 `ab`/`ba` 被计两次，对异色单词每次仍按 $\min$ 累加（两侧各走一次，恰好得到 $2\min\times 2$）。
