...
> **思考**
>
> 每根杆只需知道红、绿、蓝是否都出现过。若对 $10$ 根杆各扫描整串 $rings$ 再分别判断三种颜色，重复遍历过多，且字符串长度为偶数、颜色与杆号成对出现，适合一次扫描完成。
>
> 三种颜色是互不重叠的布尔标记，可用三位二进制分别表示。将 `'R','G','B'` 映射为 $1,2,4$，对第 $j$ 根杆做按位或，则集齐三色当且仅当掩码为 $7$。
>
> 为此用长度为 $10$ 的数组 $\textit{mask}$ 按对读取 $rings$，最后统计值为 $7$ 的杆数。
...
> **思考**
>
> 方法一已经线性扫过整串。若只关心实现是否直观，也可以按杆号 $0$ 到 $9$ 枚举，用子串查找判断该杆是否同时出现过 `'B'`、`'R'`、`'G'`。
>
> 查找本身仍是线性扫描，常数略大，但杆数固定为 $10$，复杂度同阶。本题将其作为对照实现。
