...
> **思考**
>
> 用最少直线覆盖至多 $10$ 个点。枚举直线划分是集合划分问题，点数很小，适合对子集做搜索。
>
> 状态 $\textit{state}$ 表示已覆盖的点集。每次从未覆盖点 $i$ 出发，与 $j$ 确定一条直线，并把共线的其余点一并盖上，递归剩余子集。单点也可单独用一条直线。
>
> 记忆化 $dfs(\textit{state})$，终点为全集，转移代价加一。共线用叉积判定。
