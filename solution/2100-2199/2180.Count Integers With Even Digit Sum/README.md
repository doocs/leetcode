...
> **思考**
>
> 统计 $[1,\textit{num}]$ 中数位和为偶数的个数。$\textit{num}\le 1000$，枚举每个数并求数位和即可。
>
> 对 $x$ 反复取模累加，偶则计数加一。
>
> 实现简单，复杂度带一个 $\log n$ 因子。
...
> **思考**
>
> 方法一随 $\textit{num}$ 线性增长。注意到每 $10$ 个连续整数中恰有 $5$ 个数字和为偶数，可先按整十段计算，再处理余数。
>
> 整十段贡献 $\lfloor\textit{num}/10\rfloor\times 5$，去掉 $0$ 后为减一。余下的个位个数还取决于高位数字和的奇偶，用 $s$ 的最低位调整公式中的偏移。
>
> 位数循环只需 $O(\log n)$。
