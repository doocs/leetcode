...
> **思考**
>
> 在正整数中选出 $k$ 个未出现在 $\textit{nums}$ 里的数，使其和最小。应取最小的 $k$ 个缺失正整数。若从 $1$ 起逐个判断是否在集合中，当 $k$ 与 $\textit{nums}$ 中的空洞都很大时循环次数过多。
>
> 排序并加上哨兵 $0$ 与 $2\times 10^9$ 后，相邻值 $(a,b)$ 之间有一段连续缺失。取其中前 $\min(k,b-a-1)$ 个数，其和为等差数列，一次 $O(1)$ 加入答案。
>
> 从左到右消耗 $k$ 即可。
