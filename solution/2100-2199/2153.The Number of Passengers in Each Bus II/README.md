...
> **思考**
>
> 每辆车有容量，乘客按到达顺序上车，余下的等下一辆。需要按时间合并车辆与乘客事件，并维护尚未上车的人数。
>
> 把车辆记为正容量、乘客记为 $-1$，按时间（再按 $\textit{bus\_id}$）排序后做前缀；变量 $\textit{cur\_sum}$ 在「当前积压为负」时累加乘客，遇到车辆则尝试卸下积压。
>
> 对 $\textit{bus\_id}>0$ 的行，用容量减去卸下后的剩余（或整车坐满）得到上车人数。
