...
> **思考**
>
> 三元组值和只能被三个数中的恰好一个整除。下标有序的排列都算，但 $n\le 10^5$ 不能枚举三元组。值域只有 $[1,100]$，可枚举值再按重数组合。
>
> 统计每个值的出现次数，三重循环 $a,b,c\in[1,100]$，若 $a+b+c$ 恰被其中一个整除，则按 $a,b,c$ 是否相等用排列数 $x(x-1)z$ 等公式计入。
>
> 复杂度与值域立方成正比，与 $n$ 无关。
