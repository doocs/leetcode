...
> **思考**
>
> 删去 $n$ 个元素后，前 $n$ 个之和与后 $n$ 个之和的差要最小。等价于选一个分割点，左边保留最小的 $n$ 个，右边保留最大的 $n$ 个。枚举保留集合为组合数，不可行。
>
> 用大根堆维护前缀中最小 $n$ 元的和 $\textit{pre}[i]$，用小根堆维护后缀中最大 $n$ 元的和 $\textit{suf}[i]$。分割点 $i\in[n,2n]$ 处差为 $\textit{pre}[i]-\textit{suf}[i+1]$。
>
> 两边各扫一遍堆，再枚举分割点取最小差。
