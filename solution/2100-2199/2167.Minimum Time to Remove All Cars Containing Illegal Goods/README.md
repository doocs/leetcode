...
> **思考**
>
> 非法车厢可从左端连续删、从右端连续删，或花 $2$ 单独拆掉。需要三种手段的最小总代价。枚举左右切断点若每次重算中间拆卸则为二次以上。
>
> 令 $\textit{pre}[i]$ 为处理前缀 $s[:i]$ 的最小代价：遇到 `'0'` 则等于 $\textit{pre}[i-1]$，遇到 `'1'` 则在「单独拆 $2$」与「整段从左边删掉 $i$」中取小。后缀 $\textit{suf}$ 对称。
>
> 答案为所有分割 $\textit{pre}[i]+\textit{suf}[i]$ 的最小值（前缀含 $s[i-1]$，后缀从 $s[i]$ 起）。
