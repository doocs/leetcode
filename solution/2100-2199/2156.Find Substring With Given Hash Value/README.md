...
> **思考**
>
> 需要长度为 $k$、哈希等于给定值的最靠后子串。正着滑窗时去掉左端相当于除以 $p$ 再取模，模逆元处理麻烦。$n\le 10^5$，需线性滑窗。
>
> 倒序计算 Rabin–Karp：窗口右移（在原串上左移）时，去掉旧的最高位、乘 $p$ 再加新的最低位，只需乘法和取模。
>
> 先算末尾 $k$ 个字符的哈希并记下 $p^{k-1}$，再向左滑，每次命中则更新起点。
