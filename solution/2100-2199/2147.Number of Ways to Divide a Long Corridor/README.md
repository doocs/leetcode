...
> **思考**
>
> 每段恰好两把椅子，屏风插在相邻段之间的植物上。走廊很长，按位置决定是否在「已有两椅」时切开，有重叠子问题。
>
> 状态 $(i,k)$ 表示考虑到位置 $i$、当前段已有 $k$ 把椅子的方案数。遇到椅子则 $k$ 加一，$k>2$ 非法；$k=2$ 时可选择切开（$k$ 归零）或继续。记忆化后状态数为 $O(n)$。
>
> 答案为 $\textit{dfs}(0,0)$。
...
> **思考**
>
> 方法一使用线性额外内存。座椅必须成对，相邻两对之间的植物（含空位）各自独立，乘积即方案数。
>
> 扫一遍记下座椅个数与上一座椅位置，每遇到一对的起点就把与上一对终点之间的间隔乘进答案。座椅数为 $0$ 或奇数则无解。
>
> 空间降为常数。
