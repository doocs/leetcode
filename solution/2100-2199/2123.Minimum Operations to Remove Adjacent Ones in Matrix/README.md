...
> **思考**
>
> 每次操作去掉一个 $1$，目标是消灭所有相邻的 $1$ 对，即覆盖全部「相邻 $1$」边的最小点数。网格是二分图：按 $i+j$ 的奇偶分部，相邻格必分属两侧。
>
> 二分图最小点覆盖等于最大匹配。$1$ 的个数与边数在网格规模下可用匈牙利算法求匹配。
>
> 将奇数格上的 $1$ 连向相邻的 $1$，对每个左侧点寻找增广路，匹配数即为最少操作次数。
