...
> **思考**
>
> 数组按周期 $2n$ 循环：前 $n$ 秒从左删除，后 $n$ 秒从左补回。查询可达 $10^5$，若对每个时刻真的改数组，总时间与空间都不可接受。
>
> $t\bmod 2n$ 后只需判断落在删除阶段还是补回阶段，再把询问下标映射回原数组。删除阶段剩余长度为 $n-t$，下标 $i$ 对应原位置 $i+t$；补回阶段长度为 $t-n$，下标 $i$ 对应 $\textit{nums}[i]$。
>
> 对每个询问按上述规则 $O(1)$ 取值，越界则填 $-1$。
