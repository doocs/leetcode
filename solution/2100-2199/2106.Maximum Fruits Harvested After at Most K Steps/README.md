...
> **思考**
>
> 水果位置有序，最优路线必是某一闭区间 $[l,r]$：先走到较近端再折返覆盖另一端，步数为 $r-l+\min(|\textit{startPos}-l|,|r-\textit{startPos}|)$。若枚举全部端点再求和，区间数为 $O(n^2)$，在 $n\le 10^5$ 时不可行。
>
> 固定右端 $r$ 并将左端右移时，该步数单调不增，因此可用双指针找到所有步数不超过 $k$ 的最大窗口，窗口内水果量和随指针增减维护。
>
> 为此用 $i$、$j$ 扫描 $\textit{fruits}$，累加 $s$，当区间步数超过 $k$ 时右移 $i$ 并减去左侧水果，同时记录 $s$ 的最大值。
