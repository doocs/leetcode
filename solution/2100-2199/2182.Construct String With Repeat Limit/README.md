...
> **思考**
>
> 要用完尽可能多、字典序尽可能大的字符，且同一字符不能连续超过 $\textit{repeatLimit}$ 次。应优先放当前剩余最多的最大字母，放满限额后再插入一个次大字母隔开。
>
> 计数 $26$ 个字母，从 `z` 向 `a` 扫描；指针 $j$ 指向严格更小且仍有剩余的字母。每次先放 $i$ 至多 $\textit{repeatLimit}$ 个，若 $i$ 仍剩则夹一个 $j$。
>
> $j$ 单调左移，总代价与串长及字母表成线性。
