...
> **思考**
>
> 子数组极差之和等于每个子数组最大值减最小值之和。$n\le 1000$，枚举左右端点并在扩展时维护当前区间的最大、最小值，时间为 $O(n^2)$，足以通过。
>
> 无需对每个区间重新扫描：固定左端 $i$ 后向右推进 $j$，只需用新元素更新 $\textit{mi}$、 $\textit{mx}$ 并累加差值。
>
> 因此双重循环即可得到全部极差之和。
...
> **思考**
>
> 方法一在 $n$ 更大时会偏慢。每个元素作为区间最大（或最小）值时，贡献等于它覆盖的子数组个数乘以自身，因而极差之和可改写为「最大贡献减去最小贡献」。
>
> 对每个位置，用单调栈求左侧第一个不小于它的位置与右侧第一个更大的位置，即可在线性时间内统计它作为最大值的次数；将数组取相反数再做一次即得最小值贡献。
>
> 为此我们实现函数 $f$，分别对原数组与取反数组求和，二者之差即为答案。
