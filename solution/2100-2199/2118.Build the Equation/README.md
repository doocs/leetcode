...
> **思考**
>
> 需要把多项式各项按幂次从高到低拼成一行，并处理常数项、一次项与高次项的系数符号。若在应用层循环拼串，顺序与符号分支容易出错。
>
> 用 $\texttt{CASE}$ 按 $\textit{power}$ 生成每一项的文本：零次只写系数，一次写系数与 `X`，其余写 `X^` 与次数；正系数补 `+`。
>
> 再按幂次降序 $\texttt{GROUP\_CONCAT}$，末尾接 `=0` 即得方程。
