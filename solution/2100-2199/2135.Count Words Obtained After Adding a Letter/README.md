...
> **思考**
>
> 每个单词字母互异且为小写，目标串由某个起始串加一个字母并重排得到。若对每个目标枚举删除一个字母再与起始串排序比较，字符串操作偏多，但仍与 $26$ 成正比。
>
> 字母集合可用 $26$ 位掩码表示，起始串的掩码放入哈希集合。目标掩码去掉其中一位后若落在集合中，则该目标合法。
>
> 先把 $\textit{startWords}$ 压成集合，再对每个目标试删除每个字母对应的比特。
