...
> **思考**
>
> 平衡子数组要求从 $\textit{nums1}$ 与 $\textit{nums2}$ 对应位置各选一侧后两侧和相等。子数组个数为 $O(n^2)$，若每个都真的枚举选法则为指数级。$n$ 与元素值都不大，可用和差做状态。
>
> 以右端 $i$ 结尾、两侧和之差为 $j$ 的方案数可从 $i-1$ 转移：第 $i$ 位选 $\textit{nums1}[i]$ 则差加 $a$，选 $\textit{nums2}[i]$ 则差减 $b$；该位也可单独作为新区间的起点。差可能为负，平移 $s_2=\sum\textit{nums2}$。
>
> $f[i][j]$ 按上述转移，答案累加 $f[i][s_2]$。
