...
> **思考**
>
> $k$-递增只约束下标模 $k$ 相同的子序列，这 $k$ 组互不影响，操作数之和即为答案。对一组而言，一次操作可改任意元素，最少修改次数等于长度减去最长非递减子序列长度。
>
> $n\le 10^5$，对每组求 LDS 若用 $O(L^2)$ 动态规划会超时。非递减允许相等，可用 $\texttt{bisect\_right}$ 维护堆叠数组，在 $O(L\log L)$ 内得到最长非递减长度。
>
> 为此对 $i=0,\ldots,k-1$ 分别处理 $\textit{arr}[i::k]$，将「组长度减去 LIS 长度」求和。
