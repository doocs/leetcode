...
> **思考**
>
> 反转整数会丢掉前导零，因而第二次反转能否回到原数，取决于第一次是否丢掉了末尾零。对 $num$ 做两次真实反转即可验证，但数字范围虽不大，仍可直接看数位性质。
>
> 若 $num=0$，两次反转仍为 $0$。否则，第一次反转丢掉末尾零当且仅当 $num$ 含因子 $10$，即个位为 $0$。
>
> 因此当且仅当 $num=0$ 或 $num\bmod 10\neq 0$ 时答案为真。
