...
> **思考**
>
> 墙的每一层是若干砖块拼满宽度，且相邻两层的竖缝不能对齐。层高与砖种有限，先列出一层的所有切分，再决定层与层之间的相容关系。
>
> DFS 枚举砖长组成和为 $\textit{width}$ 的序列；两个序列若除两端外存在相同前缀和则不相容。层间转移是在相容排列上的路径计数，可用 DP：$\textit{dp}[i][j]$ 表示第 $i$ 层用排列 $j$ 的方案数。
>
> 首层每种排列为 $1$，向上累加邻居，最后一层求和取模。
