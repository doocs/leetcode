...
> **思考**
>
> 每条航班同时增加出发与到达机场的流量，而表中只存单向记录。若只按 $\textit{departure\_airport}$ 聚合，则会漏掉作为目的地的流量。
>
> 将 $\textit{Flights}$ 与对调起降后的副本求并，再按出发机场求和，即可把每个机场的出入流量都算到同一列上。
>
> 最后在聚合结果中筛选流量等于全局最大值的机场编号。
