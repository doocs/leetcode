...
> **思考**
>
> 一次引爆会沿「落在爆炸半径内」的关系连锁下去，需要求从某一枚炸弹出发可达的炸弹数最大值。若把每对炸弹都当作无向关系处理，则会漏掉半径不同所造成的单向可达；若每次引爆都重新扫描全部炸弹，重复计算过多。
>
> $n\le 100$，预先对每对炸弹用距离与半径建有向图 $g$ 只需 $O(n^2)$。从图上任意一点出发的连锁引爆即为有向图上的可达点集，用 BFS 即可在 $O(n^2)$ 内求出。
>
> 为此先枚举有序对建边，再以每一枚炸弹为起点做 BFS；一旦某次搜索覆盖全部 $n$ 枚即可提前返回。
