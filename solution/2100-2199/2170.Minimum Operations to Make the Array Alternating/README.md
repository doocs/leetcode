...
> **思考**
>
> 交替数组要求奇偶下标各自恒等且两值不同。一次操作改一个位置，最少修改等于「该奇偶位上不选目标值的个数」。需为偶数位、奇数位各选一个出现最多的值，且二者不能相同。
>
> 分别统计偶、奇下标的最高频与次高频。若二者众数不同，保留它们；若相同，则在「偶位众数 + 奇位次众数」与相反组合中取保留更多的那种。
>
> 函数 $f$ 扫一遍计数并维护前两名，答案为 $n$ 减去所选频次之和。
