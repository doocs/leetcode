...
> **思考**
>
> 相邻交换把串变成回文，保证有解。交换次数等于把配对字符移到对称位置的距离之和。搜索全部交换序列过长，$n\le 2000$ 可用 $O(n^2)$ 贪心。
>
> 固定左端字符 $a$，在右侧找最近的 $a$ 并邻接交换到右端，去掉头尾后变成子问题。若左侧字符在右侧没有配对（奇数次字符），则把它换到正中，代价为到中点的距离。
>
> 每次对外层字母配对不劣于先处理内层，因而该贪心最优。
