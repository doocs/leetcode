...
> **思考**
>
> 每题可选做或不做，做则跳过随后 $\textit{brainpower}$ 题。子问题重叠，若对每条路径单独展开则为指数级。$n\le 10^5$，需线性转移。
>
> 从下标 $i$ 出发的最优分只取决于「做第 $i$ 题后跳到 $i+b+1$」与「跳过到 $i+1$」二者之较大者，可用记忆化搜索。
>
> $\textit{dfs}(i)$ 返回该最优值，答案为 $\textit{dfs}(0)$。
