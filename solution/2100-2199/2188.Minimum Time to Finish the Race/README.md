...
> **思考**
>
> 同一轮胎连续使用时第 $i$ 圈耗时按公比增长，一旦超过「换胎再跑一圈」的代价就不应再连用。连用圈数因此有很小的上界（约 $17$）。总圈数到 $10^3$，需在「何时换胎」上做决策。
>
> 先对每条轮胎预处理连跑 $i$ 圈的最小总耗时 $\textit{cost}[i]$。再令 $f[i]$ 为跑完 $i$ 圈的最小耗时，最后一段连跑 $j$ 圈则 $f[i]=f[i-j]+\textit{cost}[j]+\textit{changeTime}$。
>
> $f[0]=-\textit{changeTime}$ 以抵消第一段并不存在的换胎。
