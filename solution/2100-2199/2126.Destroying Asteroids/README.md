...
> **思考**
>
> 行星可吸收质量不超过自身的小行星并增加质量。若先打大的，可能因当前质量不足而失败，即使较小的本可先被吸收。质量总和与顺序都影响可行性。
>
> 先吸收较小者只会让质量单调变大，不会阻碍之后对更大者的比较，因而按质量升序贪心是安全的。$n\le 10^5$，排序后一次扫描即可。
>
> 将 $\textit{asteroids}$ 排序，若途中出现 $mass<x$ 则失败，否则把 $x$ 加入 $mass$。
