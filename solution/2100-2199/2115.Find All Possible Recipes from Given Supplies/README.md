...
> **思考**
>
> 一道菜能否做成取决于其原料是否都能从初始供应或已做成的菜中获得，依赖关系构成有向图。若对每道菜反复检查原料，可能因相互等待而重复扫描，也难以处理链式解锁。
>
> 将原料指向依赖它的菜，入度为该菜所需原料种数。初始供应相当于入度为零的起点，拓扑序中入度降为零的菜即可做成，并可作为新的供应继续解锁。
>
> 为此建图后把 $\textit{supplies}$ 当作队列做拓扑排序，入度归零的菜加入答案。
