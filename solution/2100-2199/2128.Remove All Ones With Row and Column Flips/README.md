...
> **思考**
>
> 行翻转与列翻转各做一次或零次，最终要变成全零。两行若能同时被同一组列翻转清零，则它们要么完全相同，要么处处相反。枚举 $2^{m+n}$ 种翻转方案在规模较大时不可行。
>
> 把每行按与第一行首元是否相同规范化到以 $0$ 开头的等价类后，合法矩阵至多对应一种行模式。哈希表中若出现第二种模式，则无法用统一的列翻转消去。
>
> 遍历各行，与 $\textit{grid}[0][0]$ 不同则整行取反，将元组放入集合，集合大小为 $1$ 当且仅当可行。
