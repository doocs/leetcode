...
> **思考**
>
> 要在给定下标处插入空格且保持原相对顺序。若每次对字符串做插入，后面字符反复移动，总代价可达二次。
>
> $\textit{spaces}$ 已递增，与 $s$ 的下标同步扫描即可在线性时间内决定每个位置是否先写空格。
>
> 用指针 $j$ 指向下一个空格位置，遍历 $s$ 时若当前下标等于 $\textit{spaces}[j]$ 则先追加空格，再追加字符，最后拼接。
