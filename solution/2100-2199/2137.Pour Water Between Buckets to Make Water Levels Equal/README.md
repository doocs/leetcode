...
> **思考**
>
> 倒水有损耗，最终公共水位越高越难达到，具有单调性。水位是实数，无法枚举。直接模拟多次倾倒也难以控制精度。
>
> 对候选水位 $v$，多出的水汇总为 $a$，不足的一侧需计入损耗后的需求 $b$；若 $a\ge b$ 则可达到 $v$。在 $[0,\max\textit{buckets}]$ 上对浮点二分。
>
> $\texttt{check}$ 扫描全部桶，二分直到区间宽度小于 $10^{-5}$。
