...
> **思考**
>
> 等值子数组允许删除至多 $k$ 个其它元素。窗口内「非众数」个数超过 $k$ 时即不合法。用哈希表维护频次并记录窗口内最大频次 $mx$，当 $r-l+1-mx>k$ 时右移左端；最终 $mx$ 即为最长等值长度。
...
> **思考**
>
> 方法一在同一窗口内同时维护所有值的频次。若先按值收集下标，则每个值上的双指针只统计「该值下标之间夹杂的其它位置」，语义更直接，复杂度同为线性。
