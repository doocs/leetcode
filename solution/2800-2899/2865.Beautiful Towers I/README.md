...
> **思考**
>
> $n\le 10^3$，可枚举峰顶，再向两侧按上限递减地取高度并求和。每次扩展保持「不超过峰顶且不高于相邻外侧」的单峰约束。
...
> **思考**
>
> 方法一每次从峰顶线性扩展，总时间 $O(n^2)$，在更大 $n$ 下不足。单调栈求出每座塔左侧（右侧）最近的更矮位置，前缀美丽值 $f[i]$ 便可在常数时间内由 $f[j]$ 递推；左右各算一遍后在峰顶处拼接。
