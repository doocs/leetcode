...
> **思考**
>
> 合法路径恰好含一个质数结点。先筛出质数，再把两端都非质数的边用并查集合并，得到不含质数的连通块。枚举质数 $i$，相邻各块大小为 $c_1,c_2,\ldots$，则以 $i$ 为一端的路径有 $\sum c$，以 $i$ 为中间点的路径有两两之积。
...
> **思考**
>
> 并查集需要预先建块。改从树根 DFS，每个子树返回「不含质数 / 恰含一个质数」的路径条数，在当前结点按是否为质数与子树贡献相乘累加，一次遍历即可。
