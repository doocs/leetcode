...
> **思考**
>
> 每次拆分要求切出的两段要么长度为 $1$，要么元素和至少为 $m$。区间状态有 $O(n^2)$ 个，可以记忆化。预处理前缀和后，$dfs(i,j)$ 枚举切点 $k$，检查左右段是否合法并递归两端。
...
> **思考**
>
> 记忆化仍要枚举切点。元素非负，反复拆分后总会留下某段长度为 $2$ 的子数组，且更长段的和只会更大。因此当 $n>2$ 时，存在相邻之和不小于 $m$ 的一对即充分必要；$n\le 2$ 则无需拆分。
