...
> **思考**
>
> $low$ 与 $high$ 以数字串给出，直接枚举区间内每个整数并检查相邻数位之差是否为 $1$，在位数很大时不可行。区间计数可改为 $F(high)-F(low-1)$。步进数只约束相邻数位，与具体数值无关，适合数位 DP。按位置 $pos$、前一位 $pre$、前导零 $lead$ 与上界 $limit$ 记忆化：前导零不参与相邻比较，非前导零仅当尚无前一位或 $|i-pre|=1$ 时转移。
