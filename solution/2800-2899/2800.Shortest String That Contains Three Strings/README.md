...
> **思考**
>
> 三个字符串长度均不超过 $100$，排列只有 $6$ 种，因而可以穷尽所有拼接顺序。答案须同时最短且字典序最小，包含关系与首尾重叠都会改变候选。若某一串已是另一串的子串，则不必再单独拼接；否则最优超串对应某一种排列下的依次合并。函数 $f$ 先处理包含，再从大到小检查后缀与前缀的重合，并按长度与字典序维护最优解。
...
> **思考**
>
> 方法一在合并时对重叠长度做线性扫描，单次合并达到 $O(n^2)$。将 $t\#s$ 做 KMP 前缀函数后，最末一项即为 $t$ 的前缀与 $s$ 的后缀的最长重合，合并可在线性时间内完成，排列枚举不必改动。
