...
> **思考**
>
> 每次预订给闭区间 $[\textit{first},\textit{last}]$ 加上同一增量。若对区间内每个航班逐格修改，总时间与区间长度之和成正比，最坏达 $O(nm)$。
>
> 差分数组在左端点 $+\textit{seats}$、在 $\textit{last}+1$ 处 $-\textit{seats}$，把一次区间更新降为两次单点操作；最后对差分数组求前缀和，即还原每个航班的座位数。
...
> **思考**
>
> 方法一的差分数组依赖最后一次整体前缀和，适合离线出答案。树状数组把「单点加、前缀查」均做到 $O(\log n)$：对 $\textit{first}$ 加、$\textit{last}+1$ 减后，查询位置 $i$ 的前缀和即航班 $i$ 的座位数。本题无需在线查询，渐近略慢，作为区间更新的另一种实现。
