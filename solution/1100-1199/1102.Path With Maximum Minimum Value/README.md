...
> **思考**
>
> 路径得分是路径上的最小格子值，要求从左上到右下最大化该值。枚举全部路径不可行。
>
> 将格子按值从大到小加入图中，并与已经加入的四邻用并查集合并。当前加入的值即「目前允许的最低分」；一旦起点与终点落入同一连通块，更小的格子尚未出现，无法再提高路径最小值，该值即为答案。
...
> **思考**
>
> 方法一用数组直接做路径压缩。方法二改为带按大小合并的并查集类，合并与查询接口更规整，最坏链长更短。仍按格子值从大到小加入，并在起点与终点连通时停止。
