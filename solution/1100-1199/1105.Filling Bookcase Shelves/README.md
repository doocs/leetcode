...
> **思考**
>
> 每本书可单独开一层，或与前面若干本同层，决策随层划分指数增长。$n\le 1000$，可用 $O(n^2)$ 动态规划。
>
> 令 $f[i]$ 为摆放前 $i$ 本书的最小总高度。最后一层以 $books[i-1]$ 结尾，自后向前扩张同层书籍并累加宽度，一旦超过 $shelfWidth$ 即可停止；层高取这些书的最大高度，再接上 $f[j-1]$。从后往前枚举同时保证宽度单调增加。
