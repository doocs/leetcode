...
> **思考**
>
> 中序叶序列固定，树形由切分位置决定。枚举所有切分的递归若不记忆会重复计算同一区间。
>
> $dfs(i,j)$ 返回区间 $[i,j]$ 的最小非叶代价与叶最大值；枚举切点 $k$，左右代价加上两侧叶最大值之积。区间数 $O(n^2)$、切点 $O(n)$，记忆化后为 $O(n^3)$。
...
> **思考**
>
> 方法一的递归栈与哈希缓存可改成区间 DP。先预处理 $g[i][j]$ 为区间最大叶，再按区间长度递推 $f[i][j]$，转移式与记忆化相同，避免递归开销。
