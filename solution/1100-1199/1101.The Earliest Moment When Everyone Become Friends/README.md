...
> **思考**
>
> 问题等价于求最早时刻使 $n$ 人落在同一连通块。日志按时间乱序给出，因此须先按时间戳排序再依次处理。
>
> 每次结交若两人尚属不同集合，则用并查集合并并把连通块计数减一；查找与合并均可摊到近常数。计数减至 $1$ 时全体已互通，返回该条日志的时间；若扫完仍多于一块则返回 $-1$。
...
> **思考**
>
> 方法一将路径压缩写在主循环里，未按集合大小合并。方法二抽出带路径压缩与按大小合并的 `UnionFind`，`union` 仅在真正合并时返回真，主过程只维护连通块个数。判定「全体成为朋友」的时机与方法一相同，树高更受控。
