...
> **思考**
>
> 每次替换须使用 $arr2$ 中的值，求最少替换次数。对 $arr1$ 每个位置枚举替换值代价过高。令 $f[i]$ 为前缀变为严格递增且位置 $i$ 不替换的最少次数；若需把 $i$ 前连续 $k$ 个位置换成 $arr2$ 中恰小于 $arr[i]$ 的 $k$ 个数，则由 $f[i-k-1]+k$ 转移。$arr2$ 排序去重后可用二分定位，首尾哨兵保证最后一位不替换。
