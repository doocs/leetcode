...
> **思考**
>
> $n$ 个 $k$ 面骰子组成 $target$，枚举每个骰子朝向为 $k^n$。令 $f[i][j]$ 为 $i$ 个骰子组成 $j$ 的方案数，第 $i$ 个骰子点数 $h$ 时由 $f[i-1][j-h]$ 转移。空骰子组成 $0$ 为 $1$ 种，取模防止溢出。
...
> **思考**
>
> 方法一的 $f[i]$ 只依赖 $f[i-1]$。用两个一维数组滚动，空间从 $O(n\times target)$ 降为 $O(target)$，转移不变。
