...
> **思考**
>
> 段式回文要求左右段文字相等。为使段数最多，应尽量切出最短的一对相等前后缀：更长的一对总可以再拆。双指针从两端增长长度 $k$，一旦 `text[i:i+k]` 与右端对应串相等就切下并各进 $2$，中间剩一段则再加 $1$。
...
> **思考**
>
> 方法一每次用切片比较，最坏 $O(n^2)$。预计算前缀哈希后，等长子串的相等性可 $O(1)$ 判定，贪心切分策略不变，比较不再扫字符。
