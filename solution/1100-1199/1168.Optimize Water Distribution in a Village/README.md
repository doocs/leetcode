...
> **思考**
>
> 每座房屋可自建井或经管道连到已有水源。引入虚点 $0$，建井视为 $0$ 到该房屋的边，问题化为 $n+1$ 个点的最小生成树。边按费用排序后用并查集合并，连通块从 $n$ 减到 $0$（虚点与 $n$ 座房屋已全连通）即得最小费用。
...
> **思考**
>
> 方法一将并查集内联。方法二抽出按大小合并的 `UnionFind`，`union` 回报是否真正合并，主循环只累加费用并计数。构图与 Kruskal 顺序不变。
