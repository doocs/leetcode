...
> **思考**
>
> 排序关键字由 $arr2$ 中的相对次序决定，未出现的元素再按自身大小排在后面。哈希表记下 $arr2$ 的下标，自定义比较键为「在表中则用下标，否则用 $1000+x$」，一次排序即可同时处理两类元素。$arr2$ 中的值互不相同，下标作为次序是稳定的。
...
> **思考**
>
> 方法一的比较排序为 $O(n\log n)$。值域不大时，先统计 $arr1$ 频次，再按 $arr2$ 顺序依次输出，最后按值扫描剩余元素，可在线性时间内完成。
