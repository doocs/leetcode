...
> **思考**
>
> `first`、`second`、`third` 可能被任意线程以任意次序调用，必须强制打印顺序。两把锁（或计数为 $0$ 的许可）初始即被占用，分别挡住 `second` 与 `third`：`first` 打印后释放第二把，`second` 打印后释放第三把，形成单向依赖。
...
> **思考**
>
> 方法一用互斥锁模拟「先到先执行」的许可。方法二改为三个信号量，初始计数分别为 $1,0,0$，语义与「先 `first` 再 `second` 再 `third`」一致，接口更贴近计数许可。
