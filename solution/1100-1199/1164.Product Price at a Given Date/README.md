...
> **思考**
>
> 每个产品取 $\le 2019$-$08$-$16$ 的最后一次改价，若无改价则默认为 $10$。子查询按产品聚合该日期前的最大 `change_date`，再连回得到价格；全部产品的去重表左连该结果，空价格补 $10$。
...
> **思考**
>
> 方法一用聚合子查询定位最后一次改价。方法二先左连日期范围内的改价，再按产品对 `change_date` 降序 `RANK`，取 $rk=1$；无改价行的窗口名次仍为 $1$，`IFNULL` 同样补 $10$。
