...
> **思考**
>
> 若枚举每个长度为 $k$ 的子串再扫描一遍判断字符是否互异，时间约为 $O(nk)$。在 $n,k\le 10^4$ 时尚可接受，但相邻子串仅相差一头一尾两个字符，重复扫描没有必要。
>
> 为此维护长度为 $k$ 的窗口，并用哈希表记录窗内各字符频次：右端字符入窗、左端字符出窗，频次降为零则删键。表中键的个数恰为 $k$，当且仅当窗口内每个字符出现一次，据此累加答案。
