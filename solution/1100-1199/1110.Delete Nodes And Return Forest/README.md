...
> **思考**
>
> 删除若干结点后，原树裂成森林；新根是「自身未删，且父结点被删（或本为原根）」的结点。后序 DFS 先处理完左右子树，再决定当前结点是否删除：若删除，则仍然存活的孩子各自成为新树的根。
>
> 待删集合放入哈希表，判断为期望常数时间。根若未被删除，最后再加入答案。
...
> **思考**
>
> 方法一依赖递归栈完成后序删除。BFS 自顶向下遍历：若孩子待删则切断指针，若当前结点待删则把它尚未切断的孩子记为新根。层序即可完成切边与收集，无需函数返回值，原根未删时最后加入。
