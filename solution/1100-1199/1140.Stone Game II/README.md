...
> **思考**
>
> 双方轮流从剩余前缀取 $1..2M$ 堆，状态为起点下标与当前 $M$。直接搜索重复子问题很多。前缀和给出「从 $i$ 到末尾的石子总和」，当前人得到的是该总和减去对手在新状态的最优。
>
> 若 $2M$ 已覆盖剩余堆，则一次取完；否则枚举取堆数并记忆化。爱丽丝从 $(0,1)$ 出发。
...
> **思考**
>
> 方法一在能取完时直接返回后缀和。方法二统一枚举取堆数，用「后缀和减去对手最小值」表达同一最优，边界改为下标越界返回 $0$。状态含义不变，写法更整齐。
