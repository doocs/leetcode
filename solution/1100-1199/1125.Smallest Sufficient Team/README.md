...
> **思考**
>
> 技能种数 $m\le 16$，子集可用 $2^m$ 个状态表示；对人的组合枚举则达 $2^n$。把每人掌握的技能压成掩码 $p[j]$，令 $f[i]$ 为覆盖技能集合 $i$ 的最少人数。
>
> 对每个已达状态 $i$ 尝试加入第 $j$ 人，转移到 $i\mid p[j]$。另用 $g,h$ 记下最后加入的人与上一状态，从全集沿指针回溯即可还原下标列表。
