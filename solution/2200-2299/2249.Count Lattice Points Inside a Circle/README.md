...
> **思考**
>
> 统计至少被一个圆覆盖的整点。圆至多 $200$ 个，坐标与半径不超过 $100$，整点范围很小。对每个圆枚举其外接正方形再判重，需要去重集合；对边界盒内每个整点测试是否落入任一圆，实现更直接。
>
> 取所有 $x+r$、$y+r$ 的最大值作为枚举上界，对每个 $(i,j)$ 检查是否存在圆满足距离平方不超过 $r^2$，命中即计数并跳出。$O(XY n)$ 在本题范围内可通过。
