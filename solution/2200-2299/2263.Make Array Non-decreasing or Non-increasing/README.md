...
> **思考**
>
> 每次可将一个元素加一或减一，要使数组变成非递减或非递增，代价之和最小。$n$ 与值域均为 $10^3$，可以把「第 $i$ 位改成 $j$」做成 DP。非递增只要把数组反转后再求非递减。
>
> 令 $f[i][j]$ 为前 $i$ 个位置、第 $i$ 个改成 $j$ 的最小代价。$j$ 必须不小于前一位的终值，因此 $f[i][j] = \min_{k\le j} f[i-1][k] + |j-nums[i-1]|$。内层最小值可以随 $j$ 递增维护。对原数组与反转各算一次取较小。
