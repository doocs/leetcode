...
> **思考**
>
> 从 $i$ 只能跳到「下一个 $\ge nums[i]$」或「下一个 $< nums[i]$」的位置，求到达终点的最小花费。$n$ 较大时不能对每个 $i$ 向右扫描。这两个后继正是单调栈能在线性时间内求出的。
>
> 从右往左分别维护严格递增、非严格递减的栈，得到邻接表 $g[i]$。再按 $i$ 从小到大松弛 $f[j] = \min(f[j], f[i]+costs[j])$。边数 $O(n)$，最短路即一遍 DP。
