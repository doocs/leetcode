...
> **思考**
>
> 峰谷定义依赖「最近的不相等邻居」，连续相等段应视为同一高度。$n \le 100$，即使对每个 $i$ 向两侧找最近不同值也能通过，但会重复扫描平台。
>
> 从左到右走时，上一个已经结算过的不同高度位置可以用指针 $j$ 记住。若 $nums[i] = nums[i+1]$，说明仍在平台内部，跳过；否则用 $nums[j]$ 与 $nums[i+1]$ 判断 $i$ 是否严格高于或低于两侧，再把 $j$ 更新为 $i$。一遍线性扫描即可数出峰谷。
