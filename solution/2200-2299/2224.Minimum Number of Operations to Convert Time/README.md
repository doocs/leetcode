...
> **思考**
>
> 每次可把当前时间增加 $1$、$5$、$15$ 或 $60$ 分钟，从 $current$ 加到 $correct$，次数尽量少。时间差至多不到一天，完全背包式搜索可以，但四种面额能凑出任意正整数，且大面额总是不亏。
>
> 先把两个时刻换成从零点起的分钟数，得到正差值 $d$。再按 $60$、$15$、$5$、$1$ 依次取尽可能多的步，余数交给更小面额。这与用最少硬币找零在面额整除链上的贪心相同。
