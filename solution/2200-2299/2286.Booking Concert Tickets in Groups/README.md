...
> **思考**
>
> $\textit{gather}$ 要在某一行找到连续 $k$ 个空位且行号尽量小；$\textit{scatter}$ 只要从前若干行凑齐 $k$ 个座位。行数 $5\times 10^4$，每行座位数达 $10^9$，不能按座位开数组，按行线性扫描也会超时。
>
> 线段树下标从 $1$ 的行，节点维护区间剩余座位和 $s$ 与单行剩余最大值 $mx$。$\textit{gather}$ 在 $[1,\textit{maxRow}]$ 上找最左满足 $mx\ge k$ 的行，再把该行剩余改为 $m'-k$。$\textit{scatter}$ 先查区间和是否够，再从前往后逐行扣减。单次对数。
