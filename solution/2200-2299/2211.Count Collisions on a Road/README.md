...
> **思考**
>
> 相向而行记 $2$ 次碰撞，与静止车相撞记 $1$ 次，最终所有会相遇的车都停下。$n \le 10^5$，不宜逐步模拟每对碰撞。
>
> 前缀中一直向左开的车左边没有障碍，后缀中一直向右开的车右边没有障碍，这两段永远不会撞上。去掉它们之后，中间剩下的每个非静止车最终都会停下来，各贡献 $1$。
>
> 因此先剥去前缀 $\texttt{L}$ 与后缀 $\texttt{R}$，再统计剩余串中非 $\texttt{S}$ 的个数。
