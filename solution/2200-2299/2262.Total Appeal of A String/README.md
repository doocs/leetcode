...
> **思考**
>
> 子串的引力是其中不同字符的个数，要求和。$n \le 10^5$，枚举全部子串是平方级。考虑以 $s[i]$ 结尾的子串：新字符 $c$ 会让「上次 $c$ 出现之后」的那些子串各多一个不同字符。
>
> 记下每个字母上次出现位置 $pos[c]$（初始 $-1$）。走到 $i$ 时，增量 $t \mathrel{+}= i-pos[c]$，再把 $t$ 累入答案。$t$ 始终等于以当前位置结尾的全部子串引力之和。
