...
> **思考**
>
> 连续相同按键可分成若干长度不超过该键字母数的段，求解码方案数。$n \le 10^5$，按整串搜索不可行。不同字符的段互相独立，方案数相乘。一段长度为 $m$ 的相同按键，等价于爬楼梯：普通键一步 $1..3$，键 $7$、$9$ 一步 $1..4$。
>
> 预先打表 $f[i]$、$g[i]$ 到 $10^5$，再按 $\textit{groupby}$ 分段相乘取模。
