...
> **思考**
>
> 要分别给出只在 $nums1$ 中出现、以及只在 $nums2$ 中出现的不同元素。若对每个值都去另一数组线性查找，$n \le 10^3$ 可以过，但重复元素会被反复判断。
>
> 先把两个数组转成集合，去重后做差：$s_1 \setminus s_2$ 与 $s_2 \setminus s_1$。哈希集合上的成员判断是期望常数时间，一次遍历即可。
