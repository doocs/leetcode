...
> **思考**
>
> 路径只能先水平再竖直（或相反），拐角尾零取决于路径上 $2$ 与 $5$ 个数的较小值。$mn \le 10^5$，枚举路径上的格子会超时。拐角一旦固定，路径就是「该行的一段前缀或后缀」加上「该列的一段前缀或后缀」。
>
> 预处理每行、每列的 $2$、$5$ 前缀和后，枚举拐点 $(i,j)$，用前缀和拼出四种「左/右 + 上/下」组合（拐点本身只计一次），取 $\min(2,5)$ 的最大值。
