...
> **思考**
>
> 子串的方差是出现最多与最少的字母次数之差。$n \le 10^4$，枚举全部子串再统计是平方级偏紧。字母只有 $26$ 个，方差只由一对字母决定，可枚举 $(a,b)$ 后在串上做「$a$ 为 $+1$、$b$ 为 $-1$」的最大子段和，且子段必须同时含 $a$ 和 $b$。
>
> $f[0]$ 是以当前为尾、只含 $a$ 的连续段长度；$f[1]$ 是已同时出现过 $a$、$b$ 的最大差值。遇 $a$ 两者加一，遇 $b$ 则 $f[1]$ 取 $\max(f[1],f[0])-1$ 并清空 $f[0]$。$f[1]$ 初值为负无穷，保证尚未见到 $b$ 时不更新答案。
