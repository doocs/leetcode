...
> **思考**
>
> 矩形从原点出发，问每个点被多少矩形包含。矩形与询问均达 $5\times 10^4$，高只有 $100$，宽可达 $10^9$。对每个点扫描全部矩形不可行。高度很小，可按高度分组。
>
> 把矩形宽度按高度放入列表并排序。询问点 $(x,y)$ 能被高度 $h \ge y$ 且宽度 $\ge x$ 的矩形盖住，于是对 $h$ 从 $y$ 到 $100$ 做一次下界查找。单点 $O(H\log n)$。
