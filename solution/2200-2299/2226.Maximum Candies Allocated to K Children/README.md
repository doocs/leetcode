...
> **思考**
>
> 每名儿童得到的糖果数必须相同，且只能来自同一堆的连续分割。$k$ 可达 $10^{12}$，不能按人模拟。若每人能分到 $v$ 颗，则也能分到任意更小的正整数，答案关于 $v$ 单调。
>
> 在 $[0, \max(\textit{candies})]$ 上二分 $v$：一堆大小为 $x$ 的糖可切出 $\lfloor x/v \rfloor$ 份，总和不少于 $k$ 则可行。注意 $v=0$ 表示无法分配，二分下界取 $0$ 即可覆盖。
