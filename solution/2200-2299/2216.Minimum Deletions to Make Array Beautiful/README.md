...
> **思考**
>
> 美丽数组要求长度为偶数，且每个偶数下标与其后一位不相等。$n \le 10^5$，不能搜索删哪些位置。从左到右配对时，一旦 $nums[i]=nums[i+1]$，这一对无法保留，必须删掉其中一个。
>
> 贪心删掉当前这个 $nums[i]$，让后面的元素前移再试，等价于指针只前进一位并计一次删除；若二者不同则这一对合法，指针跳过两位。扫完后若剩余长度仍为奇数，再删一个尾元素即可。
...
> **思考**
>
> 方法一按相邻两项决定是否删除，指针一次走一或两步。也可以先把与 $nums[i]$ 相同的连续段压掉：段内多出的元素都必须删，只留下一个作为偶数位，再与段后第一个不同值配成一对。
>
> 内层循环跳过相等元素并累计删除，然后指针再跨过配对的后一项。收尾同样按剩余奇偶补删一次。时间仍是 $O(n)$，只是按相等段推进。
