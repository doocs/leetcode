...
> **思考**
>
> 地毯长度为 $\textit{carpetLen}$，砖块是若干互不相交的区间。地毯左端若停在某段中间，覆盖不会优于把它对齐到某一段的左端。因此只需枚举「地毯左端贴在 $tiles[i]$ 的左端」。
>
> 砖块先按左端排序。指针 $j$ 维护右端仍被整段盖住的最远砖块，并用 $s$ 记下这些整段的白砖数。若下一段被部分盖住，再补上 $li+\textit{carpetLen}-tiles[j][0]$。$j$ 只前进，总时间 $O(n\log n)$。
