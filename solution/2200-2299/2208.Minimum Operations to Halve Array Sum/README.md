...
> **思考**
>
> 每次把一个数减半，要使总和降到不超过原来的一半，操作次数尽量少。$n \le 10^5$，不能搜索操作序列。减半对总和的贡献等于被选中的那个数的一半，因此每一步都应选当前最大的数。
>
> 用大根堆维护全部数值，预先记下需要削减的总量 $s = \mathrm{sum}(nums)/2$。反复取出堆顶 $t$，从 $s$ 中减去 $t/2$，再把 $t/2$ 放回堆中，直到 $s \le 0$。操作次数即为答案。
