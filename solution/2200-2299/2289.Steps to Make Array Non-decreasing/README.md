...
> **思考**
>
> 每一轮同时删掉所有「左边相邻且严格更大」的元素，问多少轮后数组非递减。$n \le 10^5$，不能真的一轮轮模拟。一个元素被删掉的时刻，取决于它右边连续上升段被左侧更大值「吃掉」需要的步数。
>
> 从右往左维护单调栈：栈中是尚未被当前值消灭的下标。弹出时，$dp[i] = \max(dp[i]+1, dp[\textit{stk.top}])$，表示 $i$ 要多一轮才能轮到更远的那个。答案是 $dp$ 的最大值。
