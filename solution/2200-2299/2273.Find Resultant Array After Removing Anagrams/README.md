...
> **思考**
>
> 反复删除与前一个词互为字母异位词的词，直到相邻都不是。词数与词长都很小，一次从左到右保留「与答案末尾不是异位词」的词即可，因为删除不影响更早已经合法的相邻对。
>
> 先放入 $words[0]$，其后仅当 $\textit{check}(s,t)$ 判定不是异位词时追加。$\textit{check}$ 用计数数组比较，长度不同则一定保留。
