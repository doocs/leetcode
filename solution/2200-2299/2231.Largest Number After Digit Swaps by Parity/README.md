...
> **思考**
>
> 只允许交换奇偶性相同的数位，要使十进制值最大。$num \le 10^9$，数位很少，但若搜索所有合法排列仍显多余。同一奇偶类内部可以任意重排，因此每位应换成该类中尚未使用的最大数码。
>
> 先统计 $0$ 到 $9$ 的出现次数。从左到右看原数每一位的奇偶，从 $9$ 或 $8$ 起向下找仍有剩余的同奇偶数码写入答案。按位重构即可。
