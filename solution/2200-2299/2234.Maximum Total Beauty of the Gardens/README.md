...
> **思考**
>
> 每种花园要么补到 $\textit{target}$ 成为完善花园，要么把不完善花园的最小值尽量抬高。$n \le 10^5$，新花数量达 $10^{10}$，不能模拟每一朵。完善花园的个数确定后，剩余花朵应全部用于垫高一段最小前缀。
>
> 先排序并做前缀和。从已有的完善花园数起，枚举最终完善个数 $x$，扣掉补齐最贵的那几座所需的花。剩余花在未完善前缀上二分：把前 $l+1$ 座都抬到 $flowers[l]$ 的代价为 $flowers[l]\cdot(l+1)-s[l+1]$，再把多出的花均分，但不超过 $\textit{target}-1$。$x\cdot\textit{full}+y\cdot\textit{partial}$ 取最大。
