...
> **思考**
>
> 只向右或向下走，路径括号序列必须合法。$m,n \le 100$，路径长度固定为 $m+n-1$。长度为奇数、起点是右括号或终点是左括号时必不合法。其余情况若搜索全部路径则指数过多，但合法前缀的平衡 $k$ 有上界：后面剩下的格子数必须还能把 $k$ 降到 $0$。
>
> 记忆化 $\textit{dfs}(i,j,k)$：先按当前格更新 $k$，若 $k<0$ 或 $k$ 大于剩余步数则剪枝；终点要求 $k=0$。状态数 $O(mn(m+n))$。
