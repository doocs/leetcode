...
> **思考**
>
> $k$ 美是长度为 $k$ 且能整除原数的子串个数。$num \le 10^9$，十进制位数很少，枚举每个长为 $k$ 的子串转成整数即可，注意除数不能为 $0$。
>
> 把 $num$ 转成字符串，对每个起点 $i$ 取 $s[i:i+k]$。
...
> **思考**
>
> 方法一每次切片再转整数，有字符串开销。窗口在十进制上滑动时，去掉最低位、在最高位加上新数字，即可 $O(1)$ 更新窗口值。
>
> 先从低位取出 $k$ 位得到 $x$，再反复 $x \leftarrow x//10$ 并在高位补上新数位，同时判断整除。额外空间 $O(1)$。
