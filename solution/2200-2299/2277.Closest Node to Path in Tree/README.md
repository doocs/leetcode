...
> **思考**
>
> 树上询问路径 $start\to end$ 上离 $node$ 最近的点。$n$ 与询问均为 $10^3$，对每次询问 BFS 路径再比距离可以过，但会重复求路径。树上任一点到一条路径的最近点，是 $node$ 与路径两端、以及路径两端彼此的三个 LCA 中深度最大者。
>
> 预处理深度与倍增父亲后，每次询问算三个 LCA 并取最深的那个。题面未附实现，按 LCA 或对每次询问 BFS 均可。
