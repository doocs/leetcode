...
> **思考**
>
> 题目要求所有与某个 $key$ 下标距离不超过 $k$ 的位置。若对每个 $i$ 都向左右各看 $k$ 格，或对每个 $i$ 再扫一遍全数组判断是否存在合格的 $j$，时间均为 $O(n^2)$。$n \le 10^3$，该复杂度可以通过。
>
> 真正需要判断的只是「区间 $[i-k, i+k]$ 内是否出现过 $key$」，不必列出全部配对。因此固定 $i$ 后枚举 $j$，一旦发现 $|i-j| \le k$ 且 $nums[j] = key$ 即可收入 $i$ 并结束内层循环。
...
> **思考**
>
> 方法一对每个 $i$ 都重新扫描，没有利用 $key$ 的出现位置可以预先收集这一事实。$n$ 虽小，仍可将「区间内是否存在 $key$」从线性查询降为对数查询。
>
> 先扫一遍数组，把所有等于 $key$ 的下标按升序记入 $idx$。对每个 $i$，在有序的 $idx$ 上二分查找是否存在落在 $[i-k, i+k]$ 内的下标：左端用 $\textit{bisect\_left}$，右端用 $\textit{bisect\_right}$。若左右指针仍满足 $l \le r$，则 $i$ 合法。时间降为 $O(n \log n)$。
...
> **思考**
>
> 方法二已经避免重复扫描 $key$，但仍要维护下标数组，并对每个 $i$ 做两次二分。注意到 $i$ 从左到右递增时，满足 $j \ge i-k$ 且 $nums[j] = key$ 的最左下标也只会右移。
>
> 为此用指针 $j$ 单调前进：当 $j < i-k$，或当前格不是 $key$ 时将 $j$ 加一。若前进后仍有 $j \le i+k$，则 $i$ 是 K 近邻下标。一遍扫描即可，额外空间 $O(1)$。
