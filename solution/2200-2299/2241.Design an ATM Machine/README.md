...
> **思考**
>
> 面额只有五种，存取调用合计 $5000$ 次，金额可达 $10^9$。按张模拟每一种组合不可行，题目要求尽量用大面额，这正是贪心取整。
>
> 用长度为 $5$ 的数组记下库存。存款按位累加。取款从 $500$ 到 $20$ 依次取 $\min(\lfloor \textit{amount}/d_i \rfloor, \textit{cnt}[i])$ 张；若最后仍有余额则整笔失败且不改库存，否则再扣减。面额数是常数，单次 $O(1)$。
