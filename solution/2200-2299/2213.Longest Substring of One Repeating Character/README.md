...
> **思考**
>
> 每次单点改写后都要回答整串中「同一字符连续段」的最大长度。$|s|$ 与询问次数均为 $10^5$，每次重扫或重建游程都会超时。单点修改只影响邻近游程，需要能在对数时间内合并左右信息。
>
> 线段树每个节点维护区间左端连续长 $lmx$、右端连续长 $rmx$、以及区间内最长连续长 $mx$。向上合并时，若左右子区间在中缝处字符相同，则可以拼出跨越中点的新段，并在整段被同字符填满时延长前缀或后缀。
>
> 修改落到叶子后沿路径 $\textit{pushup}$，再查询根节点的 $mx$。单次 $O(\log n)$，总时间 $O((n+q)\log n)$。
