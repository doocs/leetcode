...
> **思考**
>
> 要列出从未输过、以及恰好输过一场的选手，并各自升序。比赛场次达 $10^5$，选手编号也达 $10^5$，不宜开稠密数组再扫完全部编号。只关心出现过的人，输场次数是唯一需要累计的量。
>
> 哈希表 $\textit{cnt}$ 记下每人的败场：胜者若尚未出现则置 $0$，败者加一。再按编号排序后，把败场为 $0$ 或 $1$ 的人分别放入两个列表。排序决定时间 $O(n\log n)$。
