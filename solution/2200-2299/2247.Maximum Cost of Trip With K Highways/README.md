...
> **思考**
>
> 恰好走 $k$ 条边、城市不重复，求最大边权和。$k \ge n$ 时边数已超过点数减一，必不可能。$n \le 15$，适合用访问集合做状态。
>
> 令 $f[S][j]$ 为访问集合 $S$、停在 $j$ 的最大费用。单点出发为 $0$。枚举 $S$ 与终点 $j$，从 $j$ 的邻居 $h$ 转移：$f[S][j] \leftarrow f[S\setminus\{j\}][h]+\textit{cost}(h,j)$。当 $|S|=k+1$ 时更新答案。
