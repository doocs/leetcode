...
> **思考**
>
> 统计满足「能被 $p$ 整除的元素个数 $\le k$」的不同子数组个数。$n \le 200$，枚举所有子数组可行，难点在去重。把子数组拷进集合需要比较整段，哈希可以把一段压成常数。
>
> 枚举左端 $i$，向右扩展 $j$ 并累计整除个数，超过 $k$ 则停止。同时用双模滚动哈希更新，把 $(h_1,h_2)$ 放入集合。集合大小即为答案。
...
> **思考**
>
> 方法一用双哈希避免把整段放进集合。$n$ 很小，也可以把子数组的元素直接拼成字符串作为键，实现更短，只是常数与内存略大。枚举与剪枝方式不变。
