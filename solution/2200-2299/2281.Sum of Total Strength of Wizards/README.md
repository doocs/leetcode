...
> **思考**
>
> 子数组强度是「最小值乘上子数组和」，要求和。$n \le 10^5$，枚举子数组不可行。把贡献按最小值位置拆开：对每个 $i$，找到它作为严格最小（左侧大于等于、右侧严格大于）的管辖区间 $[l,r]$，再乘上这些子数组的和的总和。
>
> 单调栈求出 $left[i]$、$right[i]$。子数组和的区间求和可用前缀和的前缀和 $ss$ 在常数时间算：右侧各段和乘左段个数，减去左侧各段和乘右段个数。最后乘 $strength[i]$ 累加取模。
