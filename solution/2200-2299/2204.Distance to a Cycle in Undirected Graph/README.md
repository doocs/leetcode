...
> **思考**
>
> 图由一个环加上若干棵挂在环上的树构成。若从每个点 BFS 找环，时间可达 $O(n^2)$，在 $n$ 与边数同阶时偏贵。环上的点距离为 $0$，树上的点距离等于沿唯一路径走到环的边数。
>
> 注意到度为 $1$ 的点一定不在环上，可以像剥洋葱一样反复删除。用队列收集当前度为 $1$ 的点，删除时记录父节点 $f[i]$，并把邻居度数减一。删完后剩下的连通分量即是环。
>
> 再按删除的逆序回填：环上距离已是 $0$，被删点 $i$ 的距离为 $ans[f[i]]+1$。一遍拓扑即可得到所有点到环的距离。
