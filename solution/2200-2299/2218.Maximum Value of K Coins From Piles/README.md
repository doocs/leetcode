...
> **思考**
>
> 每堆只能从顶部连续取，总共取 $k$ 枚，价值和尽量大。若对取法做搜索，堆数与 $k$ 均可达千、总硬币数 $2000$，会重复计算同一前缀。各堆之间互相独立，属于分组背包：一堆是一组物品，取前 $h$ 枚对应一件体积为 $h$ 的物品。
>
> 令 $f[i][j]$ 为前 $i$ 堆取 $j$ 枚的最大价值。对第 $i$ 堆先做前缀和 $s[h]$，再枚举 $j$ 与 $h$，用 $f[i-1][j-h]+s[h]$ 更新。总转移量与「堆大小之和乘 $k$」同阶，可以通过。
...
> **思考**
>
> 方法一的转移只依赖上一行的 $f[i-1][\cdot]$，二维表中同时存放多行并无必要。把 $f$ 压成一维后，必须从大到小枚举容量 $j$，以免同一堆被取两次。
>
> 对每一堆仍先算前缀和，再倒序枚举 $j$ 与 $h$ 做同样的 $\max$ 更新。时间不变，额外空间降为 $O(k)$。
