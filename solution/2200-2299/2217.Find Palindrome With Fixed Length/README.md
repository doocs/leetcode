...
> **思考**
>
> 要输出长度为 $\textit{intLength}$ 的第 $q$ 个回文数，$q$ 可达 $10^9$，长度可达 $15$，既不能按顺序生成全部回文，也不能对每个询问暴力构造。回文由前半段唯一决定：后半段是前半段的镜像。
>
> 前半长度为 $\lceil \textit{intLength}/2 \rceil$，合法前半是从 $10^{l-1}$ 到 $10^l-1$ 的全体整数。第 $q$ 个对应 $v = 10^{l-1}+q-1$；若 $v$ 越界则答案为 $-1$，否则把 $v$ 的字符串镜像拼上（奇数长度时丢掉中间位的重复）。每个询问 $O(\textit{intLength})$。
