...
> **思考**
>
> 一件文物可取出，当且仅当其矩形内每个格子都已被挖到。若对每次挖掘都去改写 $n \times n$ 网格，再对每件文物扫描其格子，$n \le 10^3$ 且挖掘次数可达 $10^5$，网格本身可以承受，但文物至多占四个格子，不必真的铺开整张图。
>
> 挖掘位置互不相同，适合放入哈希集合 $s$。对每件文物枚举其 $[r_1, r_2] \times [c_1, c_2]$ 中的格子，全部落在 $s$ 中则答案加一。文物格子总数与挖掘次数同阶，时间 $O(m+k)$。
