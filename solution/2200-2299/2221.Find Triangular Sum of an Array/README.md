...
> **思考**
>
> 每一轮把相邻两数之和模 $10$ 写成新数组，直到剩一个数。$n \le 10^3$，模拟 $n$ 轮、第 $k$ 轮做 $k$ 次加法，总时间 $O(n^2)$，可以通过。无需闭式或组合数，按题意原地更新即可。
>
> 从长为 $n-1$ 的一轮做到长为 $1$，内层把 $nums[i]$ 写成 $(nums[i]+nums[i+1]) \bmod 10$。最后留下的 $nums[0]$ 即三角和。
