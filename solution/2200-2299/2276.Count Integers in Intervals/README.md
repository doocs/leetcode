...
> **思考**
>
> 要维护若干可能重叠的整数区间，支持加入一段并查询已被覆盖的整数个数。坐标到 $10^9$，操作 $10^5$ 次，不能开数组，也不能每次重建。覆盖次数不重要，区间赋值「整段已覆盖」即可。
>
> 动态开点线段树的值域为 $[1,10^9]$，节点只在访问时创建。修改把区间赋成满覆盖并用懒标记下传；查询根上的覆盖长度。单次 $O(\log 10^9)$。
