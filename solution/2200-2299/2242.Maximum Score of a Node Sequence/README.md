...
> **思考**
>
> 要在树上选出四个两两相邻的不同点，使分值和最大。$n \le 5\times 10^4$，枚举所有长为 $3$ 的路径过慢。四元组中间必是一条边 $(a,b)$，两端 $c$、$d$ 分别是 $a$、$b$ 的邻居，且四点互异。
>
> 每个点只需保留得分最高的三个邻居：枚举边时，$c$ 与 $d$ 各只有常数个候选，去掉与 $a$、$b$ 冲突的即可。预处理 $O(n)$，枚举边 $O(m)$。
