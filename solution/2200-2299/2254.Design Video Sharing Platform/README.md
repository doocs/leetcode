...
> **思考**
>
> 平台要分配最小可用 $\textit{videoId}$，并支持删除后回收编号；观看、点赞与计数都按编号随机访问。调用次数 $10^5$，视频总长也是 $10^5$，线性扫描空闲编号不可行。
>
> 小根堆保存已释放的编号，再配一个递增的下一新编号：堆空则发新号，否则弹出堆顶。内容、观看、点赞、点踩放在哈希表里，删除时把编号丢回堆。观看返回子串 $[\textit{start},\min(\textit{end},|v|-1)]$。题面未附实现，按堆加哈希表即可满足复杂度。
