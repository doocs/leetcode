...
> **思考**
>
> 每次操作只能弹出栈顶或把先前弹出的某一元素放回顶部，$k$ 可达 $10^9$，按步模拟不可行。需要把「恰好 $k$ 次操作后的栈顶」收成几种可比较的情况。
>
> $k = 0$ 时栈顶即为 $nums[0]$。若数组只有一个元素，奇数次操作会把唯一元素弹出且无法放回，答案为 $-1$；偶数次则仍是该元素。
>
> 长度至少为 $2$ 时，前 $k-1$ 次弹出腾出的数都可以在第 $k$ 次放回成为栈顶，故候选为 $nums[0..k-2]$ 的最大值；若 $k < n$，也可以第 $k$ 次继续弹出，使 $nums[k]$ 暴露为栈顶。二者取较大值即可。
