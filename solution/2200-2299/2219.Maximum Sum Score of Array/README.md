...
> **思考**
>
> 下标 $i$ 的得分为「$i$ 左侧含自身的前缀和」与「$i$ 右侧含自身的后缀和」中的较大者，要求所有 $i$ 上的最大得分。$n \le 10^5$，对每个 $i$ 重新求和不可行。
>
> 两个和都可以在扫描时维护：先记下总和 $r$，再从左往右把 $x$ 加入前缀 $l$，此时 $l$ 与 $r$ 恰是当前下标的两个候选，取较大者更新答案，然后从 $r$ 中减去 $x$。一遍即可。
