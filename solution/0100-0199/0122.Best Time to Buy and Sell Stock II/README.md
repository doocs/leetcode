...
> **思考**
>
> 可以交易任意次，但不能同时持有。把一段上涨拆成相邻两天的差价再求和，与「谷买峰卖」得到的利润相同。$n \le 3\times 10^4$。因此遍历相邻两天，只累加正差价。
...
> **思考**
>
> 贪心没有显式状态，不便于推广到有交易次数或冷冻期的变体。用 $f[i][0/1]$ 表示第 $i$ 天持有或不持有的最大利润，转移是继续持有、今日买入、继续空仓或今日卖出。与方法一答案相同，但状态更一般。
...
> **思考**
>
> 方法二第 $i$ 天只依赖第 $i-1$ 天的两个状态，两个变量滚动即可，空间降为 $O(1)$。
