...
> **思考**
>
> 与上一题不同，树不必是完美的，一层里孩子可能缺失，不能默认「左孩子 next 指向右孩子」。层序仍然正确：按层出队，把相邻节点依次接上。$n \le 6000$。
...
> **思考**
>
> 方法一的队列与一层宽度同阶，进阶要 $O(1)$ 空间。当前层的 $\textit{next}$ 已是一条链表，遍历它时可以把下一层的非空孩子按出现顺序串起来，并记下下一层的头。一层结束后改走这一条新链，不再使用队列。
