...
> **思考**
>
> 排序后扫连续段是 $O(n\log n)$，题目要求 $O(n)$。$n \le 10^5$。放入哈希集合后，从每个尚未删除的 $x$ 向右延伸，直到断掉，并把这段长度记在起点上，供更早的起点拼接。每个数进出集合一次。
...
> **思考**
>
> 方法一额外用哈希表记下每段长度以便拼接。若只从「$x-1$ 不在集合中」的位置开始向右数，则 $x$ 一定是某段起点，不必再保存 $d$。实现更短，仍保证每个数被访问常数次。
