...
> **思考**
>
> 找出至少连续三次出现的数字。把日志自连接两次，约束相邻行 $\textit{id}$ 相差 $1$ 且 $\textit{num}$ 相同，即可锁住长度为 $3$ 的窗口，再对数字去重。
...
> **思考**
>
> 方法一写两次连接。窗口函数 $\textit{LAG}/\textit{LEAD}$ 直接取出前后两行的 $\textit{num}$，三值相等即连续三次，语句更短。
...
> **思考**
>
> 方法二只看长度为 $3$ 的窗口，更长的连续段会被切开多次。用「与上一行是否相同」打分组标记再求前缀和，同一段共享一个组号，$\textit{HAVING}\,\textit{COUNT}\ge 3$ 一次选出所有足够长的段。
