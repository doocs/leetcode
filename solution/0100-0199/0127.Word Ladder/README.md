...
> **思考**
>
> 求最短变换长度，每次只改一个字母且必须在词表中。词表可达 $5000$、词长 $10$，枚举全部路径不可行。无权图的最短路用 BFS：对当前词的每个位置试 $26$ 个字母，命中词表则入队并删除，避免重复入队。
...
> **思考**
>
> 方法一词表较大时，单向搜索的前沿可能很宽。从起终点同时扩张，每次选更小的一侧扩展，两侧访问集合相交即为最短长度，搜索空间通常更小。
