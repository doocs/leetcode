...
> **思考**
>
> 求共线点的最大个数。点互不相同，$n$ 较小，枚举两点确定直线再扫其余点是否共线，用叉积避免除法，时间 $O(n^3)$ 可以过。
...
> **思考**
>
> 方法一是 $O(n^3)$。固定一点后，其余点按与它的斜率分组，同一斜率即共线。斜率用约分后的 $(\Delta y,\Delta x)$ 作键，避免浮点误差。对每个点 $O(n)$ 统计，总时间 $O(n^2)$。
