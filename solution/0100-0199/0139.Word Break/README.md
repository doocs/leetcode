...
> **思考**
>
> 判断 $s$ 能否拆成词典中的词。$|s|\le 300$，回溯所有切分会重复计算同一后缀。$f[i]$ 表示前缀 $s[0..i)$ 能否拆分：枚举切点 $j$，若 $f[j]$ 为真且 $s[j..i)$ 在词典中则成功。词典放进哈希集合，判断子串是否成词为期望 $O(1)$。
...
> **思考**
>
> 方法一每次切分都要切片并查表，子串哈希是 $O(n)$。词放进前缀树后，从位置 $i$ 沿树往前走，只有树上的边才继续，避免无用切片，时间降为 $O(n^2)$。
