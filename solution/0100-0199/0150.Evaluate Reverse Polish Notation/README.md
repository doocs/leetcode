...
> **思考**
>
> 逆波兰式把运算符写在操作数后面，求值顺序已经确定，不必处理括号与优先级。$n\le 10^4$。遇到数字入栈，遇到运算符弹出两个操作数计算再压回，栈中始终是尚未消费的中间结果，最后只剩答案。
...
> **思考**
>
> 方法一用运算符到函数的映射。本写法在栈顶原地修改：判断 token 是数字还是四则运算，减法、除法注意操作数顺序与向零取整。结构相同，少一张函数表。
