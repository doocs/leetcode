...
> **思考**
>
> 相邻房屋不能都偷，求最大金额。枚举子集要检查相邻约束，房屋数虽不大，重叠子问题很多。从第 $i$ 间出发：偷则跳到 $i+2$，不偷则到 $i+1$，取较大者。记忆化后每个下标只算一次。
...
> **思考**
>
> 方法一是递归。$f[i]$ 表示前 $i$ 间的最大金额，转移只看「偷第 $i$ 间则加 $f[i-2]$，否则继承 $f[i-1]$」，自底向上填表。
...
> **思考**
>
> 方法二的 $f[i]$ 只依赖前两项，两个变量滚动即可，空间降为 $O(1)$。
