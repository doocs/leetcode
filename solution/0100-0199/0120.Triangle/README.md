...
> **思考**
>
> 自顶向下每步只能走到下一行相邻位置，枚举路径的数量随行数指数增长。行数不大，但重叠子问题很多：从某格出发的最小路径只依赖它下方两个格子。
>
> 自底向上定义 $f[i][j]$ 为从该格走到最后一行的最小和，每个格子取下方左右的较小值再加自身，顶点 $f[0][0]$ 即答案。
...
> **思考**
>
> 方法一的 $f[i][j]$ 只依赖下一行。用一维数组从下往上覆盖，空间从 $O(n^2)$ 降到 $O(n)$，符合进阶要求。
