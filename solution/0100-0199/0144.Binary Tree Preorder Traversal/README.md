...
> **思考**
>
> 前序是「根、左、右」。树按递归定义，先记录根再递归左右即可。$n\le 100$，递归栈可以接受。进阶才要求改成迭代。
...
> **思考**
>
> 方法一已经得到正确序列，进阶要去掉系统递归。显式栈先压右再压左，弹出时先处理根，顺序与前序相同，空间仍为 $O(n)$。
...
> **思考**
>
> 方法二仍把空间留在 $O(h)$。Morris 把左子树最右空指针临时接到当前根，沿左链走下去，第一次到达时输出根，回来再拆掉。树自身的空指针充当栈，空间 $O(1)$。
