...
> **思考**
>
> 后序是「左、右、根」。递归先走完左右再记录根。$n\le 100$，递归栈可以接受。进阶要求迭代。
...
> **思考**
>
> 方法一依赖递归。注意到「根、右、左」恰是前序交换左右孩子，再反转即得「左、右、根」。用栈按这个顺序遍历后整体倒序，不必判断节点是否第二次访问。
...
> **思考**
>
> 方法二仍要 $O(n)$ 栈与一次反转。Morris 按「根、右、左」穿针，最后再反转结果，额外空间降为 $O(1)$。
