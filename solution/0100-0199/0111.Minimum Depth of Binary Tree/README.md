...
> **思考**
>
> 最小深度是根到最近叶子的节点数。注意「只有一侧孩子」的节点不是叶子，必须继续走向仅有的那一侧，否则会把缺失的孩子当成深度 $0$。$n \le 10^5$。
>
> 递归时：空树为 $0$；缺左则 $1$ 加上右子树最小深度；缺右同理；两侧都在则取较小者加 $1$。
...
> **思考**
>
> 方法一可能先走完整棵深树才遇到最近叶子。最小深度对应最短路径，BFS 按层扩张，第一次到达叶子即可停止，偏斜树时常数更好。
