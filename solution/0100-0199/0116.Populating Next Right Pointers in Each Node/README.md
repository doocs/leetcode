...
> **思考**
>
> 完美二叉树的每一层都是满的，层序时把同一层相邻节点用 $\textit{next}$ 串起来即可。$n < 2^{12}$，队列空间与一层宽度同阶。进阶要求常数额外空间，方法一先给出正确的层序连法。
...
> **思考**
>
> 方法一要用队列保存整层。完美二叉树中，一个节点的左右孩子相邻，右孩子的下一个是兄弟的左孩子。父层的 $\textit{next}$ 已经连好，可以当作遍历下一层的线索，递归把相邻指针接上，不再需要队列。
