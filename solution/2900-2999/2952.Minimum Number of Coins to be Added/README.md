...
> **思考**
>
> 要用现有及新加的硬币凑出 $[1,target]$。若已能表示 $[0,s-1]$，加入 $x \le s$ 可将区间扩到 $s+x-1$；若下一个硬币太大，则必须先补一枚 $s$，区间翻倍。这是构造完全覆盖的经典贪心。
>
> 硬币先排序，指针只前进。$s \le target$ 时循环，每次要么吃掉当前硬币，要么 $s \leftarrow 2s$ 并计数。
