...
> **思考**
>
> 每一步只能改行或改列，$k$ 步后到达 $dest$。网格可达 $10^9$ 量级，不能按格子建状态。相对 $source$ 仅有四类位置：自身、同列不同行、同行不同列、行列皆不同。
>
> 用四维向量 $f$ 记录这四类格子的方案数，一步转移只在这四类之间发生，系数由 $n,m$ 决定。迭代 $k$ 次后，按 $dest$ 与 $source$ 的行列关系选取对应分量。
