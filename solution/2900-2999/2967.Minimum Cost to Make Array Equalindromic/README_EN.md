...
> **Thinking**
>
> All values become one palindrome; the $L_1$ cost is minimized near the median. Palindromes are generated by mirroring halves in $[1,10^5]$ and stored in sorted $ps$.
>
> Sort $nums$, binary-search $ps$ around the median, and evaluate a few neighbors. $n \le 10^5$, so the target is not enumerated exhaustively.
