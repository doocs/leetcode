...
> **思考**
>
> $n \le 100$，按子序列定义枚举或用 $O(n^2)$ 动态规划均可得到最长长度。注意到 $groups$ 仅取 $0$ 或 $1$，相邻项合法当且仅当组别翻转，因此任意一组连续相同组别中至多保留一个下标。
>
> 保留每一段的第一个下标即可：它既能与上一段衔接，又不缩短后续可选长度。题目允许任意一个最长子序列，故无需在同组内再比较 $words$。一次从左到右扫描即可构造答案。
