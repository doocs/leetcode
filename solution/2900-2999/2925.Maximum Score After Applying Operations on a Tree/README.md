...
> **思考**
>
> 树上每条根到叶的路径必须留下至少一个未选节点，其余节点值可计入得分。若直接枚举每个点选或不选，需保证所有路径约束，状态过多。树形 DP 把约束收束到子树：要么不选根，则子树可全选；要么选根，则每个子树内部仍须各自留下一个未选点。
>
> $dfs$ 返回子树总和以及「满足路径约束的最大选取」。叶子只能不选自身才能留下未选点，故第二维为 $0$。非叶取 $\max(values[i]+b, a)$，其中 $a$ 为子树全和、$b$ 为子树均已合法。答案是根的第二维。
