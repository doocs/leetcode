...
> **思考**
>
> 关系改为 DAG 上的有向边「更强者指向较弱者」，冠军是唯一入度为 $0$ 的点；若不唯一则不存在。$n \le 100$，扫一遍边统计入度即可。
>
> 入度为 $0$ 的个数不是 $1$ 时返回 $-1$，否则返回该下标。
...
> **思考**
>
> 方法一显式存储全部入度。冠军等价于「从未作为边的终点出现的唯一顶点」。用集合存放 $0 \ldots n-1$，遍历边时删去终点，最后集合大小为 $1$ 则取其仅有元素。语义与入度相同，只是用集合表达「尚未输掉任何比赛」的队伍。
