...
> **思考**
>
> 数组以接口提供，长度可达 $10^{15}$，不能逐格扫描。相同值已经连成一段，故每一段只需定位右端。在 $[i,n)$ 上二分「第一个不等于 $nums.at(i)$ 的下标」，段数即二分次数。
>
> 相邻两格已不同时可直接右移一格，减少一次二分。总访问次数与段数的对数有关，受元素和上界约束。
...
> **思考**
>
> 二分依赖随机访问。若将区间对半切开，两半的段数之和在中点两侧值相等时多算了一段，减去即可。分治同样只在段边界附近访问，避免扫描整段。
>
> 当区间左右端点值已不同且长度为 $2$ 一类的平凡情形时直接返回。与方法一相比，少一次全局的「找右端」循环，改为递归合并。
