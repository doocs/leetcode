...
> **思考**
>
> 排列满足 $\gcd(i, perm_i)=1$。$n$ 很小，可用二进制 $mask$ 记录已用数字。$dfs(mask)$ 的下一个位置是 $bit\_count+1$，枚举未用且与位置互质的 $j$。
>
> 记忆化状态 $2^n$，每位再乘 $n$，可接受。
...
> **思考**
>
> 方法一按「已用集合」递归。改成 $f[mask]$ 表示达到该集合的方案，从子集差一位转移，需保证新数字与其位置互质。初始 $f[0]=1$，答案 $f[2^n-1]$。
>
> 与搜索相比只是迭代顺序不同，便于去掉递归。
