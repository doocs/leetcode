...
> **思考**
>
> 两人要走到某一栋 $j \ge \max(a,b)$ 且严格高于较矮者（若较矮者就在右侧且已更矮，则可直接在右侧会面）。在线对每个询问扫右侧不可行，$q$ 与 $n$ 均为 $5 \times 10^4$。
>
> 将询问按右端点从大到小处理，指针 $j$ 把 $r$ 右侧的楼插入树状数组：下标为离散化后的「高度后缀」，值是最小下标。查询「高度大于 $heights[l]$ 的最小下标」即为会面楼。
