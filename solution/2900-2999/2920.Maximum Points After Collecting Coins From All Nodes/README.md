...
> **思考**
>
> 树上每个点可选「减 $k$」或「全体再右移一位」。后一种操作对整棵尚未处理的子树生效，且金币 $\le 10^4$，右移约 $14$ 次后归零，故位移次数是很小的额外状态。
>
> $dfs(i,fa,j)$ 表示当前位移为 $j$ 时在 $i$ 的最优得分：方案甲取 $(coins[i] \gg j)-k$ 且子节点仍用 $j$；方案乙取 $coins[i] \gg (j+1)$ 且子节点用 $j+1$（以 $j<14$ 为限）。记忆化后从根出发即可。
