...
> **思考**
>
> 每个位置要填「其余元素之积」并对 $12345$ 取模。模数并非质数，不能对 $grid[i][j]$ 求逆；若某格为 $12345$ 的倍数，直接相除也会失效。$n \cdot m \le 10^5$，按行展开后与「除自身以外的乘积」相同。
>
> 先自右下向左上累计后缀乘积 $suf$（不含自身）写入 $p$，再自左上向右下用前缀乘积 $pre$ 乘入。两次扫描后 $p[i][j]$ 即为两侧之积，全程在模意义下更新。
