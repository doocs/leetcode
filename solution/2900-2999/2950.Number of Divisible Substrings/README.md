...
> **思考**
>
> 每个字母映射到 $1 \ldots 9$，求数字和能被长度整除的子串个数。串长通常不大，枚举左右端并累加映射值，判断 $s \bmod (j-i+1)=0$ 即可。
>
> 映射表一次性建好，避免循环中重复查表。
...
> **思考**
>
> 方法一在较长串上达到 $O(n^2)$。子串平均值必为 $1 \ldots 9$ 中的整数，故可枚举平均值 $t$，把每个映射减去 $t$ 后转化为「子数组和为 $0$」的计数。
>
> 前缀和配合哈希表统计历史出现次数，对每个 $t$ 扫一遍。总时间 $O(9n)$，适合更长的输入。
