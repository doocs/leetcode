...
> **思考**
>
> 同尾子串由区间内某字母的任意两次出现（含单字符）构成。询问多达与 $n$ 相当，不能每次重新扫描。对每个出现过的字母建前缀计数，区间内次数 $x$ 贡献 $x + C(x,2)$，即 $x(x+1)/2$，实现里写成先加区间长度再加 $C(x,2)$。
>
> 字母集合取 $set(s)$，避免对未出现字母建 $26$ 张表。每个询问枚举实际出现的字母。
