...
> **思考**
>
> 与 I 的方法二相同：枚举中间商品，左侧查询「更低价格的最大利润」，右侧查询「更高价格的最大利润」。数据更大，必须用树状数组而不能 $O(n^2)$ 扫描。
>
> $tree1$ 从左插入利润并查询 $[1, price-1]$；$tree2$ 从右对价格取对称后插入，从而把「大于 $price$」变成前缀最值。两侧皆为正时更新 $left+profit+right$。
