...
> **思考**
>
> 改写对象变成整段子串，且 $source$、$target$ 须同步替换。候选串不超过 $100$，先用字典树编号，再在编号点上 Floyd 得到任意替换代价。剩余问题是按位置切分：能匹配一对原串/目标串则付 $g[p][q]$，或单字符相等则免费前进。
>
> $dfs(i)$ 记忆化从 $i$ 切到末尾的最小代价，两指针同步走字典树。串长 $1000$，状态线性，转移与匹配长度同阶。
