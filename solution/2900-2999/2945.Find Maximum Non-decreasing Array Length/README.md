...
> **思考**
>
> 每次把一段相邻元素合并为它们的和，最终数组须非递减，求最多段数。$n \le 10^5$，不能枚举所有分割。设 $f[i]$ 为前 $i$ 个元素的最多段数，且最后一段之和应尽量小以便后续衔接；记该最后一段的起点相关信息于 $pre$。
>
> 前缀和 $s$ 上，下一段之和至少为上一段之和，故用二分查找最小的 $j$ 满足 $s[j]-s[i] \ge s[i]-s[pre[i]]$，并允许 $pre$ 取前缀最大以继承更优起点。一次扫描加二分得到 $f[n]$。
