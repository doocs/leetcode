...
> **思考**
>
> 美丽子串含恰好 $k$ 个 $1$。$n \le 100$，枚举全部 $O(n^2)$ 个子串并统计 $1$ 的个数可行。对每个合法子串，按「更短优先，长度相同则字典序更小」更新答案即可。
>
> 实现中左端点 $i$ 固定后，右端点从 $i+k$ 起扫，因至少需要 $k$ 个字符才能放下 $k$ 个 $1$。
...
> **思考**
>
> 枚举在 $n=100$ 时尚可，但对每个子串调用 $count$ 会重复扫描同一区间。美丽子串对应窗口内 $1$ 的个数恰为 $k$，且去掉前导 $0$ 不会减少 $1$ 的个数，因而最短候选必无前导零。
>
> 用双指针维护窗口：右端纳入字符，当 $1$ 的个数超过 $k$ 或左端为 $0$ 时收缩左端。每当个数恰为 $k$，再与当前最优串比较长度与字典序。
