...
> **思考**
>
> 把 $s$ 分成 $k$ 段半回文，修改次数最少。$n \le 200$，可先对每个子串枚举因子 $d$，按半回文配对统计改写次数，得到代价 $g[i][j]$。
>
> 划分本身是经典的「切 $k$ 段」DP：$f[i][j]$ 表示前 $i$ 个字符切 $j$ 段的最小代价，枚举上一段右端 $h$ 转移。预处理 $O(n^3)$ 量级，加上划分后仍可接受。
...
> **思考**
>
> 方法一将全部子串代价存成 $O(n^2)$ 数组，划分再用一层下标。并非每个子串都会在最优划分中出现，代价可以改为按需记忆化。
>
> 划分维对上一层 $j-1$ 滚动到一维 $dp$，按段数从少到多、右端从右到左更新，避免覆盖尚未使用的状态。空间降为线性，语义与方法一相同。
