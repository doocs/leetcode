...
> **思考**
>
> 美丽数组要求每个长度为 $3$ 的窗口最大值至少为 $k$。$n \le 10^5$，不能对每个窗口单独枚举加多少。对位置 $i$ 加 $\max(k-nums[i],0)$ 后，它能覆盖包含它的窗口，且一个窗口只需其中一个位置达标。
>
> 因此状态只需记住「最近三个位置谁被选为覆盖者」的最小代价。滚动 $f,g,h$ 分别对应以倒数第三、二、一项为覆盖点的最优值，读入 $x$ 时用 $\min(f,g,h)+\max(k-x,0)$ 推进。最终三者取最小。
