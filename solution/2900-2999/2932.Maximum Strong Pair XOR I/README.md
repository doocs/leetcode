...
> **思考**
>
> 强数对满足 $|x-y| \le \min(x,y)$。$n \le 50$，枚举全部有序对并取合法异或的最大值即可。
>
> 实现中双重生成器直接过滤条件，无需额外结构。
...
> **思考**
>
> 方法一在 $n=50$ 时足够，但同一题的 II 将 $n$ 提到 $5 \times 10^4$。不等式在 $x \le y$ 时化为 $y \le 2x$。排序后枚举较大值 $y$，双指针维持窗口内 $x$ 满足该式。
>
> 窗口内求与 $y$ 的最大异或可用 0-1 字典树：插入 $y$，弹出过期的 $nums[i]$，再查询。此写法与 II 共用，便于对照。
