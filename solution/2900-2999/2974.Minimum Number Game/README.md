...
> **思考**
>
> 每轮取出两个当前最小数，却按「先放较大、再放较小」写入答案。等价于反复弹出堆顶两个元素并交换顺序追加。$n \le 100$ 且为偶数，堆模拟与题面过程一致。
...
> **思考**
>
> 方法一维护堆，但全局最小值序列在排序后即确定：相邻的 $(a_{2i},a_{2i+1})$ 正是每轮弹出的一对。交换这两位后写入，无需动态结构。时间仍由排序主导。
