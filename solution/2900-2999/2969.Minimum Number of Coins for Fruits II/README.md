...
> **思考**
>
> 转移与「水果 I」相同：$f[i]=prices[i-1]+\min_{i+1 \le j \le 2i+1} f[j]$，但 $n$ 达到 $10^5$，双重循环不可行。窗口右端随 $i$ 减小而收缩，单调队列可 $O(1)$ 取最小。
>
> 从后往前弹出超过 $2i+1$ 的下标，用队首更新 $prices[i-1]$，再按代价保持递增。原地滚动后 $prices[0]$ 即为答案。
