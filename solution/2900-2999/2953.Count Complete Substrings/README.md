...
> **思考**
>
> 完全子串要求每种出现的字符恰 $k$ 次，且相邻字符码差不超过 $2$。后一条件把原串切成若干互不跨越的段，答案为各段之和。段内字符种类 $\le 26$，可枚举种类数 $i$，窗口长固定为 $i \cdot k$。
>
> 窗口内用 $cnt$ 与 $freq$ 维护「出现次数的次数」，$freq[k]=i$ 时计入。段与窗口都是线性滑动，$n \le 10^5$。
