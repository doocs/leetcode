...
> **思考**
>
> 山形要求 $i<j<k$ 且 $nums[i]<nums[j]>nums[k]$，目标和最小。$n \le 50$，三重循环可行。固定峰顶 $j$ 后，真正需要的只是左侧最小值与右侧最小值，前提是二者都严格小于 $nums[j]$。
>
> 为此先预处理 $right[i]$ 为后缀最小值，再从左扫描同时维护 $left$。对每个峰顶检验两侧是否更小，并更新三数之和。不存在则返回 $-1$。
