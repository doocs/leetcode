...
> **思考**
>
> 与上一题不同，相邻两项还须长度相同且汉明距离恰为 $1$，贪心按组别交替选取无法保证连通。$n$ 通常不超过数百，单词长度很短，因此可以接受 $O(n^2 \cdot L)$ 的转移。
>
> 以 $f[i]$ 表示以 $i$ 结尾的最长长度，$g[i]$ 记录前驱。枚举 $j < i$，仅当组别不同且 $check$ 成立时用 $f[j]+1$ 更新。最后从达到全局最大值 $mx$ 的下标沿 $g$ 回溯并反转，即得一条具体子序列。
