...
> **思考**
>
> 子数组得分是「元素和 $\times$ 最大公约数」，长度至少为 $k$。枚举所有子数组并求 GCD 在 $n$ 较大时不可行。注意到将右端延伸一位时，历史 GCD 只减不增，且不同 GCD 值至多 $O(\log A)$ 个。
>
> 用数组 $f$ 保存「该 GCD 首次出现的左端」。读入 $v$ 后与旧对做 $gcd$，去重得到新的 $g$，再并上 $(i,v)$。对长度达到 $k$ 的段，用前缀和乘以该 GCD 更新答案。
