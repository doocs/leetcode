...
> **思考**
>
> 题意与 I 相同，但 $n \le 10^5$，双重循环不可行。合法右端点 $i$ 对应的左端集合仍是前缀 $[0, i-indexDifference]$，该前缀的最值随 $i$ 单调扩张。
>
> 为此只需在扫描过程中维护前缀最小值、最大值的下标，并随时检验与 $nums[i]$ 的差。时间 $O(n)$，与 I 的实现一致，仅数据范围迫使必须采用这一线性写法。
