...
> **思考**
>
> 统计 $[start,finish]$ 中、每位 $\le limit$ 且以 $s$ 为后缀的个数。区间计数化为 $f(finish)-f(start-1)$。数位 DP 按位填写，高位受 $limit$ 与上界紧约束，低 $|s|$ 位必须匹配 $s$（紧约束时还须 $s \le$ 上界剩余）。
>
> $t$ 比 $s$ 短则方案为 $0$。记忆化 $(pos,lim)$ 即可，上界长度不超过 $16$。
