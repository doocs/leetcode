...
> **思考**
>
> 同值必须分到若干组，且任意两组大小相差不超过 $1$，故全局组大小只能是某 $k$ 或 $k+1$。组数最少时 $k$ 应尽可能大，其上界不超过最小频次。
>
> 从 $min(cnt.values())$ 递减枚举 $k$。对频次 $v$，若 $\lfloor v/k \rfloor < v \bmod k$，则无法拆成若干个 $k$ 或 $k+1$；否则最少组数为 $\lceil v/(k+1) \rceil$。第一个完全可行的 $k$ 即为最优。
