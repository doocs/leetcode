...
> **思考**
>
> 非负整数解 $x+y+z=n$ 且 $0 \le x,y,z \le limit$。$n$ 可达 $10^{18}$ 量级，三重循环不可用。无上界时隔板法给出 $C_{n+2}^{2}$；再容斥减去某变量至少 $limit+1$ 的方案，并加回两个变量同时越界的方案。
>
> 若 $n>3\cdot limit$ 则直接为 $0$。实现时对组合数自变量加判断，以免负数阶乘。公式为常数时间。
