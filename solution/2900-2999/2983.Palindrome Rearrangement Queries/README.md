...
> **思考**
>
> 每次可重排 $s$ 的两段（左半一段、右半一段），问能否变成回文。询问 $10^5$，必须预处理。把右半反转成 $t$ 后，问题变为：两段可重排的区间能否让 $s$ 与 $t$ 处处相等。
>
> 前缀字符计数比较可重排区间的多重集，前缀不等位数 $diff$ 检查不可动区间是否已对齐。按两区间包含、分离、交错分类，减去重叠后再比剩余计数。
