...
> **思考**
>
> 题意与 I 相同，但 $n \le 5 \times 10^4$，枚举端点不可行。元辅音相等即前缀上「元音 $+1$、辅音 $-1$」的和相同；乘积条件 $v^2 \equiv 0 \pmod k$ 在 $v=len/2$ 时化为长度须是某 $l$ 的倍数，其中 $l$ 由 $4k$ 的平方核决定。
>
> 扫描时把 $(i \bmod l,\ 前缀和)$ 丢进哈希表，相同键的历史位置都能与当前 $i$ 组成美丽子串。偏移 $n$ 避免负数下标。
