...
> **思考**
>
> 删除一段后剩余须严格递增，即剩余为递增前缀、递增后缀，或二者拼接且衔接处仍递增。先找到最长严格递增前缀右端 $i$；若已覆盖全数组，则任意子数组都可删，答案为 $n(n+1)/2$。
>
> 否则从右扫递增后缀，左指针 $i$ 回退到 $nums[i]<nums[j]$，每个后缀起点贡献 $i+2$ 种前缀（含空前缀）。$n \le 50$，双指针已足够，并与 II 共用。
