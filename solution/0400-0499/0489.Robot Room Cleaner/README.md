...
> **思考**
>
> 房间地图未知，只能通过机器人接口移动、转向与清扫。不能随机走，否则无法保证覆盖且难以回到起点。
>
> 把起点当作 $(0,0)$，DFS 四个方向：未访问且 $\textit{move}$ 成功则走进去递归；四个方向试完后原地掉头走回父格，再转回原来朝向。
>
> 访问集合按相对坐标去重。回溯必须先转 $180^\circ$ 再 $\textit{move}$，否则朝向与坐标会和递归栈不一致。
