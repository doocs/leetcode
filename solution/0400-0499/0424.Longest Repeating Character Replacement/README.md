...
> **思考**
>
> 窗口内最多修改 $k$ 个字符使之全相同，即「窗口长度减去众数次数 $\le k$」。枚举左右端点是 $O(n^2)$，$n\le 10^4$ 偏紧。
>
> 右指针扩张并维护窗口内最大频次 $\textit{mx}$；当 $r-l+1-\textit{mx}>k$ 时左端右移一格。只需最大合法窗口，答案为 $n-l$。
>
> 左移时不必减小 $\textit{mx}$：窗口只关心最大长度，偏大的 $\textit{mx}$ 只会使判定更严，不会漏掉更长的合法段。
