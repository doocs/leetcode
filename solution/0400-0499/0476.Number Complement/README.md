...
> **思考**
>
> 补数是在不含前导零的二进制长度内按位取反。直接取反会连高位的 $0$ 一起翻成 $1$。
>
> 用 $2^{\textit{bit\_length}}-1$ 做掩码，与原数异或，只翻转有效位。
>
> 掩码长度恰好等于最高位 $1$ 的位置，前导零不会进入结果。
...
> **思考**
>
> 方法一用异或。也可以先按位取反，再与同样的低位全 $1$ 掩码相与，只保留有效宽度。实现语言不同，语义一致。
