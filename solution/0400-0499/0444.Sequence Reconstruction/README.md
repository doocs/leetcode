...
> **思考**
>
> 要判断 $nums$ 是否为所有序列的唯一最短超序列。多个拓扑序意味着超序列不唯一。
>
> 把每个 $\textit{seq}$ 的相邻对建成有向边并统计入度。队列里始终只能有一个入度为 $0$ 的点，才能逐步确定唯一顺序；一旦同时出现两个，返回失败。
>
> 循环结束时队列为空则全部排出，顺序唯一。不必与 $nums$ 逐位比较：唯一拓扑序若存在，只能是 $nums$（题目保证 $nums$ 是 $[1,n]$ 的排列且覆盖）。
