...
> **思考**
>
> 要区分 IPv4、IPv6 与非法。正则可以写，但前导零、空段、字符集等边界更容易在拆分后逐段检查。
>
> 按 `.` 分成 $4$ 段判 IPv4：禁止前导零，必须是 $0$–$255$ 的数字。按 `:` 分成 $8$ 段判 IPv6：长度 $1$–$4$ 且均为十六进制。都不满足则 $\texttt{Neither}$。
>
> 先 IPv4 再 IPv6，两种格式的分隔符不同，不会同时成立。空段会在长度或 $\texttt{isdigit}$ 检查中失败。
