...
> **思考**
>
> 两人轮流从 $1..n$ 里不放回取数，累计先达到目标者胜。$n\le 20$，状态是已选集合，全集枚举博弈树会重复。
>
> 总和不足则无人能赢。否则 $dfs(\textit{mask},s)$：枚举未选的 $i$，若 $s+i$ 已达目标或对手在新状态下必败，则当前赢。对 $\textit{mask}$ 记忆化。
>
> 位掩码把集合压成整数，子问题至多 $2^{n}$ 个。先判总和，避免在必输局面上搜索。
