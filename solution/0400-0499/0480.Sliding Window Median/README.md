...
> **思考**
>
> 滑动窗口中位数。每次排序窗口是 $O(nk\log k)$。堆可以维护中位数，但堆不能任意删除离开窗口的元素。
>
> 对顶堆：大根堆存较小一半，小根堆存较大一半，另用哈希表延迟删除。加入、弹出后只在堆顶清理过期值，再平衡两边的有效大小。
>
> 延迟删除把「任意删」变成堆顶弹出；有效大小用计数维护，堆顶始终是当前中位数的候选。
...
> **思考**
>
> 方法一的延迟删除实现较长。有序集合支持 $O(\log k)$ 插入删除。左边集合存较小一半，右边存较大一半，每次先入右再把最小移到左并平衡。窗口满后读两端，再删滑出的值。
>
> 代码更短，代价是需要有序表结构。
