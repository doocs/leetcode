...
> **思考**
>
> 用均匀的 $rand7$ 造均匀的 $rand10$。直接 $\textit{rand7}\bmod 10$ 会偏斜。两次调用可生成 $[1,49]$ 的均匀整数。
>
> 拒绝采样：保留 $[1,40]$，映射为 $x\bmod 10+1$；落在 $41$–$49$ 则重来。$40$ 能被 $10$ 整除，十个残差各出现四次，因而均匀。
>
> 期望调用次数为常数。拒绝区间保证每个结果概率相同。
