...
> **思考**
>
> $nums1$ 是 $nums2$ 的子集，求各自在 $nums2$ 中下一个更大元。对每个 $nums1$ 的值在 $nums2$ 里向右扫是 $O(nm)$。
>
> 从右向左扫 $nums2$，维护单调递减栈：弹出所有更小的栈顶后，栈顶就是当前值的下一个更大元，记入哈希表。再按 $nums1$ 查表。
>
> 栈里从顶到底递增（右到左扫时保持递减栈），每个元素入出栈一次。先处理 $nums2$ 再回答 $nums1$，避免重复扫描。
