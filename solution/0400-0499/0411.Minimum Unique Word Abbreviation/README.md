...
> **思考**
>
> 仓库中该题的代码标签为空，尚无实现。题意是：在 $\textit{target}$ 的所有合法缩写中，找一个长度最短、且不会与字典里任何单词冲突的缩写。
>
> $m\le 21$，且 $\log_2 n+m\le 21$，说明可以按「保留哪些位置」枚举缩写。相邻被替换的片段必须合并计数，长度等于保留字母数加上替换段数。
>
> 冲突判定只需对与 $\textit{target}$ 等长的字典词进行：缩写在被替换位置上不约束字母，若某字典词在所有保留位上都相同，则该缩写不唯一。
