...
> **思考**
>
> $n_1$ 个 $s_1$ 里能抽出多少个 $n_2$ 个 $s_2$。直接模拟拼接，$n_1$ 很大时重复扫 $s_2$ 的同一起点。
>
> 预处理：从 $s_2$ 的每个位置 $i$ 出发，扫完一个 $s_1$ 后得到「匹配了几个 $s_2$」以及下一个下标。然后循环 $n_1$ 次按表递推，答案除以 $n_2$。
>
> $s_2$ 长度有限，状态只有起点下标，一次 $s_1$ 的匹配被做成 $O(1)$ 转移。
