...
> **思考**
>
> 强密码同时约束长度、字符种类与连续三段。插入、删除、替换对三类缺口的收益不同，不能只算一种操作。
>
> 按长度分类：$n<6$ 时插入既补长度也补种类；$6\le n\le 20$ 时用替换拆开长度为 $3$ 的连续段，并与缺的种类取最大；$n>20$ 必须先删除，且优先删掉「长度 $\bmod 3=0$」的连续段，因为一次删除就能少一次替换。
>
> 分类讨论把互相耦合的三种操作拆开，使每种长度只处理当时真正必需的修改。
