...
> **思考**
>
> 链表高位在前，不能像「两数相加」那样直接从头部进位。先反转再加需要再反转回来，改指针较多。
>
> 把两条链的值依次压栈，从栈顶（低位）弹出相加，新结点头插到答案前面，进位留到下一轮。栈空仍有进位则再插一位。
>
> 栈把低位变成先处理的一端，头插自动得到高位在前的结果，无需三次反转链表。
...
> **思考**
>
> 方法一已经用两个栈从低位加到高位。方法二是同一结构的完整叙述：弹出、求和、头插，直到两栈与进位都耗尽。
