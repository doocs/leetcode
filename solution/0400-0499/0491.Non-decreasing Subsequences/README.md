...
> **思考**
>
> 所有长度至少为 $2$ 的非严格递增子序列，且结果不能重复。数组未排序，不能先排序再选。
>
> DFS 到下标 $u$：若 $nums[u]\ge last$ 则可选入；仅当 $nums[u]\ne last$ 才走「不选」分支。后者避免「跳过一个值再选相同值」与「选了再继续」重复。
>
> $last$ 初值为足够小的哨兵，保证第一个数总能被选。长度大于 $1$ 才收录。
