...
> **思考**
>
> 删除 $k$ 位后希望剩下的数字最小。相同位数的大小由最高不同位决定，因此应从高位起尽量丢掉左侧偏大的数字。
>
> 从左到右维护单调不降栈：当前位更小则弹出栈顶，相当于删掉更高位的大数，并消耗一次删除额度。删满 $k$ 位后截取剩余长度，再去掉前导零。
>
> 扫描必须自左向右：只有先删掉左侧较大数字，右侧较小数字才能进入更高位。
