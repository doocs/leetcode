...
> **思考**
>
> 重要翻转对满足 $i<j$ 且 $nums[i]>2\,nums[j]$。双重循环是 $O(n^2)$。归并时左右两半已有序，可以在合并前数跨半对。
>
> 先递归左右，再对右半每个 $nums[j]$ 推进左指针直到 $nums[i]>2\,nums[j]$，剩下的左半都构成翻转对。然后再按值归并。
>
> 计数与归并分开：计数用 $2\,nums[j]$ 比较，归并用原值，两者条件不同。
...
> **思考**
>
> 方法一改写数组。离散化 $nums$ 与 $2\,nums$ 后，从右向左把 $2\,nums[j]$ 插入树状数组，查询小于 $nums[i]$ 的个数，即右侧有多少 $j$ 满足条件。
>
> 不破坏原数组相对位置的语义之外的结构，动态维护「已扫过的 $2\,nums$」。
...
> **思考**
>
> 树状数组换成线段树，仍是离散化后从右插入 $2\,nums$、查询小于 $nums[i]$ 的点数。功能相同，结点显式表示区间。
