...
> **思考**
>
> 缩写由字母与数字交错组成，数字表示跳过的长度，且不能有前导零。若先把缩写展开再比较，数字可能很大，不必真的构造新串。
>
> 双指针扫描 $\textit{abbr}$：遇数字则累加跳过长度并拒绝前导零；遇字母时先跳过再比对字符。结束时 $\textit{word}$ 指针加上剩余跳过应恰好到末尾。
>
> 跳过与比对必须在同一趟完成，才能同时检查越界、失配与长度是否用尽。
