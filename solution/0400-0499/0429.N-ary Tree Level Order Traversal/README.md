...
> **思考**
>
> 层序结果按层分组。深度优先若不分层，会把不同层的值混在一起。
>
> 队列记录当前层：每次先记下队长，弹出这些节点写入本层列表，并把它们的孩子入队。空根返回空列表。
>
> 用队长界定一层，保证同一层从左到右，且孩子在下一轮才被处理。
...
> **思考**
>
> 方法一依赖队列。若按深度递归，把节点值追加到 $\textit{ans}[i]$，层数不足时先扩一层，同样能得到层序。
>
> 少一个队列，递归栈承担层信息；同一层仍按孩子从左到右的访问顺序写入。
