...
> **思考**
>
> LFU 要在 $O(1)$ 内读写，容量满时淘汰频率最低且最久未用的键。单一哈希表能取值，不能同时维护频率与 LRU 次序。
>
> 键到结点的哈希表保证查找；$\textit{freq}\to$ 双向链表存放同频结点，新结点插到链表头表示最近。另记 $\textit{minFreq}$，淘汰时删该链表尾。
>
> 访问或更新后把结点移到 $\textit{freq}+1$ 的链表；若原链表空且频率等于 $\textit{minFreq}$，最小频率加一。新键频率为 $1$，最小频率重置为 $1$。
