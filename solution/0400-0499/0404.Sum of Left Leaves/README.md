...
> **思考**
>
> 需要把「左孩子且为叶子」与普通左子树分开。若对每个节点都再扫一遍子树，会重复访问。
>
> 一次递归即可：右子树一律下探；左孩子若无子女则累加其值，否则继续递归。空树返回 $0$。判定叶子用左右指针同为空，避免额外遍历。
...
> **思考**
>
> 方法一的判定规则不变，只是把递归改成显式栈：弹出节点后，左叶子直接累加，否则将左孩子入栈；右孩子存在则入栈。
>
> 渐近复杂度相同，实现上去掉递归深度限制。
