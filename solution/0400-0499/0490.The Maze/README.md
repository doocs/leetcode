...
> **思考**
>
> 球沿一个方向滚到墙前才停下，判断能否停在终点。逐步走格会把「路过」和「停下」混在一起。
>
> DFS：从当前停点沿四向一直滚到障碍或边界，对停点递归。访问数组标记停点，避免重复。终点被访问过则可达。
>
> 内层 $\textit{while}$ 只滚动、不标记路过的格子，状态空间是停点而不是每一个空格。
...
> **思考**
>
> 方法一用递归。同样的滚动规则改成队列 BFS：停点入队，滚到终点可提前返回。无递归深度，适合只问可达性。
