...
> **思考**
>
> 把债务清零的最少转账次数。余额为 $0$ 的人可以忽略。对剩余 $m$ 个人枚举转账顺序，$m\le 12$ 仍偏大，但可以按子集划分。
>
> 一个和为 $0$ 的子集最少需要 $|S|-1$ 次转账。令 $f[i]$ 为集合 $i$ 清零的最少次数：仅当收支和为 $0$ 时，$f[i]$ 不超过 $|i|-1$，再与所有非空真子集之和取最小。
>
> 子集枚举是 $O(3^m)$。先把交易收成余额数组，再只对和为零的集合做转移。
