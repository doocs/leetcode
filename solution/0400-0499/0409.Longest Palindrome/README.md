...
> **思考**
>
> 回文至多一个出现奇数次的字符，其余必须成对。不必枚举排列，只需看每个字符能贡献多少对。
>
> 统计频次后，每个次数 $v$ 取 $v//2\times 2$ 累加；若总和仍小于 $|s|$，还可以再放一个字符到中心。
>
> 先取偶数对、再决定中心，保证长度最大且构造存在。
...
> **思考**
>
> 方法一先完整计数再求和。也可以边扫边用异或标记每个字符的奇偶，并维护奇数种类数 $\textit{cnt}$：答案为 $n-\textit{cnt}+1$（留一个中心）或 $n$。
>
> 少一次对频次数组的二次遍历，空间仍是字符集大小。
