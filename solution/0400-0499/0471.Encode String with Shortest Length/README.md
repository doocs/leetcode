...
> **思考**
>
> 要把 $s$ 收成最短编码，可整体写成 $k[t]$，也可在某处切开分别编码。$n\le 150$，适合区间 DP。
>
> $f[i][j]$ 为 $s[i..j]$ 的最短编码。先用「倍增串查找周期」得到整段压缩 $g(i,j)$（长度小于 $5$ 则不压），再枚举切点与 $f[i][k]+f[k+1][j]$ 比长度。$i$ 从大到小、$j$ 从小到大，保证子区间已算完。
>
> 周期检测复用第 $459$ 题的技巧；括号里填的是周期串自己的最短编码，而不是原文。
