...
> **思考**
>
> 枚举所有长度至少为 $3$ 的子数组再验公差，需要 $O(n^2)$。$n\le 5000$ 勉强可行，但相邻公差一旦确定，整段贡献可以闭式累加。
>
> 一次扫描维护当前公差与连续段内新增切片数 $\textit{cnt}$：公差不变则 $\textit{cnt}$ 加一并计入答案，否则重置。
>
> 长度为 $L$ 的等差段恰好贡献 $1+2+\cdots+(L-2)$ 个切片，边走边加 $\textit{cnt}$ 即完成求和。
