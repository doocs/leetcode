...
> **思考**
>
> 去掉旧破折号后，从右每 $k$ 个字符一组，第一组可以更短，字母须大写。从右扫描再反转也可以，但要先知道第一组长度。
>
> 有效字符个数对 $k$ 取模得到第一组长度（模为 $0$ 则整组为 $k$）。从左写入大写，计数用尽就加破折号并重置为 $k$，最后去掉可能多写的尾部破折号。
>
> 第一组长度先算出来，从左输出就能保持「首组较短」而不用二次反转。
