...
> **思考**
>
> 一维接雨水可用左右最高柱，二维无法用两根指针。水从边界溢出，某格最终水位由「从边界进入的最低瓶颈」决定。
>
> 把边界高度放入小根堆并向内扩展：弹出当前最低围栏 $h$，邻格若更矮则积 $h-\textit{height}$，再以 $\max(h,\textit{height})$ 作为新围栏入堆。
>
> 小根堆保证先处理最矮瓶颈，水位单调不降，每个格子只入堆一次。
