...
> **思考**
>
> 枚举全部分割点组合，段数与长度都会爆炸。观察到：子数组和的上限越大，所需段数越少，可行性关于上限单调。
>
> 对上限 $\textit{mid}$ 二分，贪心从左累加，超过 $\textit{mid}$ 就新开一段，判断段数是否不超过 $k$。左界为 $\max(\textit{nums})$，右界为总和。
>
> 二分找最小可行上限，把组合搜索换成线性校验。
