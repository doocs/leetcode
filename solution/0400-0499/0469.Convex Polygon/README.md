...
> **思考**
>
> 凸多边形要求所有转向同号。枚举每三个顶点的夹角方向即可，不必求凸包。
>
> 对顶点 $i$，算 $\overrightarrow{p_i p_{i+1}}\times \overrightarrow{p_i p_{i+2}}$。叉积非零时若与上一次非零叉积异号，则出现凹角。共线（叉积为 $0$）不更新方向。
>
> 下标对 $n$ 取模封闭多边形。只在转向真正发生时比较符号，共线点不会误判。
