...
> **思考**
>
> 几近缺失整数在全部长度为 $k$ 的窗里恰好出现一次。不同 $k$ 下候选集合不同，统一枚举每个值的出现次数会漏掉窗口覆盖关系。
>
> $k=1$ 时每个元素自成窗口，答案是出现恰好一次的最大值；$k=n$ 时只有一个窗口，答案是全局最大。
>
> $1<k<n$ 时，中间元素至少落在两个窗里，只有两端 $\textit{nums}[0]$、$\textit{nums}[n-1]$ 可能恰好出现一次。检查它们是否在别处重复，取较大者，否则 $-1$。
