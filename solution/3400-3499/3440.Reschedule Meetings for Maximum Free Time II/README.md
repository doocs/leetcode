...
> **思考**
>
> 与 I 不同，这里只能移动一个会议，且必须放进某段已有空闲（不能与其它会议重叠）。$n\le 10^5$，需线性处理。
>
> 若会议 $i$ 只在原地左右平移，空闲为 $r_i-l_i-w_i$。若它能整段搬进左侧或右侧的某段空闲，则原来占据的 $[l_i,r_i]$ 整段空出。
>
> 预处理 $\textit{pre}[i]$、$\textit{suf}[i]$ 为左右两侧的最大空闲。枚举被搬走的会议 $i$，能搬则答案取 $r_i-l_i$，否则取 $r_i-l_i-w_i$。
