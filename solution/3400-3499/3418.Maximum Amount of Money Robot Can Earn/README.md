...
> **思考**
>
> 机器人从左上走到右下，只能向右或向下，并至多感化两次强盗（把负格当作 $0$）。网格 $500\times 500$，若再乘上感化次数，状态量约为 $500\times 500\times 3$，记忆化可行。
>
> 朴素路径计数不含「在此格是否动用一次感化」这一维，会把不同剩余次数的最优混在一起。
>
> 状态 $(i,j,k)$ 表示从该格出发、仍可感化 $k$ 次的最大收益。转移先加上本格金币再走向下或右；若本格为负且 $k>0$，还可不取本格金币并消耗一次感化。终点按剩余次数决定是否把负数抬成 $0$。
