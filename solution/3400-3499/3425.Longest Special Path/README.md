...
> **思考**
>
> 特殊路径要求树上一段路径中结点值互不相同（或按题意至多一种重复）。$n\le 5\times 10^4$，不能枚举所有路径。
>
> 树路径的窗口可以用 DFS 深度与「每种值上次出现的深度」来刻画：一旦当前路径上出现重复值，左端必须收到该值上一次出现位置之后。
>
> 因此从根向下走，维护前缀边权和与各值最近下标，用双指针把合法起点压在栈里。路径长度与结点数同时更新，得到最长特殊路径及其最少结点数。
