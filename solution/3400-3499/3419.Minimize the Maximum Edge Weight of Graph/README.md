...
> **思考**
>
> 需要让每个点都能沿反向边走到 $0$，且每个点出度不超过 $\textit{threshold}$，同时最小化所用边的最大权。$n\le 10^5$，不能枚举边子集。
>
> 「最小化最大值」适合二分答案。判定时只保留权 $\le x$ 的边，再检查是否存在一份满足出度限制、且全部点可达 $0$ 的子图。
>
> 把边反向后，从 $0$ 做 BFS/DFS：每个点至多保留 $\textit{threshold}$ 条入边（原图出边）。若全部点被访问，则 $x$ 可行。二分最小可行 $x$。
