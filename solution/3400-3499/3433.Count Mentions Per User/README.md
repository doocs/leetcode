...
> **思考**
>
> 事件带时间戳，且离线用户在 $60$ 秒内对 HERE 不可见。若按输入顺序处理，同一时刻的 OFFLINE 可能排在 MESSAGE 之后，状态会错。
>
> 先按时间排序，时间相同时把 OFFLINE 放在 MESSAGE 前，再按类型更新。
>
> $\textit{online\_t}[i]$ 记录用户 $i$ 重新上线的时刻；ALL 只累加懒标记 $\textit{lazy}$，最后统一加到每个人。HERE 则扫描当前已上线的用户。具名提及直接加一。
