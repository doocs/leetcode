...
> **思考**
>
> 会议已按时间排好且互不重叠。重排至多 $k$ 个会议，等价于把它们挤到一边，从而合并两侧的空闲段。
>
> $n\le 10^5$，不能枚举被移动的子集。相邻空闲段有 $n+1$ 个，移动 $k$ 个会议最多把连续 $k+1$ 段空闲拼在一起。
>
> 先算出各空闲长度放入 $\textit{nums}$，再对长度为 $k+1$ 的窗口求和取最大。窗口和即一次合法重排能得到的最长空闲。
...
> **思考**
>
> 方法一已经在线性时间内得到正确窗口和，但额外数组保存了全部空闲长度。
>
> 第 $i$ 段空闲由端点公式直接算出，不必预先存下。
>
> 用 $f(i)$ 在滑动窗口中即时取值，空间降为 $O(1)$，时间和结论与方法一相同。
