...
> **思考**
>
> 统计使左右两段和之差为偶数的切分。差与两段和的奇偶完全由「总与左段是否同奇偶」决定，不必每次重算整段。
>
> $l-r$ 为偶数当且仅当 $l$ 与 $r$ 同奇偶，也即 $2l$ 与总和同奇偶——实际上 $l-r=2l-\textit{total}$，故只需 $l$ 与 $\textit{total}/2$ 的整数差为偶数，等价于 $\textit{total}$ 为偶数时任意切分都合法，但实现上直接维护 $l,r$ 更直观。
>
> 从左向右把元素从 $r$ 移到 $l$，对前 $n-1$ 个切点检查 $(l-r)\bmod 2=0$。
