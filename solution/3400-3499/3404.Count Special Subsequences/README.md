...
> **思考**
>
> 特殊子序列要求下标满足间隔条件，且 $a/b=c/d$。直接四重枚举在 $n\le 1000$ 时为 $O(n^4)$，不可行；即使枚举两对端点也需避免重复的比例判断。
>
> 比例相等等价于约分后的有序对相同。若先固定右侧对 $(c,d)$ 的约分结果，再查左侧对 $(a,b)$，则每次查询为 $O(1)$。
>
> 为此先统计所有合法 $(r,s)$ 的约分键 $(\lfloor d/g\rfloor,\lfloor c/g\rfloor)$。随后按 $q$ 从左向右扫描：用已有计数回答以 $q$ 为第二元的询问，再把将要越过的 $(c,d)$ 从计数中删去，保证 $p<q<r<s$ 的间隔。
