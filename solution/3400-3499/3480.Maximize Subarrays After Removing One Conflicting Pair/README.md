...
> **思考**
>
> 合法子数组不能同时包含任一冲突对的两端。删掉恰好一对后，合法子数组尽量多。$n$ 与冲突数需线性处理。
>
> 不删时，左端 $a$ 的右端不能碰到「$a$ 及其右侧出现过的最小冲突右端 $b_1$」。贡献为 $b_1-a$。
>
> 若删掉造成 $b_1$ 的那对，右端放到次小 $b_2$，多出 $b_2-b_1$。按 $b_1$ 累加这些增量，加上不删时的总和，再取最大增量。
