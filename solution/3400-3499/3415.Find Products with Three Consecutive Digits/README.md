...
> **思考**
>
> 需要找出名称中含有连续三位数字的产品。若用循环逐字符数长度，边界（数字夹在字母或串端）容易写漏。
>
> 正则一次扫描即可同时处理「恰好三段数字被非数字或边界隔开」的情形。模式 $(^|[^0-9])[0-9]{3}([^0-9]|$)$ 避免把更长数字串中的局部三段误判为独立片段的同时，仍能命中孤立的三位。
>
> 过滤后按 $\textit{product\_id}$ 排序返回，与题面输出顺序一致。
