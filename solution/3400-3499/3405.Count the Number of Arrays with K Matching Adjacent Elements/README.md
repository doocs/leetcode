...
> **思考**
>
> $n,m\le 10^5$，无法枚举数组。长度为 $n$ 的数组有 $n-1$ 个相邻位置，其中恰好 $k$ 处相等，其余必须相异。
>
> 相等位置把数组分成 $n-k$ 段常值块。块的划分相当于从 $n-1$ 个缝中选 $k$ 个「不切开」的位置，方案数为 $C_{n-1}^{k}$。
>
> 第一段有 $m$ 种取值，其后每段只需与前一段不同，各有 $m-1$ 种。答案因此为 $C_{n-1}^{k}\cdot m\cdot(m-1)^{n-k-1}$。阶乘与逆元预处理后，组合数与快速幂即可在对数时间内求出。
