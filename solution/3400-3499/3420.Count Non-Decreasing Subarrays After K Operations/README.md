...
> **思考**
>
> 一次操作把某个元素加一。要把子数组变成非递减，等价于把每个位置抬到它左侧的前缀最大值，代价为各处抬升量之和。$n\le 10^5$，不能对每个子数组重算。
>
> 子数组越长，所需代价单调不减。因此对每个右端，存在最远的左端使得代价 $\le k$。
>
> 用单调栈维护「作为前缀最大」的一段段贡献，滑动左端弹出过期段并退回代价。对每个右端累加合法左端个数，即答案。
