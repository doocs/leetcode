...
> **思考**
>
> $s[i]=\texttt{T}$ 强制 $ans[i..i+m)$ 等于 $t$；$\texttt{F}$ 则禁止相等。答案长 $n+m-1$，希望字典序最小，故先全部填 $\texttt{a}$。
>
> T 约束可能互相冲突，必须先落盘并标记 $\textit{fixed}$。F 约束若此时仍等于 $t$，只能改一个未固定位置。
>
> 从右往左把未固定格改成 $\texttt{b}$，尽量不动更靠左的 $\texttt{a}$。找不到可改位置则无解。
