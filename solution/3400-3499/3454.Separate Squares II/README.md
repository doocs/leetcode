...
> **思考**
>
> 与 I 相同要平分面积，但正方形重叠，简单把每个正方形切入面积相加会重复计算。总面积上界升到 $10^{15}$。
>
> 重叠并面积要用扫描线：按 $y$ 排序水平边，用线段树维护当前被覆盖的 $x$ 轴长度。
>
> 相邻两条扫描线之间的矩形高度乘以树上的覆盖长度，得到并面积随 $y$ 的前缀。再在这些段上二分（或直接扫）找到面积过半的高度。
