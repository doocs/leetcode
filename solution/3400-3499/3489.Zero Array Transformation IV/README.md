...
> **思考**
>
> 每次查询把 $[l,r]$ 中每个下标至多减 $val$（选或不选）。$n\le 10$，询问 $\le 1000$，可对每个下标单独做背包。
>
> 下标 $i$ 必须用覆盖它的若干 $val$ 恰好凑出 $\textit{nums}[i]$（或任意不超过的组合，视题面「至多减到零」而定）。询问有序，要最小前缀使全体下标都能凑齐。
>
> 对每个位置维护可达和的布尔数组，按询问顺序加入 $val$，一旦所有位置都能表示目标值，该询问下标即答案；始终不能则 $-1$。
