...
> **思考**
>
> $p$ 中恰有一个 $*$，且 $|s|,|p|\le 50$，即使回溯也能通过。真正需要保证的是：星号两侧的字面量在 $s$ 中按顺序出现，中间允许任意（含空）间隔。
>
> 若把 $p$ 整段当成正则去匹配整串，会把「子串」误写成「全串匹配」。题目只要求 $p$ 替换后是 $s$ 的某个子串。
>
> 因此按 $*$ 拆开得到至多两段字面量，从左到右在 $s$ 中依次 $\textit{find}$，指针只前进不回退。任一段找不到则失败。
