...
> **思考**
>
> 与 I 类似，但路径上允许恰好一种值出现两次，其余仍唯一。$n\le 5\times 10^4$，仍走树上滑窗。
>
> 窗口左端现在由「第二种重复值」决定：第一次重复可以留下，再出现重复必须把左端推过更早的那次。
>
> DFS 维护每种值的出现下标列表与前缀边权。用至多一个「已使用的重复」标记收缩左端，同时更新最长路径与最少结点数。
