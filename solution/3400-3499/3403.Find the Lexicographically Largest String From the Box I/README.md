...
> **思考**
>
> 需要把 $word$ 切成 $\textit{numFriends}$ 段非空子串，并取出其中字典序最大的一段。若 $\textit{numFriends}=1$，整串就是唯一一段。
>
> $n$ 通常不大，枚举所有合法切法会重复比较同一子串。真正决定字典序的是「左端点固定时，更长的子串不会更小」。
>
> 其余朋友至少各占一个字符，故以 $i$ 为左端的一段最长为 $n-(\textit{numFriends}-1)$。枚举左端点，取该长度的切片，再在这些候选中取字典序最大者即可。
