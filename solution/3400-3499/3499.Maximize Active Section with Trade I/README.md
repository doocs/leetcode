...
> **思考**
>
> 一次交易把一段 $1$ 两侧的 $0$ 段一起翻成 $1$，收益是这两段 $0$ 的长度之和，再加原本所有 $1$。$n\le 10^5$，按段扫描。
>
> 不相邻的两段 $0$ 无法被同一次交易合并，因此只需相邻的 $0$ 段对。
>
> 双指针切出 runs：$1$ 的长度直接累加；相邻两段 $0$ 用 $\textit{pre}+\textit{cur}$ 更新 $\textit{mx}$。答案为 $1$ 的总数加 $\textit{mx}$。
