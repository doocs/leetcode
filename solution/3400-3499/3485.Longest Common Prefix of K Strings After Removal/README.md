...
> **思考**
>
> 对每个被删掉的单词，问剩下字符串中任意 $k$ 个的最长公共前缀。总长 $\le 10^5$，不能每次重算。
>
> Trie 上结点计数 $\ge k$ 的最大深度就是全局答案。删掉一个词只会让「计数恰好为 $k$」的祖先变得不足 $k$，这些深度是脆弱的。
>
> 用线段树按深度维护「仍有多少结点计数 $\ge k$」。删词时把脆弱深度减一，查询最大仍合法深度，再恢复。若 $n-1<k$ 则全 $0$。
