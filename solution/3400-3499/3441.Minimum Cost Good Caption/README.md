...
> **思考**
>
> 好标题要求每个字符都落在一段长度至少为 $3$ 的连续相同段中。修改一个字母的代价为字母表距离，$n\le 5\times 10^4$。
>
> 一段可以比 $3$ 更长，但太长不如拆成若干段。决策是「从位置 $i$ 开始下一段涂成字母 $c$、长度为 $L\ge 3$」。
>
> DP $f[i][c]$ 表示从 $i$ 做到结尾、且当前段字母为 $c$ 的最小代价；转移枚举下一段长度与字母。为复原字典序最小串，再记录前驱。滚动或按段长优化后可达线性对数。
