...
> **思考**
>
> 操作把一个字母改成相邻字母（可绕 $z\leftrightarrow a$），最多 $k$ 次，求最长回文子序列。两端配对的代价是环形字母距离。
>
> 经典 LPS 再加一维剩余次数。状态 $(i,j,k)$ 数量为 $n^2(k+1)$，在约束内可记忆化。
>
> 转移：丢掉左或右，或花 $t=\min(|s_i-s_j|,26-|s_i-s_j|)$ 把两端配成一对。区间空或单字符为边界。
