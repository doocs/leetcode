...
> **思考**
>
> V 形对角线从 $1$ 出发，沿对角走 $2,0,2,0,\ldots$，至多顺时针转一次。网格 $500\times 500$，搜索所有折线会重复经过同一状态。
>
> 状态由「上一格、方向、是否还能转」决定，记忆化后每个格子四个方向两种折次，总量线性。
>
> $\textit{dfs}(i,j,k,\textit{cnt})$ 尝试沿 $k$ 继续，或在 $\textit{cnt}>0$ 时转到 $(k+1)\bmod 4$。从每个 $1$ 出发四向搜索，加一得到含起点的长度。
