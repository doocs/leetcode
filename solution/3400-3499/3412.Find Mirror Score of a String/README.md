...
> **思考**
>
> 每个尚未配对的字符要与其左侧最近的、未标记的镜像字符配对，得分为下标差。$n\le 10^5$，若每次都向左扫描，最坏平方。
>
> 镜像关系是对合： $x$ 的镜像为 $y$ 当且仅当 $y$ 的镜像为 $x$。配对总是取「最近的未使用镜像」，这正是按字符分栈时的栈顶。
>
> 用哈希表把每种字符尚未配对的下标压栈。扫到 $x$ 时若镜像 $y$ 的栈非空，则弹出最近的 $j$ 并加上 $i-j$；否则把 $i$ 压入 $x$ 的栈。
