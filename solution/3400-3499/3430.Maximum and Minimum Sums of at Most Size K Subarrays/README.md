...
> **思考**
>
> 与子序列版不同，这里统计的是长度不超过 $k$ 的连续子数组的最大、最小值之和。$n$ 较大时，每个子数组重算最值不可行。
>
> 以右端 $i$ 为界，所有合法左端上的最大（最小）值，会在单调栈弹出时把「作为最值的次数」转移给新元素。
>
> 维护带 $\textit{shares}$ 的单调栈，并在窗口左端滑过 $i-k$ 时扣掉过期贡献。每个 $i$ 的 $\textit{MaxSum}_i+\textit{MinSum}_i$ 相加即答案。
