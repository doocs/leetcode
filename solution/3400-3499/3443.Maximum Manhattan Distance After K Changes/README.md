...
> **思考**
>
> 每一步改变方向，至多改 $k$ 次，最大化某一前缀结束后的曼哈顿距离。若搜索改哪些步，$4^{n}$ 不可行。
>
> 曼哈顿距离在平面上由最终的主导象限决定。把目标象限固定为四种对角方向之一后，朝该象限的步直接加一，其它步优先用改写次数变成加一，用尽则减一。
>
> 对 $\textit{SE}/\textit{SW}/\textit{NE}/\textit{NW}$ 各跑一遍贪心，取过程中 $\textit{mx}$ 的最大值。
