...
> **思考**
>
> 选出 $k$ 段长度至少为 $m$ 的不重叠子数组，最大化它们的元素和。$n$ 与 $k$、$m$ 的乘积需控制在 DP 可承受范围。
>
> 是否「正在取当前段」要把段长约束写进状态，否则无法保证每段 $\ge m$。
>
> 设 $f[i][j][0/1]$ 为考虑前 $i$ 个数、已完成 $j$ 段、当前是否在段内的最大和。转移跳过 $i$，或开启/延长一段（延长时前缀和差分保证长度）。取 $f[n][k][*]$。
