...
> **思考**
>
> 普通最大子段和可用 Kadane 在线性时间内求出。本题允许删掉某一种值的全部出现，等价于把这些位置视为不存在后再做最大子段和。
>
> $n\le 10^5$，不能对每个不同值重建数组。需要把「删掉值 $x$」的影响表示成对原数组贡献的修改。
>
> 若 $x$ 非负，删除只会减小或切断子段，最优往往是不删；若 $x$ 为负，删除相当于把若干负贡献抹去。按值分组下标，用线段树或前后缀 Kadane 合并被 $x$ 切开的各段，取所有选择中的最大值。
