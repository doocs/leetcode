...
> **思考**
>
> 环形数组上查询每个下标到最近相同值的距离。$n,q\le 10^5$，不能对每个询问扫描。
>
> 把数组复制一份接到后面，环就变成直线上的相邻两次出现。
>
> 正反各扫一遍，用哈希表记左右最近同值，写入 $\textit{d}$。原下标取 $\textit{d}[i]$ 与 $\textit{d}[i+n]$ 的较小者；距离 $\ge n$ 表示只有自己，答案为 $-1$。
