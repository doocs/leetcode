...
> **思考**
>
> 网格至多 $50\times 50$，按题面模拟走法即可。偶数行走从左到右，奇数行从右到左，并且沿途隔一个格子取一个。
>
> 若先生成完整之字形序列再下标取偶数位，需要额外数组；把「是否收取」做成沿途翻转的布尔量，可以在遍历时直接写入答案。
>
> 因此按行扫描，奇数行先就地反转，再用 $\textit{ok}$ 交替决定是否加入。行间连续性由 $\textit{ok}$ 跨行保持，无需记录全局下标。
