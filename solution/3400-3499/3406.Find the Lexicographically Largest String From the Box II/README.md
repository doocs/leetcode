...
> **思考**
>
> 与「盒子 I」相同，字典序最大的一段必是某个后缀的前缀，且长度不超过 $n-\textit{numFriends}+1$。若仍枚举所有左端点再两两比较，在更紧的时限下常数偏大。
>
> 整串的字典序最大后缀一旦确定，以其为起点、截到允许的最大长度，即为所求。
>
> 为此用双指针求 $\textit{lastSubstring}$：维护当前最优起点 $i$ 与挑战起点 $j$，按最长公共前缀推进；失配时丢弃已不可能更优的一段。$\textit{numFriends}=1$ 时仍直接返回原串。
