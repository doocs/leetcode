...
> **思考**
>
> 每天吃四块：奇数天拿最大值，偶数天拿次大值。$n$ 为 $4$ 的倍数且可达 $2\times 10^5$，不能搜索分组。
>
> 奇数天应独占最大的若干块；偶数天必须「浪费」一块更大的来取出次大。排序后这两种贡献的位置是固定的。
>
> 设奇数天 $\textit{odd}=\lceil\textit{days}/2\rceil$。答案先加上最大的 $\textit{odd}$ 块，再从剩余里每隔一块取一个作为偶数天的次大值。
