...
> **思考**
>
> 与 I 不同，这里要在长度至少为 $k$ 的子串上最大化「奇数频次 $a$ 减偶数频次 $b$」。字符只有五种，但子串有 $O(n^2)$ 个。
>
> 差 $f_a-f_b$ 是前缀计数之差。奇偶约束把前缀状态压成两位，滑动左端即可查询最小的 $\textit{preA}-\textit{preB}$。
>
> 枚举有序对 $(a,b)$，右端前进并在长度与 $b$ 的次数足够时收左端，用 $t[2][2]$ 存各奇偶的最小前缀差，再与当前 $\textit{curA}-\textit{curB}$ 组合。
