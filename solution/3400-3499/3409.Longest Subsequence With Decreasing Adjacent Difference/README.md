...
> **思考**
>
> 子序列要求相邻差的绝对值严格递减。$n\le 10^4$，若按子序列子集枚举不可行；值域却只有 $[1,300]$，相邻差至多 $299$。
>
> 决策只依赖于「当前结尾的值」与「上一段相邻差」，据此转移可以在值域上完成。
>
> 设 $f[v][d]$ 为以值 $v$ 结尾、最后一段差为 $d$ 的最长长度。加入 $x$ 时枚举上一个值 $y$ 与差 $d>|x-y|$，用 $f[y][d]$ 更新 $f[x][|x-y|]$。值域有限，总时间可接受。
