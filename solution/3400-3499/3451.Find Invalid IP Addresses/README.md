...
> **思考**
>
> 统计日志里非法 IPv4 及其出现次数。规则是四段、每段为无前导零的 $0..255$。
>
> SQL 正则能写，但前导零与区间一起约束时更易出错；在 Pandas 里拆段判断更直观。
>
> 按 `.` 分成四段，逐段检查是否为数字、数值范围以及 $\textit{str}(\textit{int})$ 复原。非法行按 IP 计数，再按次数与地址降序排序。
