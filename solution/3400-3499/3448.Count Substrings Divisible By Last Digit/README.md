...
> **思考**
>
> 子串看成十进制数，要能被其末位整除。$n\le 10^5$，不能枚举全部子串再做大整数除法。
>
> 末位 $d\in[1,9]$。可整除等价于数值模 $d$ 为 $0$。从左到右维护「以当前为末位、模 $d$ 为 $r$ 的前缀个数」。
>
> 设前缀模为 $p$，则子串 $[l,i]$ 模 $d$ 为 $0$ 当且仅当 $p\equiv 10^{i-l+1}\cdot(\textit{prefix}_{l-1})\pmod d$。按末位分组做数位 DP / 前缀计数，遇到 $0$ 则该末位无贡献。
