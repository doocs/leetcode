...
> **思考**
>
> 对应位置差的绝对值之和是不重排时的代价。重排需要先付 $k$，之后可以把 $\textit{arr}$ 的元素任意对应到 $\textit{brr}$。
>
> $n\le 10^5$，不能搜索排列。把两个数组排序后按位配对，正是使 $\sum|a_i-b_i|$ 最小的对法。
>
> 因此算两种方案：不分割的 $c_1$，以及排序后配对再加上 $k$ 的 $c_2$，取较小者。部分重排不会优于「完全不排」或「付 $k$ 后全排」。
