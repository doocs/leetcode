...
> **思考**
>
> 击败顺序有 $n!$ 种，$n \le 17$ 无法直接枚举。每天伤害等于「已击败数 $+1$」，同一存活集合上的最优天数可由该集合完全描述。
>
> 用二进制 $\textit{mask}$ 表示仍存活的怪物，令 $dfs(\textit{mask})$ 为清空该集合的最少天数。当前法力由已击败数量决定，对 $\textit{mask}$ 中每位枚举下一个击败目标并记忆化，状态数 $2^n$，每次转移 $O(n)$。
...
> **思考**
>
> 方法一的转移已经是 $O(2^n n)$，但仍依赖递归与缓存。同一转移可按 $\textit{mask}$ 从小到大填表：令 $f[\textit{mask}]$ 表示已击败集合为 $\textit{mask}$ 的最少天数，由去掉其中一只怪物的前驱转移而来。渐近复杂度不变，实现改为自底向上；此处位为 $1$ 表示已击败，与方法一相反。
