...
> **思考**
>
> 对每个起点 $i$ 向右扫描求最短使 OR 最大的子数组，最坏 $O(n^2)$，$n \le 10^5$ 不可行。从 $i$ 出发的最大 OR 由「每一位最靠左的 $1$」决定，这些位置只会出现在 $i$ 右侧。
>
> 倒序扫描，用长度 $32$ 的数组记下每位最近一次出现 $1$ 的下标。若当前位已是 $1$ 则更新该下标；否则该位必须延伸到更右侧的记录，子数组长度取这些延伸的最大值。
