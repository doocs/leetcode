...
> **思考**
>
> 经典 LIS 为 $O(n^2)$，$n \le 10^5$ 无法通过，且本题还要求相邻差不超过 $k$。以值 $v$ 结尾的最长长度等于区间 $[v-k,v-1]$ 上的最大 $f$ 再加一，需要在值域上做区间最值查询与单点更新。
>
> 值域为 $[1,10^5]$，用线段树维护 $f[v]$ 的区间最大值：对每个 $v$ 先查询再更新，即可在 $O(n\log V)$ 内完成。
