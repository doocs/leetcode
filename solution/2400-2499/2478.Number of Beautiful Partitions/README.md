...
> **思考**
>
> 每段必须以质数数字开头、非质数结尾，长度至少 $\textit{minLength}$，共 $k$ 段。$n \le 1000$，令 $f[i][j]$ 为前 $i$ 个字符分成 $j$ 段的方案。仅当 $s[i-1]$ 可作段尾时，$f[i][j]$ 等于此前合法段尾的 $f[\cdot][j-1]$ 之和。
>
> 用前缀和 $g$ 把求和变成 $g[i-\textit{minLength}][j-1]$。首字符非质数或末字符为质数则无解。
