...
> **思考**
>
> 每一步仅能左移或右移一格，若枚举全部长度为 $k$ 的路径，方案数为 $2^k$，$k \le 1000$ 无法接受。按绝对坐标与剩余步数记忆化，坐标落在长度为 $O(k)$ 的区间内，状态量已与 $k^2$ 同阶，但数轴可任意平移，不必保存真实位置。
>
> 注意到方案数只由「当前点到终点的距离」与剩余步数决定：朝终点走一步距离变为 $|i-1|$，反向则变为 $i+1$。因此令 $dfs(i,j)$ 表示距离为 $i$、还剩 $j$ 步的方案数并记忆化。若 $i>j$，剩余步数不足以走完这段距离，可直接返回 $0$。
