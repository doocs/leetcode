...
> **思考**
>
> $n \le 4000$，每次操作删去一段与其后紧邻相等的前缀。若搜索所有删法，状态可由剩余后缀的起点 $i$ 描述。从 $i$ 出发枚举长度 $j$，切片比较 $s[i:i+j]$ 与 $s[i+j:i+2j]$，相等则可转移到 $i+j$ 并多一次操作；也可一次删完全部，答案至少为 $1$。
>
> 对 $dfs(i)$ 记忆化，比较代价与长度同阶，总时间 $O(n^2)$，对本题足够。
...
> **思考**
>
> 方法一每次用切片判等，常数较大，且递归有缓存开销。先预处理 $g[i][j]$ 为 $s[i:]$ 与 $s[j:]$ 的 LCP，则 $g[i][i+j]\ge j$ 即可 $O(1)$ 判断。自后向前填 $f[i]$，空间降为 $O(n)$ 的一维表外加 LCP 表。
