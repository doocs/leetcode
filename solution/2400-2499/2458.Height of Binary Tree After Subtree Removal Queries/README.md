...
> **思考**
>
> 每次删除一个非根子树后问树高，$n \le 10^5$、$q \le 10^4$，不能每次重算。删去 $x$ 后的高度，是根到其他叶子的最长链，等于「经过 $x$ 的兄弟侧」与「祖先上其他分叉」的较大者。
>
> 第一次 DFS 记下每棵子树高度 $d$。第二次带着「删除当前点后的高度 $\textit{rest}$」下行：进入左子时，$\textit{rest}$ 与「当前深度 $+$ 右子高度」取大；右子对称。结果写入 $\textit{res}[val]$。
...
> **思考**
>
> 方法一用两次 DFS 预填每个点删除后的高度。也可以一次 DFS 记录每层节点的「子树最深」，层内按该深度排序。查询 $q$ 时看其所在层：若它贡献了层上最大深度，则改用次大；否则仍用最大。同层仅一个节点时高度退回 $level-1$。
