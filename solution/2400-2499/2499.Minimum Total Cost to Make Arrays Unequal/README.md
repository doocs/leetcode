...
> **思考**
>
> 相等位置必须换出去，下标之和先计入这些强制交换。若某值在强制位置中超过一半，则无法在内部两两交换消化，还得到其他位置找与该值无关的一对，再累加那些下标。
>
> 先统计 $a_i=b_i$ 的次数；若最大频次 $v$ 满足 $2v>same$，超额 $2v-same$ 必须向外交换。再扫 $a\ne b$ 且两端都不是该主导值的位置。仍有剩余则无解。
