...
> **思考**
>
> 枚举全部子数组再检查两两按位与是否为 $0$，子数组有 $O(n^2)$ 个，$n \le 10^5$ 无法通过。对每个右端点向左扫描，最坏仍是平方级。
>
> 「任意两数按位与为 $0$」等价于各数置 $1$ 的二进制位互不重叠，因此窗口内所有数的按位或即可作为占用位集合。窗口一旦与新元素冲突，必须从左侧移出元素直至冲突消失，故左端点单调右移。
>
> 用 $\textit{mask}$ 维护窗口按位或，右指针扫描；冲突时用异或撤销左端元素。每个下标进出窗口至多一次，即可在线性时间内得到最长合法窗口。
