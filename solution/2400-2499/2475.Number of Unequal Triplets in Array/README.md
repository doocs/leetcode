...
> **思考**
>
> $n \le 100$，三元组下标有序即可。三重循环比较三数互不相等，约束很小，可以接受。
...
> **思考**
>
> 方法一已是 $O(n^3)$。排序后相等值相邻：枚举中间下标 $j$，左侧严格小于 $nums[j]$ 的个数与右侧严格大于的个数之积即贡献。两次二分即可定位边界。
...
> **思考**
>
> 方法二仍付排序与二分。三元组只要求三值两两不同，与下标无关，故按值计数：枚举中间值的次数 $b$，左侧已扫过的个数 $a$，右侧 $c=n-a-b$，累加 $a\cdot b\cdot c$。线性。
