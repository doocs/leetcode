...
> **思考**
>
> 第二大元素是右侧第二个严格更大的值。$n \le 10^5$，从大到小处理时，已出现的下标都对应更大的值。在有序集合中查 $i$ 右侧的第二个下标即可。
>
> 将 $(值,下标)$ 按值降序，遍历时 $\textit{bisect}$ 有序下标集，取其后第二个位置对应的原值。
...
> **思考**
>
> 方法一依赖有序集合，多一个对数因子。用两个单调递减栈：一号栈尚未遇到第一个更大者，二号栈已遇到第一个但未遇到第二个。当前值弹出二号栈即写下答案，弹出一号栈的元素按原序转入二号栈，每个下标进出各栈常数次。
