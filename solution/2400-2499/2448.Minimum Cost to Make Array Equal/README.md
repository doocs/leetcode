...
> **思考**
>
> 把所有数变成同一 $x$ 的代价是 $\sum |a_i-x|b_i$，$n \le 10^5$ 不能对每个可能 $x$ 暴力。按 $a$ 排序后，代价由左右加权和组成，可用 $a_i b_i$ 与 $b_i$ 的前缀和在枚举 $x=a_i$ 时 $O(1)$ 求出。
>
> 最优 $x$ 必取某个 $a_i$（分段线性），故枚举排序后的每个位置取最小代价。
...
> **思考**
>
> 方法一枚举了每个 $a_i$ 作为目标。将 $b_i$ 视为 $a_i$ 的出现次数后，加权中位数即为最优目标，只需找到前缀权超过一半的那个 $a$，再算一次总代价。
