...
> **思考**
>
> 长度为 $5$ 的回文子序列形如 $abxba$。$n \le 10^4$、字符为数字，枚举中心 $i$ 与两端数字对 $(j,k)$，左侧 $jk$ 出现次数乘右侧 $jk$ 出现次数。
>
> 预处理前缀 $\textit{pre}[i][j][k]$ 与后缀 $\textit{suf}$：扫到数字 $v$ 时，对每个已出现的 $j$ 累加二元组 $(j,v)$。中心两侧相乘后取模。
