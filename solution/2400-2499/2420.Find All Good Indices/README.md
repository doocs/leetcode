...
> **思考**
>
> $n \le 10^5$，对每个下标 $i$ 单独检查左右各 $k$ 个元素是否非增/非减，最坏接近 $O(nk)$。左右条件互不依赖，可预先算出「以 $i-1$ 结尾的非增长度」与「以 $i+1$ 开头的非增长度」。
>
> 从左递推 $\textit{decr}[i]$，从右递推 $\textit{incr}[i]$，再在 $[k,n-k)$ 上判断两端是否都不短于 $k$。注意题设左右都是非递增（右侧为 $\textit{nums}[i+1] \ge \cdots$ 的反向叙述），与代码中两次比较 $ \le $ 一致。
