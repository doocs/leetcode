...
> **思考**
>
> 用尽量少的正方形铺满 $n\times m$ 矩形，$n,m \le 13$，状态可用每行一个比特掩码表示占用。朴素切分方案过多，但可用已用块数剪枝。
>
> 按行优先扫描第一个空格，枚举从该格能放下的边长，写入 $filled$ 后递归下一空位；回溯时异或清除。若当前块数已不优于已知答案则停止。状态压缩使「是否占用」成为位运算，回溯保证覆盖完整。
...
> **思考**
>
> 方法一从小到大放置正方形。较大块更能立刻降低剩余空洞，对 $ans$ 的剪枝更早生效。方法二先铺满当前位置能放的最大正方形，再从大到小收缩边框并继续搜索，转移集合相同，只是枚举顺序利于剪枝。
