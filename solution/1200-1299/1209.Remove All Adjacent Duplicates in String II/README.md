...
> **思考**
>
> 反复扫描删除连续 $k$ 个相同字符，$n \le 10^5$ 时最坏会多次重扫。删除具有局部性：只有相邻的相同字符会合并，且删除后两侧可能再相遇。
>
> 栈按从左到右的顺序保存尚未消除的字符及其连续次数。当前字符与栈顶相同则累加；次数达到 $k$ 则整段弹出，相当于一次消除。
>
> 为此栈中存 $(字符, 次数)$，入栈时对 $k$ 取模以便刚好满 $k$ 时弹出。一遍扫描后栈内剩余即最终串。
