...
> **思考**
>
> 要把巧克力分成 $k+1$ 段并最大化自己那一段的最小甜度。$n \le 10^4$，枚举切点不可行。若最小甜度 $x$ 可达，则更小的阈值也可达，具有单调性。
>
> 判定时从左到右累加，每凑满 $x$ 就切一刀；能切出多于 $k$ 段（自己再加 $k$ 位朋友）则 $x$ 可行。在 $[0,\sum sweetness]$ 上二分最大可行 $x$。贪心切分保证判定正确：尽早成段只会留下更多剩余去凑后面的段。
