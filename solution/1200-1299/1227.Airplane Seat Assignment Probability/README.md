...
> **思考**
>
> $n$ 达 $10^5$，不能模拟每位乘客的随机选择。第 $n$ 位能坐到自己座位的概率满足：一人时为 $1$，此后乘客 $1$ 要么坐对、要么坐到 $n$ 的位子、要么坐到某位中间乘客的位子并把问题缩成规模更小的同构实例。
>
> 该递推化简后，$n\ge 2$ 时概率恒为 $1/2$。因此直接按 $n$ 是否为 $1$ 返回 $1$ 或 $0.5$，无需迭代。
