...
> **思考**
>
> $f$ 对两个自变量均严格递增，$z \le 100$ 且解在 $1\ldots 1000$。双重线性枚举最坏 $10^6$ 次调用，可以接受但可再降。固定 $x$ 后 $f(x,\cdot)$ 单调，可在 $y$ 上二分定位等于 $z$ 的位置。
>
> 对每个 $x$ 二分 $y$，命中则记录。单调性把内层从线性变成对数。
...
> **思考**
>
> 每个 $x$ 各做一次二分，调用次数仍带对数因子。从 $(1,1000)$ 出发：值偏小只能增大 $x$（再减小 $y$ 会更小），值偏大只能减小 $y$。相等则收入并同时移动两端。总调用 $O(z)$ 级，利用了双单调性。
