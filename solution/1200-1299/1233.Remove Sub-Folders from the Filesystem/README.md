...
> **思考**
>
> 子文件夹在字典序下一定排在其父文件夹之后，且以父路径加 $/$ 为前缀。$n$ 达 $4\times 10^4$，两两检查前缀过慢。
>
> 排序后只需与答案中最近保留的文件夹比较：若当前路径是它的子孙则丢弃，否则它本身是新的根文件夹。排序把父子关系变成相邻前缀关系，一遍扫描即可。
...
> **思考**
>
> 排序依赖全局字典序，比较字符串前缀的常数较大。字典树按路径分量插入，节点标记文件夹终点；搜索时一旦碰到终点便整棵子树都是子文件夹，无需再往下走。时间与路径总长成正比，省去排序。
