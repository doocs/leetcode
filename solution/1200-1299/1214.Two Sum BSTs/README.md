...
> **思考**
>
> 在一棵 BST 中查找 $target-x$ 需 $O(n\log n)$ 次检索，节点数达 $5000$ 可以接受，但要处理两棵树的配对。中序遍历得到两个有序数组后，问题化为有序数组的两数之和。
>
> 两指针分居 $nums[0]$ 左端与 $nums[1]$ 右端：和偏小则左指针右移，和偏大则右指针左移。BST 的中序保证有序，双指针保证线性配对。
