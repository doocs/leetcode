...
> **思考**
>
> 一支球队可能从未出场，内连接会丢掉其零分记录。左连接 $Teams$ 与 $Matches$，条件为担任主队或客队，使无赛球队仍保留一行。
>
> 积分只取决于该队在该场是主是客以及进球比较：胜 $3$、平 $1$、负 $0$。$CASE$ 按行给出当场得分，再按 $team\_id$ 求和。
>
> 最后按积分降序、$team\_id$ 升序输出。左连接保证名单完整，$CASE$ 把胜平负规则写成可聚合的标量。
