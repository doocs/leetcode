...
> **思考**
>
> 开盒子需要先拥有该盒且状态为开（或已有钥匙）。盒子、钥匙互相嵌套，$n \le 1000$，适合按「当前能打开的盒子」做 BFS。
>
> 集合 $has$ 记录拥有的盒子，$took$ 防止重复处理。队列只放已打开且未处理的盒子：取出后用钥匙把关闭盒打开，并用其中嵌套盒扩充 $has$。初始开启盒入队并累加糖果，之后每打开一盒加一次糖果。
