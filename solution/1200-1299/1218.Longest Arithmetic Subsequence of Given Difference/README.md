...
> **思考**
>
> 子序列需保持原相对顺序且公差固定为 $difference$。$n \le 10^5$，枚举子集不可行。以 $x$ 结尾的最长长度只依赖以 $x-difference$ 结尾的长度，且该前驱若存在必已在此前出现。
>
> 从左到右扫描，令 $f[x]=f[x-difference]+1$。哈希表按值索引前驱，一遍得到所有结尾处的最长长度，取最大即可。
