...
> **思考**
>
> 子串费用是对应位置 ASCII 差的绝对值和。$n \le 10^5$，枚举全部 $O(n^2)$ 子串不可行。
>
> 固定长度时，存在费用不超过 $maxCost$ 的子串具有单调性：长度 $x$ 可行则 $x-1$ 也可行。任意区间费用可由前缀和 $O(1)$ 求出。
>
> 为此先建差分数组的前缀和，再对长度二分，并用滑动窗口式枚举验证。二分利用单调性，前缀和去掉内层求和。
...
> **思考**
>
> 方法一额外需要 $O(n)$ 前缀数组，总时间为 $O(n\log n)$。费用非负，窗口右端右移时费用只增，左端右移时费用只减，因此可用双指针在一次扫描中维护最大可行窗口，线性时间且常数空间。
...
> **思考**
>
> 方法二在费用超标时可能把窗口缩短后再拉长，并显式维护答案。题目只要最大长度，可让左指针至多前进、窗口单调变长，最终 $n-l$ 即为该最大长度，省去对 $ans$ 的更新。
