...
> **思考**
>
> 棋盘仅 $8\times 8$，皇后不足 $64$ 个。能攻击国王的皇后必须与国王同行、列或斜线，且中间无其他皇后。从国王沿八个方向外走，每个方向遇到的第一枚皇后即该方向唯一攻击者。
>
> 先把皇后坐标放入集合以便 $O(1)$ 查询，再从国王位置沿八个单位向量延伸，命中则记录并停止该方向。方向搜索保证遮挡关系，集合保证每步判定为常数。
