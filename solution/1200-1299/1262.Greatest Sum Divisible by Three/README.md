...
> **思考**
>
> 要最大子序列和能被 $3$ 整除。$n \le 4\times 10^4$，不能枚举子集。和模 $3$ 只有三种余数，选或不选当前数只在这三种状态间转移。
>
> $f[i][j]$ 为前 $i$ 个数中模 $3$ 余 $j$ 的最大和。不选则继承，选则从余数 $j-x$ 加上 $x$。答案为 $f[n][0]$。余数把目标约束压成常数个状态。
...
> **思考**
>
> 方法一用 $O(n)$ 行。第 $i$ 行只读上一行三个值，滚动长度为 $3$ 的数组即可，空间降为常数，转移不变。
