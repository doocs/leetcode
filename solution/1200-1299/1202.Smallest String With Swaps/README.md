...
> **思考**
>
> 若对每次交换都尝试所有排列，$n$ 与 $pairs$ 均可达 $10^5$，搜索空间过大。
>
> 交换关系具有传递性：可交换的下标构成若干连通分量，分量内字符可任意重排。字典序最小串即在每个分量内把字符按升序填回原下标。
>
> 并查集维护下标连通性后，按根收集字符并逆序排序，再按位置依次弹出最小字符写回。并查集给出分量，排序给出该分量的最优填充顺序。
