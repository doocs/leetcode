...
> **思考**
>
> 两行矩阵的列和给定，且两行 $1$ 的个数分别为 $upper$、$lower$。$n \le 10^5$，回溯不可行。列为 $2$ 时两行都只能填 $1$；为 $0$ 时都填 $0$；为 $1$ 时必须把仅剩的 $1$ 给当前余额更多的那一行，否则稍后可能不够用。
>
> 从左到右按此规则填写，余额变负或结束时两行余额未用尽则无解。贪心把 $1$ 留给更紧的行，局部选择不增加全局冲突。
