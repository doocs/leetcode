...
> **思考**
>
> 要把数组分成若干长度为 $k$ 的连续段。长度不能被 $k$ 整除则无解。$n \le 10^5$，从最小剩余值开始贪心取 $k$ 个连续数：若某值次数不够则失败。
>
> 计数后按值排序扫描，遇到仍有剩余的 $x$ 就对 $[x,x+k)$ 各减一。排序保证每次从当前最小缺口开始，避免打乱后续段。
...
> **思考**
>
> 方法一排序整阵并反复查哈希。有序字典按键维护剩余次数，每次取最小键开新段，次数降到零则删除。不必对重复元素排序，取最小是 $O(\log n)$。
