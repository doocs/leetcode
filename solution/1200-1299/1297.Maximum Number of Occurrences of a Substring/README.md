...
> **思考**
>
> 满足字母种类与长度约束的子串中，出现次数最大者。更长的合法串若出现 $t$ 次，其长度为 $minSize$ 的前缀也至少出现 $t$ 次，故只需统计长度为 $minSize$ 且字母种类不超过 $maxLetters$ 的子串。
>
> $n \le 10^5$、$minSize \le 26$，枚举所有该长窗口并用集合判种类，哈希表计数取最大。
...
> **思考**
>
> 方法一以字符串为键，每窗口 $O(minSize)$ 建串。滑动窗口维护种类数，字符串哈希使子串键变成 $O(1)$ 整数，总时间线性。
