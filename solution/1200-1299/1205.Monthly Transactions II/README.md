...
> **思考**
>
> 月度报表既要统计当月批准的交易，也要把退款记在退款发生的月份，二者日期字段不同，不宜在原表上直接分组。
>
> 将退款改写成与交易同构的行（状态为 chargeback、日期为退款日）后与原交易做并集，所有事件便落在同一套 $(month, country)$ 维度上。
>
> 为此用 $UNION$ 拼出事件表，再按月与国家汇总批准与退款的笔数和金额，并用 $HAVING$ 丢掉两端均为零的组。
