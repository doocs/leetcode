...
> **思考**
>
> 蛇有平移与旋转，状态为尾部格子与朝向。$n \le 100$，状态数 $O(n^2)$，求最少步数适合 BFS。
>
> 每步可向右或向下平移（头尾同时移动且不越界、不撞墙）；水平时可顺时针立起，垂直时可逆时针倒下，前提是旋转扫过的格子为空。把二维坐标压成一维下标后，用 $(尾, 朝向)$ 去重。
>
> 队列存 $(尾, 头)$，按层扩展；到达 $(n^2-2, n^2-1)$ 时的层数即答案。BFS 保证首次到达为目标最短路径。
