...
> **思考**
>
> 需找最长前缀，删除恰好一个元素后其余值的出现次数全部相同。$n \le 10^5$，不能对每个前缀重新统计。
>
> 维护值的频次 $cnt$ 与「频次的频次」$ccnt$，以及当前最大频次 $mx$。前缀合法当且仅当：全是 $1$；或次数只有 $mx$ 与 $mx-1$ 且恰有一个值达到 $mx$；或除一个只出现一次的值外全是 $mx$。
>
> 从左到右更新两个计数器，每步用 $ccnt$ 与 $mx$ 在 $O(1)$ 内检验三种形态，记录满足条件的最大下标。
