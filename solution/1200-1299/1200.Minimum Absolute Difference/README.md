...
> **思考**
>
> 若枚举全部数对计算绝对差，时间复杂度为 $O(n^2)$。$n \le 10^5$ 时不可行。
>
> 注意到排序后任意两数之差不小于它们之间相邻对的差，因此全局最小绝对差只可能出现在相邻元素上。
>
> 为此先将 $arr$ 排序，一遍扫描相邻差得到最小值 $mi$，再收集所有差恰为 $mi$ 的相邻对。排序决定了候选集合，两次线性扫描分别求极值与收集答案。
