...
> **思考**
>
> 每枚硬币正面概率不同，求恰好 $target$ 枚朝上的概率。$n \le 1000$，枚举子集不可行。前 $i$ 枚中恰有 $j$ 枚朝上的概率只依赖前 $i-1$ 枚：第 $i$ 枚反面则 $j$ 不变，正面则由 $j-1$ 转移。
>
> 令 $f[i][j]$ 为该概率，初值 $f[0][0]=1$，按硬币顺序填表，答案为 $f[n][target]$。二维表把独立试验的卷积展开成线性递推。
...
> **思考**
>
> 方法一的 $f[i][\cdot]$ 只读上一行。用一维数组并从大到小更新 $j$，避免新值覆盖尚未使用的 $f[j-1]$，空间降为 $O(target)$，转移不变。
