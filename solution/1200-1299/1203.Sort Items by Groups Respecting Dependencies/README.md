...
> **思考**
>
> 仅对项目做一次拓扑排序会打散同一小组，无法保证组内项目相邻。$n \le 3\times 10^4$，需要线性级的构图与排序。
>
> 约束分两层：组与组之间有先后，组内项目之间另有先后。无组项目可各成一组，使每个项目都属于某个组。组间边来自跨组依赖，组内边来自同组依赖。
>
> 为此先对组图拓扑，再按组序对组内项目拓扑。任一层出现环则无解。两层图把「组必须成块」与「项目先后」拆开，各做一次 Kahn 算法即可。
