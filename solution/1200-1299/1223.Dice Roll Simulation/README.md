...
> **思考**
>
> 连续同一点数受 $rollMax$ 限制，$n$ 达 $5000$，直接枚举序列不可行。方案数只依赖「还剩几次、上次点数、该点已连续几次」。
>
> 记忆化 $dfs(i,j,x)$：第 $i$ 次起、上一次为 $j$ 且已连续 $x$ 次。枚举下一点 $k$，不同则重置连续次数，相同则须 $x$ 仍低于上限。初态 $(0,0,0)$ 表示尚未掷出。
>
> 状态 $O(n\times 6\times 15)$，转移枚举 $6$ 个点数，记忆化避免重复子问题。
