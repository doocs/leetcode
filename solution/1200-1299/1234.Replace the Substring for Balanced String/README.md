...
> **思考**
>
> 平衡要求四种字符各占 $n/4$。$n \le 10^5$，不能枚举被替换子串。窗外四种计数都不超过 $n/4$ 当且仅当该窗口覆盖了全部超额字符，且窗口越长越容易满足，具有单调性。
>
> 先统计全串频次；已平衡则答案为 $0$。否则右端吞入字符（窗外计数减一），当窗外均合法时收缩左端并更新最短窗口。双指针维护「窗外已平衡」的最短覆盖。
