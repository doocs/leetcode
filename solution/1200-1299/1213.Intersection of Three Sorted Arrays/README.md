...
> **思考**
>
> 三数组均已有序且长度不超过 $1000$，值域在 $2000$ 以内。将三个数组一并计数后，值为 $3$ 的元素即公共元素；因各数组内部元素互不相同，该计数不会被同一数组重复抬高。按 $arr1$ 的顺序输出即可保持升序。
...
> **思考**
>
> 计数需要与值域成正比的额外数组。三个数组已有序，可在 $arr2$、$arr3$ 上二分判断 $arr1$ 的每个值是否存在，额外空间降为常数，时间变为 $O(n\log n)$。
