...
> **思考**
>
> 在长为 $arrLen$ 的数组上走 $steps$ 步回到原点。$arrLen$ 可达 $10^6$，但 $steps \le 500$，能到达的位置不超过 $steps$。方案数由位置与剩余步数决定。
>
> $dfs(i,j)$ 为位置 $i$、剩余 $j$ 步回到原点的方案；可左、右或停。$i>j$ 时不可能赶回，直接剪掉。记忆化状态 $O(steps^2)$。
