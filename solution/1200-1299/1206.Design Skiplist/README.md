...
> **思考**
>
> 每次操作可达 $5\times 10^4$，有序数组插入删除为线性，平衡树虽可达 $O(\log n)$ 但实现偏重。跳表用多层前向指针模拟平衡树的分层索引：底层存全部键，上层稀疏抽样。
>
> 查找、插入、删除都从最高层向下降到不超过目标的最近节点，再进入下一层，期望跳跃次数为对数。新节点层数以概率 $p$ 向上生长，使高层保持稀疏。
>
> 实现上用带头节点的多层链表，$find\_closest$ 在一层内滑到最接近目标的位置；$random\_level$ 决定新节点高度。期望每步 $O(\log n)$，空间与节点数及层高成正比。
