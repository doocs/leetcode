...
> **思考**
>
> 五位哲人共用五把叉，邻座争用同一把叉时若获取顺序不一致可能死锁。每次就餐只触及本人左手与右手两把叉。
>
> $scoped\_lock$ 按给定顺序锁定两把互斥量并在作用域结束时反向释放，保证同一对叉上的临界区互斥，而不波及不相邻的哲人。
>
> 将叉子 $i$ 与 $(i+1)\bmod 5$ 交给第 $i$ 位哲人，拿叉、进食、放叉均在锁内完成，从而避免循环等待。
