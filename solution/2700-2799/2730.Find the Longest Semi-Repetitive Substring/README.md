...
> **思考**
>
> 半重复串至多有一对相邻相同字符，求最长长度。枚举左右端点再检查相邻对，在 $n\le 10^5$ 时不可行。
>
> 右端点右移时累计相邻相等的次数；次数超过 $1$ 则左端点右移直至合法。窗口内始终最多一对重复，长度的最大值即为答案。
...
> **思考**
>
> 方法一在窗口不合法时可能连续收缩并维护全局最大。题目只要最长长度，每次越界只需把左端点右移一格，窗口长度单调不减，最后用 $n-l$ 即得最长合法窗口。
