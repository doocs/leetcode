...
> **思考**
>
> 统计 $[num1,num2]$ 中数位和落在 $[min\_sum,max\_sum]$ 的个数。区间长度可达 $10^{22}$ 量级，枚举整数不可行。
>
> 转化为 $[0,num2]$ 与 $[0,num1-1]$ 的差。数位 DP 按高位到低位填写，状态为 $(pos,s,limit)$：已放位数、当前数位和、是否贴上界。末位时判断 $s$ 是否落在给定闭区间，记忆化后对 $10^9+7$ 取模。
