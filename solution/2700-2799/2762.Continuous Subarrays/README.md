...
> **思考**
>
> 子数组不间断当且仅当最大值与最小值之差不超过 $2$，统计这样的子数组个数。枚举端点再求最值是平方，长度 $10^5$ 不可行。
>
> 右端点纳入新元素后，有序列表中的最大最小之差若超过 $2$，就弹出左端元素直至合法。此时以右端结尾的合法子数组个数等于表的大小。
...
> **思考**
>
> 有序列表每次插入删除带对数。窗口只需最大与最小，用两个单调队列维护下标，差值超过 $2$ 时弹出更靠左的极值并移动左端，计数方式相同而额外空间为线性、操作为均摊 $O(1)$。
