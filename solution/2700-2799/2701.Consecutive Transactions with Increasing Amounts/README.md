...
> **思考**
>
> 连续递增交易要求同一顾客相邻两天的金额严格上升，且连续天数不少于 $3$。逐顾客排序后再扫描可行，但用自连接把「前一天存在更小金额」标出来，可以把断点变成一次窗口聚合。
>
> 对每条记录左连前一天且金额更小的同行；连不上则该行开启新段。按顾客与日期排序后对断点求前缀和，得到段号 $s$，再按 $(customer\_id, s)$ 分组并保留长度至少为 $3$ 的区间。
