...
> **思考**
>
> 划分 $s$ 使被字典覆盖的段尽可能多，剩下的字符尽量少。枚举所有划分在 $n\le 50$ 下虽不至于过大，但重复前缀会被反复计算。
>
> 前 $i$ 个字符的最少额外数只依赖更短前缀：要么把 $s[i-1]$ 当作额外字符，要么存在 $j$ 使 $s[j..i)$ 落在字典中并接到 $f[j]$。哈希集合给出 $O(1)$ 的单词判定，按 $i$ 递增递推即可。
...
> **思考**
>
> 方法一对每个 $i$ 枚举全部 $j$ 并切片查表，字符串构造把时间推到立方。将单词逆序插入字典树后，可从 $i-1$ 向左沿树走，一旦失配即停止，去掉切片并把判定摊到边上。
