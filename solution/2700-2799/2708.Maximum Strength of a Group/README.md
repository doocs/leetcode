...
> **思考**
>
> 实力是非空子集的乘积，需取其最大。$n\le 13$，子集共 $2^n-1$ 个，枚举每个掩码并连乘对应元素即可得到全局最大。
>
> 空集被排除，负数与零也一并参与乘积，无需事先分类。
...
> **思考**
>
> 二进制枚举带 $n$ 的指数。按绝对值与符号分类后，零应尽量避开，负数成对相乘才增大乘积。排序后从左扫描：连续两个负数一并乘入，单个负数与零跳过，正数全部乘入；全零或单元素等边界单独处理。
