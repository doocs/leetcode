...
> **思考**
>
> 逐层比较两个 JSON 结构时，可对每个对象做键集合的并集再逐键对照。序列化长度不超过 $10^4$，完全展开可行，但只出现在一侧的键按题意必须丢弃，且类型不一致时整段应作为叶子差异，不能再往下拆。
>
> 因此真正需要处理的是双方共有的键。标量直接比较；对象或数组则递归得到子差异，仅当子差异非空时才写入结果。类型用内部标签区分，以免数组与普通对象被混为一谈。
