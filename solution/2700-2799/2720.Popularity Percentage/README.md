...
> **思考**
>
> 流行度是「与该用户互为好友的人数」除以用户总数。好友表只存单向边，直接按 $user1$ 分组会漏掉作为 $user2$ 出现的关系，也会把同一对重复计算。
>
> 先把 $(user2,user1)$ 并入，得到无向好友表 $F$。用户总数是 $F$ 中不同 $user1$ 的个数；对每个用户用窗口统计其度数再除以该总数并四舍五入到两位。
