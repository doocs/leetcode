...
> **思考**
>
> 把二进制串切成尽量少的段，每段无前导零且值为 $5$ 的幂。枚举切分在 $n\le 15$ 下可行，但带前导零的非法前缀应立刻剪枝。
>
> 预处理长度范围内所有 $5$ 的幂。$dfs(i)$ 从下标 $i$ 起的最少段数：若 $s[i]=0$ 则非法；否则向右累加二进制值，落在幂集合中则尝试 $1+dfs(j+1)$。记忆化后取不到有限值则返回 $-1$。
