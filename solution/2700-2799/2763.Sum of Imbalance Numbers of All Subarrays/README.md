...
> **思考**
>
> 子数组的不平衡数是排序去重后相邻差大于 $1$ 的对数，需求和。对每个子数组排序是 $O(n^2\log n)$ 再乘上构造，在 $n\le 1000$ 下偏紧但若能增量维护则可接受。
>
> 固定左端，右端每加入一个值，在有序集合中看它与前驱、后继是否形成新的大于 $1$ 的空隙，以及是否拆开原有空隙。用 $cnt$ 增量更新不平衡数并累加到答案。
