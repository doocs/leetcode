...
> **思考**
>
> 最长子数组须从偶数起手、相邻奇偶交替，且每个元素不超过 $threshold$。枚举左端再向右延伸，在 $n\le 100$ 下可以接受。
>
> 对每个满足条件的左端 $l$，向右走到第一处奇偶相同或越阈值的位置，用 $r-l$ 更新答案。
...
> **思考**
>
> 方法一会从已判定失败的内部下标重新开窗口，重复扫描同一段。合法段互不相交，找到一段后把左端直接接到 $r$，整趟变为线性。
