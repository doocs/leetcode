...
> **思考**
>
> 买第 $i$ 种巧克力的花费是环形左移若干次后该位置见过的最小值，每次左移另付 $x$。枚举每种糖的购买时机互相耦合，直接搜索不可行。
>
> 全局左移次数 $j$ 对所有种类相同，且 $j\ge n$ 不会再降低购买价。预处理 $f[i][j]$ 为第 $i$ 种糖在至多 $j$ 次左移下的最低价，再枚举 $j$ 取 $\sum_i f[i][j]+x\cdot j$ 的最小。
