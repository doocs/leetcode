...
> **思考**
>
> 完全子数组须包含原数组的全部不同元素。枚举左右端点后对每个窗口建集合，在 $n\le 1000$ 下可以接受。
>
> 先求出全局种类数 $cnt$，再枚举左端、向右扩张集合，一旦种类数达到 $cnt$ 就计入一个完全子数组。
...
> **思考**
>
> 方法一对每个左端重建集合，重复统计同一窗口。种类数随右端单调不减：右端纳入新元素后，一旦窗口已完全，以该右端为结尾、左端不超过当前 $i$ 的子数组都完全，个数为 $n-j$；再右移左端并维护计数。
