...
> **思考**
>
> 按给定顺序把单词接到当前串的左端或右端，相邻端点相同则删一个字符，求最短终长。每步两种选择，深度为 $n\le 1000$，裸搜索不可行。
>
> 终长只依赖已接串的首尾字符。$dfs(i,a,b)$ 表示从第 $i$ 个词起、当前首尾为 $a,b$ 时还需付出的最小长度；接到右侧比较 $s[0]$ 与 $b$，接到左侧比较 $s[-1]$ 与 $a$。记忆化后状态为 $O(n\cdot 26^2)$。
