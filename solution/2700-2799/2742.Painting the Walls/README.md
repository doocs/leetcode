...
> **思考**
>
> 付费油漆匠刷第 $i$ 面墙花费 $cost[i]$ 并耗时 $time[i]$，其间免费匠可刷同样数量的墙。枚举付费集合后验证时间，子集数为 $2^n$、$n\le 500$，不可行。
>
> 从左到右决定当前墙给谁刷：付给费匠则剩余免费额度增加 $time[i]$，给免费匠则额度减 $1$。状态 $(i,j)$ 表示从第 $i$ 面起、当前免费时能为 $j$ 的最少花费；剩余墙数不超过 $j$ 时花费为 $0$。记忆化后复杂度与 $n^2$ 同阶。
