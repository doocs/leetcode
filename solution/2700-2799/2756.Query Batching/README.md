...
> **思考**
>
> 多次 $getValue$ 要在节流窗口 $t$ 内合并成一次 $queryMultiple$，且首次调用立即发出。对每个键单独请求正确但浪费；无节流则会把并发键拆成多次往返。
>
> 记录上次实际发出的时间：若距今不少于 $t$ 或尚无在途批次，立刻用当前键调用；否则把键与对应 $resolve$ 推进队列，在窗口结束时一次性提交。返回的 Promise 在批次结果按键位对齐后兑现。
