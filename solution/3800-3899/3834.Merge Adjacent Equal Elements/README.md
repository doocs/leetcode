...
> **思考**
>
> 反复合并当前最左的相邻相等元为其和，直到不能再合。$n \le 10^5$，每次从左扫描会平方。
>
> 每次合并只影响新和与其左侧是否再次相等，右侧尚未处理的元素不受影响。栈恰好保存「当前已稳定的左段」。
>
> 从左扫入栈，只要栈顶两元相等就弹出相加再压回，循环直到栈顶不再相等。
>
> 因题目总是合并最左一对，从左处理与反复取最左对等价，且每个元素进出栈常数次。
