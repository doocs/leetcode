...
> **思考**
>
> 每个用户在任意长 $\textit{window}$ 的闭区间内请求不得超过 $k$ 次，可丢弃请求求保留最多。用户之间独立。$n \le 10^5$。
>
> 对同一用户，按时间排序后，贪心保留能放进窗口的请求、超出则丢弃当前，使丢弃尽量发生在已满窗口的右端。
>
> 用双端队列保存已保留时间，弹出与 $t$ 相差超过 $\textit{window}$ 的队首；队列长度已达 $k$ 则丢弃 $t$。
>
> 答案初值为总请求数，每丢弃一次减一。
