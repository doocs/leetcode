...
> **思考**
>
> 好子数组的按位或等于其中某个元素。$n \le 10^5$，不能枚举区间。
>
> 若或值等于 $nums[i]$，则区间内每个数都是 $nums[i]$ 的位子集，且该区间须包含 $i$。可用 $i$ 作为「或值贡献者」。
>
> 单调栈找出 $i$ 向左、向右能延伸到的最远边界，使中间元素均为 $nums[i]$ 的子集。贡献为 $(i-l[i])\cdot(r[i]-i)$。
>
> 每个元素作为或值被恰好统计在以其为「栈意义下控制者」的区间内。
