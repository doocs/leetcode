...
> **思考**
>
> 可删除出现至少两次的字母的一次出现，求字典序最小结果。$|s| \le 10^5$，不能枚举删除方案。
>
> 每个字母至少保留一次；在仍有余量时，栈顶若比当前字符大，则弹栈更优。
>
> 先统计频次，再从左到右用单调栈：当栈顶更大且该字母之后仍会出现时弹出。扫完后再去掉栈尾仍多余的重复。
>
> 栈中剩余字符即为在保留每种字母至少一次的约束下的最小序列。
