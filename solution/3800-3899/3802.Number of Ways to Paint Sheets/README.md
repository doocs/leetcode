...
> **思考**
>
> 须用恰好两种颜色、各涂一段连续纸张，且张数不超过对应 $\textit{limit}$。$n \le 10^9$，不能按纸张枚举分割点。
>
> 对有序颜色对 $(i,j)$，合法分割点 $x$ 落在一个由 $n$ 与两个上限决定的整数区间内，区间长度可 $O(1)$ 求出。
>
> 颜色数 $m \le 10^5$，两两枚举达 $O(m^2)$，在 $n$ 已闭式处理后仍过慢。瓶颈在于如何对大量上限做批量计数。
>
> 注意到每种颜色的贡献只取决于其上限与 $n$ 的相对大小，可按 $\textit{limit}$ 排序后用前缀信息累加区间长度，再对 $10^9+7$ 取模。
