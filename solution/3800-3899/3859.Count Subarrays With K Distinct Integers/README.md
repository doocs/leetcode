...
> **思考**
>
> 子数组须恰好 $k$ 个不同整数，且每个不同整数至少出现 $m$ 次。$n \le 10^5$，不能枚举区间。
>
> 「恰好 $k$ 种」等于「至少 $k$ 种」减「至少 $k+1$ 种」。再叠加「至少 $k$ 个值达到频次 $m$」的窗口约束。
>
> 双指针维护不同数个数与已达 $m$ 次的种类数：窗口一旦同时满足 $\textit{lim}$ 种与 $t \ge k$，就右移左端。此时左端之前的每个起点都合法。
>
> $f(k)-f(k+1)$ 即恰好 $k$ 种的个数。
