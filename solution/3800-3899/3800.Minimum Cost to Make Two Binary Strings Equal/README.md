...
> **思考**
>
> 使 $s$ 与 $t$ 相等，只需处理对应位不同的位置。$n \le 10^5$，若按操作序列搜索，状态空间无法承受。
>
> 对应位相同则无需改动。不同位只有两类：$s[i]=\texttt{0}$ 而 $t[i]=\texttt{1}$，或相反。记二者数量为 $d_0,d_1$。
>
> 翻转可单独修正任一类；同串交换可把一类错位与另一类配对；跨串交换可改变两类数量之差。最优方案因此只在「全部翻转」「尽量配对再翻转余量」「先用跨串平衡再配对」之间取最小。
>
> 为此先统计两类错位个数，再按上述三种代价公式比较即可，无需模拟每一次操作。
