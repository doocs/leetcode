...
> **思考**
>
> 相同字符若在当前串中下标距离 $\le k$ 则右侧并入左侧，且总是合并最左一对。$n \le 100$，但按题意模拟每次改写字符串易乱。
>
> 合并等价于：新字符若与答案中上一次该字符的位置距离不超过 $k$，则被吸收而不追加。
>
> 用哈希表记录每个字符在答案中的最近下标，遍历原串：能合并则跳过，否则追加并更新位置。
>
> 因总是优先最左，从左构造答案与反复选最左对一致。
