...
> **思考**
>
> 循环数组上只能 $+1$，要使峰值至少 $k$ 个。$n \le 5000$。
>
> 循环峰不能相邻，故 $k$ 过大时无解。每个峰对应把该位抬到严格大于两侧。
>
> $n$ 允许 $O(n^2)$ 或按奇偶位分类的贪心：选定一组不相邻下标作为峰，代价为抬到超过邻域的增量之和，邻域会因相邻候选互相影响。
>
> 可枚举循环数组的旋转缺口（哪个位置不取峰）再在链上 DP 选至少 $k$ 个峰的最小代价。
