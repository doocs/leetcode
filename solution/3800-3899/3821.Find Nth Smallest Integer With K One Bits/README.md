...
> **思考**
>
> 求二进制恰好 $k$ 个 $1$ 的第 $n$ 小正整数，答案小于 $2^{50}$。枚举自然数并计数位会远超时限。
>
> 从高位到低位决定该位是否放 $1$：若低 $i$ 位放满剩余 $k$ 个 $1$ 的组合数小于 $n$，则当前位必须为 $1$。
>
> 预处理 $\binom{i}{k}$，自第 $49$ 位向下：若 $n>C(i,k)$ 则置位、扣除组合数并减少剩余 $1$ 的个数。
>
> 试填保证字典序意义下第 $n$ 个恰好 $k$ 个 $1$ 的数被唯一确定。
