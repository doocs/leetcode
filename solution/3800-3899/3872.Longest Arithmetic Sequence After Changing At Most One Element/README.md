...
> **思考**
>
> 至多改一个元素后，求最长连续等差子数组。$n \le 10^5$，枚举修改位置再扩展过慢。
>
> 不修改时的最长等差由相邻差相等的游程给出。修改 $i$ 可延长左侧游程、右侧游程，或把两侧接到同一公差。
>
> 预处理以 $i$ 结尾、以 $i$ 开头的等差长度 $f,g$，再枚举修改点按三种方式更新。
>
> 连接两侧时要求 $nums[i+1]-nums[i-1]$ 为偶数，且公差与左右游程一致才把额外长度加上。
