...
> **思考**
>
> 中心子数组要求区间和等于区间内某个元素。$n \le 500$，三重循环枚举左右端点再查找元素仍偏重。
>
> 固定左端点后向右延伸时，区间和与元素集合均可增量维护。
>
> 为此枚举起点 $i$，用累加和 $s$ 与哈希集合记录 $nums[i..j]$ 中的值，若 $s$ 落在集合中则计数。
>
> $O(n^2)$ 枚举全部子数组，哈希判断将内层查找降为均摊常数。
