...
> **思考**
>
> 奇妙数：本身数位严格单调，或其数位和为好数。$r \le 10^{15}$，枚举区间不可行。
>
> 计数区间 $[l,r]$ 上的数位性质，标准做法是数位 DP，再做前缀差。
>
> 状态记录位置、数位和、前一位、单调状态（未定/递增/递减/已破坏）以及是否贴上界。终点时若仍单调则计入，否则检查数位和是否为好数。
>
> 好数判定对数位和（至多 $9 \times 16$）特判即可。分别算到 $r$ 与 $l-1$ 作差。
