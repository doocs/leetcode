...
> **思考**
>
> 将数组恰好分成 $k$ 段，每段贡献 $\textit{sum}(\textit{sum}+1)/2$，求总分最小。$n \le 1000$，段内平方和对前缀和二次，直接枚举切点达 $O(n^2 k)$ 量级尚可，但常数需控制。
>
> 多段划分具有最优子结构：前 $i$ 个元素分成 $t$ 段的最优值，只依赖更短前缀的 $t-1$ 段最优。
>
> 注意到代价是前缀和的凸函数，相邻决策单调，可用单调队列优化转移，将每一层从二次降到线性。
>
> 因此按段数递推，用前缀和计算段值，再依凸性维护决策队列。
