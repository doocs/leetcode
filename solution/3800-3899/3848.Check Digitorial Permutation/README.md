...
> **思考**
>
> 问 $n$ 的某个无前导零排列是否等于其各位阶乘之和。$n \le 10^9$，枚举排列不可行。
>
> 阶乘和只取决于数位多重集，与排列无关。算出该和 $x$ 后，只需比较 $x$ 与 $n$ 的数位多重集是否相同。
>
> 预处理 $0..9$ 的阶乘，对 $n$ 各位累加，再把两数的十进制串排序比较。
>
> 排序后相等即存在同一多重集的排列；前导零约束由「和与 $n$ 位数集合一致」自然排除非法写法。
