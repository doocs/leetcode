...
> **思考**
>
> 删最少元素使剩余异或为 $\textit{target}$。$n \le 40$，值 $\le 10^4$，异或值域约 $2^{14}$。
>
> 等价于选尽可能多的元素使其异或为 $\textit{target}$，再倒推删除数。
>
> 令 $f[i][j]$ 为前 $i$ 个元素异或为 $j$ 时最多能选多少个，转移为选或不选当前值。
>
> 若 $\textit{target}$ 已超出值域上界则无解；否则答案为 $n-f[n][\textit{target}]$。
