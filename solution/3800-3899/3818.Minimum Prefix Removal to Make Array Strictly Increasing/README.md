...
> **思考**
>
> 必须删去一个前缀（可空）使剩余严格递增，并要前缀尽量短。$n \le 10^5$，不能枚举每个前缀再检查。
>
> 剩余段必须是原数组的某个后缀，且该后缀本身严格递增。最短前缀对应最长合法后缀。
>
> 从右往左走，遇到第一处 $nums[i-1] \ge nums[i]$ 即无法再延长，答案为 $i$。
>
> 若一直走到头，则整个数组已递增，答案为 $0$。
