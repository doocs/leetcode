...
> **思考**
>
> 构造非递减、$0 \le arr[i] \le 5000$ 且数位和等于 $\textit{digitSum}[i]$ 的数组个数。$n \le 1000$，数位和 $\le 50$。
>
> 相邻非递减把问题收成按位置选值，且当前值 $\ge$ 上一个。每个数位和对应有限个候选整数。
>
> 可对每个和预处理合法数，再 DP：位置 $i$、上一个取值，转移为不小于它的候选。
>
> 模 $10^9+7$。值上界 $5000$，需控制状态，按候选排序后前缀和加速转移。
