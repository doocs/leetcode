...
> **思考**
>
> 每次给一个 $k \times k$ 子矩阵全体 $+1$，求使网格全等的最少次数。操作只增不减，目标 $T$ 至少为当前最大值。
>
> 从左上扫到右下：位置 $(i,j)$ 之后更靠右下的操作覆盖不到它，故不足 $T$ 的部分必须在此刻以 $(i,j)$ 为左上角补齐。
>
> 二维差分在 $O(1)$ 记录 $k \times k$ 的未来增量，并以差分前缀和读取当前累计。越界或已超过 $T$ 则失败。
>
> 若 $T=\max$ 与 $T=\max+1$ 均不可行，则无法摊平。
