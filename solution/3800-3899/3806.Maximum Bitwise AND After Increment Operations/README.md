...
> **思考**
>
> 至多 $k$ 次加一后，任选 $m$ 个数使其按位与最大。$n \le 5 \times 10^4$，不能枚举子集。
>
> 按位与越大，通常要求更高位为 $1$。从高位到低位试填：假设答案已含更高位，再问当前位能否也取 $1$。
>
> 将一个数抬到不低于目标值 $\textit{target}$，只需修到 $\textit{target}$ 中第一个冲突位及以下，代价由低位掩码差给出。
>
> 对每个试填目标算出全体代价，取最小的 $m$ 个之和与 $k$ 比较，能放下则保留该位。试填法保证高位优先。
