...
> **思考**
>
> 需按用户统计反应次数、主导反应及其占比，并筛掉反应不足 $5$ 次或占比低于 $0.60$ 的用户。直接在原表上多次扫描会重复聚合。
>
> 同一用户的多种反应要同时参与「最大次数」与「总次数」的计算，再回表取出达到最大次数的反应类型。
>
> 为此先按 $(\textit{user\_id},\textit{reaction})$ 分组得到次数表 $t$，再在 $t$ 上按用户聚合出最大次数与比例，筛入 $s$。
>
> 最后用 $s$ 与 $t$ 按用户连接、保留次数等于最大值的行，并按比例与编号排序。分组后再连接，避免在明细上反复过滤。
