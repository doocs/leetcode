...
> **思考**
>
> 主动玩家得分，奇数分或每第 $6$ 场会交换角色。求第一位减第二位的总分差。$n \le 1000$，按规则模拟。
>
> 差可用符号 $k=\pm 1$ 表示当前主动方属于第一位或第二位，把分数乘 $k$ 累加。
>
> 先按奇数翻转 $k$，再按 $i \bmod 6=5$ 翻转，然后计入 $k \cdot x$。
>
> 顺序与题目「先判奇数、再判第 $6$ 场、最后得分」一致。
