...
> **思考**
>
> 交替指相邻比较在 $<$ 与 $>$ 之间轮换，可删至多一个元素。$n \le 10^5$，枚举删除位置再扫会平方。
>
> 不删除时的最长交替段可由左到右按最后一次比较方向递推。删除 $i$ 则试图把 $i$ 左段与右段在 $i-1$ 与 $i+1$ 处接上。
>
> 为此预处理以 $i$ 结尾、以 $i$ 开头的两种方向长度 $l_1,l_2,r_1,r_2$。
>
> 答案先取不删时的最大，再枚举删除点，仅当两侧比较仍交替时把对应前后缀相加。
