...
> **思考**
>
> 行为稳定要求存在连续至少 $5$ 天、每天恰好一个且相同的操作。同一用户可能有多段，只保留最长段。
>
> 先排除一天内多条操作的日期，否则连续性定义被破坏。
>
> 对 $(\textit{user\_id},\textit{action})$ 按日期排序后，用日期减去行号的经典手法把连续日期归到同一 $\textit{grp}$。
>
> 按组聚合长度并筛 $\ge 5$，再对每用户按长度取第一名，最后按长度与编号排序。
