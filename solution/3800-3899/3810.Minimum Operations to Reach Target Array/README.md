...
> **思考**
>
> 一次操作会把某个值 $x$ 的全部极大连续段同时改成 $\textit{target}$ 对应位置。$n \le 10^5$，模拟每次改写会反复扫描。
>
> 值为 $x$ 的所有段在同一次操作中一起被替换，故每个仍不等于目标的 $x$ 只需操作一次，与段数无关。
>
> 已相等的位置不必计入。答案即「存在失配」的不同 $nums[i]$ 个数。
>
> 用集合收集失配处的原值，其大小即为最少操作次数。
