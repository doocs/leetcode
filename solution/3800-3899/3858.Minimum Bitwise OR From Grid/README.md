...
> **思考**
>
> 每行恰选一个数，最小化所选数的按位或。格子总数 $\le 10^5$，不能枚举选取。
>
> 或值希望高位尽量为 $0$。从高位试填：假设更高位已定，问当前位能否全体选到在已定高位与更低位任意的掩码下都不迫使该位为 $1$。
>
> 对试填掩码 $\textit{ans} \mid (2^i-1)$，若每行都存在 $x$ 使 $x$ 被该掩码覆盖，则当前位可保持 $0$，否则必须置 $1$。
>
> 高位优先的试填给出最小或值。
