...
> **思考**
>
> 特殊下标为严格峰。只能 $+1$，先最大化峰数再最小化总增量。$n \le 10^5$。
>
> 峰不能相邻。奇数长数组可占满所有奇数下标；偶数长则必须在 $[1,n-2]$ 中跳过一处。
>
> 把下标 $i$ 抬到大于左右邻的代价为 $\max(0,\max(nums[i-1],nums[i+1])+1-nums[i])$。记忆化搜索 $\mathrm{dfs}(i,j)$：从 $i$ 起还可跳过 $j$ 次。
>
> 选择抬高则跳到 $i+2$；若仍有跳过额度则可改走 $i+1$ 并耗尽额度。起点为 $1$，初值 $j=n\bmod 2$ 的反。
