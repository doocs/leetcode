...
> **思考**
>
> 得分只取决于元音个数 $v$ 与辅音个数 $c$，与字符位置无关。$|s| \le 100$，一次扫描足够。
>
> 空格与数字既非元音也非辅音，不可计入 $c$。
>
> 为此只对字母计数：先把字母记入 $c$，若属于 $\texttt{aeiou}$ 再记入 $v$，最后从 $c$ 中减去 $v$ 得到真正的辅音数。
>
> $c=0$ 时规定得分为 $0$，否则为 $\lfloor v/c \rfloor$。
