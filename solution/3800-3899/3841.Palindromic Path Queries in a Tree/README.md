...
> **思考**
>
> 路径字符可重排成回文，当且仅当奇数次字符至多一个。$n,q \le 5 \times 10^4$，不能每次走路径。
>
> 每个字母出现次数的奇偶可用 $26$ 位掩码表示，路径掩码为两端到根的前缀异或再消去 LCA。
>
> 修改一个点的字符只改变该点子树内外的前缀关系，可用树上差分或欧拉序数据结构维护点到根的掩码。
>
> 查询先求 LCA，再判断路径掩码的弹出计数是否不超过 $1$；更新则改写该点字符并同步掩码。
