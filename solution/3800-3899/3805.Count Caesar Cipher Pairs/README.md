...
> **思考**
>
> 相似指两串可通过整体循环移位对齐。$n \cdot m \le 10^5$，两两比较移位次数不可行。
>
> 同一相似类中的串只差一个公共偏移。把每串平移到首字符固定为 $\texttt{z}$，则同类映射到同一规范串。
>
> 因此对每个单词做一次规范化并计数，同类内部的配对数为 $\binom{v}{2}$。
>
> 哈希表按规范串聚合后，对出现次数求和组合数即可。
