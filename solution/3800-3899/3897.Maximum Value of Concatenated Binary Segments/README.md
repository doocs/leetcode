...
> **思考**
>
> 每段为若干 $1$ 后接若干 $0$，可重排列段，使拼接二进制最大。总位数 $\le 2 \times 10^5$。
>
> 二进制比大小即字典序。两段 $A,B$ 的相对顺序由 $AB$ 与 $BA$ 的字典序决定。
>
> 全 $1$ 段应最前且 $1$ 越多越前；既有 $1$ 又有 $0$ 的按 $1$ 降序、$0$ 升序；全 $0$ 段垫后。
>
> 排序后用预计算的 $2$ 的幂累加每位 $1$ 的权值，不必真正构造字符串。
