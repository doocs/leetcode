...
> **思考**
>
> 需按到达顺序匹配最早司机与最早乘客，并支持取消未匹配乘客。调用至多 $1000$ 次，但顺序查询要能取最小时间戳。
>
> 先进先出若用普通队列，取消中间乘客需要扫描。应按时间戳有序存储。
>
> 用有序集合保存 $(t,\textit{id})$，全局时间戳自增以区分先后；哈希表记下乘客的 $t$ 以便 $O(\log n)$ 取消。
>
> 匹配时若任一侧为空则失败，否则弹出两侧最小元。
