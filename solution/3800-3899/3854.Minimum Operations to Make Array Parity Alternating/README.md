...
> **思考**
>
> 每次 $\pm 1$ 改变一个元素，使相邻奇偶交替，并在最少次数下最小化极差。$n \le 10^5$。
>
> 交替只有两种模式：偶数下标为偶或为奇。每种模式中，错位元素各改一次即可，次数与改成 $+1$ 还是 $-1$ 无关。
>
> 为压极差，错位时若该值已是全局最小则只能 $+1$，已是最大则只能 $-1$，其余方向任选但不影响能否取到边界。
>
> 对两种模式各算 $(\textit{次数},\textit{极差})$，取字典序较小者；长度为 $1$ 时无需操作。
