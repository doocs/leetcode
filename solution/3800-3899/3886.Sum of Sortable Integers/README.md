...
> **思考**
>
> $k$ 为可排序整数当且仅当 $k \mid n$，且每段长 $k$ 的子数组可经循环移位拼成全局非递减。$n \le 10^5$。
>
> 因数个数少，枚举 $n$ 的因数即可。对每个 $k$，每段必须是某段循环移位，等价于段内相对循环秩序能对接相邻段。
>
> 全局非递减意味着把各段旋转后拼接应单调。可检查每个位置与「排序后应在的循环位置」是否一致，或比较相邻段的最小旋转。
>
> 对通过检查的因数求和。
