...
> **思考**
>
> 元素只能改到 $[1,6]$，要让两数组和相等且改动次数最少。$n\le 10^5$，需按「一次改动能缩小的差距」贪心。
>
> 不妨设 $s_1<s_2$。较小数组每个数最多加到 $6$，较大数组最多减到 $1$。把这些增量收集后降序，能一次减多少差距就减多少，直到差距非正。
...
> **思考**
>
> 增量只可能是 $0..5$，不必排序数组。用长度为 $6$ 的计数桶从 $5$ 扫到 $1$，按出现次数消耗差距，时间降为线性。
