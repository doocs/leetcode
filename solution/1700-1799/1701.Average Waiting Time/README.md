...
> **思考**
>
> 每位顾客的等待时间由厨师何时空闲与到达时刻共同决定。顾客按到达时间给出，且厨师严格按订单顺序做菜，因此不必重排，按序模拟即可。
>
> 维护厨师完成上一单的时刻 $t$。下一位到达时刻为 $a$、做菜时间为 $b$ 时，开工时刻为 $\max(t,a)$，完成时刻为 $\max(t,a)+b$，等待为完成时刻减去 $a$。
>
> 累加全部等待再除以人数，得到平均等待时间。
