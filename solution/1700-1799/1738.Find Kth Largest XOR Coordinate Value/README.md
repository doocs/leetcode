...
> **思考**
>
> 坐标 $(i,j)$ 的值是左上矩形的异或。直接对每个矩形重算是 $O(m^2n^2)$，$m,n\le 1000$ 不可行。
>
> 二维前缀异或满足 $s[i][j]=s[i-1][j]\oplus s[i][j-1]\oplus s[i-1][j-1]\oplus matrix[i-1][j-1]$，可 $O(mn)$ 列出全部值。
>
> 在列表上取第 $k$ 大（排序或堆选择）即可。
