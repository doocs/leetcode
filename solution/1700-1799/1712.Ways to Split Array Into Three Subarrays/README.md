...
> **思考**
>
> 三段非空且前缀和满足 $s_{\textit{left}}\le s_{\textit{mid}}\le s_{\textit{right}}$。两重枚举分割点为 $O(n^2)$，$n\le 10^5$ 不可行。
>
> 元素非负，前缀和单调。固定左段右端 $i$ 后，$\textit{mid}$ 的右端落在一段连续区间，可用二分求出。
>
> 条件化为 $s[j]\ge 2s[i]$ 与 $s[k]\le (s[-1]+s[i])/2$。对每个 $i$ 做两次二分，方案数对 $10^9+7$ 取模。
