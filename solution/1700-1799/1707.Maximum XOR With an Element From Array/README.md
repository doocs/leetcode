...
> **思考**
>
> 每个询问要在 $nums[j]\le m_i$ 的约束下最大化 $x_i\oplus nums[j]$。在线对每个询问扫描数组是 $O(nq)$，$n,q\le 10^5$ 无法接受。
>
> 询问互不依赖且与 $nums$ 的顺序无关。按 $m_i$ 排序后，可把「不超过 $m$ 的数」单调插入同一棵结构。
>
> 将 $nums$ 排序，用指针把 $\le m_i$ 的数插入 $0$-$1$ 字典树；在树上贪心走相反位即可得到最大异或。空树时答案为 $-1$。
