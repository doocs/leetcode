...
> **思考**
>
> 询问形如从 $x$ 起每隔 $y$ 求和。若每次暴力沿步长累加，$q\le 1.5\times 10^5$、$n\le 5\times 10^4$ 在步长很小时会退化到接近 $O(nq)$。
>
> 步长大时路径短，直接求和即可；步长小时路径长，应预处理后缀和。以 $\sqrt{n}$ 为界把两种情形分开。
>
> $\textit{suf}[i][j]$ 表示从 $j$ 起步长为 $i$ 的后缀和。$y\le\sqrt{n}$ 时查表，否则按步长扫描。整体 $O((n+q)\sqrt{n})$。
