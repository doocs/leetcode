...
> **思考**
>
> 每列顶端放一球，挡板将球导向左或右。网格规模有限，可对每颗球独立模拟下落。
>
> 卡住的情形只有四类：贴边且挡板把球推出网格，或相邻两格形成 $V$ 形。除此以外，球沿挡板斜向进入下一行。
>
> 用 $\textit{dfs}(i,j)$ 表示从 $(i,j)$ 出发的出口列：先判卡住返回 $-1$，否则递归到 $(i+1,j\pm 1)$；到达第 $m$ 行则返回当前列。
