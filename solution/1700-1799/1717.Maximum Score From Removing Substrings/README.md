...
> **思考**
>
> 每次删除 $\textit{ab}$ 得 $x$、删除 $\textit{ba}$ 得 $y$，删除会改变邻接。搜索所有删除顺序在长串上不可行。
>
> 一段仅含 $a,b$ 的子串无论怎么删，最终只剩一种字符，操作次数由两种字母的个数决定。应先删分值更高的模式。
>
> 若 $x<y$ 则交换字母与分值。扫描时优先用计数匹配高分对；遇到其它字符或结束时，再用 $\min(cnt_a,cnt_b)$ 结算低分对。
