...
> **思考**
>
> 卡车容量按箱计数，而目标是单元数最大。同类箱单元数相同，应优先装单元密度高的箱子。
>
> 按每箱单元数降序排序后，依次装入 $\min(\textit{truckSize},\textit{箱数})$ 个，更新剩余容量，容量耗尽即停。
...
> **思考**
>
> 方法一的排序代价为 $O(n\log n)$。每箱单元数不超过 $1000$，可用计数数组按值下标存放箱数。
>
> 从 $1000$ 扫到 $1$，按单元数从大到小装车，线性完成同一贪心。
