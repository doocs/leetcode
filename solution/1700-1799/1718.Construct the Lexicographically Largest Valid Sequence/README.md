...
> **思考**
>
> 长度为 $2n-1$ 的序列中，$1$ 出现一次，$2..n$ 各出现两次且间距恰为该数。$n$ 较小，可回溯搜索。
>
> 字典序最大要求优先放置较大的数。从左到右填空位，对每个空位从 $n$ 试到 $2$，再试 $1$。
>
> $\textit{path}$ 记录已填值，$\textit{cnt}$ 记录剩余次数。数 $i$ 占用 $u$ 与 $u+i$；一旦填满 $2n-1$ 个位置即得到题目保证存在的解。
