...
> **思考**
>
> 把所有球移到盒子 $i$ 的代价是各球下标与 $i$ 的距离之和。$n\le 2000$，对每个 $i$ 扫全部球可行，但可线性完成。
>
> 左（右）侧代价可由相邻位置递推：多一个左侧球，整体代价增加该球数。预处理 $left[i]$ 与 $right[i]$ 后相加。
...
> **思考**
>
> 方法一的左右数组只依赖前一格，可去掉额外数组：从左到右、再从右到左把递推值累加进 $ans$，额外空间降为常数。
...
> **思考**
>
> 若只求实现简单，可先收集所有球的下标，再对每个盒子累加 $|i-j|$。$n$ 不大时可以通过，常数与空间都比前缀和差。
