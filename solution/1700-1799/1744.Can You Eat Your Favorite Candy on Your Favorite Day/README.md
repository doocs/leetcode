...
> **思考**
>
> 糖果必须按种类顺序吃，每天至少 $1$ 颗、至多 $\textit{dailyCap}$ 颗。询问多，不能对每个询问模拟逐日食用。
>
> 前缀和 $s[t]$ 是第 $t$ 类之前的糖果总数。要在第 $\textit{day}$ 天吃到类型 $t$，最慢也得在那天之前吃掉不到 $s[t+1]$ 颗，最快也不能在到达 $t$ 之前就把 $s[t]$ 吃完。
>
> 即 $\textit{day}<s[t+1]$ 且 $(\textit{day}+1)\cdot mx>s[t]$。对每个询问 $O(1)$ 判断。
