...
> **思考**
>
> 允许交换的下标对可以复合，同一连通分量内的值任意重排。$n\le 10^5$，需按分量处理而不能枚举排列。
>
> 并查集把可交换下标连成分量。每个分量内，$\textit{source}$ 的多重集应尽量覆盖 $\textit{target}$ 对应位置。
>
> 用分量哈希表统计源值频次；扫 $\textit{target}$ 时若分量中该值已用尽则汉明距离加一。
