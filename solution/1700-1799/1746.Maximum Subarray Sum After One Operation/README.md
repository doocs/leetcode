...
> **思考**
>
> 必须将恰好一个元素改成平方，再求最大子段和。枚举替换位置后做 Kadane 是 $O(n^2)$，$n\le 10^5$ 不可行。
>
> 以当前位置结尾时，替换可以尚未发生或已经发生。前者即普通 Kadane；后者来自「此前未替换并在此平方」或「此前已替换并加上当前值」。
>
> 两个滚动变量 $f,g$ 分别维护这两种结尾状态，取全程最大。
