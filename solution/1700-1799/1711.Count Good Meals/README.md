...
> **思考**
>
> 两数之和为 $2$ 的幂的无序对，若枚举全部下标对则需 $O(n^2)$。$n\le 10^5$ 不可行。
>
> 元素上界为 $2^{20}$，可能的幂次只有 $O(\log M)$ 个。对每个已出现的数 $d$，枚举幂 $s$ 并在哈希表中查 $s-d$。
>
> 边遍历边把 $d$ 写入计数表，只与左侧配对，避免重复。答案对 $10^9+7$ 取模。
...
> **思考**
>
> 方法一按元素顺序边扫边查。也可以先统计全部频次，再枚举幂与哈希表中的 $a$，用 $cnt[s-a]$ 成对累加。
>
> 当 $a=s-a$ 时用 $m(m-1)$，否则用 $m\cdot cnt[s-a]$；每对被算两次，最后右移一位再取模。复杂度同为 $O(n\log M)$。
