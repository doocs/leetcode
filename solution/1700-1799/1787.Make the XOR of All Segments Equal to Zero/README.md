...
> **思考**
>
> 每个长 $k$ 的窗口异或为 $0$，推出 $nums[i]=nums[i+k]$，即答案以 $k$ 为周期，且一个周期异或为 $0$。值不超过 $2^{10}$，可按余数组 DP。
>
> $f[j]$ 表示当前已处理的组异或为 $j$ 的最少修改。一组要么全改成新值（代价为组大小），要么改成组内已有的 $v$（省下 $cnt[v]$）。滚动到第 $k$ 组后取 $f[0]$。
