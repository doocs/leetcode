...
> **思考**
>
> 列可任意重排，求全 $1$ 子矩阵的最大面积。$m\cdot n\le 10^5$，不能对每种排列枚举。
>
> 重排只改列序，不改一列里向上连续 $1$ 的高度。把每个 $1$ 改写成向上高度后，同一行的高度可当作柱状图。
>
> 每行降序排序，第 $k$ 大高度 $v$ 可与前 $k$ 列组成高为 $v$ 的全 $1$ 矩阵，面积 $k\cdot v$。对所有行取最大。
