...
> **思考**
>
> 操作允许把 $00$ 换成 $10$、把 $10$ 换成 $01$。若对每一次替换做搜索，状态空间随串长指数增长，难以在约束内完成。
>
> 操作 $2$ 可将任意 $1$ 向右挪动，操作 $1$ 则把连续 $0$ 压成「若干 $1$ 后接一个 $0$」。因此最优形态至多含一个 $0$，且该 $0$ 应尽量靠右。
>
> 第一个 $0$ 之前的前缀 $1$ 无法被改动；其后所有 $0$ 可汇聚到同一位置。设首个 $0$ 的下标为 $k$，再加上其后 $0$ 的个数，即得最终 $0$ 的下标，其余位填 $1$。
