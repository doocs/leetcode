...
> **思考**
>
> 数对给出祖先关系：边出现当且仅当一方是另一方的祖先。需要判断这样的有根树有 $0$、$1$ 还是多于 $1$ 棵。
>
> 度数（邻接对数）越大的点越靠近根。将点按度数升序排列后，每个点的父候选是度数不更小且与之相邻的点。
>
> 检验该候选是否覆盖当前点的全部邻接；多个点无法找到父则不能成树。若存在度数相同的父子，则方案多于一；否则恰好一棵。
