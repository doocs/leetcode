...
> **思考**
>
> 子序列和最接近 $\textit{goal}$。$n\le 40$，全集枚举 $2^n$ 不可行，但 $2^{n/2}$ 可以。
>
> 将数组对半，DFS 得到左右所有子序列和。对每个左和在有序的右和中二分 $\textit{goal}-l$，取最接近的两个候选。
...
> **思考**
>
> 方法二仍是折半枚举与二分，只是 DFS 写成先不选再选的递归形态，逻辑与方法一相同，实现更紧凑。
