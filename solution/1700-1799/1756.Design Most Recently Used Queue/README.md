...
> **思考**
>
> 每次把第 $k$ 个元素移到队尾。$n$ 与操作次数不大时，用数组删除再追加即可模拟。
>
> $\textit{fetch}(k)$ 取出下标 $k-1$，从原处删掉并接到末尾。
...
> **思考**
>
> 方法一每次删除是线性的。若不真正删除、只在末尾追加，则需快速定位「逻辑上的第 $k$ 个未删除位置」。
>
> 树状数组记录各下标被移走的次数，二分第一个满足 $i-\textit{query}(i)\ge k$ 的 $i$，将该值追加到 $q$ 末尾并在 $i$ 处标记删除。单次 $O(\log^2 n)$。
