...
> **思考**
>
> 每次修改后要最大化「前缀不同质数个数 + 后缀不同质数个数」。一个质数若同时出现在两侧会被算两次，只在一侧则一次。
>
> $n$、$q$ 达 $5 \cdot 10^4$，需动态维护每个质数出现的最左、最右位置。分裂点 $k$ 的贡献可由这些端点用线段树（或分治）维护「跨过 $k$ 的质数能贡献 $1$ 还是 $2$」。修改后更新该质数的端点并查询全局最大。
