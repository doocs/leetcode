...
> **思考**
>
> 将数组分成恰好 $k$ 段，最小化各段 XOR 的最大值。$n$ 与 $k$ 不大，可用 DP：$f[i][j]$ 表示前 $i$ 个数分成 $j$ 段时该最大值的最小可能。
>
> 前缀 XOR $g[i]$ 使子段 $[h+1,i]$ 的 XOR 为 $g[i]\oplus g[h]$。枚举上一段终点 $h$，取 $\max(f[h][j-1], g[i]\oplus g[h])$ 的最小。答案为 $f[n][k]$。
