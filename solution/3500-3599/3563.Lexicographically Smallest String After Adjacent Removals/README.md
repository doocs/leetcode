...
> **思考**
>
> 相邻连续字母可删，且要字典序最小的最终串，而非任意消完。$n \le 250$，可对区间做 DP 判断一段能否删空，再据此构造最小串。
>
> 令 $g[i][j]$ 表示 $s[i..j]$ 能否被删光；转移枚举与 $s[i]$ 配对的位置。再令 $f[i]$ 为 $s[i..]$ 能得到的最小串：要么保留 $s[i]$ 接 $f[i+1]$，要么若某 $[i,k]$ 可删则跳到 $f[k+1]$。两者取字典序较小者。
