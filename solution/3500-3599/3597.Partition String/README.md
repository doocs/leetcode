...
> **思考**
>
> 从左到右必须切出从未出现过的最短新段。用集合保存已输出段，当前缓冲 $t$ 每追加一个字符就查询是否未见过；未见则输出并清空。
>
> 段长大致按 $1+2+\cdots$ 增长，查找代价与段长有关，总时间约 $O(n\sqrt{n})$，对题目规模足够。
...
> **思考**
>
> 方法一用整段字符串做哈希表键，比较与拷贝随段长增加。将判重改为多项式字符串哈希后，子串哈希 $O(1)$，查找与插入也是期望常数。
>
> 双指针表示当前段 $[l,r]$，扩展 $r$ 并查询哈希；未出现则切下该段。总时间降为线性，逻辑仍是同一贪心切分。
