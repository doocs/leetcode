...
> **思考**
>
> $\textit{numWays}[i]$ 是用未知面值凑 $i+1$ 的完全背包方案数。从 $1$ 到 $n$ 扫描：若当前金额在已有面值下的方案数比 $\textit{numWays}$ 少 $1$，则必须引入该金额作为新面值；多了或对不上则无解。
>
> 每认定一个面值，就用完全背包更新后续方案数。扫描结束即得递增的面值列表，或在矛盾时返回空数组。
