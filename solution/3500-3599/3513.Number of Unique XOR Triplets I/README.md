...
> **思考**
>
> $\textit{nums}$ 是 $[1,n]$ 的排列，下标可重复，三元组异或的取值集合固定。暴力枚举为 $O(n^3)$，与 $n$ 的规模不符。
>
> $n \le 2$ 时结果即为 $n$。$n \ge 3$ 时三个数的异或恰好铺满 $[0, 2^{\lfloor \log_2 n \rfloor + 1} - 1]$，因此答案为 $1 \ll \textit{bitLength}(n)$。
