...
> **思考**
>
> 上一题 $O(n^2)$ 模拟在 $n \le 10^5$ 下不可行。仍须反复合并相邻和最小的一对，但每次扫描全数组过慢。
>
> 用有序集合按 $(\textit{sum}, i)$ 取最小对，另一有序集合维护存活下标以便 $O(\log n)$ 定位左右邻居；再用 $\textit{inv}$ 统计当前逆序相邻对数，只在受影响的两三处更新。当 $\textit{inv}=0$ 时停止。
