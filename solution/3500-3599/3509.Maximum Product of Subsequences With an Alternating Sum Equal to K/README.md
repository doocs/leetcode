...
> **思考**
>
> 子序列有 $2^n$ 个，$n \le 150$，不能枚举。交错和与乘积需同时记录，而 $k$ 范围大、$\textit{limit} \le 5000$、$nums[i] \le 12$，乘积超过 $\textit{limit}$ 后对答案无贡献，可以截断。
>
> 按位置做 DP，状态取已选长度的奇偶、当前交错和以及截断后的乘积，转移为选或不选当前数。存在 $0$ 时需单独处理乘积为零的情形。
