...
> **思考**
>
> $s$ 与 $t$ 各自内部的最长回文可用中心扩展在多项式时间内求出，但拼接后的回文还可以左右各取一段，并在一侧再接一个回文核。
>
> 将 $t$ 反转后，$s$ 的前缀与反转 $t$ 的前缀相等，即对应原问题中 $s$ 的一段与 $t$ 的一段对称。$f[i][j]$ 记录这对前缀的匹配长度，再接上从该位置出发的最长回文 $g_1[i]$ 或 $g_2[j]$，即可覆盖「只取一侧」与「两侧匹配后再延长」两类最优。
