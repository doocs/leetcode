...
> **思考**
>
> 公司层级是树，折扣依赖上司是否已买，预算为容量，属于树上背包。对每个结点维护 $f[j][\textit{pre}]$：子树预算不超过 $j$、上司购买状态为 $\textit{pre}$ 时的最大利润。
>
> 先把子女的背包按预算做多重合并得到 $\textit{nxt}$，再决定本结点是否购买（费用为 $\lfloor \textit{present}/(\textit{pre}+1)\rfloor$）。根的上司视为未买，答案为 $f_1[\textit{budget}][0]$。
