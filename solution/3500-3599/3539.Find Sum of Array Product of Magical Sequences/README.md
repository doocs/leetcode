...
> **思考**
>
> 长度为 $m$ 的序列每位取自 $\textit{nums}$，序列乘积再按出现次数的二进制进位统计置位数，直接枚举序列不可行。
>
> 按 $\textit{nums}$ 的下标分配出现次数 $t$，方案数为 $\binom{j}{t} \cdot \textit{nums}[i]^t$，进位与置位由 $t+\textit{st}$ 的奇偶与右移更新。记忆化 $\textit{dfs}(i,j,k,\textit{st})$，并预处理阶乘逆元以算组合数。
