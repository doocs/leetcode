...
> **思考**
>
> 「长度超过 $2$ 的子数组之和皆为正」若逐段检查，需要 $O(n^2)$ 个区间，而 $n \le 10^5$。一次替换可将某位置改成任意值，等价于在该处切断此前的前缀约束。
>
> 从左扫描前缀和，维护窗口左端与此前长度至少 $2$ 的前缀最大值 $\textit{pre\_mx}$。若当前前缀和 $\le \textit{pre\_mx}$，则出现非正区间，将当前位置视为一次替换并重置窗口。贪心在冲突处切断，操作次数最少。
