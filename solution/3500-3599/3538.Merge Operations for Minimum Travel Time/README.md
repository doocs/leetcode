...
> **思考**
>
> $n \le 50$ 且必须恰好合并 $k \le 10$ 次，枚举合并顺序过多。合并后一段的行驶速度是被并入的 $\textit{time}$ 之和，路程由相邻保留路标的 $\textit{position}$ 差决定。
>
> 定义在某个路标结束、已用若干次合并、当前段速度来自一段连续 $\textit{time}$ 之和时的最小旅行时间，枚举本段覆盖的路标个数转移。$n$ 与 $k$ 均小，三维 DP 可行。
