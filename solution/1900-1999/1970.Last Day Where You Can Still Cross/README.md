...
> **思考**
>
> 陆地逐日变成水域，能从首行走到末行的天数具有单调性：第 $k$ 天可行则更早也可行。$mn$ 达 $2\times 10^4$，可对天数二分再用 BFS 判定。
>
> 对中点 $k$ 把前 $k$ 格填成水，从第一行所有陆地出发四向扩展，能抵达最后一行则增大 $k$。
>
> 二分次数为对数，每次 BFS 线性于格子数。
...
> **思考**
>
> 二分每次重建网格并搜索，常数偏大。倒序把水域变回陆地，用并查集把相邻陆地连通，并增设顶、底两个虚拟结点。
>
> 某天恢复后若顶底连通，则前一天仍可穿过，即为最晚日期。只需一次线性扫描。
