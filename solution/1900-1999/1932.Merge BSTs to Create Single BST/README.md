...
> **思考**
>
> 每棵树至多三个结点，合并只能把一棵树的根接到另一棵的叶子上。$n\le 5\times 10^4$，不能反复模拟所有配对。
>
> 叶子值若与某棵树的根相同，则该根必须接到此叶；最终只能剩下一棵根（其值不出现在任何叶子上）。按此唯一方式拼接后，再中序验证是否为 BST 且无重复。
>
> 用哈希把根值映射到树，遍历叶子完成挂接；若根不唯一、挂接失败或中序不严格递增，则返回空。
