...
> **思考**
>
> 长度为 $3$ 的回文子序列形如 $c\_c$，中间字符任意。$n\le 10^5$，枚举全部三元组不可行。
>
> 字母仅 $26$ 种，两端字符可枚举。对固定 $c$，取其首次与末次出现之间的不同字符个数，即为以 $c$ 为两端的不同回文个数。
>
> $\texttt{find}/\texttt{rfind}$ 定位两端，中间用集合去重，总时间 $O(n|\Sigma|)$。
