...
> **思考**
>
> 双方交替填 `?`，Alice 要使左右半和不同。完整博弈树随问号个数指数增长，但最优策略可归结为问号奇偶与两侧已有数字之差。
>
> 问号总数为奇数时 Alice 先手多填一次，必能使两侧不等。偶数时双方总能把剩余问号对冲到同一侧，Bob 能逼平当且仅当已有和之差恰好等于 $9$ 乘以两侧问号数之差的一半。
>
> 因此扫描左右半的问号个数与数字和，按上述等式判断即可，无需模拟对局。
