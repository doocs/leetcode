...
> **思考**
>
> 若按回合完整模拟淘汰赛，每一轮需在 $\lfloor n/2\rfloor$ 场对决中枚举胜负，状态随剩余人数指数增长，且 $n$ 最大为 $28$，直接搜索难以复用中间结果。
>
> 注意到两名指定选手以外的胜负，只影响下一轮他们的相对名次与剩余人数；一旦两人相遇即可终止。因此状态可压缩为 $(l,r,n)$，即当前序列中两人的下标与总人数。
>
> 为此对前半区用二进制枚举胜者，并强制 $l$、$r$ 晋级，再按原顺序重编号后递归。记忆化后同一三元组只计算一次，最早与最晚回合分别取最小值与最大值加一。
