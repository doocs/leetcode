...
> **思考**
>
> 武器每分钟只能消灭一只，若按到达时间任意穿插，需要比较的顺序很多。每只怪物的最晚击杀时刻由距离与速度唯一确定。
>
> 第 $i$ 分钟开始时已用掉 $i$ 次射击，因此应按最晚可击杀时间升序消灭：若排序后第 $i$ 只的时限小于 $i$，则无法再多杀。
>
> $\lfloor(d-1)/s\rfloor$ 给出「到达前最后一分钟」，排序后扫描即可。
