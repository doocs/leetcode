...
> **思考**
>
> 逐格比较四邻可在 $O(mn)$ 内找出峰值，但题目要求 $O(n\log m)$ 或 $O(m\log n)$，全表扫描不满足时间限制。
>
> 若只看某一行的最大值 $mat[i][j]$，其与下一行使该列单调上升或下降，峰值必落在对应的半侧：向上则上半部分必有峰，向下则下半部分必有峰。反证可知，若该半侧无峰，边界处的行最大值将与外围 $-1$ 矛盾。
>
> 因此对行下标二分，每次在中行求最大值列 $j$，再与 $mat[mid+1][j]$ 比较以收缩区间。行数按对数收缩，每步扫描一列宽为 $n$ 的行，总时间为 $O(n\log m)$。
