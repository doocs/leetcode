...
> **思考**
>
> 对每个长度 $k$ 求所有窗口最小值的最大值。滑动窗口最值再取 $\max$ 为 $O(n)$ 每个 $k$，总 $O(n^2)$。
>
> 以 $nums[i]$ 为最小值的最长区间由其左右严格更小的位置决定，该区间长度 $m$ 意味着 $nums[i]$ 可充当所有 $\le m$ 的答案候选。
>
> 单调栈求左右边界，写入 $ans[m-1]$，再从大到小用 $ans[i+1]$ 更新 $ans[i]$，因为更长窗口的最优值也可用于更短窗口。
