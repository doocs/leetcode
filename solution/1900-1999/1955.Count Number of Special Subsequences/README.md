...
> **思考**
>
> 特殊子序列为若干 $0$、若干 $1$、若干 $2$ 依次拼接。子序列个数指数级，$n\le 10^5$ 必须按结尾数字计数。
>
> $f[i][j]$ 表示前 $i$ 个元素中以 $j$ 结尾的特殊子序列数。遇 $0$ 可在旧的 $0$ 序列后追加或单开；$1$、$2$ 同理只能接在前一阶段或同阶段之后。
>
> 模 $10^9+7$ 后答案为 $f[n-1][2]$。
...
> **思考**
>
> 转移只读上一行三个状态，可压成长度为 $3$ 的数组原地更新，空间降为常数。
