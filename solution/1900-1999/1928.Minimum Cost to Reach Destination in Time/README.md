...
> **思考**
>
> 边权含时间与过路费两维，最短路若只优化费用会超时限，只优化时间则费用不是最小。$\textit{maxTime}\le 1000$、$n\le 1000$，可用时间作为一层 DP。
>
> 令 $f[i][j]$ 为恰好用时 $i$ 到达城市 $j$ 的最小费用。按时间升序，对每条边做一次松弛，相当于在「时间—城市」图上的最短路。
>
> 终点在所有可行时间上取最小费用，不存在则返回 $-1$。
