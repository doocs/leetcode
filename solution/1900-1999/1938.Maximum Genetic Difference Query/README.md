...
> **思考**
>
> 询问的是树上从 $\textit{node}$ 到根的路径中，与给定值异或最大的结点。每次从该点爬到根再枚举，在 $n,q\le 10^5$ 时过慢。
>
> 最大异或可用二进制字典树在路径上在线查询。把询问挂到对应结点，DFS 进入时插入结点值、离开时删除，则树上当前路径恰为根到该点。
>
> 每位优先走相反位，即可在 $O(\log V)$ 内回答该点的全部询问。
