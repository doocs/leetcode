...
> **思考**
>
> 交替和按子序列中的奇偶位置加减。枚举所有子序列为指数级；$n\le 10^5$ 必须线性递推。
>
> 记 $f[i]$ 为前 $i$ 个元素、末项处于奇数位（需减去）时的最大交替和，$g[i]$ 为末项处于偶数位（需加上）时的最大值。当前值 $x$ 要么接在另一奇偶之后，要么不选。
>
> 转移为 $f[i]=\max(g[i-1]-x,f[i-1])$，$g[i]=\max(f[i-1]+x,g[i-1])$，答案取二者在 $n$ 处的较大值。
...
> **思考**
>
> 相邻状态只依赖上一位置的 $f$、$g$，数组可以压成两个变量，按从左到右同时更新，空间由 $O(n)$ 降为 $O(1)$。
