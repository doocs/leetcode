...
> **思考**
>
> 「至多一个字母出现奇数次」若对每个子串统计频次，时间为 $O(n^2\cdot\Sigma)$，$n\le 10^5$ 不可行。
>
> 字母仅 $10$ 个，奇偶可用 $10$ 位掩码表示。前缀异或相等则区间全部为偶；相差恰好一位则恰有一个字母为奇。
>
> 扫到位置 $i$ 时，答案累加历史中与当前掩码相同或只差一位的前缀个数，再把当前掩码计入哈希。
