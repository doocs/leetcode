...
> **思考**
>
> 完整回合对齐到 $15$ 分钟整点，直接按字符串比较跨日区间容易出错。将时刻化为当天分钟数后，区间长度至多再加 $1440$。
>
> 登录时刻须向上取整到下一回合起点，登出时刻须向下取整到上一回合终点，二者之差除以 $15$ 即为完整回合数。
>
> 实现上对起点加 $14$ 再整除 $15$，对终点直接整除 $15$，并与 $0$ 取最大，避免登出早于第一个完整回合时得到负数。
