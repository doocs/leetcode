...
> **思考**
>
> 相邻两档高度差超过 $\textit{dist}$ 时必须插入新档，且新档也受 $\textit{dist}$ 限制。$n\le 10^5$、高度达 $10^9$，不能逐步模拟每一档。
>
> 从高度 $a$ 到 $b$ 最少插入 $\lfloor(b-a-1)/\textit{dist}\rfloor$ 档。在起点补 $0$ 后对相邻高度求和即可。
>
> 贪心把插入位置尽量拉满 $\textit{dist}$，不会比该公式更优。
