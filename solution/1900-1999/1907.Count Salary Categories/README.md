...
> **思考**
>
> 按收入分段计数时，某一档可能没有任何账户，直接 $\texttt{GROUP BY}$ 会缺行。题目要求三档始终出现且空档为 $0$。
>
> 因此先构造含 Low、Average、High 的类别表，再对 $\texttt{Accounts}$ 按 $\texttt{CASE}$ 分组计数，最后左连接，使缺档由 $\texttt{IFNULL}$ 补零。
...
> **思考**
>
> 临时表再左连接需要两次扫描与一次连接。三档互斥，也可分别用条件求和各扫一遍表，再用 $\texttt{UNION}$ 拼成三行，空档同样得到 $0$，实现更短。
