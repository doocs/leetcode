...
> **思考**
>
> 访问奇偶决定下一步去 $nextVisit[i]$ 还是 $i+1$，模拟天数可达 $10^{9}$ 量级。第一次走到房间 $i$ 的日期 $f[i]$ 只依赖更小下标。
>
> 从 $i-1$ 首次到达再折返 $nextVisit[i-1]$，区间 $[nextVisit[i-1],i-1]$ 的奇偶与第一次走过时相同，故这段耗时再次为 $f[i-1]-f[nextVisit[i-1]]$，外加两天。
>
> 线性递推并取模。
