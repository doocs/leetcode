...
> **思考**
>
> 若干青蛙的 `croak` 交错出现，长度为 $10^5$，不能回溯匹配每一只青蛙。五个字母必须按 $c\to r\to o\to a\to k$ 的顺序推进。
>
> 用长度为 $5$ 的计数表示当前停在各阶段的叫声。遇到 $c$ 表示新开一只未完成的青蛙；遇到后续字母必须消耗前一阶段，否则非法。$k$ 完成一只，并发数减一。
>
> 答案是过程中未完成青蛙数的最大值；结束时若仍有未完成者，或长度不是 $5$ 的倍数，则返回 $-1$。
