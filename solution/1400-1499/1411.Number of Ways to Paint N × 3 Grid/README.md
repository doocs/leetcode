...
> **思考**
>
> 每格 $3$ 色且相邻不同色，$n\le 5000$，直接给整张网格染色是 $3^{3n}$ 量级。一行仅三格，合法染色可按对称性分成两类：$010$ 型（两色）与 $012$ 型（三色）。
>
> 统计两类各自能转移到下一行的方案数，得到 $g_0=3f_0+2f_1$、$g_1=2f_0+2f_1$。第一行两类各 $6$ 种，递推 $n-1$ 步后求和即可。
...
> **思考**
>
> 方法一依赖人工分类转移系数。列数固定为 $3$，一行至多 $3^3=27$ 种涂色，可把合法行状态显式枚举，再预处理行间是否相邻合法。
>
> $f[i][j]$ 表示第 $i$ 行状态为 $j$ 的方案数，由上一行所有相容状态累加。滚动数组后空间只保留一层，实现不必推导封闭系数。
