...
> **思考**
>
> $a+b+c\le 300$，搜索所有合法串可以做，但最优解具有明确的局部规则：应尽量使用剩余次数最多的字母，同时禁止连续三个相同字符。
>
> 若当前结尾已是两个相同字母，则必须改用次多的字母打断。用大根堆按剩余次数取顶，不合法时改取堆中下一个，用完再将剩余次数压回。
>
> 当无法再放入任何字母而不破坏约束时停止，得到的即最长快乐字符串。
