...
> **思考**
>
> 子序列须非空，元素可正可负，$m,n\le 500$。若按 LCS 只取正贡献会漏掉「只能选一对负数乘积」的情形。
>
> $f[i][j]$ 表示两数组前缀的最大点积：可丢掉 $nums1[i-1]$ 或 $nums2[j-1]$，也可强制配对，并允许丢弃此前的负前缀（$\max(0,f[i-1][j-1])+x\cdot y$）。初值 $-\infty$ 保证至少选一对。
