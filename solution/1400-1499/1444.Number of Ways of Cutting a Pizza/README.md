...
> **思考**
>
> 每次横切或竖切，且切下的部分必须含苹果。$rows,cols\le 50$，$k\le 10$，枚举切法树会重复计算同一剩余矩形与剩余刀数。
>
> 二维前缀和在 $O(1)$ 内判断一块是否含苹果。$dfs(i,j,k)$ 表示左上角为 $(i,j)$ 的剩余披萨再切 $k$ 刀的方案：枚举合法水平/垂直切线，递归右下块。$k=0$ 时剩余块有苹果则计 $1$。
