...
> **思考**
>
> 两个机器人同行向下，各可左/中/右一列。$rows,cols\le 70$，分别搜索路径会重复计算。同行同步后状态只需「行、两人列号」。
>
> $f[i][j_1][j_2]$ 为两人在第 $i$ 行处于 $j_1,j_2$ 时的最大樱桃；同格只计一次。由上一行九种相邻列转移。答案取最后一行最大值。
...
> **思考**
>
> 方法一的第 $i$ 层只依赖第 $i-1$ 层，用两张 $n\times n$ 表滚动即可，时间不变、空间少一维。
