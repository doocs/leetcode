...
> **思考**
>
> $n\le 10$，每位至多 $2$ 种合法后继（首位 $3$ 种），开心串总数不超过 $3\cdot 2^{9}=1536$，按字典序 DFS 生成后取第 $k$ 个即可。
>
> 当前串长为 $n$ 时收入答案；已凑满 $k$ 个则剪枝。相邻字符必须不同，字符集仅为 $a,b,c$。
...
> **思考**
>
> 方法一生成了至多全部开心串。固定前缀后，剩余 $n-i-1$ 位各有 $2$ 种选择，分支大小为 $2^{n-i-1}$，可直接按 $k$ 跳过整段，不必枚举。
>
> 先判断 $k$ 是否超过总数 $3\cdot 2^{n-1}$。随后从左到右枚举与上一位不同的字母，用剩余计数决定写入还是把 $k$ 减去该分支。
