...
> **思考**
>
> 共 $2n$ 个球均分，总方案为 $C_{2n}^n$。颜色种数 $\le 8$、每种至多 $6$ 个，可按颜色枚举放进第一盒的个数。
>
> $dfs(i,j,\textit{diff})$：处理第 $i$ 色、第一盒还剩 $j$ 个空位、两盒颜色数之差为 $\textit{diff}$。全放一侧会改变 $diff$，两边都有则差不变，乘以组合数 $\mathrm{comb}(balls[i],x)$。合法终态除以总方案即概率。
