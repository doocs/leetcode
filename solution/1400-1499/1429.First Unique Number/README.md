...
> **思考**
>
> 需要在多次 `add` 后仍能 $O(1)$ 回答「最早出现且只出现一次的数」。仅用计数器无法保留出现顺序，仅用队列则删除中间重复元素不便。
>
> 计数器记录频次，`OrderedDict` 按首次成为唯一的顺序保存当前频次为 $1$ 的键。`add` 时若频次变为 $1$ 则插入，变为 $2$ 则删除；`showFirstUnique` 取字典的第一个键。
...
> **思考**
>
> 方法一在重复出现时立即从有序字典删除。也可以只把新值追加到双端队列，查询时从队首弹出已经不再唯一的元素，懒删除同样保证队首即答案，实现更短。
