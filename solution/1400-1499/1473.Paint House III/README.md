...
> **思考**
>
> 要把 $m$ 栋房子涂成恰好 $target$ 个街区，未涂色的房子需付费。房子与颜色都不多，状态取「涂到第 $i$ 栋、当前颜色 $j$、街区数 $k$」。
>
> 已涂色则颜色固定、花费为 $0$；未涂色则枚举颜色并加 $cost$。与上一栋同色则街区数不变，异色则加一。取最后一栋各颜色在 $target$ 街区下的最小花费，不可行则为 $-1$。
