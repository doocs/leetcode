...
> **思考**
>
> 子数组的最大最小之差随窗口变长而不减。$n\le 10^5$，枚举所有区间是 $O(n^2)$。固定右端点，左端只需右移到窗口极值差不超过 $limit$。
>
> 有序集合同时给出当前窗口的最小、最大值，超限则删除左端元素。最长合法窗口即答案。
...
> **思考**
>
> 方法一对每个右端点动态收缩，长度本身具有单调性：若存在长为 $mid$ 的合法段，则更短的也存在。可二分长度，再用两个单调队列检查是否有窗口满足差值约束。
>
> 时间仍为 $O(n\log n)$，把「求最长」改成了判定问题。
...
> **思考**
>
> 方法一、二都带对数因子。窗口最大、最小本身可用两个单调双端队列在摊还 $O(1)$ 内维护，右端扩张、左端在差值超限时收缩，总体线性。
