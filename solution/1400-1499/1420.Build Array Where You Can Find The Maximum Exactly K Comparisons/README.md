...
> **思考**
>
> 搜索代价等于从左到右刷新最大值的次数。$n\le 50$、$m\le 100$，枚举全部数组不可行，需按「长度、代价、当前最大值」递推。
>
> 第 $i$ 位若不刷新最大值，可在 $[1,j]$ 中任取，方案数为上一层同代价、同最大值乘 $j$；若恰好写入新最大值 $j$，则由更小最大值、代价减一的状态累加。
>
> $k=0$ 不可能。对 $dp[n][k][\cdot]$ 求和即答案。
