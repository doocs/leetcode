...
> **思考**
>
> 对每个子集再枚举烹饪顺序，$n\le 500$ 无法承受。满意度可正可负，空菜单的得分为 $0$，因此只需在「按时间系数放大后的加权和」中取最大者。
>
> 同一组菜应让满意度高的菜获得更大的时间系数，故最优顺序必为升序烹饪。从高到低依次纳入一道菜时，总得分的增量恰好等于当前已选菜的满意度之和。
>
> 将数组降序排列，累加前缀和 $s$，仅当 $s>0$ 时把 $s$ 加入答案；一旦前缀非正，再纳入更小的菜只会降低得分，随即停止。
