...
> **思考**
>
> 子序列相邻下标差至多为 $k$，求最大和。$n\le 10^5$，若对每个 $i$ 枚举 $[i-k,i-1]$ 的最优 $f$，则达到 $O(nk)$。
>
> $f[i]=\textit{nums}[i]+\max(0,\max_{i-k\le j<i}f[j])$，即长度为 $k$ 的窗口最大值。单调递减队列按 $f$ 维护下标，队首过期则弹出，转移成为摊还 $O(1)$。
>
> 元素可为负，因此允许「不接前缀」而单独从 $i$ 开始。答案取所有 $f[i]$ 的最大者。
