...
> **思考**
>
> 子数组评分为和与长度之积。枚举全部子数组需 $O(n^2)$，$n \le 10^5$ 不可行。元素均为正，故固定右端后，窗口越长积分越大，合法左端构成前缀。
>
> 用前缀和把任意区间和变成常数查询，再对长度二分，找到使 $(s[i]-s[i-l])\times l < k$ 的最大 $l$。每个右端点贡献 $l$ 个子数组。
...
> **思考**
>
> 方法一额外需要前缀数组，且每个右端点付出对数代价。正数保证积分随左端右移单调下降，因此左指针只需前进、不必回退。维护窗口和，一次扫过即可在线性时间内统计合法子数组。
