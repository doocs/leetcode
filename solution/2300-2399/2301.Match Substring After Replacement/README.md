...
> **思考**
>
> 映射只允许一步替换，因此判断 $sub$ 能否对准 $s$ 的某一段，只需按长度枚举起点并逐位核验。$|s| \le 5000$，对齐次数与子串长度之积约为 $2.5 \times 10^7$，可以接受。
>
> 同一字符可能对应多个新字符，逐次扫描映射表会重复工作。将每个旧字符的可达集合收入哈希表后，对齐时 $O(1)$ 查询「相等或可替换」即可。
...
> **思考**
>
> 方法一用哈希集合存储映射，常数受散列影响。字符集仅为大小写字母与数字，因此改用 $128 \times 128$ 的布尔数组标记可达关系，查询与空间均变为下标访问，算法流程不变。
