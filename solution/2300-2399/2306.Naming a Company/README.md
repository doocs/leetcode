...
> **思考**
>
> 合法公司名由两个不同想法互换首字母得到，且互换后两串均不在原集合中。两两配对需 $O(n^2)$，$n \le 5 \times 10^4$ 不可行。
>
> 首字母只有 $26$ 种，后缀是否冲突决定一对能否互换。用集合判重后，统计「以字母 $i$ 开头、换成 $j$ 后不在集合中」的个数 $f[i][j]$。再次遍历时，一个义项换成 $j$ 合法，则它可与 $f[j][i]$ 个对方配对，从而在线性于 $n|\Sigma|$ 的时间内计数。
