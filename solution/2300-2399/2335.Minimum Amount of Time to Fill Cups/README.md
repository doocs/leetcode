...
> **思考**
>
> 每秒可给两种不同杯子各接 $1$，或只接一种。总量不超过 $300$，反复取当前最多的两杯减一即可。
>
> 每次排序后减少最大的两个（第二大已为 $0$ 则只减一个），直到全为 $0$。贪心保证每秒尽量消化两份需求。
...
> **思考**
>
> 方法一按秒模拟。将三数排序为 $a \le b \le c$ 后可闭式求解：若 $a+b \le c$，较短的两杯可在 $c$ 秒内穿插接完；否则总能两两配对，答案为 $\lfloor (a+b+c+1)/2 \rfloor$。
