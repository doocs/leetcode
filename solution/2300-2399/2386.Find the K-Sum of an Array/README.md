...
> **思考**
>
> 子序列和的第 $k$ 大。最大和 $mx$ 是全体正数之和，其余和等于 $mx$ 减去某段绝对值子序列。$n \le 10^5$ 且 $k \le 2000$，不能枚举 $2^n$。
>
> 把绝对值排序后，用小根堆按不降顺序展开子序列和：每次弹出 $(s,i)$，再压入「加上 $nums[i]$」与「用 $nums[i]$ 替换 $nums[i-1]$」。第 $k-1$ 次弹出后堆顶即第 $k$ 小的减去量。
