...
> **思考**
>
> `L` 只能左移、`R` 只能右移，且二者不能穿过对方。$n \le 10^5$，模拟逐步移动过慢。去掉空位后，两串的字母序列必须相同。
>
> 抽出非 `_` 字符及其下标。对应字母不同则失败；`L` 的起点不能在目标右侧，`R` 不能在目标左侧。
...
> **思考**
>
> 方法一先物化两份位置列表。双指针同步跳过 `_`，在原串上做同一组下标比较，省去额外数组，逻辑不变。
