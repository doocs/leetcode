...
> **思考**
>
> 理想数组要求后项能整除前项。$n$ 与 $\textit{maxValue}$ 均可达 $10^4$，不能枚举数组。一条严格倍增链的长度不超过 $\log \textit{maxValue}$。
>
> 先统计以 $i$ 结尾、含 $j$ 个不同值的链数 $f[i][j]$，再把长度为 $j$ 的链用隔板法扩成 $n$ 位，乘组合数 $c_{n-1}^{j-1}$。组合数按行递推，链长上限取 $16$。
