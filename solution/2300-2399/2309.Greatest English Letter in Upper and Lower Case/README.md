...
> **思考**
>
> $s$ 长度不超过 $1000$，需要同时出现大小写的最大字母。将出现过的字符放入集合后，从 $Z$ 到 $A$ 检查即可，第一次命中便是答案。
>
> 集合保证大小写各一次哈希查询。若都不存在，返回空串。
...
> **思考**
>
> 方法一占用与字符集同阶的额外空间。字母只有 $26$ 个，可用两个整型掩码分别记录大小写出现位，再取交。最高的置位对应最大字母，从而把空间压到常数。
