...
> **思考**
>
> 木块可反复横切或竖切，每块也可整块出售。搜索所有切割序列会大量重复同一尺寸。$m, n \le 200$，不同矩形只有 $O(mn)$ 种。
>
> 子问题只由高、宽决定：整卖价格与一次切开后两块最优值之和取最大。用哈希表记下给定价格，记忆化搜索 $dfs(h,w)$，横切只枚举到一半以免对称重复。
...
> **思考**
>
> 方法一依赖递归与缓存，常数与调用开销较大。按面积从小到大填表，同一转移写成两重循环，语义不变而避免递归栈，并便于滚动或并行扩展。
