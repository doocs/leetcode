...
> **思考**
>
> 只能交换一段对应子数组，目标是使某一个数组的元素和最大。$n \le 10^5$，不能枚举区间。交换 $[l,r]$ 后，$nums2$ 的和增加 $\sum(nums1_i-nums2_i)$ 在该区间上的值。
>
> 问题化为最大子段和：对差分数组做 Kadane，得到给 $nums2$ 的最大增益；对称地再算给 $nums1$ 的增益。与原数组和相加后取较大者。
