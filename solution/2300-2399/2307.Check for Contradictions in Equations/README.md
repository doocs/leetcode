...
> **思考**
>
> 每个等式给出两个变量的比值。若沿不同路径把同一对变量连起来，比值可能冲突。变量至多两百个，等式至多 $100$ 条，适合并查集维护连通关系。
>
> 将变量映射为整数后，用权值 $w[x]$ 表示 $x$ 到根的比值。合并时按乘法更新权；若两点已在同一集合，则用路径压缩后的权检验 $v \cdot w[a]$ 是否与 $w[b]$ 在误差内一致。出现偏差即存在矛盾。
