...
> **思考**
>
> 要把 $n \le 8$ 包饼干分给 $k \le 8$ 个孩子并最小化最大份量。若无约束地枚举分配，状态数为 $k^n$，在上限附近过于庞大。
>
> 同一分配的不公平度与包的先后无关，但大包先放更容易触发剪枝。将包降序排列后回溯：若某孩子加上当前包已不小于已知答案则跳过；相邻孩子份量相同则视为对称，只尝试一种。全部包分完后用当前最大值更新答案。
