...
> **思考**
>
> 需要一段长度为 $k$ 的子数组，其最小值大于 $\textit{threshold}/k$。$n \le 10^5$，枚举区间并求最小值不可行。以某值为最小值的最长段，由其左右更小元素隔开。
>
> 从大到小加入元素并用并查集连通已加入的邻居。当前值 $v$ 恰为其所在块的最小值，块长为 $sz$ 时若 $v > \textit{threshold}/sz$ 则找到答案。
...
> **思考**
>
> 方法一并查集需排序，额外对数因子。单调栈可在线性时间内求出每个位置作为最小值的左右边界，从而直接得到 $k$ 并检验同一不等式，省去按值合并。
