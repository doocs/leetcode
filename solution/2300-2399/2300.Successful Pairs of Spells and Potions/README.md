...
> **思考**
>
> 若对每个咒语扫描全部药水，时间复杂度为 $O(nm)$。$n, m \le 10^5$，无法通过。
>
> 固定咒语 $v$ 后，成功条件化为 $potions[j] \ge \frac{success}{v}$，只取决于药水强度的单调分界。为此先将药水升序排列，再用二分定位第一瓶满足阈值的药水，其后整段后缀均可配对。每个咒语只需一次对数查找。
