...
> **思考**
>
> 把一个数拆成若干正整数之和，目标是最终非递减，且拆分次数最少。$n \le 10^5$，须从右往左确定上界。
>
> 右侧已确定的最大值 $mx$ 不应再被拆小。若 $nums[i] \le mx$ 则更新 $mx$；否则拆成 $k=\lceil nums[i]/mx \rceil$ 段，次数加 $k-1$，并把新的上界改成 $\lfloor nums[i]/k \rfloor$，以便左侧仍尽量大。
