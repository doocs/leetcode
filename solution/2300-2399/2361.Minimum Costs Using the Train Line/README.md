...
> **思考**
>
> 两线并行，特快首次上车另付 $expressCost$。$n \le 10^5$，须线性递推。到达第 $i$ 站的最小花费只与上一站所在线路有关。
>
> 令 $f[i]$、$g[i]$ 分别为坐常规、特快到达 $i$ 的最小费用。常规可由任一线路加 $a_i$；特快可由常规加首乘费再加 $b_i$，或继续特快只加 $b_i$。第 $i$ 站答案为二者较小值。
...
> **思考**
>
> 方法一保留整表。转移只用上一站的两个数，滚动为变量 $f,g$ 即可，额外空间降到常数，输出数组仍要 $O(n)$。
