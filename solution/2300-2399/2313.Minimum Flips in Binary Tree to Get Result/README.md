...
> **思考**
>
> 每个叶子可翻 $0/1$，内部结点是逻辑运算。对每个叶子尝试翻转再求值，状态数为指数级，树至多上千结点时不可行。
>
> 一棵子树对答案的贡献只体现为把它算成假或真的最少翻转。自底向上返回二元组：代价做成 $false$ 与做成 $true$。按结点类型（叶、$OR$、$AND$、$XOR$、$NOT$）组合左右儿子的四种或两种情形，根上取目标真值对应的那一维。
