...
> **思考**
>
> 删去若干元素后，剩下的最小值须整除 $numsDivide$ 中每一个。该最小值必是 $\gcd(numsDivide)$ 的因子。$n$ 可达 $10^5$。
>
> 先求 $x=\gcd(numsDivide)$，再将 $nums$ 升序扫描，第一个整除 $x$ 的值之前的个数即为删除次数；若没有则无解。
...
> **思考**
>
> 方法一为找最小合法因子而排序。直接扫描得到能整除 $x$ 的最小 $y$，再数小于 $y$ 的元素，时间降为线性。
