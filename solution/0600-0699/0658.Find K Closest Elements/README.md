...
> **思考**
>
> 有序数组中找与 $x$ 最接近的 $k$ 个数，结果仍要有序。按距离排序再取前 $k$ 个即可，代价是 $O(n\log n)$。
>
> 以 $|v-x|$ 为键排序，截取前 $k$ 个后再按值排序。
...
> **思考**
>
> 答案必是一段连续子数组。两端指针按距离把更远的一侧收进来，直到长度恰为 $k$，线性即可，且结果已有序。
...
> **思考**
>
> 双指针是线性。窗口左端在 $[0,n-k]$ 上单调：比较 $x-arr[mid]$ 与 $arr[mid+k]-x$ 即可二分左端，降到 $O(\log n)$。
