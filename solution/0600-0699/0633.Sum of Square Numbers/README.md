...
> **思考**
>
> 判断是否存在 $a^2+b^2=c$。对每个 $a$ 再开方检查 $c-a^2$ 需要多次开方。$c$ 可达 $2^{31}-1$，从 $0$ 与 $\sqrt{c}$ 两端相向移动即可。
>
> 和偏小则增大 $a$，偏大则减小 $b$。平方关于指针单调，不会漏解。
...
> **思考**
>
> 双指针是 $O(\sqrt{c})$ 的搜索。数论上，$c$ 能写成两平方和当且仅当所有形如 $4k+3$ 的素因子幂次为偶数。质因数分解后按此判定，复杂度同阶但不再枚举 $(a,b)$。
