...
> **思考**
>
> 要从 $k$ 个有序列表各取至少一个数，使覆盖区间最短。枚举每个列表的下标组合在总长度较大时不可行。
>
> 把所有 $(值, 组号)$ 排成一条有序序列后，问题变成最短窗口恰好覆盖 $k$ 个组。右端扩张、左端在组数仍为 $k$ 时收缩，用计数表维护窗口内的组。
...
> **思考**
>
> 方法一先合并再滑窗，要放下全部元素。各列表已有序，可用小根堆始终拿当前 $k$ 个指针中的最小端，并维护最大值；弹出一组后推进该组指针。空间降到 $O(k)$，无需预先展开。
