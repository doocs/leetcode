...
> **思考**
>
> $[1,n]$ 中一数重复、一数缺失。扫描标记即可，也可用和差一次求出。
>
> 记 $s$ 为数组和、$s_2$ 为去重和、$s_1$ 为 $1..n$ 之和。则重复为 $s-s_2$，缺失为 $s_1-s_2$。
...
> **思考**
>
> 数学法依赖去重集合。直接计每个值的出现次数，出现 $2$ 次与 $0$ 次的即答案，读写更直观。
...
> **思考**
>
> 前两法都要线性额外空间。把 $nums$ 与 $1..n$ 全部异或得到 $a\oplus b$，再用 $\textit{lowbit}$ 分组，两组分别异或出 $a,b$，看谁出现在数组里即知重复与缺失。
