...
> **思考**
>
> 有向树多一条边后，可能出现入度为 $2$ 的点，或指向根形成环，或二者兼有。无向并查集不能直接处理双入边。
>
> 先找入度为 $2$ 的两条边。若存在，尝试丢掉较后的那条再并查：仍成环则应删较早的。若没有双入边，则按无向情形删掉成环的那条。
...
> **思考**
>
> 判定逻辑与方法一相同，只是把路径压缩与按秩合并收进模板类，主函数只处理 `dup` 与 `union` 失败。
