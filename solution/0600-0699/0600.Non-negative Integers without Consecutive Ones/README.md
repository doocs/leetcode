...
> **思考**
>
> 若枚举 $[0, n]$ 并逐个检查二进制是否含连续 $1$，当 $n$ 达到 $10^9$ 时扫描量过大。合法计数只依赖已填位数与前一位是否为 $1$，与具体数值无关。
>
> 因此按二进制从高位向低位填写，状态取 $(i, \textit{pre}, \textit{limit})$：若 $\textit{pre}=1$ 则当前位不能再填 $1$；$\textit{limit}$ 保证不超过 $n$。记忆化后复杂度与位数成正比。
