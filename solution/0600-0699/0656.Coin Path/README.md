...
> **思考**
>
> 从下标 $0$ 跳到末尾，步长不超过 $\textit{maxJump}$，避开 $-1$，要最小花费且字典序最小的下标序列。正向记录路径时，花费相同要比较整段前缀。
>
> 倒着定义 $f[i]$ 为从 $i$ 到终点的最小花费，转移看后面至多 $\textit{maxJump}$ 格。还原时从左往右取满足 $f[i]$ 等于剩余预算的最左下标，自然得到字典序最小路径。
