...
> **思考**
>
> 无向图由树加一条边而成，要删掉使图重新成树的那条（输入中最后出现的）。DFS 找环再比边序偏繁。
>
> 按输入顺序并查集连边：两端已连通则这条边在环上，即为答案；否则合并。第一次冲突就是最后那条多余边。
...
> **思考**
>
> 方法一内嵌 `find`。抽出带按秩合并的 `UnionFind`，`union` 返回是否成功，主循环只判断失败即返回，逻辑相同、常数更好。
