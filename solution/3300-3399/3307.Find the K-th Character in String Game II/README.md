...
> **思考**
>
> $k \le 10^{14}$，不能再像 I 那样构造字符串。每次操作使长度翻倍，第 $k$ 个字符由若干次「是否后移一位」决定。
>
> 先找到刚超过 $k$ 的长度 $n=2^i$，再沿操作逆序回溯：若 $k$ 落在后半段，则它来自前半对应位置，并按 $\textit{operations}[i-1]$ 决定是否累加 $1$，同时把 $k$ 映射回前半。
>
> 直到长度为 $1$，累加量对 $26$ 取模即得字母。回溯次数为 $O(\log k)$。
