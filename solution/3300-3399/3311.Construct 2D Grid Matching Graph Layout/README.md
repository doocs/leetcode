...
> **思考**
>
> 输入保证是网格图。$n \le 5 \times 10^4$，不能回溯填格。度数刻画位置：$1$ 度为单行端点，$2$ 度为角落或单行内部，$3$ 度为边界，$4$ 度为内部。
>
> 先按度数构造第一行：存在 $1$ 度则整图是一行；否则从 $2$ 度角落沿「度数小于 $4$」的边走出第一行。
>
> 此后每一行由上一行各点尚未访问的邻居唯一确定，逐行铺满即可。
