...
> **思考**
>
> 每位字母可沿字母表正向或反向循环移动，费用分别为 $\textit{nextCost}$ 与 $\textit{previousCost}$。$n \le 10^5$，需对每位在 $O(1)$ 内求出两种方向的代价。
>
> 把费用数组复制一份接在后面，前缀和即可在环上取任意弧长。
>
> 正向用 $s1[y]-s1[x]$（必要时 $y$ 加 $26$），反向对称，取较小者累加。
