...
> **思考**
>
> 好子序列要求相邻元素差的绝对值为 $1$。$n \le 10^5$，不能枚举子序列，应按值做递推。
>
> 令 $g[x]$ 为以 $x$ 结尾的好子序列条数，$f[x]$ 为其元素和。新的 $x$ 可单独成段，也可接在已有的 $x-1$ 或 $x+1$ 之后。
>
> 接上时，新和为「旧和 + 旧条数 $\times x$」。按输入顺序更新，才能把「当前 $x$」与「此前同值」一并计入。最后对所有 $f$ 求和取模。
