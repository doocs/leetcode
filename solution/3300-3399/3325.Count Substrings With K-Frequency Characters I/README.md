...
> **思考**
>
> 统计至少有一个字符出现不少于 $k$ 次的子串。$n \le 3000$，枚举端点再扫描窗口可行，但可再压到线性。
>
> 该条件对左端点单调：窗口一旦含某个字符 $k$ 次，再向左扩张仍然合法。因此可维护「所有字符次数都小于 $k$」的最长右窗口。
>
> 右端纳入 $c$ 后，若 $c$ 的次数达到 $k$，则右移左端直至次数回到 $k$ 以下。此时左端在 $[0, l)$ 的子串均合法，答案累加 $l$。
