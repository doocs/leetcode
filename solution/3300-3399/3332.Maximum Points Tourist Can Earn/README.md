...
> **思考**
>
> 共 $k$ 天、每天停在一城或迁往另一城，得分取停留分或旅途分。$n,k \le 200$，状态数为 $O(nk)$，转移枚举来源城市即可。
>
> $f[i][j]$ 表示第 $i$ 天结束时在 $j$ 的最大得分。从 $h$ 留下则加 $\textit{stayScore}[i-1][j]$，从 $h$ 迁来则加 $\textit{travelScore}[h][j]$。
>
> 第 $0$ 天尚未行动，各城得分为 $0$。答案为 $f[k]$ 的最大值。
