...
> **思考**
>
> 长度为 $5$ 的子序列，中间项是唯一众数。$n \le 1000$，枚举中间下标后，左右各选两个下标。
>
> 暴力 $O(n^5)$ 不可行。固定中点 $x=\textit{nums}[i]$ 后，按左右出现次数分类：中点必须出现得比任何其他值更频繁。
>
> 用左右频率表组合计数，减去中点不唯一或不为众数的情况。对每个 $i$ 做 $O(n)$ 预处理后，$O(1)$ 或 $O(|\Sigma|)$ 统计。
