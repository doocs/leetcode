...
> **思考**
>
> 每组连续相同字符至少保留 $1$ 个，总长至少为 $k$。$n \le 5 \times 10^5$ 且 $k \le 2000$，不能枚举每组取值，宜把「长度 $\ge k$」拆成「无限制方案 $a$」减去「长度 $< k$ 的方案 $b$」。
>
> 每组先强制选 $1$ 个，$a$ 为各组长度之积；剩余可选个数写入 $\textit{nums}$，并把 $k$ 减去组数。若此时 $k < 1$，则 $a$ 即为答案。
>
> $f[i][j]$ 表示前 $i$ 组额外再选 $j$ 个的方案。转移是一段连续和，用前缀和优化到 $O(k)$。$b$ 为最后一行不足 $k$ 的和，$a-b$ 即所求。
