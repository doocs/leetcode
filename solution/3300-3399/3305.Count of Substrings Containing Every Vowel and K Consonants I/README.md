...
> **思考**
>
> $|\textit{word}| \le 250$，枚举子串再统计可行；若直接维护「恰含 $k$ 个辅音且五元音齐全」，左右端点会同时受两种约束，窗口单调性不完整。
>
> 将「恰好 $k$ 个辅音」写成「至少 $k$ 个」减去「至少 $k+1$ 个」。后者只含「$\ge$」型条件，可用滑动窗口：右端推进后，在五元音齐备且辅音不少于阈值时尽量右移左端。
>
> 此时以当前右端为结尾、左端落在 $[0, l)$ 的子串均合法，答案累加 $l$。哈希表只记录五种元音，辅音用计数器 $x$。
