...
> **思考**
>
> 表演者分到至多 $x$ 个非空节目，每个节目再选 $y$ 种得分。$n,x \le 1000$，可按「人数 $\times$ 已用节目数」递推。
>
> 第 $i$ 人要么加入已有 $j$ 个节目之一，要么开启一个新节目（尚余 $x-j+1$ 个空位）。两种选择互斥且覆盖全部划分。
>
> 处理完全部人后，对每个 $j$ 乘上 $y^j$，即为每个非空节目染色的方案，再对 $10^9+7$ 取模。
