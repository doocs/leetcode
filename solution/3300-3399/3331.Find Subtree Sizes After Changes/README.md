...
> **思考**
>
> 每个节点可改挂到最近的、字母相同的祖先之下。$n \le 10^5$，不能先显式改边再另做一遍子树统计。
>
> 一次 DFS 即可：用栈 $\textit{d}[c]$ 记录字母 $c$ 的祖先链。回溯前若存在更近的同字母祖先，则把当前子树大小加到该祖先上，否则加到父节点。
>
> 后序保证 $\textit{ans}[i]$ 已含全部后代；弹栈后祖先链恢复，互不干扰。
