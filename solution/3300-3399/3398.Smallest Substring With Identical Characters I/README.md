...
> **思考**
>
> 至多翻转 $\textit{numOps}$ 位，使最长的相同字符段尽量短。$n \le 1000$，对目标长度 $m$ 二分。
>
> $m=1$ 时只能变成 `0101...` 或 `1010...`，取与 $s$ 差异较小的那种。$m>1$ 时，每段连续相同字符长度为 $k$ 则至少翻 $\lfloor k/(m+1) \rfloor$ 次。
>
> 若总翻转次数不超过 $\textit{numOps}$，则 $m$ 可行。二分最小可行 $m$。
