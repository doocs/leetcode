...
> **思考**
>
> 每个 $x$ 可变成 $[x-k,x+k]$ 内任一整数，至多改 $\textit{numOperations}$ 次，使某个值频率最大。$n \le 10^5$，不能枚举目标再扫数组。
>
> 目标 $y$ 能吸引所有区间覆盖 $y$ 的元素；其中已经等于 $y$ 的不必改。差分数组在 $x-k$ 加一、$x+k+1$ 减一，并在 $x$ 处插 $0$ 以便统计原值。
>
> 按坐标排序做前缀和得覆盖数 $s$，频率为 $\min(s,\textit{cnt}[y]+\textit{numOperations})$，取最大。
