...
> **思考**
>
> 每次把某个数位加一或减一（不得有前导零），且不能走到质数，代价为经过的数值之和，求 $n$ 到 $m$ 的最小代价。值不超过 $10^4$，可在隐式图上最短路。
>
> 先筛出 $10^5$ 内的质数。$n$ 或 $m$ 为质数则无解。其余每位 $\pm 1$ 作为边，边权为到达的新值。
>
> Dijkstra 以累计和为关键字；第一次弹出 $m$ 即为最小和。
