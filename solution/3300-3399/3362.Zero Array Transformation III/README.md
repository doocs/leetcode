...
> **思考**
>
> 与前两题不同，询问可任选子集，目标是尽量少用（即多剩）。每位 $i$ 需要至少 $\textit{nums}[i]$ 次覆盖，否则无解。
>
> 按左端排序后扫描 $i$：把已开始的区间右端丢进最大堆。仅当当前覆盖不足时，才取出仍覆盖 $i$ 的最晚结束区间。
>
> 最晚结束的区间对后续更有用，因此贪心保留它们到不得不取之时。差分数组在 $r+1$ 处回退覆盖。堆中剩余即为未使用的询问数。
