...
> **思考**
>
> 与 I 相比，奇偶步交替耗时 $1$ 与 $2$，且网格更大。最短路模型不变，只是边权变成 $\max(\textit{moveTime}[x][y],d)+(i+j)\bmod 2+1$。
>
> $(i+j)\bmod 2$ 恰为到达 $(i,j)$ 后下一步的奇偶：从 $(0,0)$ 出发，下一步耗时 $1$，再下一步耗时 $2$，与坐标奇偶一致。
>
> 仍用 Dijkstra 按时间松弛；终点首次出堆即为答案。
