...
> **思考**
>
> 每个 $x$ 可变成 $[x-k,x+k]$ 内任一整数，求最多能得到多少个不同值。$n \le 10^5$，排序后从左往右贪心占最小还能用的整数。
>
> 已用最大值记为 $\textit{pre}$。当前 $x$ 取 $\min(x+k,\max(x-k,\textit{pre}+1))$，若仍大于 $\textit{pre}$ 则占用并计数。
>
> 先占较小值给后面更大的区间留下空位，因此必须先排序。
