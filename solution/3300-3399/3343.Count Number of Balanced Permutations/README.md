...
> **思考**
>
> 平衡排列要求奇数位数字和等于偶数位。总和为奇数则无解。$n \le 80$，不能枚举排列，应按数字种类分配到奇偶两位。
>
> $\textit{dfs}(i,j,a,b)$ 表示从数字 $i$ 起，奇数位还需和 $j$、还需 $a$ 个位置，偶数位还需 $b$ 个位置。对数字 $i$ 枚举分给奇数位的个数 $l$，偶数位得 $r=\textit{cnt}[i]-l$。
>
> 方案数乘组合数 $C_a^l C_b^r$，再递归下一种数字。各位独立，组合数避免对相同数字重复计数。
