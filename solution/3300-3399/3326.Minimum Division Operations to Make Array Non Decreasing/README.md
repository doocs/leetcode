...
> **思考**
>
> 每次把 $x$ 除以一个真因数。质数无法再变小；合数除以最大真因数后变成最小质因数，此后不能再除。$n \le 10^5$、$M \le 10^6$，需预处理最小质因数。
>
> 从左往右改会破坏尚未确定的右侧约束。从右往左看，右边已经固定为最终值，左边若更大则只能立刻变成 $\textit{lpf}[x]$。
>
> 若变成最小质因数后仍大于右侧，则无法非递减。每个位置至多操作一次，故从右往左扫描并计数即可。
