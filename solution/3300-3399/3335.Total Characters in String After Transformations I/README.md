...
> **思考**
>
> 每次把字母后移一位，$\texttt{z}$ 变成 $\texttt{ab}$。$t \le 10^5$，不能对字符串本身模拟。
>
> 长度只依赖 $26$ 个字母的计数。令 $f[i][j]$ 为 $i$ 次变换后字母 $j$ 的个数：$\texttt{z}$ 贡献给 $\texttt{a}$ 与 $\texttt{b}$，其余由前一字母移来。
>
> 答案为 $f[t]$ 各维之和，并对 $10^9+7$ 取模。
