...
> **思考**
>
> 要判断是否存在两个相邻、长度均为 $k$ 的严格递增段。$n \le 100$，枚举起点可行，一次扫描也能直接得到最大可行 $k$。
>
> 相邻两段要么来自同一段递增被从中切开（长度为 $\lfloor \textit{cur}/2 \rfloor$），要么来自两段紧邻递增的较短者 $\min(\textit{pre},\textit{cur})$。
>
> 在递增断裂处更新这两种候选，最后看最大值是否至少为 $k$。
