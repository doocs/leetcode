...
> **思考**
>
> $a \lor (a+1)$ 总会把 $a$ 最低的 $0$ 置 $1$，结果必为奇数。本题元素为质数，偶数只有 $2$，故 $2$ 无解。
>
> 对奇数 $x$，最小的 $a$ 是把 $x$ 中最低 $0$ 的下一位 $1$ 打掉，使 $a \lor (a+1)$ 仍等于 $x$。
>
> 从低到高找第一个为 $0$ 的位 $i$，令 $a = x \oplus 2^{i-1}$。$n \le 100$ 且 $x \le 10^3$，逐位扫描即可。
