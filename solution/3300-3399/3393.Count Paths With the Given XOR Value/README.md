...
> **思考**
>
> 从左上走到右下，只能向右或向下，路径 XOR 等于 $k$。网格至多 $300 \times 300$，格子值小于 $16$，可用 $f[i][j][x]$ 计数。
>
> XOR 值域只有 $16$，第三维很小。转移到 $(i,j)$ 时把来自上方与左方的 $x$ 异或上 $\textit{grid}[i][j]$。
>
> 答案为 $f[m-1][n-1][k]$，并对 $10^9+7$ 取模。
