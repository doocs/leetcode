...
> **思考**
>
> $n \le 50$，每个窗口重算频率再取前 $x$ 也可；为与更大规模的 II 共用结构，这里用有序集合维护「当前 top-$x$」与其余元素。
>
> 窗口滑动时，频率变化会使同一数值在两个集合之间移动。先按旧频率删除、更新计数、再按新频率插入，才能保持集合有序。
>
> 变量 $s$ 同步 top-$x$ 的加权和，避免每次从集合重算。当 $|l| \ne x$ 时从另一侧补进或弹出，即可得到该窗口的 $x$-sum。
