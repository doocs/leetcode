...
> **思考**
>
> 题意与 I 相同，但 $n \le 10^5$，每个窗口重算频率不可行，必须在滑动时 $O(\log n)$ 维护 top-$x$。
>
> 两个有序集合分别存放当前 top-$x$ 与其余元素，哈希表记录频率。插入删除仍按「先删旧对、再插新对」的顺序。
>
> 用 $s$ 维护 top-$x$ 的加权和，窗口两端进出后调整集合大小至 $x$，即可线性扫完所有窗口。
