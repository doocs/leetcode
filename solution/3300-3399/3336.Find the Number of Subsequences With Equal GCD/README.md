...
> **思考**
>
> 要统计两段不相交子序列 GCD 相等的方案。$n,M \le 200$，按「下标、第一段 GCD、第二段 GCD」记忆化，状态约为 $n M^2$。
>
> 每个元素有三种去向：不选、进入第一段、进入第二段，GCD 用 $\gcd(\textit{nums}[i], \cdot)$ 更新；空段的 GCD 约定为 $0$。
>
> 终点要求两段 GCD 相等。初始调用含两个空段，答案需减 $1$ 后再取模。
...
> **思考**
>
> 方法一的递归深度与三维缓存带来额外常数。同一转移可写成滚动数组：处理 $x$ 时用新表 $g$ 接收三种去向。
>
> 空间从 $O(n M^2)$ 降为 $O(M^2)$，且避免递归开销。答案仍为对角元之和减 $1$。
>
> 跳过 $f[j][k]=0$ 的格子，实际常数往往更小。
