...
> **思考**
>
> 各塔高度须互不相同且不超过各自上界。$n \le 10^5$，枚举分配方案不可行。若从矮塔开始占位，高塔可能失去本可使用的较大高度。
>
> 总和最大要求优先把更大的整数分给上界更大的塔。将 $\textit{maximumHeight}$ 降序处理后，用 $mx$ 记录上一座已分配高度。
>
> 当前塔取 $\min(x, mx-1)$：既不超过自身上界，又与已分配高度严格递减。若得到非正值，则无法构造合法方案。
