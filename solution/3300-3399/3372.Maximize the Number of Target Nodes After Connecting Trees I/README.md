...
> **思考**
>
> 把第一棵树的 $i$ 连到第二棵的某个 $j$，目标节点是距离不超过 $k$ 的点。$n,m \le 1000$，可对每个起点 DFS。
>
> 连边后，$i$ 侧贡献为深度 $\le k$ 的点数；$j$ 侧深度预算变成 $k-1$。第二棵树上这个量与 $i$ 无关，取全局最大即可。
>
> 因此先算第二棵每个点深度 $\le k-1$ 的点数的最大值 $t$，再对第一棵每个 $i$ 加上深度 $\le k$ 的点数。
