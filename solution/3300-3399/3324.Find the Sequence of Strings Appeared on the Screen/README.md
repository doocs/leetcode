...
> **思考**
>
> 屏幕从空串开始，每次追加一个字母或把末位沿字母表推进。$|\textit{target}| \le 400$，按目标逐位模拟即可。
>
> 对目标的每一位，从 $\texttt{a}$ 依次追加到当前前缀，直到追加出该位为止。中间每一个串都是题目要求记录的屏幕状态。
>
> 因此不必搜索按键序列，沿 $\textit{ascii\_lowercase}$ 走到目标字符即得到全部中间结果。
