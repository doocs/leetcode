...
> **思考**
>
> 题目要求返回值为 $\textit{val}$ 的节点及其整棵子树。节点数不超过 $5000$，整树扫描在时间上可以接受，但会丢掉二叉搜索树「左小右大」这一结构。
>
> 若当前节点值已经等于目标，答案就是它本身；若更大，目标只可能在左子树，反之只可能在右子树。因此每次比较后只需进入一侧，不必同时展开两棵子树。
>
> 递归自然对应这一过程：空节点或命中则返回当前节点，否则按大小关系进入左或右。最坏仍可能走到一条链，时间与栈空间均为 $O(n)$。
