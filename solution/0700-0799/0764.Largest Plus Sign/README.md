...
> **思考**
>
> 在 $n\le 500$ 的网格上找最大阶加号，$O(n^3)$ 对每个中心扩臂偏紧。阶数等于中心向四方向连续 $1$ 的最小值。
>
> 四个方向的连续长度可各做一次线性扫描。把地雷先标成 $0$，其余初始化为 $n$，再取与四向计数的最小值。
>
> 同一趟里用 $j$ 与 $n-1-j$ 同时更新左右、上下四个计数，最后取全表最大。时间 $O(n^2)$。
