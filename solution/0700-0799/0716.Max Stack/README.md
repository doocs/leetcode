...
> **思考**
>
> 除普通栈的入出栈与读顶外，还要 $O(1)$ 或接近对数地读最大、并删除最近的那个最大值。仅用一个栈无法在中间删元；两个栈能取最大，却不能按节点摘除。
>
> 双向链表保持入栈顺序，任意节点都可以 $O(1)$ 拆掉；再把节点放进按值排序的有序集合，即可在对数时间内找到当前最大。二者指向同一批节点，删除时两边同时更新。
>
> $\textit{push}$ 链尾追加并插入有序集合；$\textit{pop}$ 删链尾；$\textit{popMax}$ 取有序集合末元并从链表摘除。各写操作 $O(\log n)$。
