...
> **思考**
>
> 要找能由词典中其他词每次加一个字母拼出的最长词，并列时取字典序最小者。词数与总长都不大，但若对每个词反复在集合里查全部前缀，比较与去重都容易写繁。
>
> 「每次加一个字母」等价于该词的每个前缀都是词典中的完整词。把全部词插入字典树后，沿路径检查每个节点是否都标了词尾，即可一次判断。
>
> 先建树再扫一遍单词，用 $\textit{search}$ 验证前缀链，同时按长度优先、同长取更小字典序更新答案。总时间与字符数成正比。
