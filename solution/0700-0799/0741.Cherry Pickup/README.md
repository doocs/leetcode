...
> **思考**
>
> 要从起点走到终点再走回，格子至多 $50$，两条路径分别搜索状态过多。同一格子的樱桃只能摘一次，分开走两次难以处理重叠。
>
> 往返等价于两人同时从起点各走 $k$ 步：步数相同则列由行决定，状态可压成 $(k,i_1,i_2)$。两人落到同一格时樱桃只加一次，障碍格不可达。
>
> $f[k][i_1][i_2]$ 由上一步行号 $i_1$ 或 $i_1-1$、$i_2$ 或 $i_2-1$ 转移，加上当前格的樱桃。答案为 $\max(0, f[2n-2][n-1][n-1])$。
