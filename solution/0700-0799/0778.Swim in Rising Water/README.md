...
> **思考**
>
> 水位随时间上涨，时刻 $t$ 只能走高度 $\le t$ 的格子，求从左上到右下的最早时刻。高度是 $0..n^2-1$ 的排列，可按高度从低到高加入格子。
>
> 加入高度为 $t$ 的格并与已加入的四邻连通后，若起点与终点连通，则 $t$ 就是答案。并查集维护连通。
>
> 先把高度映射到坐标，再按 $t$ 递增合并。时间 $O(n^2\alpha)$。
