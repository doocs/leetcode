...
> **思考**
>
> 判断全局逆序是否等于局部（相邻）逆序。$n\le 10^5$，数全部逆序对可以，但更弱的条件是：不存在跨度大于 $1$ 的逆序。
>
> 即不存在 $i\le j-2$ 且 $nums[i]>nums[j]$。维护 $nums[0..i-2]$ 的最大值，一旦大于 $nums[i]$ 则失败。
...
> **思考**
>
> 方法一用充要条件直接判断。若要分别计数，局部逆序扫相邻即可，全局逆序用树状数组数左侧更大的个数。
>
> 边走边比较两者之差，一旦全局更多即可返回。复杂度多一个对数。
