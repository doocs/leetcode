...
> **思考**
>
> 需要实现键值映射，键与值均在 $[0, 10^6]$，操作不超过 $10^4$ 次。与哈希集合类似，键可直接当作下标。
>
> 开辟长度为 $10^6+1$ 的数组，用 $-1$ 表示缺省。$\textit{put}$、$\textit{get}$、$\textit{remove}$ 分别写成赋值、读取和写回 $-1$，均为 $O(1)$。
>
> 值域与键域相同，哨兵 $-1$ 不会与合法值冲突。空间按值域分配，在本题限制下可行。
