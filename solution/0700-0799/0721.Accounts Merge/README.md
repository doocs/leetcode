...
> **思考**
>
> 同一人的多个账户会通过共享邮箱连通，需要按人合并邮箱并排序。账户数约 $1000$，若两两比较邮箱集合，既慢也难维护姓名。
>
> 连通关系发生在「账户」这一层：两个账户只要出现同一个邮箱就应合并。并查集按账户下标连边，邮箱到首次出现账户的映射用来发现这条边。
>
> 合并后再按根下标把邮箱收入集合，姓名取该根账户的名字，最后对邮箱排序输出。查找带路径压缩，总代价接近线性对数。
