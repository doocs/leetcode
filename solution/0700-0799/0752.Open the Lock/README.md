...
> **思考**
>
> 四位转盘每次转动一位，避开死亡数字，求到目标的最少步数。状态空间 $10^4$，每点至多 $8$ 个邻居，最短路即 BFS。
>
> 死亡数字与已访问点一并加入集合，避免入队。起点本身若在死亡集合中则无解；目标为 `0000` 则零步。
>
> 按层扩展，邻居由四位各自 $\pm 1$（环形）生成。第一次到达目标的层数即答案。
...
> **思考**
>
> 单向 BFS 可能扩展近半个状态空间。起点与终点已知，两端同时扩展，在中间相遇即可得到最短路，队列更短。
>
> 每次选当前更小的一侧扩一层；若邻居已在对侧出现，步数相加即答案。死亡数字两侧都跳过。
...
> **思考**
>
> 双向 BFS 仍均匀扩展。每位到目标的环形距离之和是可采纳启发，A* 用 $\textit{dist}+f$ 作堆键，终点首次出堆即为最短路。
>
> $f$ 不超过真实剩余步数（忽略死亡数字），因此可采纳。死亡格不扩展；松弛时更新 $\textit{dist}$ 并入堆。
