...
> **思考**
>
> 要把从某个 IPv4 出发的连续 $n$ 个地址表示成尽量少的 CIDR 块。仓库中各语言题解代码为空，思路仍按贪心最长前缀来写。
>
> 当前起点能覆盖的最长合法块，受对齐（低位零的个数）与剩余个数的限制，取二者较小者作为掩码长度。输出一块后起点加上块大小、减少剩余，直到 $n=0$。
>
> IP 与整数互转后，块大小为 $2^{32-\textit{mask}}$。这样每块都尽量大且不越出给定范围。
