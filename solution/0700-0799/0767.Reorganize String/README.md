...
> **思考**
>
> 重排使相邻字符不同。若某字符超过 $\lceil n/2\rceil$ 次则不可能。长度有限，先判断再隔位填充。
>
> 出现最多的字符先放在偶数下标，放满后从下标 $1$ 继续。这样最高频字符被尽量隔开。
>
> 按频次从大到小写入答案数组，越界则改到奇数位。
...
> **思考**
>
> 隔位填充依赖「最多隔一个」这一特解。写成与「间隔 $k$」相同的堆+冷却队列，可复用到 $k=2$。
>
> 每次取当前剩余最多的字符，用过后进入长度为 $k$ 的队列，出队后再回堆。拼不出全长则失败。
