...
> **思考**
>
> 行星只在「右行遇到左行」时相撞，同向或相离都不会撞。$n\le 10^4$，模拟每一对碰撞需要按存活序列处理，栈正好保存仍向右、尚未被摧毁的行星。
>
> 正数不会与左侧相撞，直接入栈。负数则不断与栈顶的正数比质量：栈顶更小则爆炸并继续，相等则双方消失，栈顶更大则当前行星消失。
>
> 从左到右扫一遍，每个行星至多入栈出栈一次，时间 $O(n)$。
