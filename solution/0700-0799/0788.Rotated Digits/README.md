...
> **思考**
>
> 好数旋转后仍合法且与原数不同。$n\le 10^4$，枚举每个数拆位即可。
>
> 非法数字直接否；合法数字按映射拼出旋转结果，与原数比较。
...
> **思考**
>
> 方法一对更大的 $n$ 会偏慢。合法性只取决于数位，可用数位 DP 统计 $[1,n]$。
>
> $dfs(i,ok,limit)$：$ok$ 表示是否已出现 $2/5/6/9$，$limit$ 为上界约束。非法位不递推，末位返回 $ok$。
