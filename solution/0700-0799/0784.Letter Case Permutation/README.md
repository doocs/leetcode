...
> **思考**
>
> 每个字母可改大小写，数字不动，输出全部串。长度短，回溯即可。
>
> 走到字母时先保持原样继续，再异或 $32$ 翻转大小写走第二支；数字只有一支。到达末尾收集。
...
> **思考**
>
> 回溯按位置展开。字母个数为 $n$ 时方案恰为 $2^n$，可用一个整数的比特表示第 $j$ 个字母用小写还是大写。
>
> 枚举 $0..2^n-1$，按位决定每个字母的大小写，数字原样抄写。
