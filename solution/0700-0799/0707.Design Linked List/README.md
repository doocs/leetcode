...
> **思考**
>
> 题目禁止使用内置链表库，需要自行支持按下标访问、头尾插入与按位置删除。调用次数不超过 $2000$，每次从头部走 $O(n)$ 都可以通过。
>
> 单链表的瓶颈在于定位第 $\textit{index}$ 个节点必须顺序走过前驱。用哑节点统一空表与非空表的插入删除，再用 $\textit{cnt}$ 记录长度，即可把越界判断从遍历中拆出。
>
> 头插、尾插都归约为 $\textit{addAtIndex}$：先走到目标位置的前驱，再改指针。删除同样先定位前驱。各操作时间由遍历长度决定，为 $O(n)$。
...
> **思考**
>
> 方法一每次插入都动态分配节点。本题节点数很小，分配开销不明显，但在节点更多时频繁 $\textit{new}$ 会拉高常数。
>
> 预先申请两块平行数组分别存放值与后继下标，用递增的 $\textit{idx}$ 充当节点池，插入改为在池中取下一空位并改写下标，语义与指针链表相同。
>
> $\textit{head}$ 存首元下标，$\textit{cnt}$ 仍表示长度。时间复杂度与方法一一致，避免了逐次分配。
