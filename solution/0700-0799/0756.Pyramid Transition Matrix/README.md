...
> **思考**
>
> 底层长度不超过 $6$，允许的三元组有限。自底向上枚举每一层的所有合法顶，层短但分支仍会重复同一剩余串。
>
> 相邻两块决定上一层对应位置的候选字母，一层的全部候选做笛卡尔积，得到所有可能的上一层字符串。
>
> 记忆化 $dfs(s)$：长度为 $1$ 则成功；某对相邻无候选则失败；否则对 $\textit{product}$ 出的每一层递归。
