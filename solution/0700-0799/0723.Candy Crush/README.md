...
> **思考**
>
> 要反复消除横竖连续三个及以上的相同糖果并下落，直到稳定。棋盘不大，直接按规则模拟是可行的。
>
> 若边消边落，后一次消除可能破坏前一次尚未结算的连通，题目要求同一时刻所有稳定的三连一起消去。因此先整盘标记，再统一下落。
>
> 用负数标记待消格（比较时取绝对值），一行一列扫长度为 $3$ 的窗口；若本轮有标记，则每列自底向上把正数落下、上空填零，并继续下一轮。
