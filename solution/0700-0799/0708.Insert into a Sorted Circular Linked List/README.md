...
> **思考**
>
> 在非降序的循环链表中插入一个值。节点数可达 $5 \times 10^4$，需要在一圈内找到插入缝隙。空表时新节点自环即可。
>
> 有序环上存在唯一的「下降沿」：从最大值连回最小值。插入点要么落在某一段升序中间，要么恰好越过这条下降沿（新值不小于最大值或不大于最小值）。
>
> 为此用相邻指针 $\textit{prev}$、$\textit{curr}$ 绕环一周，满足上述两种条件之一便停下；若全程单调则插在起点前，循环自然落到最后一对。接上新节点后返回原头。
