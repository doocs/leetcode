...
> **思考**
>
> 要维护实数轴上半开区间的添加、删除与覆盖查询，端点可达 $10^9$，操作约 $10^4$ 次。若每次把区间拆成列表再线性合并，单次可能扫过大量片段。
>
> 覆盖关系是区间加、区间查的经典形态，线段树能在对数时间内处理。值域太大不能预先建满整棵树，需要动态开点，并用懒标记把整段赋值下推。
>
> 节点记录该段是否被完全覆盖。修改时若当前段被询问区间包住，打上添加或删除标记；查询则在完全覆盖时直接返回 $\textit{v}$，否则下推后左右与起来。单次 $O(\log n)$。
