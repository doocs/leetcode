...
> **思考**
>
> 要把含变量、括号与加减乘的表达式展开成多项式，同类项合并后按次数与字典序输出。仓库中各语言题解为空，按多项式运算的常规写法说明。
>
> 数字与变量是单项式，加减即合并系数，乘法则对两项的变量多重集合做并。括号改变运算顺序，应用栈或递归下降按优先级求值，遇赋值表则把变量换成常数。
>
> 最终把系数非零的项按次数降序、同次按变量字典序输出。
