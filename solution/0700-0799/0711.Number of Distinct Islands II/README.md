...
> **思考**
>
> 统计网格中四连通岛屿的个数，旋转与镜像视为同一形状。网格不超过 $50\times 50$，枚举每个岛屿后比较形状是可行的。
>
> 普通「不同岛屿」只需把相对坐标规范化；本题同一块陆地在 $8$ 种二面体变换下都算相同，因此要把每种变换后的坐标都算出来，再平移到原点并取其中字典序最小者作为签名。
>
> DFS 收集一座岛的格子，对 $(i,j)$ 生成 $(\pm i,\pm j)$ 与 $(\pm j,\pm i)$ 共八组，各组排序后减去首点，再在八组中取最小元组放入集合。集合大小即为答案。
