...
> **思考**
>
> 统计由四个 $1$ 构成的轴对齐矩形个数。$200\times 200$ 若枚举四个顶点则过慢；$1$ 至多 $6000$ 个，宜利用「一行中的一对 $1$」。
>
> 两行能构成矩形，当且仅当某两列在这两行都是 $1$。枚举当前行的每一对 $1$ 列，此前已出现该列对的行数就是能配成的矩形数。
>
> 哈希表统计列对出现次数：当前行累加 $\textit{cnt}[(i,j)]$，再把该对加一。时间 $O(mn^2)$。
