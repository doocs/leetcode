...
> **思考**
>
> $2\times 3$ 棋盘状态只有 $720$ 种，空格四向滑动求最少步数，即无权最短路。把局面编成六字符便于入队去重。
>
> 每次还原棋盘、找到 $0$ 的位置、交换生成邻居。第一次到达 `123450` 的层数即答案，搜完则无解。
...
> **思考**
>
> 全状态 BFS 已经很小。先用逆序对奇偶剪掉不可达局面，再用曼哈顿距离作可采纳启发，优先扩展更接近目标的状态。
>
> 堆键为 $f+\textit{dist}$，终点首次出堆即为最短路。
