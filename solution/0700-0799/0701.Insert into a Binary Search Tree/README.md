...
> **思考**
>
> 需要在二叉搜索树中插入一个尚未出现的值，并返回新的根。节点数不超过 $10^4$，任意正确的插入路径都可以通过。
>
> 插入位置由 BST 的有序性唯一确定：沿比较路径走到空位，在该处挂上新节点即可，无需旋转或重构。空树则新节点自身即为根。
>
> 因此按当前值与 $\textit{val}$ 的大小递归进入左或右子树，并把返回的子树根接回当前节点。最坏沿一条链下降，时间与栈空间均为 $O(n)$。
