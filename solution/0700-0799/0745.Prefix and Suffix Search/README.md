...
> **思考**
>
> 询问同时匹配前缀与后缀的最大下标。词长不超过 $7$，词数与询问均为 $10^4$，把每个词的全部前缀–后缀对预先存起来，询问即可 $O(1)$。
>
> 后出现的词下标更大，按顺序覆盖哈希表即可保留最大下标。空前缀、空后缀也要存，以支持整词匹配的边界。
>
> 键为 $(pref,suff)$，值为下标。$\textit{f}$ 查表，缺失返回 $-1$。
...
> **思考**
>
> 方法一把一对前缀后缀都存进哈希，空间是词长平方之和。改用两棵字典树分别索引前缀与反转后缀，节点上记下经过的下标列表。
>
> 询问时取出前缀路径与反转后缀路径上的下标序列（均按插入顺序递增），从右同步双指针找最大的公共下标。
>
> 词长短，列表也不长，避免了存全部配对。
