...
> **思考**
>
> 要构造最短字符串，使每个长度为 $n$ 的 $k$ 进制串都作为子串出现。总长下界为 $k^n+n-1$，对应德布鲁因序列。
>
> 把长 $n-1$ 的串看作点，再接一个字符就是一条边，边即一个密码。每个点出入度均为 $k$，存在欧拉回路，回路上的边序便是最短覆盖。
>
> Hierholzer：从 $0$ 出发，边用整数 $u\cdot 10+x$ 去重，递归走完再把字符追加（后序），最后补上起点的 $n-1$ 个零。
