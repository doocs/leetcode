...
> **思考**
>
> 要从 $[0, n)$ 中等概率抽取不在黑名单中的整数，$n$ 很大而黑名单相对较短，每次现场扫描或拒绝采样都可能反复撞上黑名单。
>
> 合法整数恰好有 $k=n-|B|$ 个。若能把 $[0, k)$ 中「被拉黑」的位置一一映射到 $[k, n)$ 中的白名单，则只需在 $[0, k)$ 上均匀取数，再按映射替换即可。
>
> 预处理时把黑名单放入集合，从 $i=k$ 起跳过黑名单，将每个小于 $k$ 的黑名单值映到下一个可用的大号。$\textit{pick}$ 时取 $x\in[0,k)$，查表得 $d.get(x,x)$。
