...
> **思考**
>
> 把链表尽量均匀地分成 $k$ 段，多出来的节点分给前面的部分。长度不超过 $1000$，先数长度再切是直接的。
>
> 平均每段 $\lfloor n/k\rfloor$，余数 $n\bmod k$ 决定前若干段多一个。空位用空指针补齐到 $k$ 段。
>
> 第二遍按算出的长度走到每段末尾，断开 $\textit{next}$ 并记下下一段头。只需两次遍历。
