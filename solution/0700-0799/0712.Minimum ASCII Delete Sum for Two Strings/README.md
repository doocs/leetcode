...
> **思考**
>
> 要使两串相等，只能删除字符，代价为被删字符的 ASCII 之和。长度至多 $1000$，若对每个对齐位置递归删除，子问题会大量重叠。
>
> 这与编辑距离同类：若末字符相同则不必删；否则必须删掉其中一侧的末字符，并累加其码值。空串对另一串的代价是该串全部码值之和。
>
> 令 $f[i][j]$ 为前缀 $s_1[:i]$ 与 $s_2[:j]$ 的最小删除和，按上述转移填表，答案为 $f[m][n]$。时间与空间均为 $O(mn)$。
