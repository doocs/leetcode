...
> **思考**
>
> 允许双重预订、禁止三重。端点仍到 $10^9$，调用 $1000$ 次。若维护每个已订区间再两两查重，需要额外记录双重区间，实现较碎。
>
> 覆盖次数随时间的变化可用差分数组描述：起点 $+1$、终点 $-1$。扫描有序时间点上的前缀和，一旦超过 $2$ 便拒绝并回滚本次差分。
>
> 值域大，用有序字典代替数组即可。每次预订扫描当前关键点，时间 $O(n)$，总 $O(n^2)$，在 $1000$ 次调用下可通过。
...
> **思考**
>
> 方法一每次要扫完全部关键点。区间加一、查询区间最大覆盖，正是带懒标记的线段树。
>
> 值域 $10^9$，动态开点维护区间最大值。先查询 $[\textit{start},\textit{end})$ 的最大覆盖，若已为 $2$ 则拒绝，否则区间加一。单次 $O(\log n)$。
