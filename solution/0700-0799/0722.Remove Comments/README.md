...
> **思考**
>
> 要从源码中去掉行注释与块注释，并保留跨行块注释之外的有效字符。源码总长有限，一次扫描即可，难点在状态：是否正处在块注释中，以及行注释会吃掉本行剩余部分。
>
> 块注释可以跨行，因此状态必须带到下一行；行注释只作用于当前行。两种注释都以两字符记号开头，扫描时要优先识别 `/*`、`*/`、`//`，避免把记号拆开当普通字符。
>
> 用 $\textit{blockComment}$ 与缓冲 $t$：块内只寻找结束符；块外遇 `//` 结束本行，遇 `/*` 进入块，否则收入 $t$。一行结束且不在块内、缓冲非空时才形成答案的一行，这样跨行拼接也自然成立。
