...
> **思考**
>
> 数组可有重复，求最多切成多少块，使各块分别排序后整体有序。$n\le 2000$，枚举切点需快速判断「左侧都 $\le$ 右侧」。
>
> 从左到右，已形成的块的最大值应单调不降。新值若小于栈顶，说明它必须并入前面某几块，弹出直到栈顶不大于它，再把原先的块最大值压回。
>
> 栈里留下的每个最大值对应一块，栈长即答案。
...
> **思考**
>
> 单调栈没有显式表达切点条件。在 $i$ 切开当且仅当前缀最大 $\le$ 后缀最小。
>
> 预处理前缀最大，从右扫后缀最小，每处满足不等式则多一块。整段本身算一块，故初值 $1$。
