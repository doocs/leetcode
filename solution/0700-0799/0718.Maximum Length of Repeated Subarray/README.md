...
> **思考**
>
> 求两个数组的最长公共连续子数组。长度均为 $1000$，枚举起点再匹配最坏 $O(n^3)$，不能接受；公共子序列的 DP 也不要求连续。
>
> 连续意味着只有在两端字符相等时，长度才能由「去掉末元后的公共后缀」加一得到，否则这段后缀长度为 $0$。
>
> 令 $f[i][j]$ 为以 $nums1[i-1]$、$nums2[j-1]$ 结尾的公共后缀长，相等则 $f[i-1][j-1]+1$，否则为 $0$。遍历取全局最大。时间 $O(mn)$。
