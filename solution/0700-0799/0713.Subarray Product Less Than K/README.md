...
> **思考**
>
> 统计乘积严格小于 $k$ 的连续子数组个数。数组长度可达 $3\times 10^4$，枚举左右端点再求积为 $O(n^2)$，偏紧。
>
> 元素均为正整数，窗口乘积随右端扩张而单调不减。一旦乘积达到 $k$，必须右移左端才能恢复合法，因此每个端点至多移动一次。
>
> 维护乘积 $p$ 与左端 $l$：右端纳入 $x$ 后，若 $p\ge k$ 则不断除掉 $\textit{nums}[l]$。此时以 $r$ 结尾的合法子数组个数为 $r-l+1$。一遍扫描即可。
