...
> **思考**
>
> 统计最大值落在 $[left,right]$ 的子数组。$n\le 10^5$，枚举两端再求最大过慢。最大值 $\le R$ 的子数组减去最大值 $\le L-1$ 的，即所求。
>
> $f(x)$ 扫一遍：当前值 $\le x$ 则延长合法后缀，个数加其长度，否则清零。
...
> **思考**
>
> 差分把「落在区间」拆开。也可枚举谁当最大值：用单调栈找左严格更大、右非严格更大，贡献 $(i-l)(r-i)$，仅当该值在 $[left,right]$ 内计入。
>
> 左右边界不对等是为了每个子数组只算一个最大元（相同时取最左）。
