...
> **思考**
>
> 旋转有序数组中查找 $target$，且可能有重复。线性扫描总能找到最左出现，但未利用半段有序。
>
> 中点将数组分成至少一段有序区间；比较 $arr[mid]$ 与 $arr[r]$ 可判断哪一段有序，再看 $target$ 落在哪一侧。
>
> 两端相等时先收缩 $r$，避免无法判定。相等则 $r$ 减一。结束后检查 $arr[l]$。重复使最坏接近线性，有序段上仍接近对数。
