...
> **思考**
>
> 通常删除节点需要其前驱以改写 `next`。本题只给出待删节点本身，且保证它不是尾节点，因此无法沿链表回溯。
>
> 删除的可见效果是该位置的值与后继关系消失。把后继的值拷到当前节点，再跳过后继，从调用者视角即等价于删掉了当前节点。
>
> 两步赋值 `node.val = node.next.val` 与 `node.next = node.next.next` 即可，时间和空间均为常数。
