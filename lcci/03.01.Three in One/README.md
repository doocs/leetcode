...
> **思考**
>
> 要用一份存储实现三个固定容量的栈。三个独立数组显然可行，但题目要求共享一块连续内存。
>
> 把区间均分为三段，并用三个计数记录各自栈顶偏移。下标 $cap \times stackNum + size$ 即该栈的下一个空位。
>
> 数组长度为 $3cap+3$，末三位存三个栈的元素个数；`push`/`pop`/`peek` 只改对应计数与槽位，均为 $O(1)$。满栈拒绝压入，空栈 `pop`/`peek` 返回 $-1$。
