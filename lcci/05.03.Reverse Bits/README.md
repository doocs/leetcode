...
> **思考**
>
> 允许把一个 $0$ 翻成 $1$ 后，求最长连续 $1$。枚举翻转位置再向两侧扩展，约 $32^2$ 次，可通过，但重复扫描多。
>
> 所求即「窗口内至多一个 $0$」的最长区间，可用滑动窗口在一次扫描中维护。
>
> 右端 $i$ 扫过 $32$ 位，用 $cnt$ 记窗口内 $0$ 的个数；超过 $1$ 则右移 $j$ 并回退计数。`num>>i & 1 ^ 1` 恰在该位为 $0$ 时加一。
