...
> **思考**
>
> 把有序的 $B$ 并入 $A$ 末尾的空位。若从前往后写入，尚未处理的 $A$ 会被覆盖。另开数组再拷回则多用 $O(m+n)$ 空间。
>
> $A$ 的尾部空位足够容纳全部结果，从后往前填入较大者就不会破坏尚未合并的前缀。
>
> 指针 $i$、$j$ 指向两数组有效末尾，$k$ 从 $m+n-1$ 递减。当 $B$ 耗尽或 $A[i]$ 更大时写入 $A[i]$，否则写入 $B[j]$。原地完成。
