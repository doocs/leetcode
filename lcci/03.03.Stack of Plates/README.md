...
> **思考**
>
> 容量为 $cap$ 的栈满后要新开一叠，并支持按叠弹出。用一维数组手动切分边界，下标计算易错，且中间一叠变空后的收缩不自然。
>
> 把「若干有界栈」直接建成栈的列表：$stk[-1]$ 为当前叠。
>
> `push` 在末叠满时追加空列表再压入；`pop` 委托 `popAt` 最后一叠；`popAt` 弹出指定叠，空则从列表删除以免留下空洞。$cap=0$ 时拒绝一切压入。
