...
> **思考**
>
> 旋转即把某前缀挪到末尾。枚举所有切割点再比较，共 $n$ 次、每次 $O(n)$，在长度相等时可行，但重复比较较多。
>
> 长度不等则必不是旋转。长度相等时，$s_1+s_1$ 覆盖 $s_1$ 的全部旋转，于是问题化为一次子串判定：$s_2$ 是否出现在 $s_1+s_1$ 中。
>
> 代码先比长度，再用 `s2 in s1 * 2` 完成包含检查，与上述观察一致。
