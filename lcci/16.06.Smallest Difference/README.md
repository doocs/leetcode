...
> **思考**
>
> 两数组中各取一数，使差的绝对值最小。枚举全部数对是 $O(nm)$。
>
> 固定 $a$ 中的 $x$ 后，只需 $b$ 里最接近 $x$ 的值，有序数组上即一次二分。
>
> 先排序 $b$，对每个 $x$ 用 `bisect_left` 看插入点及其左侧，更新最小差。总时间由排序与 $n$ 次对数查找主导。
...
> **思考**
>
> 对每个 $x$ 单独二分，常数上仍有 $n$ 次对数。
>
> 两数组都排序后双指针同步前进：较小一端移动才可能缩小差，一遍线性扫完，省去反复二分。
