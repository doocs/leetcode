...
> **思考**
>
> 倒数第 $k$ 个节点等价于正数第 $n-k+1$ 个。先走一遍求 $n$ 再走第二遍，正确且 $n$ 通常不大；题目希望一次遍历。
>
> 若快指针比慢指针提前 $k$ 步，则快指针抵达末尾时，慢指针恰好停在倒数第 $k$ 个。
>
> 两指针同起于头节点，`fast` 先走 $k$ 步，再同步前进直至 `fast` 为空，返回 `slow.val`。全程 $O(1)$ 额外空间。
