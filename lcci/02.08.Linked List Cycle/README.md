...
> **思考**
>
> 求环入口。哈希表记录已访节点，首次重复即入口，空间 $O(n)$。
>
> 快慢指针若相遇则必在环上；再由头与相遇点同步各走一步，相遇处即入口。其正确性来自 $x = (k-1)(y+z)+z$。
>
> 先让 `fast` 步长为 $2$、`slow` 步长为 $1$ 直至相遇（或 `fast` 为空则无环），再将 $ans$ 从头出发与 `slow` 同速前进。不使用额外表。
