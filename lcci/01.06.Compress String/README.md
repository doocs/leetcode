...
> **思考**
>
> 压缩串由「字符 + 连续出现次数」拼接而成，短于原串才采用。暴力对每个位置向右数相同字符，最坏仍是线性，但指针会反复回退，实现不清晰。
>
> 同一段连续字符只需报告一次，关键是找出每段的左右端点。
>
> `groupby`（或显式双指针）按相等关系分段，将字符与段长写入 $t$，最后按长度取 $S$ 与 $t$ 中较短者。扫描各字符恰好一次，与「双指针找连续段」同构。
