...
> **思考**
>
> 数据流中查询「有多少个数 $\le x$」。每次排序或扫描已插入元素，单次查询线性，跟不上多次询问。
>
> 值域不超过约 $5\times 10^4$，前缀计数可用树状数组在 $O(\log U)$ 内更新与查询。
>
> `track` 对 $x+1$ 做 $+1$（下标从 $1$ 起）；`getRankOfNumber` 查询 $[1,x+1]$ 的和。树状数组长度取 $50010$ 覆盖题目范围。
