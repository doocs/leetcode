...
> **思考**
>
> 栈顶需始终为当前最小元素。若只在查询时排序，则无法用栈接口表达；若允许用另一个栈，则可在插入时恢复有序。
>
> 要让新值 $val$ 落到正确深度，须暂时弹出所有比它更小的栈顶（它们应更靠近栈顶），放入 $val$ 后再压回。
>
> 辅助栈 $t$ 承接这些较小元素，`push` 结束后 $stk$ 自顶向下仍非降。`pop`/`peek`/`isEmpty` 只碰栈顶，为 $O(1)$。
