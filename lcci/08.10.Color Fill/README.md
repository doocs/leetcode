...
> **思考**
>
> 从 $(sr,sc)$ 把四连通的原色块改成新色。扫描全图再判断连通会把不相干的同色也改掉。
>
> 只需访问与起点连通且颜色为 $oc$ 的格子，深度优先即可。
>
> 越界、颜色不是 $oc$、或已经是新色则返回；否则改色并向四邻递归。与新色相同的判断同时防止 $oc=newColor$ 时无限递归。
...
> **思考**
>
> DFS 依赖递归栈，在细长连通块上深度可达 $mn$。
>
> BFS 用队列扩展四邻，语义相同而深度变为队列长度，避免调用栈溢出。
