...
> **思考**
>
> 一次编辑包含替换、插入或删除。若枚举对较短串的所有一次编辑再与另一串比较，状态数与长度成正比，实现繁琐。
>
> 长度差超过 $1$ 时不可能一次编辑达成，可直接否定。为此不妨令 $m \ge n$：相等则只允许恰好一处替换；相差 $1$ 则较长串须恰好多一个字符。
>
> 等长时对位统计不等字符个数；不等长时用双指针在较长串上跳过至多一处失配。先交换使第一参数更长，从而只实现删除方向，避免插入与删除各写一套。
