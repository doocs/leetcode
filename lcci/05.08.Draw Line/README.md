...
> **思考**
>
> 在位图第 $y$ 行把 $[x_1,x_2]$ 涂成 $1$，存储是按 $32$ 位打包的整数数组。逐像素置位正确但与字边界交互繁琐。
>
> 整段覆盖的字可一次写成全 $1$（即 $-1$），只需修补首尾两个不完全字。
>
> 由 $y\cdot w+x$ 算出首尾字下标 $i$、$j$，先把 $[i,j]$ 置 $-1$，再按 $x_1\bmod 32$、$x_2\bmod 32$ 清掉首字高位与尾字低位以外的位。
