...
> **思考**
>
> 要删除链表中值重复的节点并保持首次出现的相对次序。对每个节点向后扫描查重，时间 $O(n^2)$，在链表较短时可通过，但每次查询都是线性的。
>
> 判定「值是否已出现」是查找问题，哈希表可将查询降到期望常数时间。
>
> 用 $vis$ 记录已保留的值，虚拟头节点 $pre$ 便于删除：若 $pre.next.val$ 已在集合中则跳过该节点，否则记入集合并前进。一次遍历完成去重。
