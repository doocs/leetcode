...
> **思考**
>
> 箱子须在长宽高上都严格更小才能叠上，求最大高度。搜索所有叠放顺序是阶乘级。
>
> 先按宽度排序后，问题接近「三维严格递增的最长链」，可用 $O(n^2)$ 的 LIS 式 DP。宽相同则深度降序，避免宽相等却被当成可叠。
>
> $f[i]$ 为以 $i$ 为底的最大高度，枚举 $j<i$ 且深、高都更小的箱子转移，再加上 $box[i]$ 的高。答案为 $f$ 的最大值。
