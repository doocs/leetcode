...
> **思考**
>
> 每行、每列递增，查找 $target$。整表扫描是 $O(mn)$。每行有序，可对每行二分。
>
> 对第 $i$ 行做 $bisect\_left$，若命中则返回。行与行之间不再利用列递增，复杂度 $O(m\log n)$。
>
> 实现直接遍历行并二分，空间 $O(1)$，写起来短，适合行数不大或只需行内有序的情形。
...
> **思考**
>
> 方法一对列递增未加利用，仍可能看遍每一行。
>
> 从左下（或右上）出发：当前值偏大则上移丢掉该列更大的一段，偏小则右移丢掉该行更小的一段，每步剔除一行或一列，时间 $O(m+n)$。
