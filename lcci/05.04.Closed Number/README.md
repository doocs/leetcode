...
> **思考**
>
> 要找与 $num$ 二进制中 $1$ 的个数相同的最近更大、更小值。逐个加减再数 $1$，最坏走过很大一段空白。
>
> 更大者来自把某一处 `01` 换成 `10` 并把更低的 $1$ 尽量靠右；更小者对称地处理 `10` $\to$ `01` 并把低位 $0$ 靠右。
>
> 对两种方向用 $(a,b)$ 表示要交换的相邻位模式，找到最低合法位置后交换，再在更低位上把 $1$ 与 $0$ 按目标方向聚拢。找不到则保持 $-1$。
