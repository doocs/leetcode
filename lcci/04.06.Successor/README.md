...
> **思考**
>
> 中序后继是中序序列中 $p$ 的下一个。完整中序需要 $O(n)$ 时间与空间；题目是 BST，可利用有序性。
>
> 后继是所有大于 $p.val$ 的节点中最小的那个。从根下行时，若 $root.val > p.val$，则 $root$ 是候选，并转入左子树寻找更小的更大值；否则后继只可能在右子树。
>
> 循环更新 $ans$ 并走左或右，高度时间内完成，无需父指针，也无需中序 Moris 线索。
