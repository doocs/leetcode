...
> **思考**
>
> 互为排列当且仅当字符多重集合相同。两串长度不等则可立即否定；长度同为 $n$ 时，若枚举 $s_1$ 的全排列再与 $s_2$ 比较，代价远高于题目规模所需。
>
> 瓶颈在于如何在线性时间内核对频次。为此先统计 $s_1$ 各字符出现次数，再扫描 $s_2$ 逐次减一：某计数变为负数，则频次不一致。
>
> 测试数据仅含小写字母，长度为 $26$ 的数组即可充当哈希表。先比较长度再计数，避免对明显不等的输入做无用统计。
...
> **思考**
>
> 哈希计数已在 $O(n)$ 时间内给出答案，但需要与字符集规模成正比的辅助空间。
>
> 若将两串按字典序排序后再比较，相等当且仅当它们是同一多重集合。排序代价为 $O(n \log n)$，实现更短，且不依赖对字符集的假设。
