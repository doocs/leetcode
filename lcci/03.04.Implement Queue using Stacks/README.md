...
> **思考**
>
> 栈是后进先出，队列要先进先出。每次入队都把已有元素倒到辅助栈再倒回，正确但使 `push` 成为线性。
>
> 入队与出队的次序相反，只需在出队侧再反转一次。为此用 `stk1` 只负责入队，`stk2` 只负责出队；仅当 `stk2` 为空时才把 `stk1` 一次性倒入。
>
> `move` 保证每个元素至多从 `stk1` 迁到 `stk2` 一次，出队与取队首均摊 $O(1)$。判空则看两栈是否都空。
