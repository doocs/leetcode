...
> **思考**
>
> 幂集大小为 $2^n$，必须枚举。对每个元素独立决定选或不选，递归树恰好覆盖全部子集。
>
> $dfs(u,t)$ 先不选 $nums[u]$，再选入后回溯弹出。到达 $u=n$ 时拷贝 $t$。
>
> 先递归不选再选，生成顺序与「从空集逐步扩张」一致；弹出保证 $t$ 被各分支共享而不互相污染。
...
> **思考**
>
> 递归枚举已经覆盖全部子集，调用栈与回溯样板仍然偏长。
>
> 每个子集对应 $[0,2^n)$ 中一个掩码，第 $i$ 位为 $1$ 则纳入 $nums[i]$。迭代即可，无需递归。
