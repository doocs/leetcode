...
> **思考**
>
> 每次可上 $1$、$2$ 或 $3$ 阶，求到达 $n$ 的方案数。递归按三路展开而不记忆，重复子问题极多。
>
> 到达 $i$ 的方案只依赖 $i-1$、$i-2$、$i-3$，线性递推即可，并按题意取模。
>
> 滚动三个变量 $a,b,c$ 表示连续三项，循环 $n-1$ 次后 $a$ 即为 $f(n)$。空间 $O(1)$，与「只保留三项」一致。
...
> **思考**
>
> 线性递推在 $n$ 很大时仍要 $\Theta(n)$ 步，模运算成为主要常数开销。
>
> 三阶常系数递推可写成 $1\times 3$ 状态乘 $3\times 3$ 转移矩阵，再用快速幂在 $O(\log n)$ 次矩阵乘内得到 $F_n$。
