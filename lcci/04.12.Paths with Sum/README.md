...
> **思考**
>
> 路径可起止于任意祖先—后代对，不必经过根。若对每个节点再向下枚举所有路径，同一条边会被反复累加，最坏接近平方。
>
> 路径和等于两端前缀和之差。若当前前缀为 $s$，则需要祖先上出现过 $s-sum$ 的次数。
>
> 哈希表 $cnt$ 统计根到当前节点的前缀；$cnt[0]=1$ 对应空前缀。先查 $cnt[s-sum]$，再把 $s$ 加一，递归左右，回溯时减一，以免影响兄弟分支。
