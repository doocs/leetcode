...
> **思考**
>
> 字母异位词排序后得到同一键。两两比较是否互为异位需要 $O(n^2 k)$，在词数较多时偏慢。
>
> 把排序后的串当作哈希键，同一键下的原串自然成组。
>
> 对每个 $s$ 计算 $k=''.join(sorted(s))$ 并追加到 $d[k]$，最后输出 $d$ 的值列表。每词 $O(k\log k)$ 的排序换一次哈希。
...
> **思考**
>
> 排序键在字符集很小（仅小写）时可以换成频次数组，避免 $O(k\log k)$。
>
> 用长度为 $26$ 的计数（或由其构成的元组）作键，分组语义不变，单词变为 $O(k+C)$。
