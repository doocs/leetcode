...
> **思考**
>
> 两数按逆序存在链表中，相加规则与竖式加法相同。先把链表转成整数再相加，在位数很大时会溢出或失去线性时间的意义。
>
> 只需从低位到高位同步前进，并维护进位 $carry$。某一链表耗尽后仍可能有进位，循环条件因此包含 $l_1$、$l_2$ 或 $carry$。
>
> 每次用 `divmod` 拆出当前位与新进位，尾插到虚拟头之后。不先物化整个整数，空间除答案外为 $O(1)$。
