...
> **思考**
>
> 在 $n$ 列上放 $n$ 个互不攻击的皇后。枚举排列再检查对角线，排列数为 $n!$，检查代价额外线性。
>
> 按行放置时，冲突只发生在列与两条对角线上，可用数组 $O(1)$ 查询。
>
> $col[j]$、$dg[i+j]$、$udg[n-i+j]$ 标记占用；$dfs(i)$ 尝试第 $i$ 行各列，成功则改棋盘并递归，返回后恢复。$n$ 不大，回溯足以列出全部棋盘。
