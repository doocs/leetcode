...
> **思考**
>
> 单词表中两个词最近距离。记下所有下标再两两比较，最坏平方。
>
> 最近的一对一定涉及某次出现与对方上一次出现，故扫一遍维护两词最近下标即可。
>
> $i$、$j$ 分别记录 $word1$、$word2$ 的最近位置，每步更新 $|i-j|$。一词多次出现时只保留最近一次，已足够。
...
> **思考**
>
> 一次遍历适合单次询问；同一文本上多次询问会重复扫全文。
>
> 预处理每词的下标列表，询问时对两个有序下标数组双指针求最近对，单次询问与出现次数成正比。
