...
> **思考**
>
> 空格替换为三位编码，结果长度与原文成正比，一次遍历即可完成。语言已提供按子串替换的接口。
>
> 直接调用 `replace`，把空格映射为 `%20`。
...
> **思考**
>
> 内置替换把构造过程藏在库函数里，面试时常要求显式建串。逐字符判断：空格写入 `%20`，其余原样收集，最后拼接。
>
> 与方法一复杂度相同，只是把映射写开。
...
> **思考**
>
> 方法二已给出逐字符拼接。本方法在 JavaScript 中用数组收集再 `join`，避免反复拼接不可变字符串，算法与方法二一致。
