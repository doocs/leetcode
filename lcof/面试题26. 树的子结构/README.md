...
3. 判断树 A 的左子树是否包含树 B 的左子树，即调用 $\textit{dfs}(A.left, B.left)$，并且判断树 A 的右子树是否包含树 B 的右子树，即调用 $\textit{dfs}(A.right, B.right)$。如果其中有一个函数返回 `false`，则树 B 是树 A 的子结构，返回 `false`；否则，返回 `true`。
