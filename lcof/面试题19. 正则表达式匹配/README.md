...
> **思考**
>
> 模式含 `.` 与 `*`，朴素回溯会在同一 $(i,j)$ 上反复尝试。$s,p$ 长度有限，状态数为乘积级，可记忆化。
>
> 令 $dfs(i,j)$ 表示从该后缀能否匹配：若下一位是 `*`，则跳过该段或在当前字符匹配时继续吞字符；否则必须一对一匹配。答案为 $dfs(0,0)$。
...
> **思考**
>
> 记忆化已是最优子结构，可改成自底向上填表，避免递归栈。$f[i][j]$ 对应前缀是否匹配，转移与方法一的三种分支相同，空串对空模式为真。
