...
> **思考**
>
> 取最小 $k$ 个数，不必得到第 $k$ 小之外的相对次序。全排序后截前 $k$ 位即可，$n$ 规模下 $O(n\log n)$ 可接受。
>
> 后续方法再分别用堆和划分降低不必要的排序开销。
...
> **思考**
>
> 全排序处理了不需要的较大元素。只需维持大小为 $k$ 的大根堆：超过 $k$ 就弹出当前最大，堆中剩下的即最小 $k$ 个，时间为 $O(n\log k)$。
...
> **思考**
>
> 堆仍有对数因子。快排一次划分后，若枢轴下标为 $k$ 则左侧即答案；否则只递归包含第 $k$ 位的那一侧，期望线性。
