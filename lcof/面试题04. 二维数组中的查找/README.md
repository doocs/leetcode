...
> **思考**
>
> 矩阵每行升序，$m,n$ 规模下逐格比较可行，但未利用有序性。行内有序意味着对每一行可用二分定位目标。
>
> 对每行做 `bisect_left`，若命中则返回。列递增尚未用到，复杂度为 $O(m\log n)$。
...
> **思考**
>
> 按行二分每次只排除半行，未同时利用列递增。从左下角出发，当前值大于目标则该行右侧都更大，上移一行；小于目标则该列上方都更小，右移一列。
>
> 每次丢掉一行或一列，至多 $m+n$ 步即可结束。
