...
> **思考**
>
> 前序首元素是根，中序把它左右切开。每次在中序里线性找根是 $O(n)$，整棵树会到 $O(n^2)$。结点值互异，可预处理中序值到下标。
>
> 递归时用 $d[preorder[i]]$ 切出左右长度，再对两段分别建树。查找降为 $O(1)$，总时间线性。
...
> **思考**
>
> 哈希表把查找降到常数，却多一份 $O(n)$ 映射。结点不多时，每次在中序里 `indexOf` 定位根，再按左右区间切开前序、中序并递归，代码更短，时间为 $O(n^2)$。

<!-- thinking:end -->

不用哈希表预处理。每次在中序序列里用线性查找定位根节点，再按左右区间切开前序、中序并递归建树。实现更短，但每次查找使时间复杂度变为 $O(n^2)$。

时间复杂度 $O(n^2)$，空间复杂度 $O(n)$。其中 $n$ 是二叉树的节点个数。
